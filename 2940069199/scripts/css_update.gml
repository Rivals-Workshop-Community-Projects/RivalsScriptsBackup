...
//Colorshift button
else if ((get_instance_x(cursor_id) >= x + button_cord[1,0] && get_instance_x(cursor_id) <= x + button_cord[1,0] + 30) && (get_instance_y(cursor_id) >= y + button_cord[1,1] && get_instance_y(cursor_id) <= y + button_cord[1,1] + 26)) {
    suppress_cursor = true;
    if (menu_a_pressed) {
        menu_a_pressed = false;
        color_shift++
