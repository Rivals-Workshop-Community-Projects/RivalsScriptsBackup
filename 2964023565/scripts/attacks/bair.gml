set_attack_value(AT_BAIR, AG_CATEGORY, 1);
set_attack_value(AT_BAIR, AG_SPRITE, sprite_get("bair"));
set_attack_value(AT_BAIR, AG_NUM_WINDOWS, 4);
set_attack_value(AT_BAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_BAIR, AG_LANDING_LAG, 9);
set_attack_value(AT_BAIR, AG_HURTBOX_SPRITE, sprite_get("bair_hurt"));

set_window_value(AT_BAIR, 1, AG_WINDOW_LENGTH, 14);
set_window_value(AT_BAIR, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_BAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_BAIR, 1, AG_WINDOW_SFX, asset_get("sfx_zetter_fireball_fire"));
set_window_value(AT_BAIR, 1, AG_WINDOW_SFX_FRAME, 6);

set_window_value(AT_BAIR, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_BAIR, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_BAIR, 2, AG_WINDOW_ANIM_FRAME_START, 3);

set_window_value(AT_BAIR, 3, AG_WINDOW_LENGTH, 3);
set_window_value(AT_BAIR, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_BAIR, 3, AG_WINDOW_ANIM_FRAME_START, 4);

set_window_value(AT_BAIR, 4, AG_WINDOW_LENGTH, 18);
set_window_value(AT_BAIR, 4, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_BAIR, 4, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_BAIR, 4, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_BAIR, 4);

set_hitbox_value(AT_BAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_BAIR, 1, HG_WINDOW, 3);
set_hitbox_value(AT_BAIR, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_BAIR, 1, HG_HITBOX_X, -60);
set_hitbox_value(AT_BAIR, 1, HG_HITBOX_Y, -35);
set_hitbox_value(AT_BAIR, 1, HG_WIDTH, 50);
set_hitbox_value(AT_BAIR, 1, HG_HEIGHT, 40);
set_hitbox_value(AT_BAIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_BAIR, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_BAIR, 1, HG_DAMAGE, 14);
set_hitbox_value(AT_BAIR, 1, HG_ANGLE, 180 - 40);
set_hitbox_value(AT_BAIR, 1, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_BAIR, 1, HG_KNOCKBACK_SCALING, .7);
set_hitbox_value(AT_BAIR, 1, HG_BASE_HITPAUSE, 12);
set_hitbox_value(AT_BAIR, 1, HG_HITPAUSE_SCALING, .65);
set_hitbox_value(AT_BAIR, 1, HG_VISUAL_EFFECT_X_OFFSET, -32);
set_hitbox_value(AT_BAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_BAIR, 1, HG_VISUAL_EFFECT, 148);
set_hitbox_value(AT_BAIR, 1, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));


//Goop interaction
set_hitbox_value(AT_BAIR, 2, HG_HITBOX_TYPE, 1); //It does nothing, meant for Goop
set_hitbox_value(AT_BAIR, 2, HG_WINDOW, 2);
set_hitbox_value(AT_BAIR, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_BAIR, 2, HG_HITBOX_X, -5);
set_hitbox_value(AT_BAIR, 2, HG_HITBOX_Y, -35);
set_hitbox_value(AT_BAIR, 2, HG_WIDTH, 100);
set_hitbox_value(AT_BAIR, 2, HG_HEIGHT, 50);
set_hitbox_value(AT_BAIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_BAIR, 2, HG_PRIORITY, 0);
set_hitbox_value(AT_BAIR, 2, HG_DAMAGE, 0);
set_hitbox_value(AT_BAIR, 2, HG_ANGLE, 1);
set_hitbox_value(AT_BAIR, 2, HG_BASE_KNOCKBACK, 0);
set_hitbox_value(AT_BAIR, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_BAIR, 2, HG_BASE_HITPAUSE, 0);
set_hitbox_value(AT_BAIR, 2, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_BAIR, 2, HG_VISUAL_EFFECT_X_OFFSET, -32);
set_hitbox_value(AT_BAIR, 2, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_BAIR, 2, HG_VISUAL_EFFECT, 148);

set_hitbox_value(AT_BAIR, 3, HG_HITBOX_TYPE, 1); //It does nothing, meant for Goop
set_hitbox_value(AT_BAIR, 3, HG_WINDOW, 3);
set_hitbox_value(AT_BAIR, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_BAIR, 3, HG_HITBOX_X, -30);
set_hitbox_value(AT_BAIR, 3, HG_HITBOX_Y, -40);
set_hitbox_value(AT_BAIR, 3, HG_WIDTH, 90);
set_hitbox_value(AT_BAIR, 3, HG_HEIGHT, 50);
set_hitbox_value(AT_BAIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_BAIR, 3, HG_PRIORITY, 0);
set_hitbox_value(AT_BAIR, 3, HG_DAMAGE, 0);
set_hitbox_value(AT_BAIR, 3, HG_ANGLE, 1);
set_hitbox_value(AT_BAIR, 3, HG_BASE_KNOCKBACK, 0);
set_hitbox_value(AT_BAIR, 3, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_BAIR, 3, HG_BASE_HITPAUSE, 0);
set_hitbox_value(AT_BAIR, 3, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_BAIR, 3, HG_VISUAL_EFFECT_X_OFFSET, -32);
set_hitbox_value(AT_BAIR, 3, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_BAIR, 3, HG_VISUAL_EFFECT, 148);

set_hitbox_value(AT_BAIR, 4, HG_HITBOX_TYPE, 1); //It does nothing, meant for Goop
set_hitbox_value(AT_BAIR, 4, HG_WINDOW, 4);
set_hitbox_value(AT_BAIR, 4, HG_LIFETIME, 5);
set_hitbox_value(AT_BAIR, 4, HG_HITBOX_X, -44);
set_hitbox_value(AT_BAIR, 4, HG_HITBOX_Y, -35);
set_hitbox_value(AT_BAIR, 4, HG_WIDTH, 90);
set_hitbox_value(AT_BAIR, 4, HG_HEIGHT, 50);
set_hitbox_value(AT_BAIR, 4, HG_SHAPE, 0);
set_hitbox_value(AT_BAIR, 4, HG_PRIORITY, 0);
set_hitbox_value(AT_BAIR, 4, HG_DAMAGE, 0);
set_hitbox_value(AT_BAIR, 4, HG_ANGLE, 1);
set_hitbox_value(AT_BAIR, 4, HG_BASE_KNOCKBACK, 0);
set_hitbox_value(AT_BAIR, 4, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_BAIR, 4, HG_BASE_HITPAUSE, 0);
set_hitbox_value(AT_BAIR, 4, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_BAIR, 4, HG_VISUAL_EFFECT_X_OFFSET, -32);
set_hitbox_value(AT_BAIR, 4, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_BAIR, 4, HG_VISUAL_EFFECT, 148);