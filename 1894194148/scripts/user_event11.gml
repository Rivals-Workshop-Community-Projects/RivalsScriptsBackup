				...
				case 7:
					...
					switch(player_id.muno_char_id){
						...
						default:
							...
							var drawn = textDraw(draw_x, draw_y, "fName", c_white, 18, draw_w, fa_center, 1, true, phone.image_alpha, "                                        ", true);
							if !selected changeCursor(draw_x - drawn[0] / 2 - 7, draw_y - drawn[1] / 2 - 4 + sin_thing, drawn[0] + 11, drawn[1] + 1);
							
							draw_set_valign(valign);
							
							textDraw(draw_x, origin_y + screen_h - 138, "fName", $aaff99, 12, 120, fa_center, 1, true, phone.image_alpha, "lets be real for a sec. if it's locked for use with a specific character why didnt they disable the app or something? maybe thats also an ad tactic. i'm conflicted about this", false);
		...
		if (state != 2 && state != 4 && stage_id != noone && stage_id.screen.low_fps = true){
			...
			case 6:
				...
				if stage_id == noone {
					...
					var tease_txt = textDraw(draw_x, draw_y, "fName", c_white, 20, draw_text_width, fa_left, 1, false, 1, "TT Framework not found in local network! (Error Code: 0627)
					This app is only available to use in following regions:
