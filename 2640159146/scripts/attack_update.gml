...
beginning = (window == 1 && window_timer == 1)
...
if ((attack == AT_FTILT || attack == AT_DATTACK) && has_hit && window == 2) 
{
	window = 4;
	window_timer = 0;
	destroy_hitboxes();
...
//Do not let bloodmeter go below the minimum threshold
...
if (attack = AT_DSPECIAL_2) //BloodrageAnimation
	blooddie = 0; 
...
switch(attack) {
	...
	case AT_BAIR: 
		if (window == 1 && window_timer == 10)
			sound_play(sound_get("swingmid"), 0, noone, 1, .9)
	break;
	case AT_USPECIAL: 
	...
	break;
	case AT_DSTRONG:
		if ((window == 3 || window == 7) && window_timer == 10)
			sound_play(sound_get("swingmid"))
	break;
}
