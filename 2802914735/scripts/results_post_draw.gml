...
if ("ayaka_initialized_victory_screen" not in self)
{
    ayaka_initialized_victory_screen = true;
    //defaults to prevent errors
    ayaka_victory_quote = "ermmmm that wasnt supposed to happen";
    ayaka_victory_icon = 0;
...
if (winner == player /* && uhc_batteries */)
{
    ...
    var quote_time  =   240;
    //Animate panel
    if ("ayaka_quote_current_pos_x" not in self)
    { ayaka_quote_current_pos_x = hide_pos_x; }
    
    //Must check with timing or if result boxes are open
    var diff = ((results_timer > quote_time && !someone_pressed) ? 
                    quote_pos_x : hide_pos_x) - ayaka_quote_current_pos_x;
    
    ayaka_quote_current_pos_x += sign(diff) 
                                * max(min(abs(diff), 5), abs(diff) * 0.25);
    
    //Draw panel
    if (ayaka_quote_current_pos_x > hide_pos_x && (string_length(ayaka_victory_quote) >= 1))
    {
        draw_sprite(sprite_get("victory_quote_bg"), 0, 
                    ayaka_quote_current_pos_x, quote_pos_y);
        draw_sprite_ext(sprite_get("victory_quote_playerflourish"),0,
                    ayaka_quote_current_pos_x, quote_pos_y, 1, 1, 0, get_player_hud_color( winner ), 1);
        draw_sprite_ext(sprite_get("stockicon"), ayaka_victory_icon, 
                    ayaka_quote_current_pos_x + 46, quote_pos_y + 10, 1.5, 1.5, 0, c_white, 1);
        draw_win_quote(ayaka_quote_current_pos_x+135, quote_pos_y+15, 
                        ayaka_victory_quote);
    }
}
...
#define get_victory_screen_data()
{
    ...
    with (asset_get("pHitBox")) if ("ayaka_victory_screen_array" in self)
    {
        data_array = ayaka_victory_screen_array;
    ...
    if (string_length(data_array[winner].status_quote) > 1)
    {
        ...
        ayaka_victory_quote = data_array[winner].status_quote;
        ayaka_victory_icon = data_array[best_player].icon;
    }
    else
    {
        ...
        ayaka_victory_quote = data_array[best_player].quote;
        ayaka_victory_icon = data_array[best_player].icon;
        if (string_length(ayaka_victory_quote) < 1)
        { ayaka_victory_quote = get_random_quote(); 
          ayaka_victory_icon = 0;    
        }
    }
