set_attack_value(AT_FTHROW, AG_SPRITE, sprite_get("fstrong"));
set_attack_value(AT_FTHROW, AG_NUM_WINDOWS, 5);
set_attack_value(AT_FTHROW, AG_CATEGORY, 2);
set_attack_value(AT_FTHROW, AG_OFF_LEDGE, 0);
set_attack_value(AT_FTHROW, AG_HURTBOX_SPRITE, sprite_get("fstrong"));


set_window_value(AT_FTHROW, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTHROW, 1, AG_WINDOW_LENGTH, 9*4);
set_window_value(AT_FTHROW, 1, AG_WINDOW_ANIM_FRAMES, 9);
set_window_value(AT_FTHROW, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTHROW, 1, AG_WINDOW_ANIM_FRAME_START, 10);


set_window_value(AT_FTHROW, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTHROW, 2, AG_WINDOW_LENGTH, 12);
set_window_value(AT_FTHROW, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTHROW, 2, AG_WINDOW_ANIM_FRAME_START, 19);
set_window_value(AT_FTHROW, 2, AG_WINDOW_ANIM_FRAMES, 2);

set_window_value(AT_FTHROW, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTHROW, 3, AG_WINDOW_LENGTH, 20);
set_window_value(AT_FTHROW, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTHROW, 3, AG_WINDOW_ANIM_FRAME_START, 21);
set_window_value(AT_FTHROW, 3, AG_WINDOW_ANIM_FRAMES, 4);

set_window_value(AT_FTHROW, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTHROW, 4, AG_WINDOW_LENGTH, 12);
set_window_value(AT_FTHROW, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTHROW, 4, AG_WINDOW_ANIM_FRAME_START, 26);
set_window_value(AT_FTHROW, 4, AG_WINDOW_ANIM_FRAMES, 2);

set_window_value(AT_FTHROW, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTHROW, 5, AG_WINDOW_LENGTH, 20);
set_window_value(AT_FTHROW, 5, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTHROW, 5, AG_WINDOW_ANIM_FRAME_START, 28);
set_window_value(AT_FTHROW, 5, AG_WINDOW_ANIM_FRAMES, 6);

set_num_hitboxes(AT_FTHROW, 3);


set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 2, HG_WINDOW, 5);
set_hitbox_value(AT_FTHROW, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FTHROW, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_X, -12);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_Y, -42);
set_hitbox_value(AT_FTHROW, 2, HG_WIDTH, 104);
set_hitbox_value(AT_FTHROW, 2, HG_HEIGHT, 84);
set_hitbox_value(AT_FTHROW, 2, HG_SHAPE, 0);
set_hitbox_value(AT_FTHROW, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_FTHROW, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_FTHROW, 2, HG_ANGLE, 140);
set_hitbox_value(AT_FTHROW, 2, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FTHROW, 2, HG_BASE_KNOCKBACK, 12);
set_hitbox_value(AT_FTHROW, 2, HG_BASE_HITPAUSE, 0);
set_hitbox_value(AT_FTHROW, 2, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_FTHROW, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_FTHROW, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_FTHROW, 2, HG_HITSTUN_MULTIPLIER, .7);
set_hitbox_value(AT_FTHROW, 2, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_GROUP, -1);

set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 1, HG_WINDOW, 3);
set_hitbox_value(AT_FTHROW, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FTHROW, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_X, 22);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_Y, -42);
set_hitbox_value(AT_FTHROW, 1, HG_WIDTH, 104);
set_hitbox_value(AT_FTHROW, 1, HG_HEIGHT, 84);
set_hitbox_value(AT_FTHROW, 1, HG_SHAPE, 0);
set_hitbox_value(AT_FTHROW, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_FTHROW, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_FTHROW, 1, HG_ANGLE, 40);
set_hitbox_value(AT_FTHROW, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FTHROW, 1, HG_BASE_KNOCKBACK, 12);
set_hitbox_value(AT_FTHROW, 1, HG_BASE_HITPAUSE, 0);
set_hitbox_value(AT_FTHROW, 1, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_FTHROW, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_FTHROW, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_FTHROW, 1, HG_HITSTUN_MULTIPLIER, .7);;
set_hitbox_value(AT_FTHROW, 1, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_GROUP, -1);


set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 4, HG_WINDOW, 3);
set_hitbox_value(AT_FTHROW, 4, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FTHROW, 4, HG_LIFETIME, 4);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_X, 22);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_Y, -42);
set_hitbox_value(AT_FTHROW, 4, HG_WIDTH, 104);
set_hitbox_value(AT_FTHROW, 4, HG_HEIGHT, 84);
set_hitbox_value(AT_FTHROW, 4, HG_SHAPE, 0);
set_hitbox_value(AT_FTHROW, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_FTHROW, 4, HG_DAMAGE, .1);
set_hitbox_value(AT_FTHROW, 4, HG_ANGLE, 50);
set_hitbox_value(AT_FTHROW, 4, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FTHROW, 4, HG_BASE_KNOCKBACK, 12);
set_hitbox_value(AT_FTHROW, 4, HG_BASE_HITPAUSE, 0);
set_hitbox_value(AT_FTHROW, 4, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_FTHROW, 4, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_FTHROW, 4, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_FTHROW, 4, HG_HITSTUN_MULTIPLIER, 1);;
set_hitbox_value(AT_FTHROW, 4, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_GROUP, -1);

