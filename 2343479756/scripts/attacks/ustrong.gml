set_attack_value(AT_USTRONG, AG_SPRITE, sprite_get("ustrong"));
set_attack_value(AT_USTRONG, AG_HURTBOX_SPRITE, sprite_get("ustrong_hurt"));
set_attack_value(AT_USTRONG, AG_NUM_WINDOWS, 7);
set_attack_value(AT_USTRONG, AG_STRONG_CHARGE_WINDOW, 1);

set_window_value(AT_USTRONG, 1, AG_WINDOW_LENGTH, 14);//16
set_window_value(AT_USTRONG, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_USTRONG, 1, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_USTRONG, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USTRONG, 1, AG_WINDOW_SFX, sound_get("swing_weak_1"));
set_window_value(AT_USTRONG, 1, AG_WINDOW_SFX_FRAME, 7);//8

set_window_value(AT_USTRONG, 2, AG_WINDOW_LENGTH, 8);
set_window_value(AT_USTRONG, 2, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_USTRONG, 2, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_USTRONG, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USTRONG, 2, AG_WINDOW_SFX, sound_get("swing_heavy_1"));
set_window_value(AT_USTRONG, 2, AG_WINDOW_SFX_FRAME, 6);

set_window_value(AT_USTRONG, 3, AG_WINDOW_LENGTH, 3);
set_window_value(AT_USTRONG, 3, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_USTRONG, 3, AG_WINDOW_ANIM_FRAMES, 1);

set_window_value(AT_USTRONG, 4, AG_WINDOW_LENGTH, 8);
set_window_value(AT_USTRONG, 4, AG_WINDOW_ANIM_FRAME_START, 7);
set_window_value(AT_USTRONG, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USTRONG, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USTRONG, 4, AG_WINDOW_SFX, sound_get("swing_heavy_2"));
set_window_value(AT_USTRONG, 4, AG_WINDOW_SFX_FRAME, 6);

set_window_value(AT_USTRONG, 5, AG_WINDOW_LENGTH, 3);
set_window_value(AT_USTRONG, 5, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_USTRONG, 5, AG_WINDOW_ANIM_FRAMES, 1);

set_window_value(AT_USTRONG, 6, AG_WINDOW_LENGTH, 14);
set_window_value(AT_USTRONG, 6, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_USTRONG, 6, AG_WINDOW_ANIM_FRAMES, 3);

set_window_value(AT_USTRONG, 7, AG_WINDOW_LENGTH, 9);
set_window_value(AT_USTRONG, 7, AG_WINDOW_ANIM_FRAME_START, 12);
set_window_value(AT_USTRONG, 7, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_USTRONG, 7, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_USTRONG, 5);

//main natsumi hit
set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 1, HG_WINDOW, 3);
set_hitbox_value(AT_USTRONG, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_X, 5);//13
set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_Y, -57);
set_hitbox_value(AT_USTRONG, 1, HG_WIDTH, 30);//75
set_hitbox_value(AT_USTRONG, 1, HG_HEIGHT, 83);
set_hitbox_value(AT_USTRONG, 1, HG_DAMAGE, 7);//10
set_hitbox_value(AT_USTRONG, 1, HG_ANGLE, 110);
set_hitbox_value(AT_USTRONG, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_USTRONG, 1, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_USTRONG, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_USTRONG, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_USTRONG, 1, HG_HITPAUSE_SCALING, 0.6);
set_hitbox_value(AT_USTRONG, 1, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_USTRONG, 1, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_USTRONG, 1, HG_HIT_SFX, sound_get("hit_med_4"));

//sub natsumi hit
set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 2, HG_WINDOW, 3);
set_hitbox_value(AT_USTRONG, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_X, -31);
set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_Y, -78);
set_hitbox_value(AT_USTRONG, 2, HG_WIDTH, 89);
set_hitbox_value(AT_USTRONG, 2, HG_HEIGHT, 69);
set_hitbox_value(AT_USTRONG, 2, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 2, HG_DAMAGE, 5);//8
set_hitbox_value(AT_USTRONG, 2, HG_ANGLE, 70);
//set_hitbox_value(AT_USTRONG, 2, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_USTRONG, 2, HG_PRIORITY, 1);//2
set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_USTRONG, 2, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_USTRONG, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_USTRONG, 2, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_USTRONG, 2, HG_HITPAUSE_SCALING, 0.6);
set_hitbox_value(AT_USTRONG, 2, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_USTRONG, 2, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_USTRONG, 2, HG_HIT_SFX, sound_get("hit_med_4"));

//ultra far front hit (of main)
set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 5, HG_WINDOW, 3);
set_hitbox_value(AT_USTRONG, 5, HG_LIFETIME, 3);
set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_X, 35);
set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_Y, -50);
set_hitbox_value(AT_USTRONG, 5, HG_WIDTH, 30);//75
set_hitbox_value(AT_USTRONG, 5, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 5, HG_DAMAGE, 7);//10
set_hitbox_value(AT_USTRONG, 5, HG_ANGLE, 145);//140
set_hitbox_value(AT_USTRONG, 5, HG_PRIORITY, 3);//1
set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_USTRONG, 5, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_USTRONG, 5, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_USTRONG, 5, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_USTRONG, 5, HG_HITPAUSE_SCALING, 0.6);
set_hitbox_value(AT_USTRONG, 5, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_USTRONG, 5, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_USTRONG, 5, HG_HIT_SFX, sound_get("hit_med_4"));

//main sign hit
set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 3, HG_WINDOW, 5);
set_hitbox_value(AT_USTRONG, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_X, -43);
set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_Y, -89);
set_hitbox_value(AT_USTRONG, 3, HG_WIDTH, 64);//60
set_hitbox_value(AT_USTRONG, 3, HG_HEIGHT, 176);
set_hitbox_value(AT_USTRONG, 3, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 3, HG_DAMAGE, 8);//3
set_hitbox_value(AT_USTRONG, 3, HG_ANGLE, 90);//80
set_hitbox_value(AT_USTRONG, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_GROUP, 2);
set_hitbox_value(AT_USTRONG, 3, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_USTRONG, 3, HG_KNOCKBACK_SCALING, 1.1);//1.1
set_hitbox_value(AT_USTRONG, 3, HG_BASE_HITPAUSE, 10);//8
set_hitbox_value(AT_USTRONG, 3, HG_HITPAUSE_SCALING, 0.7);
set_hitbox_value(AT_USTRONG, 3, HG_VISUAL_EFFECT, 306);
set_hitbox_value(AT_USTRONG, 3, HG_HIT_SFX, sound_get("hit_strong_1"));

//sub sign hit
set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 4, HG_WINDOW, 6);
set_hitbox_value(AT_USTRONG, 4, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_X, -44);
set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_Y, -78);
set_hitbox_value(AT_USTRONG, 4, HG_WIDTH, 41);
set_hitbox_value(AT_USTRONG, 4, HG_HEIGHT, 149);
set_hitbox_value(AT_USTRONG, 4, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 4, HG_DAMAGE, 7);//2
set_hitbox_value(AT_USTRONG, 4, HG_ANGLE, 85);
set_hitbox_value(AT_USTRONG, 4, HG_PRIORITY, 4);
set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_GROUP, 2);
set_hitbox_value(AT_USTRONG, 4, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_USTRONG, 4, HG_KNOCKBACK_SCALING, 0.7);
set_hitbox_value(AT_USTRONG, 4, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_USTRONG, 4, HG_HITPAUSE_SCALING, 0.6);
set_hitbox_value(AT_USTRONG, 4, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_USTRONG, 4, HG_HIT_SFX, sound_get("hit_strong_1"));




















