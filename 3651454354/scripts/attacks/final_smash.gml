atk = 49;

set_attack_value(49, AG_CATEGORY, 2);
set_attack_value(49, AG_SPRITE, sprite_get("uspecial"));
set_attack_value(49, AG_HURTBOX_SPRITE, sprite_get("hurtbox"));
set_attack_value(49, AG_NUM_WINDOWS, 7);
set_attack_value(49, AG_OFF_LEDGE, 1);

set_window_value(49, 1, AG_WINDOW_TYPE, 1);
set_window_value(49, 1, AG_WINDOW_LENGTH, 6);
set_window_value(49, 1, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(49, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(49, 1, AG_WINDOW_VSPEED, 0);
set_window_value(49, 1, AG_WINDOW_HSPEED, 0);
set_window_value(49, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(49, 1, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(49, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(49, 1, AG_WINDOW_SFX, sound_get("snd_swingweak"));

set_window_value(49, 2, AG_WINDOW_TYPE, 1);
set_window_value(49, 2, AG_WINDOW_LENGTH, 600);
set_window_value(49, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(49, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(49, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(49, 2, AG_WINDOW_SFX, sound_get("snd_joker_byebye"));

set_window_value(49, 3, AG_WINDOW_TYPE, 1);
set_window_value(49, 3, AG_WINDOW_LENGTH, 64);
set_window_value(49, 3, AG_WINDOW_ANIM_FRAMES, 8);
set_window_value(49, 3, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(49, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(49, 3, AG_WINDOW_SFX_FRAME, 15);
set_window_value(49, 3, AG_WINDOW_SFX, sound_get("snd_joker_neochaos"));

set_window_value(49, 4, AG_WINDOW_TYPE, 1);
set_window_value(49, 4, AG_WINDOW_LENGTH, 6);
set_window_value(49, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(49, 4, AG_WINDOW_ANIM_FRAME_START, 3);

set_window_value(49, 5, AG_WINDOW_TYPE, 1);
set_window_value(49, 5, AG_WINDOW_LENGTH, 500);
set_window_value(49, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(49, 5, AG_WINDOW_ANIM_FRAME_START, 3);

set_window_value(49, 6, AG_WINDOW_TYPE, 1);
set_window_value(49, 6, AG_WINDOW_LENGTH, 120);
set_window_value(49, 6, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(49, 6, AG_WINDOW_ANIM_FRAME_START, 3);

set_window_value(49, 7, AG_WINDOW_TYPE, 1);
set_window_value(49, 7, AG_WINDOW_LENGTH, 24);
set_window_value(49, 7, AG_WINDOW_ANIM_FRAMES, 8);
set_window_value(49, 7, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(49, 7, AG_WINDOW_HAS_SFX, 1);
set_window_value(49, 7, AG_WINDOW_SFX_FRAME, 1);
set_window_value(49, 7, AG_WINDOW_SFX, sound_get("snd_spearappear"));

set_num_hitboxes(49, 3);

set_hitbox_value(49, 1, HG_HITBOX_TYPE, 2);
set_hitbox_value(49, 1, HG_LIFETIME, 300);
//set_hitbox_value(49, 1, HG_WIDTH, 45);
//set_hitbox_value(49, 1, HG_HEIGHT, 95);
set_hitbox_value(49, 1, HG_PROJECTILE_VSPEED, -3);
set_hitbox_value(49, 1, HG_DAMAGE, 2);
set_hitbox_value(49, 1, HG_ANGLE, 90);
set_hitbox_value(49, 1, HG_HIT_SFX, sound_get("snd_hurt"));
set_hitbox_value(49, 1, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(49, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(49, 1, HG_BASE_HITPAUSE, 10);
set_hitbox_value(49, 1, HG_VISUAL_EFFECT, 304);
set_hitbox_value(49, 1, HG_PRIORITY, 1);
set_hitbox_value(49, 1, HG_ANGLE_FLIPPER, 3);
set_hitbox_value(49, 1, HG_PROJECTILE_MASK, sprite_get("intro_knife"));
set_hitbox_value(49, 1, HG_PROJECTILE_SPRITE, sprite_get("intro_knife"));
set_hitbox_value(49, 1, HG_PROJECTILE_COLLISION_SPRITE, sprite_get("knife_hbox"));
//set_hitbox_value(49, 1, HG_PROJECTILE_GROUND_BEHAVIOR, -1);
set_hitbox_value(49, 1, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(49, 1, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(49, 1, HG_PROJECTILE_DOES_NOT_REFLECT, true );
set_hitbox_value(49, 1, HG_PROJECTILE_IS_TRANSCENDENT, true );
set_hitbox_value(49, 1, HG_PROJECTILE_PLASMA_SAFE, true );
set_hitbox_value(49, 1, HG_PROJECTILE_UNBASHABLE, true );
set_hitbox_value(49, 1, HG_PROJECTILE_DESTROY_EFFECT, 1 );
set_hitbox_value(49, 1, HG_PROJECTILE_PARRY_STUN, 1 );
set_hitbox_value(49, 1, HG_PROJECTILE_ANIM_SPEED, 0.0 );

set_hitbox_value(49, 2, HG_HITBOX_TYPE, 2);
set_hitbox_value(49, 2, HG_LIFETIME, 22);
//set_hitbox_value(49, 2, HG_HITBOX_X, 0);
//set_hitbox_value(49, 2, HG_HITBOX_Y, 50);
//set_hitbox_value(49, 2, HG_SHAPE, 1);
//set_hitbox_value(49, 2, HG_WIDTH, 64);
//set_hitbox_value(49, 2, HG_HEIGHT, 1520);
set_hitbox_value(49, 2, HG_PRIORITY, 1);
set_hitbox_value(49, 2, HG_DAMAGE, 3);
set_hitbox_value(49, 2, HG_ANGLE, 90);
set_hitbox_value(49, 2, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(49, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(49, 2, HG_BASE_HITPAUSE, 10);
set_hitbox_value(49, 2, HG_PROJECTILE_VSPEED, 999);
set_hitbox_value(49, 2, HG_HITPAUSE_SCALING, .6);
//set_hitbox_value(AT_DSPECIAL, 2, HG_HIT_LOCKOUT, 5);
set_hitbox_value(49, 2, HG_ANGLE_FLIPPER, 3);
set_hitbox_value(49, 2, HG_HIT_SFX, sound_get("snd_hurt1"));
set_hitbox_value(49, 2, HG_VISUAL_EFFECT_Y_OFFSET, -9999);
set_hitbox_value(49, 2, HG_PROJECTILE_SPRITE, sprite_get("beam"));
set_hitbox_value(49, 2, HG_PROJECTILE_MASK, sprite_get("beam"));
set_hitbox_value(49, 2, HG_PROJECTILE_ANIM_SPEED, 0.5);
set_hitbox_value(49, 2, HG_PROJECTILE_AIR_FRICTION, 0.03);
set_hitbox_value(49, 2, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(49, 2, HG_PROJECTILE_GROUND_BEHAVIOR, 0);
set_hitbox_value(49, 2, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(49, 2, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(49, 2, HG_PROJECTILE_DOES_NOT_REFLECT, 1);
set_hitbox_value(49, 2, HG_PROJECTILE_IS_TRANSCENDENT, 1);

set_hitbox_value(49, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(49, 3, HG_WINDOW, 0);
set_hitbox_value(49, 3, HG_LIFETIME, 4);
set_hitbox_value(49, 3, HG_HITBOX_X, 32);
set_hitbox_value(49, 3, HG_WIDTH, 1000);
set_hitbox_value(49, 3, HG_HEIGHT, 600);
set_hitbox_value(49, 3, HG_SHAPE, 2);
set_hitbox_value(49, 3, HG_PRIORITY, 3);
set_hitbox_value(49, 3, HG_DAMAGE, 20);
set_hitbox_value(49, 3, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(49, 3, HG_KNOCKBACK_SCALING, 1.2);
set_hitbox_value(49, 3, HG_BASE_HITPAUSE, 60);
set_hitbox_value(49, 3, HG_HITPAUSE_SCALING, 0.6);
set_hitbox_value(49, 3, HG_ANGLE, 45);
set_hitbox_value(49, 3, HG_ANGLE_FLIPPER, 3);
set_hitbox_value(49, 3, HG_HIT_SFX, sound_get("snd_damage_strong"));
set_hitbox_value(49, 3, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(49, 3, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(49, 3, HG_PROJECTILE_MASK, -1);
set_hitbox_value(49, 3, HG_PROJECTILE_HSPEED, 10);
set_hitbox_value(49, 3, HG_PROJECTILE_VSPEED, 0);
set_hitbox_value(49, 3, HG_PROJECTILE_ANIM_SPEED, 0.3);
set_hitbox_value(49, 3, HG_PROJECTILE_DESTROY_EFFECT, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_DOES_NOT_REFLECT, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_UNBASHABLE, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_PLASMA_SAFE, 1);
set_hitbox_value(49, 3, HG_PROJECTILE_WALL_BEHAVIOR, 1);