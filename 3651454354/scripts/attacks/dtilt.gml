set_attack_value(AT_DTILT, AG_SPRITE, sprite_get("dtilt"));
set_attack_value(AT_DTILT, AG_NUM_WINDOWS, 3);
set_attack_value(AT_DTILT, AG_HURTBOX_SPRITE, sprite_get("dtilt_hurt"));

set_window_value(AT_DTILT, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DTILT, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX, sound_get("snd_swing_quick_high"));
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX_FRAME, 1);

set_window_value(AT_DTILT, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 2, AG_WINDOW_LENGTH, 13);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAME_START, 1);

set_window_value(AT_DTILT, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 3, AG_WINDOW_LENGTH, 6);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAME_START, 4);

set_num_hitboxes(AT_DTILT,1);

set_hitbox_value(AT_DTILT, 1, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW, 9);
set_hitbox_value(AT_DTILT, 1, HG_LIFETIME, 30);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_X, 44);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DTILT, 1, HG_WIDTH, 20);
set_hitbox_value(AT_DTILT, 1, HG_HEIGHT, 30);
set_hitbox_value(AT_DTILT, 1, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DTILT, 1, HG_DAMAGE, 4);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE, 80);
set_hitbox_value(AT_DTILT, 1, HG_BASE_KNOCKBACK, 7.5);;
set_hitbox_value(AT_DTILT, 1, HG_KNOCKBACK_SCALING, .2);;
set_hitbox_value(AT_DTILT, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 1, HG_HITPAUSE_SCALING, .15);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 30);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_MASK, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_SPRITE, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_VSPEED, 0.5);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 1, HG_FINAL_BASE_KNOCKBACK, 4.5);
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_clairen_hit_weak"));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_PARRY_STUN, true);

set_hitbox_value(AT_DTILT, 2, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW, 9);
set_hitbox_value(AT_DTILT, 2, HG_LIFETIME, 30);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_X, 44);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DTILT, 2, HG_EXTRA_CAMERA_SHAKE, -1);
set_hitbox_value(AT_DTILT, 2, HG_WIDTH, 20);
set_hitbox_value(AT_DTILT, 2, HG_HEIGHT, 30);
set_hitbox_value(AT_DTILT, 2, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DTILT, 2, HG_DAMAGE, 3);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE, 80);
set_hitbox_value(AT_DTILT, 2, HG_BASE_KNOCKBACK, 7.5);;
set_hitbox_value(AT_DTILT, 2, HG_KNOCKBACK_SCALING, .2);;
set_hitbox_value(AT_DTILT, 2, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 2, HG_HITPAUSE_SCALING, .15);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 30);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_MASK, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_SPRITE, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_VSPEED, 0.5);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 2, HG_FINAL_BASE_KNOCKBACK, 4.5);
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, asset_get("sfx_clairen_hit_weak"));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_PARRY_STUN, true);

set_hitbox_value(AT_DTILT, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW, 9);
set_hitbox_value(AT_DTILT, 3, HG_LIFETIME, 30);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_X, 44);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DTILT, 3, HG_EXTRA_CAMERA_SHAKE, -1);
set_hitbox_value(AT_DTILT, 3, HG_WIDTH, 20);
set_hitbox_value(AT_DTILT, 3, HG_HEIGHT, 30);
set_hitbox_value(AT_DTILT, 3, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 3, HG_PRIORITY, 2);
set_hitbox_value(AT_DTILT, 3, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE, 80);
set_hitbox_value(AT_DTILT, 3, HG_BASE_KNOCKBACK, 7.5);;
set_hitbox_value(AT_DTILT, 3, HG_KNOCKBACK_SCALING, .2);;
set_hitbox_value(AT_DTILT, 3, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 3, HG_HITPAUSE_SCALING, .15);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_X_OFFSET, 30);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_MASK, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_SPRITE, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_VSPEED, 0.5);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 3, HG_FINAL_BASE_KNOCKBACK, 4.5);
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_clairen_hit_weak"));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_PARRY_STUN, true);

set_hitbox_value(AT_DTILT, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW, 9);
set_hitbox_value(AT_DTILT, 4, HG_LIFETIME, 30);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_X, 44);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DTILT, 4, HG_EXTRA_CAMERA_SHAKE, -1);
set_hitbox_value(AT_DTILT, 4, HG_WIDTH, 20);
set_hitbox_value(AT_DTILT, 4, HG_HEIGHT, 30);
set_hitbox_value(AT_DTILT, 4, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_DTILT, 4, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 4, HG_ANGLE, 80);
set_hitbox_value(AT_DTILT, 4, HG_BASE_KNOCKBACK, 7.5);;
set_hitbox_value(AT_DTILT, 4, HG_KNOCKBACK_SCALING, .2);;
set_hitbox_value(AT_DTILT, 4, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 4, HG_HITPAUSE_SCALING, .15);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT_X_OFFSET, 30);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 4, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_MASK, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_SPRITE, sprite_get("diamond_proj"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_VSPEED, 0.5);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 4, HG_FINAL_BASE_KNOCKBACK, 4.5);
set_hitbox_value(AT_DTILT, 4, HG_HIT_SFX, asset_get("sfx_clairen_hit_weak"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_PARRY_STUN, true);