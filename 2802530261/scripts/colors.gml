// DEFAULT COLOR

// helmet
set_color_profile_slot( 0, 0, 228, 59, 68 );
set_color_profile_slot_range( 0, 5, 2, 28 );

// limbs
set_color_profile_slot( 0, 1, 0, 149, 233 );
set_color_profile_slot_range( 1, 9, 14, 38 );

// body
set_color_profile_slot( 0, 2, 192, 203, 220 );
set_color_profile_slot_range( 2, 2, 11, 16 );

// gun
set_color_profile_slot( 0, 3, 25, 60, 62 );
set_color_profile_slot_range( 3, 48, 18, 17 );

// gun lights
set_color_profile_slot( 0, 4, 254, 231, 97 );
set_color_profile_slot_range( 4, 16, 19, 1 );

// visor
set_color_profile_slot( 0, 5, 99, 199, 77 );
set_color_profile_slot_range( 5, 20, 26, 25 );

// shinespark
set_color_profile_slot( 0, 6, 129, 82, 255 );
set_color_profile_slot_range( 6, 1, 33, 51 );

// gunship
set_color_profile_slot( 0, 7, 246, 117, 122 );
set_color_profile_slot_range( 7, 64, 5, 55 );


// ALTERNATE COLORS
set_num_palettes( 24 );

// Varia Suit
set_color_profile_slot( 1, 0, 228, 59, 68 ); //helmet
set_color_profile_slot( 1, 1, 255, 149, 0 ); //limbs
set_color_profile_slot( 1, 2, 218, 220, 192 ); //body
set_color_profile_slot( 1, 3, 25, 60, 62 ); //gun
set_color_profile_slot( 1, 4, 254, 231, 97 ); //gun lights
set_color_profile_slot( 1, 5, 99, 199, 77 ); //visor
set_color_profile_slot( 1, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 1, 7, 246, 117, 122 ); //gunship

// Gravity Suit
set_color_profile_slot( 2, 0, 228, 59, 68 ); //helmet
set_color_profile_slot( 2, 1, 133, 43, 179 ); //limbs
set_color_profile_slot( 2, 2, 203, 192, 220 ); //body
set_color_profile_slot( 2, 3, 25, 60, 62 ); //gun
set_color_profile_slot( 2, 4, 115, 255, 0 ); //gun lights
set_color_profile_slot( 2, 5, 99, 199, 77 ); //visor
set_color_profile_slot( 2, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 2, 7, 246, 117, 122 ); //gunship

// Super Power Suit
set_color_profile_slot( 3, 0, 181, 38, 53 ); //helmet
set_color_profile_slot( 3, 1, 255, 149, 0 ); //limbs
set_color_profile_slot( 3, 2, 255, 191, 0 ); //body
set_color_profile_slot( 3, 3, 25, 60, 62 ); //gun
set_color_profile_slot( 3, 4, 25, 60, 62 ); //gun lights
set_color_profile_slot( 3, 5, 99, 199, 77 ); //visor
set_color_profile_slot( 3, 6, 254, 174, 52 ); //shinespark
set_color_profile_slot( 3, 7, 254, 231, 97 ); //gunship

// Super Varia Suit
set_color_profile_slot( 4, 0, 181, 38, 53 ); //helmet
set_color_profile_slot( 4, 1, 255, 85, 0 ); //limbs
set_color_profile_slot( 4, 2, 255, 191, 0 ); //body
set_color_profile_slot( 4, 3, 25, 60, 62 ); //gun
set_color_profile_slot( 4, 4, 25, 60, 62 ); //gun lights
set_color_profile_slot( 4, 5, 99, 199, 77 ); //visor
set_color_profile_slot( 4, 6, 254, 174, 52 ); //shinespark
set_color_profile_slot( 4, 7, 254, 231, 97 ); //gunship

// Super Gravity Suit
set_color_profile_slot( 5, 0, 181, 38, 53 ); //helmet
set_color_profile_slot( 5, 1, 191, 56, 186 ); //limbs
set_color_profile_slot( 5, 2, 255, 191, 0 ); //body
set_color_profile_slot( 5, 3, 25, 60, 62 ); //gun
set_color_profile_slot( 5, 4, 25, 60, 62 ); //gun lights
set_color_profile_slot( 5, 5, 0, 149, 233 ); //visor
set_color_profile_slot( 5, 6, 254, 174, 52 ); //shinespark
set_color_profile_slot( 5, 7, 254, 231, 97 ); //gunship

// Fusion Power Suit
set_color_profile_slot( 6, 0, 228, 59, 104 ); //helmet
set_color_profile_slot( 6, 1, 43, 106, 255 ); //limbs
set_color_profile_slot( 6, 2, 255, 191, 0 ); //body
set_color_profile_slot( 6, 3, 14, 64, 31 ); //gun
set_color_profile_slot( 6, 4, 139, 166, 152 ); //gun lights
set_color_profile_slot( 6, 5, 0, 149, 233 ); //visor
set_color_profile_slot( 6, 6, 255, 160, 218 ); //shinespark
set_color_profile_slot( 6, 7, 246, 117, 122 ); //gunship

// Fusion Varia Suit
set_color_profile_slot( 7, 0, 228, 59, 104 ); //helmet
set_color_profile_slot( 7, 1, 134, 237, 31 ); //limbs
set_color_profile_slot( 7, 2, 222, 98, 197 ); //body
set_color_profile_slot( 7, 3, 14, 64, 31 ); //gun
set_color_profile_slot( 7, 4, 139, 166, 152 ); //gun lights
set_color_profile_slot( 7, 5, 0, 149, 233 ); //visor
set_color_profile_slot( 7, 6, 255, 160, 218 ); //shinespark
set_color_profile_slot( 7, 7, 246, 117, 122 ); //gunship

// Fusion Gravity Suit
set_color_profile_slot( 8, 0, 228, 59, 104 ); //helmet
set_color_profile_slot( 8, 1, 140, 0, 255 ); //limbs
set_color_profile_slot( 8, 2, 110, 202, 255 ); //body
set_color_profile_slot( 8, 3, 14, 64, 31 ); //gun
set_color_profile_slot( 8, 4, 139, 166, 152 ); //gun lights
set_color_profile_slot( 8, 5, 0, 149, 233 ); //visor
set_color_profile_slot( 8, 6, 255, 160, 218 ); //shinespark
set_color_profile_slot( 8, 7, 246, 117, 122 ); //gunship

// White EMMI
set_color_profile_slot( 9, 0, 255, 255, 255 ); //helmet
set_color_profile_slot( 9, 1, 204, 204, 204 ); //limbs
set_color_profile_slot( 9, 2, 66, 66, 66 ); //body
set_color_profile_slot( 9, 3, 204, 204, 204 ); //gun
set_color_profile_slot( 9, 4, 181, 38, 53 ); //gun lights
set_color_profile_slot( 9, 5, 181, 38, 53 ); //visor
set_color_profile_slot( 9, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 9, 7, 255, 255, 255 ); //gunship

// Yellow EMMI
set_color_profile_slot( 10, 0, 255, 191, 0 ); //helmet
set_color_profile_slot( 10, 1, 255, 153, 0 ); //limbs
set_color_profile_slot( 10, 2, 66, 66, 66 ); //body
set_color_profile_slot( 10, 3, 140, 85, 14 ); //gun
set_color_profile_slot( 10, 4, 255, 250, 161 ); //gun lights
set_color_profile_slot( 10, 5, 255, 250, 161 ); //visor
set_color_profile_slot( 10, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 10, 7, 254, 231, 97 ); //gunship

// Blue EMMI
set_color_profile_slot( 11, 0, 81, 53, 227 ); //helmet
set_color_profile_slot( 11, 1, 81, 53, 227 ); //limbs
set_color_profile_slot( 11, 2, 66, 66, 66 ); //body
set_color_profile_slot( 11, 3, 81, 53, 227 ); //gun
set_color_profile_slot( 11, 4, 181, 38, 53 ); //gun lights
set_color_profile_slot( 11, 5, 181, 38, 53 ); //visor
set_color_profile_slot( 11, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 11, 7, 81, 53, 227 ); //gunship

// Green EMMI
set_color_profile_slot( 12, 0, 134, 237, 31 ); //helmet
set_color_profile_slot( 12, 1, 134, 237, 31 ); //limbs
set_color_profile_slot( 12, 2, 66, 66, 66 ); //body
set_color_profile_slot( 12, 3, 111, 204, 18 ); //gun
set_color_profile_slot( 12, 4, 110, 202, 255 ); //gun lights
set_color_profile_slot( 12, 5, 110, 202, 255 ); //visor
set_color_profile_slot( 12, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 12, 7, 134, 237, 31 ); //gunship

// Red EMMI
set_color_profile_slot( 13, 0, 228, 59, 68 ); //helmet
set_color_profile_slot( 13, 1, 228, 59, 68 ); //limbs
set_color_profile_slot( 13, 2, 66, 66, 66 ); //body
set_color_profile_slot( 13, 3, 125, 1, 15 ); //gun
set_color_profile_slot( 13, 4, 255, 85, 0 ); //gun lights
set_color_profile_slot( 13, 5, 255, 85, 0 ); //visor
set_color_profile_slot( 13, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 13, 7, 228, 59, 68 ); //gunship

// Cyan EMMI
set_color_profile_slot( 14, 0, 45, 247, 234 ); //helmet
set_color_profile_slot( 14, 1, 45, 247, 234 ); //limbs
set_color_profile_slot( 14, 2, 66, 66, 66 ); //body
set_color_profile_slot( 14, 3, 45, 247, 234 ); //gun
set_color_profile_slot( 14, 4, 140, 213, 255 ); //gun lights
set_color_profile_slot( 14, 5, 140, 213, 255 ); //visor
set_color_profile_slot( 14, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 14, 7, 45, 247, 234 ); //gunship

// The Mandalorian
set_color_profile_slot( 15, 0, 204, 204, 204 ); //helmet
set_color_profile_slot( 15, 1, 107, 51, 31 ); //limbs
set_color_profile_slot( 15, 2, 79, 58, 50 ); //body
set_color_profile_slot( 15, 3, 0, 0, 0 ); //gun
set_color_profile_slot( 15, 4, 107, 51, 31 ); //gun lights
set_color_profile_slot( 15, 5, 0, 0, 0 ); //visor
set_color_profile_slot( 15, 6, 181, 38, 53 ); //shinespark
set_color_profile_slot( 15, 7, 204, 204, 204 ); //gunship

// Phazon Suit
set_color_profile_slot( 16, 0, 140, 140, 140 ); //helmet
set_color_profile_slot( 16, 1, 66, 66, 66 ); //limbs
set_color_profile_slot( 16, 2, 140, 140, 140 ); //body
set_color_profile_slot( 16, 3, 0, 70, 79 ); //gun
set_color_profile_slot( 16, 4, 0, 149, 233 ); //gun lights
set_color_profile_slot( 16, 5, 181, 38, 53 ); //visor
set_color_profile_slot( 16, 6, 45, 247, 234 ); //shinespark
set_color_profile_slot( 16, 7, 254, 231, 97 ); //gunship

// Ridley
set_color_profile_slot( 17, 0, 107, 98, 140 ); //helmet
set_color_profile_slot( 17, 1, 131, 104, 140 ); //limbs
set_color_profile_slot( 17, 2, 107, 98, 140 ); //body
set_color_profile_slot( 17, 3, 107, 98, 140 ); //gun
set_color_profile_slot( 17, 4, 140, 83, 124 ); //gun lights
set_color_profile_slot( 17, 5, 194, 179, 96 ); //visor
set_color_profile_slot( 17, 6, 228, 59, 68 ); //shinespark
set_color_profile_slot( 17, 7, 140, 83, 124 ); //gunship

// Dark Samus
set_color_profile_slot( 18, 0, 38, 43, 68 ); //helmet
set_color_profile_slot( 18, 1, 38, 43, 68 ); //limbs
set_color_profile_slot( 18, 2, 38, 43, 68 ); //body
set_color_profile_slot( 18, 3, 20, 23, 36 ); //gun
set_color_profile_slot( 18, 4, 0, 149, 233 ); //gun lights
set_color_profile_slot( 18, 5, 0, 149, 233 ); //visor
set_color_profile_slot( 18, 6, 0, 149, 233 ); //shinespark
set_color_profile_slot( 18, 7, 38, 43, 68 ); //gunship

// Metroid Suit
set_color_profile_slot( 19, 0, 92, 163, 54 ); //helmet
set_color_profile_slot( 19, 1, 92, 163, 54 ); //limbs
set_color_profile_slot( 19, 2, 105, 93, 93 ); //body
set_color_profile_slot( 19, 3, 68, 120, 40 ); //gun
set_color_profile_slot( 19, 4, 228, 59, 68 ); //gun lights
set_color_profile_slot( 19, 5, 228, 59, 68 ); //visor
set_color_profile_slot( 19, 6, 129, 82, 255 ); //shinespark
set_color_profile_slot( 19, 7, 246, 117, 122 ); //gunship

// NES
set_color_profile_slot( 20, 0, 225, 40, 0 ); //helmet
set_color_profile_slot( 20, 1, 252, 146, 46 ); //limbs
set_color_profile_slot( 20, 2, 252, 146, 46 ); //body
set_color_profile_slot( 20, 3, 61, 147, 0 ); //gun
set_color_profile_slot( 20, 4, 252, 146, 46 ); //gun lights
set_color_profile_slot( 20, 5, 61, 147, 0 ); //visor
set_color_profile_slot( 20, 6, 252, 146, 46 ); //shinespark
set_color_profile_slot( 20, 7, 225, 40, 0 ); //gunship

// Metroid II
set_color_profile_slot( 21, 0, 165, 165, 165 ); //helmet
set_color_profile_slot( 21, 1, 82, 82, 82 ); //limbs
set_color_profile_slot( 21, 2, 255, 255, 255 ); //body
set_color_profile_slot( 21, 3, 82, 82, 82 ); //gun
set_color_profile_slot( 21, 4, 165, 165, 165 ); //gun lights
set_color_profile_slot( 21, 5, 255, 255, 255 ); //visor
set_color_profile_slot( 21, 6, 165, 165, 165 ); //shinespark
set_color_profile_slot( 21, 7, 255, 255, 255 ); //gunship

// Early Access
set_color_profile_slot( 22, 0, 83, 122, 62 ); //helmet
set_color_profile_slot( 22, 1, 35, 67, 49 ); //limbs
set_color_profile_slot( 22, 2, 211, 226, 154 ); //body
set_color_profile_slot( 22, 3, 35, 67, 49 ); //gun
set_color_profile_slot( 22, 4, 83, 122, 62 ); //gun lights
set_color_profile_slot( 22, 5, 211, 226, 154 ); //visor
set_color_profile_slot( 22, 6, 211, 226, 154 ); //shinespark
set_color_profile_slot( 22, 7, 167, 186, 74 ); //gunship

// Abyss
set_color_profile_slot( 23, 0, 86, 84, 146 ); //helmet
set_color_profile_slot( 23, 1, 79, 61, 94 ); //limbs
set_color_profile_slot( 23, 2, 166, 172, 214 ); //body
set_color_profile_slot( 23, 3, 79, 61, 94 ); //gun
set_color_profile_slot( 23, 4, 220, 113, 255 ); //gun lights
set_color_profile_slot( 23, 5, 220, 113, 255 ); //visor
set_color_profile_slot( 23, 6, 220, 113, 255 ); //shinespark
set_color_profile_slot( 23, 7, 86, 84, 146 ); //gunship


/* This is a comment used by that one RoA colors.gml generator tool to store palette data. You can safely keep it in your colors.gml if you plan to re-use the tool later, or safely remove it if you don't.
=== BEGIN JSON PALETTE ===
{"formatversion":1,"data":[{"name":"helmet","colors":[{"r":228,"g":59,"b":68,"main":true},{"r":158,"g":40,"b":53}]},{"name":"limbs","colors":[{"r":0,"g":149,"b":233,"main":true},{"r":18,"g":78,"b":137}]},{"name":"body","colors":[{"r":192,"g":203,"b":220,"main":true},{"r":139,"g":155,"b":180}]},{"name":"gun","colors":[{"r":25,"g":60,"b":62,"main":true},{"r":58,"g":68,"b":102},{"r":38,"g":43,"b":68}]},{"name":"gun lights","colors":[{"r":254,"g":231,"b":97,"main":true},{"r":254,"g":174,"b":52}]},{"name":"visor","colors":[{"r":180,"g":255,"b":164},{"r":62,"g":137,"b":72},{"r":99,"g":199,"b":77,"main":true}]},{"name":"shinespark","colors":[{"r":129,"g":82,"b":255,"main":true},{"r":62,"g":0,"b":227},{"r":35,"g":0,"b":128}]},{"name":"gunship","colors":[{"r":246,"g":117,"b":122,"main":true},{"r":181,"g":80,"b":136},{"r":104,"g":56,"b":108}]}]}
=== END JSON PALETTE ===
*/
