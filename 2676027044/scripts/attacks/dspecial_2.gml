set_attack_value(AT_DSPECIAL_2, AG_CATEGORY, 2);
set_attack_value(AT_DSPECIAL_2, AG_SPRITE, sprite_get("willo_ignite"));
set_attack_value(AT_DSPECIAL_2, AG_NUM_WINDOWS, 3);
set_attack_value(AT_DSPECIAL_2, AG_OFF_LEDGE, 1);
set_attack_value(AT_DSPECIAL_2, AG_AIR_SPRITE, sprite_get("willo_ignite"));
set_attack_value(AT_DSPECIAL_2, AG_HURTBOX_SPRITE, sprite_get("dspecial_hurt"));

set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_VSPEED, -1);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_VSPEED_TYPE, 2);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSPECIAL_2, 1, AG_WINDOW_SFX, asset_get("mfx_coin"));

set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_2, 2, AG_WINDOW_VSPEED_TYPE, 1);

set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_2, 3, AG_WINDOW_VSPEED_TYPE, 1);

set_num_hitboxes(AT_DSPECIAL_2, 1);

//set_hitbox_value(AT_DSPECIAL_2, 1, HG_HITBOX_TYPE, 1);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_WINDOW, 2);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_LIFETIME, 2);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_HITBOX_X, -4);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_HITBOX_Y, -28);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_WIDTH, 42);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_HEIGHT, 52);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_SHAPE, 0);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_PRIORITY, 3);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_DAMAGE, 3);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_ANGLE, 90);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_BASE_KNOCKBACK, 7);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_KNOCKBACK_SCALING, .2);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_BASE_HITPAUSE, 6);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_HITPAUSE_SCALING, .3);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_VISUAL_EFFECT_X_OFFSET, 18);
//set_hitbox_value(AT_DSPECIAL_2, 1, HG_HIT_SFX, sound_get("hit1"));
