...
if ("nate_initialized_victory_screen" not in self)
{
    nate_initialized_victory_screen = true;
    //defaults to prevent errors
    nate_victory_quote = "ermmmm that wasnt supposed to happen";
    nate_victory_icon = 0;
...
if (winner == player /* && uhc_batteries */)
{
    ...
    var quote_time  =   240;
    //Animate panel
    if ("nate_quote_current_pos_x" not in self)
    { nate_quote_current_pos_x = hide_pos_x; }
    
    //Must check with timing or if result boxes are open
    var diff = ((results_timer > quote_time && !someone_pressed) ? 
                    quote_pos_x : hide_pos_x) - nate_quote_current_pos_x;
    
    nate_quote_current_pos_x += sign(diff) 
                                * max(min(abs(diff), 5), abs(diff) * 0.25);
    
    //Draw panel
    if (nate_quote_current_pos_x > hide_pos_x && (string_length(nate_victory_quote) >= 1))
    {
        draw_sprite(sprite_get("victory_quote_bg"), 0, 
                    nate_quote_current_pos_x, quote_pos_y);
        draw_sprite_ext(sprite_get("stockicon"), nate_victory_icon, 
                    nate_quote_current_pos_x + 46, quote_pos_y + 10, 1.5, 1.5, 0, c_white, 1);
        draw_win_quote(nate_quote_current_pos_x+135, quote_pos_y+15, 
                        nate_victory_quote);
    }
}
...
#define get_random_quote()
{
    var quotes = [];
    var i = 0;
    quotes[i++] = "*huff* It's over, right? Good...";
    quotes[i++] = "There we go, I made it through. Onto the next fight!";
    quotes[i++] = "Alright, what can I learn from that fight...?";
...
#define get_victory_screen_data()
{
    ...
    with (asset_get("pHitBox")) if ("nate_victory_screen_array" in self)
    {
        data_array = nate_victory_screen_array;
    ...
    if (string_length(data_array[winner].status_quote) > 1)
    {
        ...
        nate_victory_quote = data_array[winner].status_quote;
        nate_victory_icon = data_array[best_player].icon;
    }
    else
    {
        ...
        nate_victory_quote = data_array[best_player].quote;
        nate_victory_icon = data_array[best_player].icon;
        if (string_length(nate_victory_quote) < 1)
        { nate_victory_quote = get_random_quote(); 
          nate_victory_icon = 0;    
        }
    }
