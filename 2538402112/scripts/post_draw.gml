...
switch (state){
    ...
    case PS_ATTACK_AIR:
        if (attack == AT_FSPECIAL && window != 4){
            if get_player_color(player) == 14 draw_sprite_ext(sprite_get("fspecial_wandEA"), image_index, x, y, 2*spr_dir,2,0,c_white,1);
            else if get_player_color(player) == 13 draw_sprite_ext(sprite_get("fspecial_wand_abyss"), image_index, x, y, 2*spr_dir,2,0,c_white,1);
        ...
        if (attack == AT_USPECIAL){
            ...
            if get_player_color(player) == 14 grov_escapebeam = 2
