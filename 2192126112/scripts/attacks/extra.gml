set_attack_value(AT_EXTRA_1, AG_SPRITE, sprite_get("charge"));

set_attack_value(AT_EXTRA_1, AG_HURTBOX_SPRITE, sprite_get("charge_hurt"));
set_attack_value(AT_EXTRA_1, AG_OFF_LEDGE, 0);
set_attack_value(AT_EXTRA_1, AG_CATEGORY, 2);

set_attack_value(AT_EXTRA_1, AG_NUM_WINDOWS, 4);

set_window_value(AT_EXTRA_1, 1, AG_WINDOW_TYPE, 0);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_LENGTH, 5);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_ANIM_FRAMES, 1);


set_window_value(AT_EXTRA_1, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_LENGTH, 50);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_ANIM_FRAMES, 10);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_SFX, 0);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HSPEED, 8);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HSPEED_TYPE, 1);


set_window_value(AT_EXTRA_1, 3, AG_WINDOW_TYPE, 0);   // cancelled
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_LENGTH, 2);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_SFX, 0);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_SFX_FRAME, 0);

set_window_value(AT_EXTRA_1, 4, AG_WINDOW_TYPE, 0); //normal ending
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_LENGTH, 10);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_SFX, 0);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_SFX_FRAME, 0);

set_num_hitboxes(AT_EXTRA_1, 1)


set_hitbox_value(AT_EXTRA_1, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_1, 1, HG_WINDOW, 2);
set_hitbox_value(AT_EXTRA_1, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_LIFETIME, 55);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITBOX_X, -6);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITBOX_Y, -49);
set_hitbox_value(AT_EXTRA_1, 1, HG_WIDTH, 20);
set_hitbox_value(AT_EXTRA_1, 1, HG_HEIGHT, 82);
set_hitbox_value(AT_EXTRA_1, 1, HG_SHAPE, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_EXTRA_1, 1, HG_DAMAGE, 6);
set_hitbox_value(AT_EXTRA_1, 1, HG_ANGLE, 361);
set_hitbox_value(AT_EXTRA_1, 1, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_EXTRA_1, 1, HG_KNOCKBACK_SCALING, 0.4);
set_hitbox_value(AT_EXTRA_1, 1, HG_EFFECT, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITPAUSE_SCALING, 0.5);
set_hitbox_value(AT_EXTRA_1, 1, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_TECHABLE, 1);
set_hitbox_value(AT_EXTRA_1, 1, HG_HIT_SFX, sound_get("demo_charge_hit_flesh2-2"));
set_hitbox_value(AT_EXTRA_1, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_EXTRA_CAMERA_SHAKE, 1);
set_hitbox_value(AT_EXTRA_1, 1, HG_THROWS_ROCK, 1); //head1