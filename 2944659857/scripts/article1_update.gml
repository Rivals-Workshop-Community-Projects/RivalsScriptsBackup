...
if x != clamp(x, blast_l, blast_r) || y > blast_b {
    if squishwalk_sound != noone {
        sound_stop(squishwalk_sound);
        squishwalk_sound = noone;
    }
    instance_destroy();
    exit;
}
...
switch state {
    ...
    case PS_WALK_TURN:
        if state_timer == 0 {
            spr_dir *= -1;
            sprite_index = sprite_get("goomba_turn");
            image_index = 0;
        }
    ...
    case PS_HITSTUN:
        if hitstop { 
            sprite_index = sprite_get("goomba_hurt");
            state_timer = -1;
        }
        else {
            if state_timer == 0 {
    ...
    case PS_BURIED:
        if state_timer == 0 {
            sprite_index = sprite_get("goomba_squish");
            vsp = 0;
            hsp = 0;
            is_hittable = false;
            ...
            is_hittable = image_index;
        }
        else {
            sprite_index = spr_squishwalk;
            image_index += walk_anim_speed;
            if squishwalk_sound == noone {
                squishwalk_sound = sound_play(sound_get("squishwalk_loop"), true);
            }
            if state_timer % 20 == 0 && !free { 
                hsp = -walk_speed + 3*random_func(0, 2, true);
            }
        }
    ...
    case PS_FIRST_JUMP:
        if state_timer == 0 {
            sprite_index = spr_fall;
            image_index = 0;
            spr_dir = 1 - 2*random_func(0, 2, true);
            vsp = -jump_speed;
            hsp = 0;
    ...
    case PS_LAND:
        if state_timer == 0 {
            sprite_index = sprite_get("goomba_land");
    ...
    case PS_DEAD:
        if state_timer == 0 {
            sprite_index = sprite_get("goomba_dead");
...
// ouchbox
switch state {
    case PS_WALK: case PS_WALK_TURN: case PS_LAND: case PS_HITSTUN:
        var x1 = bbox_left + 8;
        var y1 = bbox_top + 8;
        var x2 = bbox_right - 8;
        ...
        with oPlayer {
            if point_distance(x, y, other.x, other.y) < 150 {
                if collision_rectangle(x1, y1, x2, y2, hurtboxID, true, false) {
                    if !other.free && vsp > max_fall/2 && other.state != PS_HITSTUN {
                        vsp = -1.1 * short_hop_speed;
                        with other {
                            destroy_ouchbox();
...
if state != PS_BURIED && squishwalk_sound != noone {
    sound_stop(squishwalk_sound);
    squishwalk_sound = noone;
}
