...
switch state {
    ...
    case PS_WALK_TURN:
        if state_timer == 0 {
            spr_dir *= -1;
            sprite_index = sprite_get("koopa_turn");
            image_index = 0;
        }
    ...
    case PS_HITSTUN:
        if hitstop { 
            sprite_index = sprite_get("koopa_hurt");
            state_timer = -1;
        }
        else {
            if state_timer == 0 {
    ...
    case PS_BURIED:
        if state_timer == 0 {
            sprite_index = sprite_get("koopa_squish");
            image_index = 0;
            vsp = 0;
            hsp = 0;
            is_hittable = false;
            ...
            sprite_index = sprite_get("koopa_shell");
            image_index = 0;
        }
        else {
            sprite_index = sprite_get("koopa_shellrock")
    ...
    case PS_LAND:
        if state_timer == 0 {
            sprite_index = sprite_get("koopa_land");
    ...
    case PS_DEAD:
        if state_timer == 0 {
            sprite_index = sprite_get("koopa_dead");
        ...
        with oPlayer {
            if point_distance(x, y, other.x, other.y) < 150 {
                if collision_rectangle(x1, y1, x2, y2, hurtboxID, true, false) {
                    if !other.free && vsp > max_fall/4 && other.state != PS_HITSTUN {
                        vsp = -1.1 * short_hop_speed;
                        with other {
                            destroy_ouchbox();
                            if state == PS_BURIED && is_hittable {
                                if sign(x - other.x) != 0 { spr_dir = sign(x-other.x); }
                                queued_state = PS_DASH;
                                sound_play(sound_get("die"));
                            }
                            else { queued_state = PS_BURIED; }
                        }
                        break;
                    }
                    else {
                        with other if state != PS_BURIED {
                            create_ouchbox(2, 1);
                        }
                        break;
                    }
