set_attack_value(AT_JAB, AG_SPRITE, sprite_get("jab"));
set_attack_value(AT_JAB, AG_NUM_WINDOWS, 9);
set_attack_value(AT_JAB, AG_HURTBOX_SPRITE, sprite_get("jab_hurt"));

// -- Jab 1 --
// Start-up
set_window_value(AT_JAB, 1, AG_WINDOW_LENGTH, 4);
set_window_value(AT_JAB, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_JAB, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));

// Active
set_window_value(AT_JAB, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_JAB, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 2, AG_WINDOW_ANIM_FRAME_START, 1);

// Recovery
set_window_value(AT_JAB, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_JAB, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 3, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_JAB, 3, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_JAB, 3, AG_WINDOW_CANCEL_FRAME, 4);

// -- Jab 2 --
// Start-up
set_window_value(AT_JAB, 4, AG_WINDOW_LENGTH, 2);
set_window_value(AT_JAB, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 4, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_JAB, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_JAB, 4, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));

// Active
set_window_value(AT_JAB, 5, AG_WINDOW_LENGTH, 2);
set_window_value(AT_JAB, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 5, AG_WINDOW_ANIM_FRAME_START, 3);

// Recovery
set_window_value(AT_JAB, 6, AG_WINDOW_LENGTH, 17);
set_window_value(AT_JAB, 6, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 6, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_JAB, 6, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_JAB, 6, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_JAB, 6, AG_WINDOW_CANCEL_FRAME, 4);

// -- Jab 3 --
// Start-up
set_window_value(AT_JAB, 7, AG_WINDOW_LENGTH, 4);
set_window_value(AT_JAB, 7, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 7, AG_WINDOW_HSPEED, 6);
set_window_value(AT_JAB, 7, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_JAB, 7, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_JAB, 7, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_JAB, 7, AG_WINDOW_SFX_FRAME, 1);

// Active
set_window_value(AT_JAB, 8, AG_WINDOW_LENGTH, 5);
set_window_value(AT_JAB, 8, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 8, AG_WINDOW_ANIM_FRAME_START, 5);

// Recovery
set_window_value(AT_JAB, 9, AG_WINDOW_LENGTH, 18);
set_window_value(AT_JAB, 9, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 9, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_JAB, 9, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_JAB, 3);

// Hitbox 1
set_hitbox_value(AT_JAB, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_JAB, 1, HG_WINDOW, 2);
set_hitbox_value(AT_JAB, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_JAB, 1, HG_HITBOX_X, 10);
set_hitbox_value(AT_JAB, 1, HG_HITBOX_Y, -16);
set_hitbox_value(AT_JAB, 1, HG_WIDTH, 30);
set_hitbox_value(AT_JAB, 1, HG_HEIGHT, 25);
set_hitbox_value(AT_JAB, 1, HG_SHAPE, 2);
set_hitbox_value(AT_JAB, 1, HG_DAMAGE, 2); // 1
set_hitbox_value(AT_JAB, 1, HG_ANGLE, 60); // 2
set_hitbox_value(AT_JAB, 1, HG_BASE_KNOCKBACK, 4); // 3
set_hitbox_value(AT_JAB, 1, HG_KNOCKBACK_SCALING, 0); // 4
set_hitbox_value(AT_JAB, 1, HG_PRIORITY, 2); // 5
set_hitbox_value(AT_JAB, 1, HG_BASE_HITPAUSE, 5); // 6
set_hitbox_value(AT_JAB, 1, HG_HITPAUSE_SCALING, 0); // 7
set_hitbox_value(AT_JAB, 1, HG_ANGLE_FLIPPER, 0); // 8
set_hitbox_value(AT_JAB, 1, HG_VISUAL_EFFECT, 301);
set_hitbox_value(AT_JAB, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_JAB, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_JAB, 1, HG_HIT_SFX, asset_get("sfx_blow_weak1"));

// Hitbox 2
set_hitbox_value(AT_JAB, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_JAB, 2, HG_WINDOW, 5);
set_hitbox_value(AT_JAB, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_JAB, 2, HG_HITBOX_X, 14);
set_hitbox_value(AT_JAB, 2, HG_HITBOX_Y, -23);
set_hitbox_value(AT_JAB, 2, HG_WIDTH, 31);
set_hitbox_value(AT_JAB, 2, HG_HEIGHT, 28);
set_hitbox_value(AT_JAB, 2, HG_SHAPE, 2);
set_hitbox_value(AT_JAB, 2, HG_DAMAGE, 2); // 1
set_hitbox_value(AT_JAB, 2, HG_ANGLE, 60); // 2
set_hitbox_value(AT_JAB, 2, HG_BASE_KNOCKBACK, 4); // 3
set_hitbox_value(AT_JAB, 2, HG_KNOCKBACK_SCALING, 0); // 4
set_hitbox_value(AT_JAB, 2, HG_PRIORITY, 2); // 5
set_hitbox_value(AT_JAB, 2, HG_BASE_HITPAUSE, 5); // 6
set_hitbox_value(AT_JAB, 2, HG_HITPAUSE_SCALING, 0); // 7
set_hitbox_value(AT_JAB, 2, HG_ANGLE_FLIPPER, 0); // 8
set_hitbox_value(AT_JAB, 2, HG_VISUAL_EFFECT, 301);
set_hitbox_value(AT_JAB, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_JAB, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_JAB, 2, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_JAB, 2, HG_HITBOX_GROUP, 2);

// Hitbox 3
set_hitbox_value(AT_JAB, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_JAB, 3, HG_WINDOW, 8);
set_hitbox_value(AT_JAB, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_JAB, 3, HG_HITBOX_X, 11);
set_hitbox_value(AT_JAB, 3, HG_HITBOX_Y, -14);
set_hitbox_value(AT_JAB, 3, HG_WIDTH, 31);
set_hitbox_value(AT_JAB, 3, HG_HEIGHT, 29);
set_hitbox_value(AT_JAB, 3, HG_SHAPE, 2);
set_hitbox_value(AT_JAB, 3, HG_DAMAGE, 3); // 1
set_hitbox_value(AT_JAB, 3, HG_ANGLE, 361); // 2
set_hitbox_value(AT_JAB, 3, HG_BASE_KNOCKBACK, 6); // 3
set_hitbox_value(AT_JAB, 3, HG_KNOCKBACK_SCALING, .4); // 4
set_hitbox_value(AT_JAB, 3, HG_PRIORITY, 2); // 5
set_hitbox_value(AT_JAB, 3, HG_BASE_HITPAUSE, 6); // 6
set_hitbox_value(AT_JAB, 3, HG_HITPAUSE_SCALING, .4); // 7
set_hitbox_value(AT_JAB, 3, HG_ANGLE_FLIPPER, 0); // 8
set_hitbox_value(AT_JAB, 3, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 3, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_JAB, 3, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_JAB, 3, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
set_hitbox_value(AT_JAB, 3, HG_HITBOX_GROUP, 3);