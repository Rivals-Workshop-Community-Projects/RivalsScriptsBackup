// DEFAULT COLOR

// fruit
set_color_profile_slot( 0, 0, 225, 107, 106 );
set_color_profile_slot_range( 0, 13, 24, 27 );

// highlight
set_color_profile_slot( 0, 1, 212, 230, 147 );
set_color_profile_slot_range( 1, 1, 1, 1 );

// body 1
set_color_profile_slot( 0, 2, 174, 206, 77 );
set_color_profile_slot_range( 2, 1, 1, 1 );

// stripe 1
set_color_profile_slot( 0, 3, 129, 174, 50 );
set_color_profile_slot_range( 3, 1, 1, 1 );

// body 2
set_color_profile_slot( 0, 4, 99, 146, 50 );
set_color_profile_slot_range( 4, 1, 1, 1 );

// stripe 2
set_color_profile_slot( 0, 5, 71, 106, 31 );
set_color_profile_slot_range( 5, 1, 1, 1 );

// foliage 1
set_color_profile_slot( 0, 6, 85, 126, 41 );
set_color_profile_slot_range( 6, 1, 1, 1 );

// foliage 2
set_color_profile_slot( 0, 7, 56, 88, 13 );
set_color_profile_slot_range( 7, 1, 1, 1 );


// ALTERNATE COLORS
set_num_palettes( 20 );

// winter
set_color_profile_slot( 1, 0, 225, 107, 106 ); //fruit
set_color_profile_slot( 1, 1, 177, 250, 255 ); //highlight
set_color_profile_slot( 1, 2, 118, 228, 250 ); //body 1
set_color_profile_slot( 1, 3, 78, 191, 223 ); //stripe 1
set_color_profile_slot( 1, 4, 57, 155, 184 ); //body 2
set_color_profile_slot( 1, 5, 27, 120, 150 ); //stripe 2
set_color_profile_slot( 1, 6, 48, 184, 129 ); //foliage 1
set_color_profile_slot( 1, 7, 26, 114, 93 ); //foliage 2

// coconut
set_color_profile_slot( 2, 0, 255, 246, 237 ); //fruit
set_color_profile_slot( 2, 1, 212, 151, 60 ); //highlight
set_color_profile_slot( 2, 2, 171, 104, 38 ); //body 1
set_color_profile_slot( 2, 3, 138, 78, 1 ); //body 2
set_color_profile_slot( 2, 4, 97, 52, 0 ); //stripe 1
set_color_profile_slot( 2, 5, 74, 34, 0 ); //stripe 2
set_color_profile_slot( 2, 6, 144, 212, 0 ); //foliage 1
set_color_profile_slot( 2, 7, 92, 138, 0 ); //foliage 2

// berry
set_color_profile_slot( 3, 0, 219, 39, 198 ); //fruit
set_color_profile_slot( 3, 1, 252, 150, 148 ); //highlight
set_color_profile_slot( 3, 2, 252, 102, 98 ); //body 1
set_color_profile_slot( 3, 3, 237, 68, 93 ); //stripe 1
set_color_profile_slot( 3, 4, 203, 17, 64 ); //body 2
set_color_profile_slot( 3, 5, 151, 5, 63 ); //stripe 2
set_color_profile_slot( 3, 6, 44, 154, 62 ); //foliage 1
set_color_profile_slot( 3, 7, 18, 91, 37 ); //foliage 2

// pepper
set_color_profile_slot( 4, 0, 255, 153, 61 ); //fruit
set_color_profile_slot( 4, 1, 253, 235, 173 ); //highlight
set_color_profile_slot( 4, 2, 240, 190, 30 ); //body 1
set_color_profile_slot( 4, 3, 231, 142, 18 ); //stripe 1
set_color_profile_slot( 4, 4, 224, 108, 25 ); //body 2
set_color_profile_slot( 4, 5, 190, 79, 3 ); //stripe 2
set_color_profile_slot( 4, 6, 93, 126, 37 ); //foliage 1
set_color_profile_slot( 4, 7, 45, 87, 38 ); //foliage 2

// cantaloupe
set_color_profile_slot( 5, 0, 255, 154, 0 ); //fruit
set_color_profile_slot( 5, 1, 255, 232, 194 ); //highlight
set_color_profile_slot( 5, 2, 232, 199, 144 ); //body 1
set_color_profile_slot( 5, 3, 199, 168, 113 ); //stripe 1
set_color_profile_slot( 5, 4, 140, 136, 92 ); //body 2
set_color_profile_slot( 5, 5, 94, 115, 85 ); //stripe 2
set_color_profile_slot( 5, 6, 140, 156, 50 ); //foliage 1
set_color_profile_slot( 5, 7, 63, 97, 19 ); //foliage 2

// abyss
set_color_profile_slot( 6, 0, 50, 48, 95 ); //fruit
set_color_profile_slot( 6, 1, 226, 160, 225 ); //highlight
set_color_profile_slot( 6, 2, 220, 113, 255 ); //body 1
set_color_profile_slot( 6, 3, 187, 76, 220 ); //stripe 1
set_color_profile_slot( 6, 4, 144, 59, 206 ); //body 2
set_color_profile_slot( 6, 5, 109, 36, 134 ); //stripe 2
set_color_profile_slot( 6, 6, 86, 84, 146 ); //foliage 1
set_color_profile_slot( 6, 7, 50, 48, 95 ); //foliage 2

// gameboy
set_color_profile_slot( 7, 0, 35, 67, 49 ); //fruit
set_color_profile_slot( 7, 1, 211, 226, 154 ); //highlight
set_color_profile_slot( 7, 2, 167, 186, 74 ); //body 1
set_color_profile_slot( 7, 3, 167, 186, 74 ); //stripe 1
set_color_profile_slot( 7, 4, 83, 122, 62 ); //body 2
set_color_profile_slot( 7, 5, 83, 122, 62 ); //stripe 2
set_color_profile_slot( 7, 6, 83, 122, 62 ); //foliage 1
set_color_profile_slot( 7, 7, 35, 67, 49 ); //foliage 2

// kirby
set_color_profile_slot( 8, 0, 170, 8, 74 ); //fruit
set_color_profile_slot( 8, 1, 255, 200, 240 ); //highlight
set_color_profile_slot( 8, 2, 248, 160, 216 ); //body 1
set_color_profile_slot( 8, 3, 243, 134, 182 ); //stripe 1
set_color_profile_slot( 8, 4, 240, 112, 160 ); //body 2
set_color_profile_slot( 8, 5, 216, 72, 112 ); //stripe 2
set_color_profile_slot( 8, 6, 248, 24, 128 ); //foliage 1
set_color_profile_slot( 8, 7, 176, 0, 40 ); //foliage 2

// bob-omb
set_color_profile_slot( 9, 0, 128, 128, 128 ); //fruit
set_color_profile_slot( 9, 1, 137, 211, 245 ); //highlight
set_color_profile_slot( 9, 2, 91, 158, 181 ); //body 1
set_color_profile_slot( 9, 3, 45, 105, 117 ); //stripe 1
set_color_profile_slot( 9, 4, 34, 80, 89 ); //body 2
set_color_profile_slot( 9, 5, 23, 54, 61 ); //stripe 2
set_color_profile_slot( 9, 6, 255, 170, 71 ); //foliage 1
set_color_profile_slot( 9, 7, 171, 82, 50 ); //foliage 2

// noodle
set_color_profile_slot( 10, 0, 255, 153, 53 ); //fruit
set_color_profile_slot( 10, 1, 182, 244, 48 ); //highlight
set_color_profile_slot( 10, 2, 100, 221, 42 ); //body 1
set_color_profile_slot( 10, 3, 71, 178, 57 ); //stripe 1
set_color_profile_slot( 10, 4, 54, 134, 51 ); //body 2
set_color_profile_slot( 10, 5, 30, 91, 49 ); //stripe 2
set_color_profile_slot( 10, 6, 255, 214, 0 ); //foliage 1
set_color_profile_slot( 10, 7, 194, 124, 43 ); //foliage 2

// buzz
set_color_profile_slot( 11, 0, 255, 199, 132 ); //fruit
set_color_profile_slot( 11, 1, 255, 234, 150 ); //highlight
set_color_profile_slot( 11, 2, 249, 202, 53 ); //body 1
set_color_profile_slot( 11, 3, 176, 145, 94 ); //stripe 1
set_color_profile_slot( 11, 4, 155, 99, 59 ); //body 2
set_color_profile_slot( 11, 5, 93, 69, 50 ); //stripe 2
set_color_profile_slot( 11, 6, 83, 68, 72 ); //foliage 1
set_color_profile_slot( 11, 7, 54, 33, 39 ); //foliage 2

// NES Open Tournament Golf
set_color_profile_slot( 12, 0, 74, 113, 222 ); //fruit
set_color_profile_slot( 12, 1, 255, 255, 255 ); //highlight
set_color_profile_slot( 12, 2, 233, 206, 206 ); //body 1
set_color_profile_slot( 12, 3, 255, 128, 128 ); //stripe 1
set_color_profile_slot( 12, 4, 202, 119, 136 ); //body 2
set_color_profile_slot( 12, 5, 193, 52, 52 ); //stripe 2
set_color_profile_slot( 12, 6, 82, 106, 235 ); //foliage 1
set_color_profile_slot( 12, 7, 59, 73, 135 ); //foliage 2

// unnamed blue alt
set_color_profile_slot( 13, 0, 106, 92, 249 ); //fruit
set_color_profile_slot( 13, 1, 154, 241, 255 ); //highlight
set_color_profile_slot( 13, 2, 111, 188, 242 ); //body 1
set_color_profile_slot( 13, 3, 182, 128, 224 ); //stripe 1
set_color_profile_slot( 13, 4, 102, 109, 238 ); //body 2
set_color_profile_slot( 13, 5, 101, 65, 217 ); //stripe 2
set_color_profile_slot( 13, 6, 153, 52, 52 ); //foliage 1
set_color_profile_slot( 13, 7, 103, 11, 105 ); //foliage 2

// watermimic
set_color_profile_slot( 14, 0, 255, 63, 121 ); //fruit
set_color_profile_slot( 14, 1, 192, 255, 97 ); //highlight
set_color_profile_slot( 14, 2, 112, 233, 92 ); //body 1
set_color_profile_slot( 14, 3, 68, 199, 146 ); //stripe 1
set_color_profile_slot( 14, 4, 57, 166, 84 ); //body 2
set_color_profile_slot( 14, 5, 59, 128, 126 ); //stripe 2
set_color_profile_slot( 14, 6, 86, 130, 130 ); //foliage 1
set_color_profile_slot( 14, 7, 74, 82, 120 ); //foliage 2

// rgb
set_color_profile_slot( 15, 0, 255, 0, 0 ); //fruit
set_color_profile_slot( 15, 1, 166, 110, 110 ); //highlight
set_color_profile_slot( 15, 2, 114, 80, 80 ); //body 1
set_color_profile_slot( 15, 3, 85, 65, 65 ); //stripe 1
set_color_profile_slot( 15, 4, 60, 53, 53 ); //body 2
set_color_profile_slot( 15, 5, 38, 38, 38 ); //stripe 2
set_color_profile_slot( 15, 6, 255, 80, 80 ); //foliage 1
set_color_profile_slot( 15, 7, 128, 40, 40 ); //foliage 2

// transcendent
set_color_profile_slot( 16, 0, 143, 217, 255 ); //fruit
set_color_profile_slot( 16, 1, 255, 255, 255 ); //highlight
set_color_profile_slot( 16, 2, 241, 206, 249 ); //body 1
set_color_profile_slot( 16, 3, 255, 173, 225 ); //stripe 1
set_color_profile_slot( 16, 4, 199, 151, 201 ); //body 2
set_color_profile_slot( 16, 5, 205, 105, 207 ); //stripe 2
set_color_profile_slot( 16, 6, 128, 197, 255 ); //foliage 1
set_color_profile_slot( 16, 7, 111, 123, 236 ); //foliage 2

// infamous
set_color_profile_slot( 17, 0, 0, 255, 204 ); //fruit
set_color_profile_slot( 17, 1, 230, 185, 254 ); //highlight
set_color_profile_slot( 17, 2, 182, 132, 254 ); //body 1
set_color_profile_slot( 17, 3, 143, 93, 204 ); //stripe 1
set_color_profile_slot( 17, 4, 96, 75, 166 ); //body 2
set_color_profile_slot( 17, 5, 65, 47, 135 ); //stripe 2
set_color_profile_slot( 17, 6, 0, 255, 204 ); //foliage 2
set_color_profile_slot( 17, 7, 0, 169, 152 ); //foliage 1

// ranked gold
set_color_profile_slot( 18, 0, 249, 185, 112 ); //fruit
set_color_profile_slot( 18, 1, 255, 236, 201 ); //highlight
set_color_profile_slot( 18, 2, 255, 218, 107 ); //body 1
set_color_profile_slot( 18, 3, 252, 193, 66 ); //stripe 1
set_color_profile_slot( 18, 4, 237, 165, 64 ); //body 2
set_color_profile_slot( 18, 5, 217, 125, 20 ); //stripe 2
set_color_profile_slot( 18, 6, 207, 127, 35 ); //foliage 1
set_color_profile_slot( 18, 7, 153, 83, 12 ); //foliage 2

// imitater
set_color_profile_slot( 19, 0, 198, 144, 143 ); //fruit
set_color_profile_slot( 19, 1, 251, 251, 251 ); //highlight
set_color_profile_slot( 19, 2, 193, 199, 181 ); //body 1
set_color_profile_slot( 19, 3, 149, 168, 138 ); //stripe 1
set_color_profile_slot( 19, 4, 116, 143, 105 ); //body 2
set_color_profile_slot( 19, 5, 76, 99, 70 ); //stripe 2
set_color_profile_slot( 19, 6, 152, 179, 152 ); //foliage 1
set_color_profile_slot( 19, 7, 86, 127, 87 ); //foliage 2