set_attack_value(AT_DAIR, AG_CATEGORY, 1);
set_attack_value(AT_DAIR, AG_SPRITE, sprite_get("dair"));
set_attack_value(AT_DAIR, AG_NUM_WINDOWS, 3);
set_attack_value(AT_DAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_DAIR, AG_LANDING_LAG, 8);
set_attack_value(AT_DAIR, AG_HURTBOX_SPRITE, sprite_get("dair_hurt"));

/*
set_window_value(AT_DAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_LENGTH, 5);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAMES, 2);
//set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED_TYPE, 1);
//set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED, 0);
//set_window_value(AT_DAIR, 1, AG_WINDOW_CUSTOM_GRAVITY, 1.0);
set_window_value(AT_DAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX, asset_get("sfx_ori_dsmash_skitter_alone"));
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX_FRAME, 4);

set_window_value(AT_DAIR, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAME_START, 2);

set_window_value(AT_DAIR, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 3, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_DAIR, 3, AG_WINDOW_HAS_WHIFFLAG, 5);

set_num_hitboxes(AT_DAIR, 2);

set_hitbox_value(AT_DAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 1, HG_WINDOW, 2);
//set_hitbox_value(AT_DAIR, 1, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_DAIR, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_X, -3);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_Y, 6);
set_hitbox_value(AT_DAIR, 1, HG_WIDTH, 70);
set_hitbox_value(AT_DAIR, 1, HG_HEIGHT, 40);
set_hitbox_value(AT_DAIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 1, HG_DAMAGE, 4);
set_hitbox_value(AT_DAIR, 1, HG_ANGLE, 120);
set_hitbox_value(AT_DAIR, 1, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DAIR, 1, HG_KNOCKBACK_SCALING, 0.1);
set_hitbox_value(AT_DAIR, 1, HG_BASE_HITPAUSE, 8);
//set_hitbox_value(AT_DAIR, 1, HG_GROUNDEDNESS, 1);
//set_hitbox_value(AT_DAIR, 1, HG_HITSTUN_MULTIPLIER, 0.5);
//set_hitbox_value(AT_DAIR, 1, HG_HITPAUSE_SCALING, 1.0);
//set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, 197);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 1, HG_HIT_SFX, asset_get("sfx_blow_medium3"));
//set_hitbox_value(AT_DAIR, 1, HG_ANGLE_FLIPPER, 6);

set_hitbox_value(AT_DAIR, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 2, HG_WINDOW, 2);
//set_hitbox_value(AT_DAIR, 1, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_DAIR, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_X, 22);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_Y, -12);
set_hitbox_value(AT_DAIR, 2, HG_WIDTH, 30);
set_hitbox_value(AT_DAIR, 2, HG_HEIGHT, 60);
set_hitbox_value(AT_DAIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 2, HG_DAMAGE, 4);
set_hitbox_value(AT_DAIR, 2, HG_ANGLE, 120);
set_hitbox_value(AT_DAIR, 2, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DAIR, 2, HG_KNOCKBACK_SCALING, 0.1);
set_hitbox_value(AT_DAIR, 2, HG_BASE_HITPAUSE, 8);
//set_hitbox_value(AT_DAIR, 1, HG_HITSTUN_MULTIPLIER, 0.5);
//set_hitbox_value(AT_DAIR, 1, HG_HITPAUSE_SCALING, 1.0);
//set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, 197);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 2, HG_HIT_SFX, asset_get("sfx_blow_medium3"));
*/

set_window_value(AT_DAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAMES, 2);
//set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED_TYPE, 1);
//set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED, 0);
//set_window_value(AT_DAIR, 1, AG_WINDOW_CUSTOM_GRAVITY, 1.0);
set_window_value(AT_DAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_medium2"));
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX_FRAME, 9);

set_window_value(AT_DAIR, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAME_START, 2);

set_window_value(AT_DAIR, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 3, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_DAIR, 3, AG_WINDOW_HAS_WHIFFLAG, 5);

set_num_hitboxes(AT_DAIR, 3);

set_hitbox_value(AT_DAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 1, HG_WINDOW, 2);
//set_hitbox_value(AT_DAIR, 1, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_DAIR, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_X, -3);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_Y, 6);
set_hitbox_value(AT_DAIR, 1, HG_WIDTH, 70);
set_hitbox_value(AT_DAIR, 1, HG_HEIGHT, 40);
set_hitbox_value(AT_DAIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 1, HG_DAMAGE, 8);
set_hitbox_value(AT_DAIR, 1, HG_ANGLE, 120);
set_hitbox_value(AT_DAIR, 1, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DAIR, 1, HG_KNOCKBACK_SCALING, 0.1);
set_hitbox_value(AT_DAIR, 1, HG_BASE_HITPAUSE, 7);//8
//set_hitbox_value(AT_DAIR, 1, HG_GROUNDEDNESS, 1);
//set_hitbox_value(AT_DAIR, 1, HG_HITSTUN_MULTIPLIER, 0.5);
set_hitbox_value(AT_DAIR, 1, HG_HITPAUSE_SCALING, 0.3);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, 111);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 1, HG_HIT_SFX, sound_get("amber_magic_hit_medium1"));
//set_hitbox_value(AT_DAIR, 1, HG_ANGLE_FLIPPER, 6);

set_hitbox_value(AT_DAIR, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 2, HG_WINDOW, 2);
//set_hitbox_value(AT_DAIR, 1, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_DAIR, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_X, 22);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_Y, -12);
set_hitbox_value(AT_DAIR, 2, HG_WIDTH, 30);
set_hitbox_value(AT_DAIR, 2, HG_HEIGHT, 60);
set_hitbox_value(AT_DAIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_DAIR, 2, HG_DAMAGE, 8);
set_hitbox_value(AT_DAIR, 2, HG_ANGLE, 280);
set_hitbox_value(AT_DAIR, 2, HG_BASE_KNOCKBACK, 5);//4
set_hitbox_value(AT_DAIR, 2, HG_KNOCKBACK_SCALING, 0.6);//5
set_hitbox_value(AT_DAIR, 2, HG_BASE_HITPAUSE, 9);//12
set_hitbox_value(AT_DAIR, 2, HG_HITSTUN_MULTIPLIER, 0.75);
set_hitbox_value(AT_DAIR, 2, HG_HITPAUSE_SCALING, 0.4);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT, 112);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 2, HG_HIT_SFX, asset_get("sfx_ori_energyhit_medium"));

set_hitbox_value(AT_DAIR, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 3, HG_WINDOW, 2);
set_hitbox_value(AT_DAIR, 3, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_X, 22);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_Y, -12);
set_hitbox_value(AT_DAIR, 3, HG_WIDTH, 30);
set_hitbox_value(AT_DAIR, 3, HG_HEIGHT, 60);
set_hitbox_value(AT_DAIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_DAIR, 3, HG_DAMAGE, 8);
set_hitbox_value(AT_DAIR, 3, HG_ANGLE, 60);
set_hitbox_value(AT_DAIR, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DAIR, 3, HG_KNOCKBACK_SCALING, 0.1);
set_hitbox_value(AT_DAIR, 3, HG_GROUNDEDNESS, 1);
set_hitbox_value(AT_DAIR, 3, HG_BASE_HITPAUSE, 9);//12
set_hitbox_value(AT_DAIR, 3, HG_HITPAUSE_SCALING, 0.4);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT, 112);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 3, HG_HIT_SFX, asset_get("sfx_ori_energyhit_medium"));

//ABYSS PROJECTILES

set_hitbox_value(AT_DAIR, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_TYPE, 2 );
set_hitbox_value(AT_DAIR, 4, HG_WINDOW, 2);
set_hitbox_value(AT_DAIR, 4, HG_WINDOW_CREATION_FRAME, 2);
set_hitbox_value(AT_DAIR, 4, HG_LIFETIME, 20);
set_hitbox_value(AT_DAIR, 4, HG_WIDTH, 85);
set_hitbox_value(AT_DAIR, 4, HG_HEIGHT, 85);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_X, 0);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_Y, -16);
set_hitbox_value(AT_DAIR, 4, HG_SHAPE, 0 );
set_hitbox_value(AT_DAIR, 4, HG_PRIORITY, 1);
set_hitbox_value(AT_DAIR, 4, HG_DAMAGE, 7);
set_hitbox_value(AT_DAIR, 4, HG_ANGLE, 315);
set_hitbox_value(AT_DAIR, 4, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DAIR, 4, HG_KNOCKBACK_SCALING, .55);
set_hitbox_value(AT_DAIR, 4, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DAIR, 4, HG_HITPAUSE_SCALING, .5);
set_hitbox_value(AT_DAIR, 4, HG_HITSTUN_MULTIPLIER, .8);
set_hitbox_value(AT_DAIR, 4, HG_SDI_MULTIPLIER, .1);
set_hitbox_value(AT_DAIR, 4, HG_ANGLE_FLIPPER, 0 );
set_hitbox_value(AT_DAIR, 4, HG_GROUNDEDNESS, 0 );
set_hitbox_value(AT_DAIR, 4, HG_IGNORES_PROJECTILES, 0 );
set_hitbox_value(AT_DAIR, 4, HG_TECHABLE, 2 );
set_hitbox_value(AT_DAIR, 4, HG_FORCE_FLINCH, 0 );
set_hitbox_value(AT_DAIR, 4, HG_THROWS_ROCK, 0 );
set_hitbox_value(AT_DAIR, 4, HG_EXTRA_CAMERA_SHAKE, 0 );
set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT, 112 );
set_hitbox_value(AT_DAIR, 4, HG_HIT_SFX, asset_get("sfx_ori_energyhit_medium"));
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_SPRITE, sprite_get("abyss_projectile_down"));
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_MASK, -1); // was yarnball_cmask
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_HSPEED, 10);
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_VSPEED, 10);
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_GRAVITY, 0);
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_GROUND_FRICTION, 0);
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_AIR_FRICTION, 0);
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_WALL_BEHAVIOR, 1 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 1 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_ENEMY_BEHAVIOR, 0 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_UNBASHABLE, 0 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_PARRY_STUN, 1 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_DOES_NOT_REFLECT, 1 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_IS_TRANSCENDENT, 1 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_PLASMA_SAFE, 1 );
set_hitbox_value(AT_DAIR, 4, HG_PROJECTILE_DESTROY_EFFECT, 112); // was 109
set_hitbox_value(AT_DAIR, 4, HG_EFFECT, 99); //this effect is used for knowing if it's a rune projectile
