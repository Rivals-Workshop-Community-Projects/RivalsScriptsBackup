...
if(attack == AT_NSPECIAL && (state == PS_ATTACK_AIR || state == PS_ATTACK_GROUND))
{
  ...
  switch(window)
  {
    case 2:
      shader_start();
      draw_sprite_ext(sprite_get("n_weapon_special"), 0, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
      shader_end();
      if(arrowsremain[select_cur] != 0)
      {
        switch(select_cur)
        {
          case 0:
            shader_start();
            draw_sprite_ext(sprite_get("powerarrow"), animframe, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
            break;
          case 1:
            draw_sprite_ext(sprite_get("firearrow"), animframe, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
            break;
          case 2:
            draw_sprite_ext(sprite_get("elecarrow"), animframe, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
            break;
          case 3:
            draw_sprite_ext(sprite_get("icearrow"), animframe, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
            break;
          case 4:
            draw_sprite_ext(sprite_get("rockarrow"), animframe, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
            break;
        }
      }
      else
    ...
    case 3:
      shader_start();
      draw_sprite_ext(sprite_get("n_weapon_special"), floor(window_timer/4)+1, x-14*spr_dir, y-38, 1, 1*spr_dir, abs(aim_angle), c_white, 1);
      break;
  }
}
shader_end();
...
if(select_timer != 0) { //See that the timer is active
  ...
  {
      uiSize = (select_cur == uiIcon+1 ? 1.5 : 1)
      ...
      if(arrowsmax[uiIcon+1] != arrowsremain[uiIcon+1])
        draw_debug_text(floor(sin(uiRound)*80) + x-10, floor(cos(uiRound)*80) + y - 40, string(abs(uiCharge/60)));
  }
  if(select_cur == 0)
  { 
