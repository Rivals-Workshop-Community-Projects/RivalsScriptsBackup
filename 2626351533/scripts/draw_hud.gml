...
with (mine) {
	...
	if idx != noone{
		draw_sprite_ext(sprite_get("article_offscreen2"), idx, clamp(x - view_get_xview(), margin, view_get_wview() - margin)-30, clamp(y - view_get_yview(), margin, view_get_hview() - margin)-45, 2, 2, 0, get_player_hud_color(player), 1);
		with other shader_start();
		draw_sprite_ext(sprite_get("article_offscreen2"), 8, clamp(x - view_get_xview(), margin, view_get_wview() - margin)-30, clamp(y - view_get_yview(), margin, view_get_hview() - margin)-45, 2, 2, 0, c_white, 1);
		with other shader_end();
	}
}
	}
