...
if karmatimer != 0 {
    if karmatimer >= 300 {
    draw_sprite(sprite_get("ADmark"), 32 - ((karmatimer - 300)/5),  curse_target.x + 8,  curse_target.y + 40); 
    }
    
    if karmatimer < 300 && karmatimer >= 30 {
        if karma < 20 {
            draw_sprite(sprite_get("ADmark"), 30 + (get_gameplay_time()%15)/5,  curse_target.x + 8,  curse_target.y + 40); 
        } else {
            draw_sprite(sprite_get("ADmark"), 33 + (get_gameplay_time()%15)/5,  curse_target.x + 8,  curse_target.y + 40); 
        }
    }
    ...
    if karmatimer < 30 {
         ...
         draw_sprite(sprite_get("ADmark"), 40 - (karmatimer/5), curse_target.x + 8,  curse_target.y - 16 + karmatimer*2); 
    }
}
}
///if soultimer == 0 {
