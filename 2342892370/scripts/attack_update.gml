...
if attack == AT_NSPECIAL {
	...
	if nshit > 3 && window == 1 && window_timer == 4 {
		nshit = -10
...
if attack == AT_DSPECIAL {
	...
	if window == 2 && window_timer == 1{
		if admw > 0 {
			karma += 8*admw
			take_damage(player, -1, -1*admw)
			admw = 0
			sound_play(sound_get("slice1"),false,noone,0.6)
			    spawn_hit_fx ( curse_target.x, curse_target.y + 6, wh )
		}
	}
	...
	if window == 2 && window_timer >=  10{
		if admb > 0 {
			karma += 15*admb
			take_damage(player, -1, -3*admb)
			admb = 0
			sound_play(sound_get("strong1"),false,noone,0.6)
			    spawn_hit_fx ( curse_target.x, curse_target.y + 6, bh )
		}
	}
