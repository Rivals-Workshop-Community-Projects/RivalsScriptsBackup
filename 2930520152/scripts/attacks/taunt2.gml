set_attack_value(AT_TAUNT_2, AG_SPRITE, sprite_get("taunt2"));
set_attack_value(AT_TAUNT_2, AG_NUM_WINDOWS, 5);
set_attack_value(AT_TAUNT_2, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_TAUNT_2, AG_OFF_LEDGE, 0);
set_attack_value(AT_TAUNT_2, AG_HURTBOX_SPRITE, asset_get("ex_guy_hurt_box"));


// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_TYPE, 1);
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_LENGTH, 364);
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_ANIM_FRAMES, 52);
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_HAS_SFX, 0);//1
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_SFX, sound_get("lol_earrape"));
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_SFX_FRAME, 19);

// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_TYPE, 1);
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_LENGTH, 140);//140
// set_window_value(AT_TAUNT_2, 1, AG_WINDOW_ANIM_FRAMES, 21);

set_window_value(AT_TAUNT_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_LENGTH, 50);//140
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_SFX, sound_get("default_dance"));

set_window_value(AT_TAUNT_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_LENGTH, 7);//140
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_ANIM_FRAME_START, 5);

set_window_value(AT_TAUNT_2, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 3, AG_WINDOW_LENGTH, 112);//140
set_window_value(AT_TAUNT_2, 3, AG_WINDOW_ANIM_FRAMES, 15);
set_window_value(AT_TAUNT_2, 3, AG_WINDOW_ANIM_FRAME_START, 6);

set_window_value(AT_TAUNT_2, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 4, AG_WINDOW_LENGTH, 10);
set_window_value(AT_TAUNT_2, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_TAUNT_2, 4, AG_WINDOW_ANIM_FRAME_START, 20);

set_window_value(AT_TAUNT_2, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 5, AG_WINDOW_LENGTH, 224);
set_window_value(AT_TAUNT_2, 5, AG_WINDOW_ANIM_FRAMES, 32);
set_window_value(AT_TAUNT_2, 5, AG_WINDOW_ANIM_FRAME_START, 20);

set_num_hitboxes(AT_TAUNT_2, 0);

//early

set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_WINDOW, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_WINDOW_CREATION_FRAME, 28);
set_hitbox_value(AT_TAUNT_2, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_X, 15);
set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_Y, -48);
set_hitbox_value(AT_TAUNT_2, 1, HG_WIDTH, 25);
set_hitbox_value(AT_TAUNT_2, 1, HG_HEIGHT, 25);
set_hitbox_value(AT_TAUNT_2, 1, HG_PRIORITY, 10);
set_hitbox_value(AT_TAUNT_2, 1, HG_DAMAGE, 10);
set_hitbox_value(AT_TAUNT_2, 1, HG_ANGLE, 361);
set_hitbox_value(AT_TAUNT_2, 1, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_TAUNT_2, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_TAUNT_2, 1, HG_BASE_HITPAUSE, 2);
set_hitbox_value(AT_TAUNT_2, 1, HG_EXTRA_HITPAUSE, 133);
set_hitbox_value(AT_TAUNT_2, 1, HG_VISUAL_EFFECT, 20);
set_hitbox_value(AT_TAUNT_2, 1, HG_HIT_SFX, asset_get("sfx_absa_kickhit"));
set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_GROUP, -1);

set_hitbox_value(AT_TAUNT_2, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_TAUNT_2, 2, HG_WINDOW, 1);
set_hitbox_value(AT_TAUNT_2, 2, HG_WINDOW_CREATION_FRAME, 133);
set_hitbox_value(AT_TAUNT_2, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_TAUNT_2, 2, HG_HITBOX_X, 15);
set_hitbox_value(AT_TAUNT_2, 2, HG_HITBOX_Y, -48);
set_hitbox_value(AT_TAUNT_2, 2, HG_WIDTH, 25);
set_hitbox_value(AT_TAUNT_2, 2, HG_HEIGHT, 25);
set_hitbox_value(AT_TAUNT_2, 2, HG_PRIORITY, 10);
set_hitbox_value(AT_TAUNT_2, 2, HG_DAMAGE, 10);
set_hitbox_value(AT_TAUNT_2, 2, HG_ANGLE, 361);
set_hitbox_value(AT_TAUNT_2, 2, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_TAUNT_2, 2, HG_KNOCKBACK_SCALING, 1);
set_hitbox_value(AT_TAUNT_2, 2, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_TAUNT_2, 2, HG_VISUAL_EFFECT, 197);
set_hitbox_value(AT_TAUNT_2, 2, HG_HIT_SFX, asset_get("sfx_absa_kickhit"));
set_hitbox_value(AT_TAUNT_2, 2, HG_HITBOX_GROUP, -1);