set_attack_value(AT_DTHROW, AG_CATEGORY, 2);
set_attack_value(AT_DTHROW, AG_SPRITE, sprite_get("dthrow"));
set_attack_value(AT_DTHROW, AG_NUM_WINDOWS, 5);
set_attack_value(AT_DTHROW, AG_AIR_SPRITE, sprite_get("dthrow"));
set_attack_value(AT_DTHROW, AG_HURTBOX_SPRITE, sprite_get("dthrow_hurt"));

set_window_value(AT_DTHROW, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTHROW, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DTHROW, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DTHROW, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 1, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTHROW, 1, AG_WINDOW_SFX_FRAME, 7);
set_window_value(AT_DTHROW, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_medium1"));
set_window_value(AT_DTHROW, 1, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_DTHROW, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTHROW, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DTHROW, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTHROW, 2, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_DTHROW, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 2, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 2, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_DTHROW, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTHROW, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DTHROW, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DTHROW, 3, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DTHROW, 3, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 3, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTHROW, 3, AG_WINDOW_SFX_FRAME, 11);
set_window_value(AT_DTHROW, 3, AG_WINDOW_SFX, asset_get("sfx_zetter_downb"));
set_window_value(AT_DTHROW, 3, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_DTHROW, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTHROW, 4, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DTHROW, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTHROW, 4, AG_WINDOW_ANIM_FRAME_START, 7);
set_window_value(AT_DTHROW, 4, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 4, AG_WINDOW_VSPEED_TYPE, 1);

set_window_value(AT_DTHROW, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTHROW, 5, AG_WINDOW_LENGTH, 15);
set_window_value(AT_DTHROW, 5, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DTHROW, 5, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_DTHROW, 5, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DTHROW, 5, AG_WINDOW_VSPEED_TYPE, 1);

set_num_hitboxes(AT_DTHROW, 2);

set_hitbox_value(AT_DTHROW, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTHROW, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DTHROW, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DTHROW, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_DTHROW, 1, HG_HITBOX_X, 16);
set_hitbox_value(AT_DTHROW, 1, HG_HITBOX_Y, -36);
set_hitbox_value(AT_DTHROW, 1, HG_WIDTH, 30);
set_hitbox_value(AT_DTHROW, 1, HG_HEIGHT, 66);
set_hitbox_value(AT_DTHROW, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DTHROW, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_DTHROW, 1, HG_ANGLE, 260);
set_hitbox_value(AT_DTHROW, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DTHROW, 1, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_DTHROW, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTHROW, 1, HG_DRIFT_MULTIPLIER, .01);
set_hitbox_value(AT_DTHROW, 1, HG_SDI_MULTIPLIER, .01);
set_hitbox_value(AT_DTHROW, 1, HG_TECHABLE, 3);
set_hitbox_value(AT_DTHROW, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTHROW, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTHROW, 1, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTHROW, 1, HG_HIT_SFX, asset_get("sfx_blow_medium2"));

set_hitbox_value(AT_DTHROW, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTHROW, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DTHROW, 2, HG_SHAPE, 2);
set_hitbox_value(AT_DTHROW, 2, HG_WINDOW, 4);
set_hitbox_value(AT_DTHROW, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_DTHROW, 2, HG_HITBOX_X, 0);
set_hitbox_value(AT_DTHROW, 2, HG_HITBOX_Y, -15);
set_hitbox_value(AT_DTHROW, 2, HG_WIDTH, 90);
set_hitbox_value(AT_DTHROW, 2, HG_HEIGHT, 30);
set_hitbox_value(AT_DTHROW, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DTHROW, 2, HG_DAMAGE, 6);
set_hitbox_value(AT_DTHROW, 2, HG_ANGLE, 90);
set_hitbox_value(AT_DTHROW, 2, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(AT_DTHROW, 2, HG_KNOCKBACK_SCALING, .6);
set_hitbox_value(AT_DTHROW, 2, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_DTHROW, 2, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DTHROW, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTHROW, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTHROW, 2, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));

if(has_rune("N")){
    set_hitbox_value(AT_DTHROW, 2, HG_TECHABLE, 3);
    set_hitbox_value(AT_DTHROW, 2, HG_EFFECT, 15);
    set_hitbox_value(AT_DTHROW, 2, HG_DAMAGE, 18);
    set_hitbox_value(AT_DTHROW, 2, HG_EXTRA_HITPAUSE, 10);
}