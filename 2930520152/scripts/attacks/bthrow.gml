set_attack_value(AT_NTHROW, AG_CATEGORY, 2);
set_attack_value(AT_NTHROW, AG_SPRITE, sprite_get("bthrow"));
set_attack_value(AT_NTHROW, AG_NUM_WINDOWS, 5);
set_attack_value(AT_NTHROW, AG_AIR_SPRITE, sprite_get("bthrow"));
set_attack_value(AT_NTHROW, AG_HURTBOX_SPRITE, sprite_get("bthrow_hurt"));

set_window_value(AT_NTHROW, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_NTHROW, 1, AG_WINDOW_LENGTH, 4);
set_window_value(AT_NTHROW, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_NTHROW, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 1, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_NTHROW, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_NTHROW, 1, AG_WINDOW_SFX_FRAME, 3);
set_window_value(AT_NTHROW, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_medium1"));
set_window_value(AT_NTHROW, 1, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_NTHROW, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_NTHROW, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_NTHROW, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_NTHROW, 2, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_NTHROW, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 2, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 2, AG_WINDOW_HSPEED, -2);
set_window_value(AT_NTHROW, 2, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_NTHROW, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_NTHROW, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_NTHROW, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_NTHROW, 3, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_NTHROW, 3, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 3, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 3, AG_WINDOW_HSPEED, -2);
set_window_value(AT_NTHROW, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_NTHROW, 3, AG_WINDOW_SFX_FRAME, 11);
set_window_value(AT_NTHROW, 3, AG_WINDOW_SFX, asset_get("sfx_swipe_medium2"));
set_window_value(AT_NTHROW, 3, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_NTHROW, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_NTHROW, 4, AG_WINDOW_LENGTH, 4);
set_window_value(AT_NTHROW, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_NTHROW, 4, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_NTHROW, 4, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 4, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_NTHROW, 4, AG_WINDOW_CUSTOM_AIR_FRICTION, ground_friction);
set_window_value(AT_NTHROW, 4, AG_WINDOW_CUSTOM_GROUND_FRICTION, ground_friction);

set_window_value(AT_NTHROW, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_NTHROW, 5, AG_WINDOW_LENGTH, 16);
set_window_value(AT_NTHROW, 5, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_NTHROW, 5, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_NTHROW, 5, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_NTHROW, 5, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_NTHROW, 5, AG_WINDOW_CUSTOM_AIR_FRICTION, ground_friction);
set_window_value(AT_NTHROW, 5, AG_WINDOW_CUSTOM_GROUND_FRICTION, ground_friction);

set_num_hitboxes(AT_NTHROW, 2);

set_hitbox_value(AT_NTHROW, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NTHROW, 1, HG_SHAPE, 0);
set_hitbox_value(AT_NTHROW, 1, HG_WINDOW, 2);
set_hitbox_value(AT_NTHROW, 1, HG_LIFETIME, 0);
set_hitbox_value(AT_NTHROW, 1, HG_HITBOX_X, -15);
set_hitbox_value(AT_NTHROW, 1, HG_HITBOX_Y, -40);
set_hitbox_value(AT_NTHROW, 1, HG_WIDTH, 60);
set_hitbox_value(AT_NTHROW, 1, HG_HEIGHT, 40);
set_hitbox_value(AT_NTHROW, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_NTHROW, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_NTHROW, 1, HG_ANGLE, 145);
set_hitbox_value(AT_NTHROW, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_NTHROW, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_NTHROW, 1, HG_DRIFT_MULTIPLIER, .01);
set_hitbox_value(AT_NTHROW, 1, HG_SDI_MULTIPLIER, .01);
set_hitbox_value(AT_NTHROW, 1, HG_TECHABLE, 3);
set_hitbox_value(AT_NTHROW, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_NTHROW, 1, HG_VISUAL_EFFECT_X_OFFSET, -16);
set_hitbox_value(AT_NTHROW, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_NTHROW, 1, HG_HIT_SFX, asset_get("sfx_blow_medium2"));

set_hitbox_value(AT_NTHROW, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NTHROW, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_NTHROW, 2, HG_SHAPE, 2);
set_hitbox_value(AT_NTHROW, 2, HG_WINDOW, 4);
set_hitbox_value(AT_NTHROW, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_NTHROW, 2, HG_HITBOX_X, -15);
set_hitbox_value(AT_NTHROW, 2, HG_HITBOX_Y, -30);
set_hitbox_value(AT_NTHROW, 2, HG_WIDTH, 60);
set_hitbox_value(AT_NTHROW, 2, HG_HEIGHT, 50);
set_hitbox_value(AT_NTHROW, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_NTHROW, 2, HG_DAMAGE, 8);
set_hitbox_value(AT_NTHROW, 2, HG_ANGLE, 140);
set_hitbox_value(AT_NTHROW, 2, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_NTHROW, 2, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_NTHROW, 2, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_NTHROW, 2, HG_VISUAL_EFFECT_X_OFFSET, -22);
set_hitbox_value(AT_NTHROW, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_NTHROW, 2, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));

if(has_rune("N")){
    set_hitbox_value(AT_NTHROW, 2, HG_DAMAGE, 20);
    set_hitbox_value(AT_NTHROW, 2, HG_BASE_KNOCKBACK, 9);
    set_hitbox_value(AT_NTHROW, 2, HG_KNOCKBACK_SCALING, 1.45);
    set_hitbox_value(AT_NTHROW, 2, HG_HITSTUN_MULTIPLIER, 1.1);
}