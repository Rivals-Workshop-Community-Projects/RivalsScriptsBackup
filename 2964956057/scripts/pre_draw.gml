...
if get_player_color(player) == 4{
	...
	with pHitBox if player_id == other && type == 2{
		...
		obj_flag(make_colour_rgb(247, 168, 223), 5/10, 5/20); // pink
		obj_flag(c_white, 1/10, 9/20);//white
	}
	
	with flowey_save{
		obj_flag(make_colour_rgb(85, 205, 252), 1, 0); // mayablue
		obj_flag(make_colour_rgb(247, 168, 223), 3/10, 7/20); // pink
		obj_flag(c_white, 1/10, 9/20);//white
	}
}

shader_start();
if state == PS_ATTACK_GROUND && attack == AT_DTILT draw_sprite_ext(sprite_get("dtilt_vine"), image_index, x + ((dtilt_pos - dtilt_dist) * spr_dir), y, spr_dir * (1 + small_sprites), 1 + small_sprites, image_angle, c_white, 1);
