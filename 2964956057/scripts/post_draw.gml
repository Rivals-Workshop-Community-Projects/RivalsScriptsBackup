if state == PS_DOUBLE_JUMP draw_sprite_ext(sprite_get("wings"), image_index, x, y, spr_dir * 2, 2, 0, make_color_hsv(hue, color_get_saturation(color_rgb), color_get_value(color_rgb)), 1);
...
if "genesis_shockwave_vfx" in self if genesis_shockwave_vfx && genesis{
	shader_start();
	draw_sprite_ext(sprite_index, image_index, x, y + ((char_height / 2) * random_func_2((sprite_index % 200), 2, false) * (attack == AT_UAIR || attack == AT_FAIR)), image_xscale + (0.5 + random_func_2((sprite_index % 200), 2, false)) * spr_dir, image_yscale + 0.5 + random_func_2((sprite_index % 200), 2, false), image_angle, c_red, 1 - (hitstop_full - hitstop) / hitstop_full / 2);
	shader_end();
}
...
if object_index == asset_get("oTestPlayer"){ //this checks if it's in a playtest screen.
	if !variable_instance_exists(id, "pn__init"){ //initialization part runs once
		...
		patch_note_text = [
			...
			Jab:
			- Increased Jab 1 endlag 9 -> 14.
			+ Jab 1 hitstun multiplier normalized 0.85 -> 1.
			= Changes made to keep Jab 1 from true-comboing into itself without the hitstun multiplier.
			Fair:
			= Spike hitpause adjusted 18/0.9 -> 14/1.4.
			+ Extra camera shake added to spike.
			Ftilt:
			+ Hitstun multiplier normalized 0.8 -> 1.
			= V_fork why did you leave this in here.
			- Kb decreased to compensate 7/0.75 -> 7/0.7.
			Ustrong:
			...
			"Version 1.7.1
			= Fixed issue where NDSpecial instantly obliterated everyone if it didn't have a homing target.
			= Fixed issue where NSpecial still homed on players taunting on the respawn platform.",
			
			"Version 1.7.2
			= DTilt now positions itself so it doesn't go off the ledge.
			= Reorganized Runes.
			+ Cleaned up a fuckton of code."
		];
		...
		pn__list_max = array_length(patch_note_title) - 1; //print(string(pn__list_max));
	...
	if variable_instance_exists(id, "pn__init"){ //this is very unnecessary i just want to be safe to avoid error log
		...
		//offset
		if pn__opener_timer < pn__opener_timer_max pn__opener_timer++;
		if pn__select_timer < pn__select_timer_max pn__select_timer++;
		if pn__open var bc_yA = ease_expoOut(bc_yA - 40, bc_y-page_h, pn__opener_timer, pn__opener_timer_max); //(if opened) 
		else var bc_yA = ease_expoOut(bc_y + 40 - page_h, bc_yA, pn__opener_timer, pn__opener_timer_max); //(if closed)
		...
		//txt alpha
		if pn__open var txt_a = ease_expoOut( 0, 1, pn__opener_timer, pn__opener_timer_max); //(if opened)
		else var txt_a = ease_expoOut( 1, 0, pn__opener_timer, pn__opener_timer_max); //(if closed)
		...
		if pn__open{ //(if open)
			...
			if pn__displaying == false{
				...
				if pn__list_old == "up" var l_off_y = ease_cubeOut( 4, 0, pn__select_timer, pn__select_timer_max); //prev input is up
				if pn__list_old == "down" var l_off_y = ease_cubeOut( -4, 0, pn__select_timer, pn__select_timer_max); //prev input is down
				...
				if pn__open{ //scroll stuff n select stuff
					if up_down{
						if pn__up_track == false{
							pm__list_select_old = pn__list_select;
							pn__list_select--;
			...
			if pn__displaying == true{ //txt set
				draw_set_halign(fa_left);
				draw_set_valign(fa_top);
				draw_set_font(asset_get("fName"));
