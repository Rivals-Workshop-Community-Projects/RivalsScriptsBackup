...
if alpher > 0 alpher -= 0.05;

if alpher_alt > 0 alpher_alt -= 0.025;

if get_player_color(real_player) != prev_alt || champ_cur != prev_champ{
	alpher_alt = 3;
	if get_player_color(player) == 22 sound_play(sound_get("glitch_select"));
}
...
switch get_player_color(real_player){
	...
	case 23:
		...
		if hovering == 40 draw_sprite_ext(sprite_get("css_champ_flash"), 0, temp_x + 180, temp_y + 102 + runes_mode, 1, 1, 0, c_white, 0.8 + (sin(hovering_timer / 20) * -0.8));
	break;
	case 24:
...
// genesis css glitch
if get_player_color(player) == 22 && random_func_2(69, 2, 1) == 0{
...
else if get_player_color(real_player) == 24 textDraw(temp_x + 100, temp_y + 36, "fName", colorgroup, 14, 120, 1, 1, alpher_alt, "Press A on icon to customize", c_black);
