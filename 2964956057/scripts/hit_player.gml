...
switch my_hitboxID.attack{
	...
	break;
	case AT_UTILT:
		if instance_exists(hit_player_obj) && !hit_player_obj.clone && !hit_player_obj.super_armor && hit_player_obj.soft_armor < check_kb hit_player_obj.x = lerp(hit_player_obj.x, x, 0.1);
	break;
	case AT_FSTRONG:
	...
	case AT_USTRONG:
		switch my_hitboxID.hbox_num{
			...
			case 2: case 3: //Catchers
				...
				if instance_exists(hit_player_obj) && !hit_player_obj.clone && !hit_player_obj.super_armor && hit_player_obj.soft_armor < check_kb hit_player_obj.x = lerp(hit_player_obj.x, x, 0.15);
			break;
		}
	break;
	case AT_NAIR:
	...
	case AT_DAIR:
		...
		if my_hitboxID.hbox_num == 1 sound_play(asset_get("sfx_blow_medium2"), false, noone, 0.3, 0.9);
		else sound_play(asset_get("sfx_blow_medium3"), false, noone, 0.5, 1.2);
		old_hsp /= 2;
		old_vsp = -((check_kb * 0.7) - ((my_hitboxID.hbox_num == 2) / 10));
	break;
	case AT_UAIR:
		if my_hitboxID.hbox_num < 3 sound_play(asset_get("sfx_ori_energyhit_medium"), false, noone, 0.7, 1.6);
		else sound_play(asset_get("sfx_ori_energyhit_heavy"), false, noone, 0.7, 1.9);
		sound_play(asset_get("sfx_clairen_hit_med"), false, noone, 0.6, 1.5 + ((my_hitboxID.hbox_num == 2) / 2));
