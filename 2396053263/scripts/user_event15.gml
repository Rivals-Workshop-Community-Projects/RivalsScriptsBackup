...
if (object_index == asset_get("cs_playerbg_obj")){
	num_alts = 15; // Number of alt costumes; controls how many appear on the CSS
	exit;
}
...
with phone{
	...
	initTip("Pulse Curse");
	if ("spr_heart" in player_id) initTipImage_ext(player_id.spr_heart, 1, fa_center, 1, c_white, 0, 32, 32, 30, 32);
	initTipWords("Many of Heartbrake's attacks inflict a Pulse Curse, a stage control element that tails behind the opponent as they move.
	
	By using DSpecial, you can activate the heart, which will cause it to stop moving and split apart. Any enemy in between the two halves will trigger them, causing them to close together. If hit by this, the opponent will be stunned for a long time.
	
	Here's a list of all the attacks that inflict a Pulse Curse:
	UTilt (Sweetspot)
	FAir (Sweetspot)
	BAir (Sweetspot)
	FSpecial
	...
	Having your Pulse Curses active as much as possible can control a lot of space, but may not pressure the opponent too much.
	Instead, try to activate them when they are in the worst spot for your opponent, such as covering a major part of the stage, or when they're recovering.");
	
	initTip("V-Brake");
	initTipWords("Pressing taunt in the air allows you to perform a V-Brake, which has different properties based on when it is pressed.
	When not holding a direction, V-Braking allows you to float in the air for a brief time.
	When holding a direction, or when pressing a direction, you preform an aerial dash that halts your vertical speed, but grants a horizontal boost.
	You can use V-Braking during attacks, but not to float with; you also only get one per airtime (restored on walljump).");
	initTipImage_ext(player_id.spr_tauntair, 5, fa_center, 1, c_white, 3, 62, 62, 62, 0);
	initTipWords("This is Heartbrake's best way to move around. Its applications are extremely broad, so have fun experimenting!
	
	Although, you might need to bind your taunt button to something more accessible. Sorry, them's the brakes!");
	
	
	initTip("NSpecial: Cardiac Arrest");
	initTipWords("This attack throws opponents either left or right, completely horizontally until they reach the end of the distance.
	Its knockback is completely fixed, and never scales. Which means that it's great for killing off the side blastzone offstage!
	
	You can also use it to trap opponents in your Pulse Brake, by tossing them into it, or simply holding them in place until it finishes closing.");
	initTipImage_ext(player_id.spr_nspecial, 14, fa_left, 1, c_white, 0, 62, 62, 80, 0);
	
	initTip("FSpecial: Piercing Shards");
	initTipWords("This move throws 3 projectiles at once in a narrow fan. Interestingly, they deal less damage and knockback as they travel, every 3 frames until they disappear.
	The move also has very little endlag, which makes it great as a combo starter point blank.
	It inflicts a Pulse Curse on hit, regardless of projectile age.");
	initTipImage(player_id.spr_fspecial, -5, fa_center, 1, c_white, 0);
	
	initTip("USpecial: Bore");
	initTipWords("This multihitting drill kick can be V-Braked, and its knockback sends in the direction you are travelling.
	Which means...");
	initTipWords_ext("Dragdown USpecial.", fa_left, c_red, 0, 1);
	initTipImage(player_id.spr_uspecial, 2, fa_left, 1, c_white, 0);
	initTipImage(player_id.spr_uspecial, 6, fa_right, 1, c_white, 0);
	initTipWords("The final hit also inflicts a Pulse Curse.");
	
	initTip("DSpecial: Pulse Brake");
	initTipWords("This special activates your Pulse Curse from any range. See the Pulse Curse section for more details.
	
	It also has a hitbox! A very weak one, but it's funny and can sometimes even be practical.
	On top of that, it stalls you in the air for a bit, which can be useful as a recovery mixup.");
	initTipImage(player_id.spr_dspecial, -5, fa_center, 1, c_white, 0);
	
	initTip("Super Tilt Boost");
	initTipWords("If you tilt-cancel Heartbrake's second hit of Jab, you'll be sent forward a decent distance. It's pretty telegraphed, but can be a useful mixup option.
	When combined with FTilt, you can move forward at an incredible speed!
	
	Alternatively, try hitting an opponent with Jab2, then using FTilt in the opposite direction of them. They might not see it coming!");
	
	initTip("Did You Know?");
	initTipWords("Heartbrake's real name is Lily.
	She once was in a relationship with Callie, but that eventually went sour. 
	Rumor has it that their relationship formed Heartbrake into who she is today. Just what happened between those two...?");
...
with phone{
	...
	initPatch("1.0", "February 14, 2021");
	initPatchWords_ext("Released! The beginning of the end is now.", fa_center, c_white, 0, 0);
...
with self{
	...
	// update.gml
	initCheat("Infinite V-Brakes", "cheat_inf_vbrake", [0, 1], ["No", "Yes"], "Gain infinity V-Brakes instead of only one.");
	
	// update.gml
	initCheat("No Cooldowns", "cheat_cooldown", [0, 1], ["No", "Yes"], "FSpecial and DSpecial no longer have cooldowns.");
...
with phone{
	...
	initAbout("About Heartbrake", "Heartbrake was created by Trail Mix, with coding help from Delta Parallax and design help from both FungiWizard and Delta Parallax.");
