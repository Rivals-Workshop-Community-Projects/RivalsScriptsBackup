...
if (ncharge >= 1 && ncharge <= 59) {
	draw_sprite(sprite_get("Charge1Post"), get_gameplay_time()*.15, x, y);
}


if (ncharge >= 30 && ncharge <= 59) {
	draw_sprite(sprite_get("Charge2Post"), get_gameplay_time()*.15, x, y);
}

if (attack == AT_NSPECIAL && window == 2 && state == PS_ATTACK_GROUND) {
	draw_sprite(sprite_get("Ground1Post"), get_gameplay_time()*.15, x, y);
}

if (ncharge >= 60 && ncharge <= 119) {
	draw_sprite(sprite_get("Charge3Post"), get_gameplay_time()*.15, x, y);; 
}

if (ncharge >= 90 && ncharge <= 119) {
	draw_sprite(sprite_get("Charge4Post"), get_gameplay_time()*.15, x, y);; 
	draw_sprite(sprite_get("Charge4Post2"), get_gameplay_time()*.15, x, y);
}
