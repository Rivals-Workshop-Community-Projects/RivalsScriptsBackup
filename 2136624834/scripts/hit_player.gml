...
if my_hitboxID.attack == AT_NSPECIAL && my_hitboxID.hbox_num == 3 {
	...
	create = create_hitbox(AT_NSPECIAL,2,floor(my_hitboxID.x ),floor(my_hitboxID.y))
	create.spr_dir = my_hitboxID.spr_dir
	create.hsp = 3*my_hitboxID.spr_dir
	create.vsp = -6
	create.nomorehit = 20
...
if my_hitboxID.attack == AT_NSPECIAL && my_hitboxID.hbox_num == 2 {
	...
	create = create_hitbox(AT_NSPECIAL,1,floor(my_hitboxID.x ),floor(my_hitboxID.y))
	create.spr_dir = my_hitboxID.spr_dir
	create.hsp = 4*my_hitboxID.spr_dir 
	create.vsp = -5
	create.nomorehit = 20
	my_hitboxID.destroyed = true 
}
...
if timestop < 100 {
move_cooldown[AT_FSPECIAL_2] = min(10 + hit_player_obj.hitstop*4, 60)
}
...
if my_hitboxID.attack == AT_USTRONG && my_hitboxID.hbox_num == 3 {
	sound_play(sound_get("RI"),false,noone,.6,.85 )
}


}

if timestop < 100 && timefreeze < 1 && move_cooldown[AT_DSPECIAL] <= 10{
    if timestop < 95 {
    timestop += my_hitboxID.damage * 0.8
    } else {
     timestop = 101
    }
}
...
if my_hitboxID.attack == AT_FSPECIAL{
   ...
   if move_cooldown[AT_NSPECIAL] == 0 && ncharge < 60{
   	...
   	snox = hit_player_obj.x
   	snoy = hit_player_obj.y - 30
   }
}
