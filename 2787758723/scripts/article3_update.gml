...
switch _action[P.LOAD][L.ACTION_TYPE] {
    ...
    case ACT.CHECK:
    	var _param = _action[P.LOAD][L.PARAM];
    	var _action_true = false;
    	if (_param[0] < 0)
    		with obj_stage_main if (_param[1] in self) && variable_instance_get(id,_param[1]) == _param[2] _action_true = true;	
    	if (_param[0] >= 0) {
			with obj_stage_article if "action_article_index" in self && action_article_index == _param[0] if variable_instance_get(id,_param[1]) == _param[2] _action_true = true;
			with obj_stage_article_solid if "action_article_index" in self && action_article_index == _param[0] if variable_instance_get(id,_param[1]) == _param[2] _action_true = true;
			with obj_stage_article_platform if "action_article_index" in self && action_article_index == _param[0] if variable_instance_get(id,_param[1]) == _param[2] _action_true = true;
			}
    	_action[@P.DIE] = _action_true;
    	break;
    case ACT.CONTROL:
    	with oPlayer {
    ...
    case ACT.SET:
	    switch _param[3] { //ease_type?
	    	case 1:
					...
					with obj_stage_article_solid if "action_article_index" in self && action_article_index == _param[0] {
						variable_instance_set(id,_param[1],_param[2]);
					} with obj_stage_article_platform if "action_article_index" in self && action_article_index == _param[0] {
						variable_instance_set(id,_param[1],_param[2]);
					}
		    		_action[@P.DIE] = true;
	    		break;
...
if new_action[P.ROOM_ID] != 0 && new_action[P.ROOM_ID] != room_id return false; //Only process actions effecting the current room, or All Rooms
if new_action[P.SCENE_ID] != 0 && new_action[P.SCENE_ID] != scene_id return false; //Only process actions effecting the current scene, or All Scenes
//On action start
...
switch new_action[P.LOAD][L.ACTION_TYPE] {
	...
	case ACT.WINDOW:
		...
		with obj_stage_main {
			...
			if win_over != [] for(var i = 0; i < array_length_1d(win_over) - 1; i++) for(var j = 0; j < array_length_1d(win_over[i]) - 1; j++) win_data[@_param[0]][@(i+1)][@(j+1)] = win_over[i][j];
	...
	case ACT.CHECK:
    	...
    	var _action = false	
    	if (_param[0] < 0)
    		with obj_stage_main if (_param[1] in self) && variable_instance_get(id,_param[1]) == _param[2] _action = true;	
    	if (_param[0] >= 0)
			with obj_stage_article if "action_article_index" in self && action_article_index == _param[0] if variable_instance_get(id,_param[1]) == _param[2] _action = true;
    ...
    case ACT.DESPAWN:
    	var _param = new_action[P.LOAD][L.PARAM];
    	with obj_stage_article if "action_article_index" in self && action_article_index == _param[0] {
    		if "destroyed" in self destroyed = true; //Enemies have their own distruction methods
	...
	case ACT.SPAWN_ART: //Spawn an article. Not counted in room!
		var _param = new_action[P.LOAD][L.PARAM];
		var _rel_pos = cell_to_real([_param[1][1]+room_manager.grid_offset,_param[1][2]-room_manager.grid_offset],_param[0]);
		var _art;
		switch _param[1][3] {
            case 2:
        ...
        _art.cell_pos = room_manager.cell_pos;
