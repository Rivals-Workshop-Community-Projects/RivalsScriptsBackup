...
if state == 0 && (active_scene == 0 || active_scene == room_manager.cur_scene){ //trigger active
	if (!instance_exists(collis_obj) || collis_obj.mask_index != asset_get("empty_sprite")) {
    ...
    if instance_exists(collis_obj) { // && room_manager.cur_room_time > 30 
		...
		if (collis_obj.clone || collis_obj.custom_clone) in_cam = true;
        if in_cam {
            with obj_stage_main {
                cam_override_obj = other.id;
    ...
    if collis_obj == noone && obj_stage_main.cam_override_obj == id {
        in_cam = false;
        with obj_stage_main {
