...
if !_init {
    ...
    if trigger_shape > 1 { //If the variable looks to be an index, use it
        ...
        img_spd = spawn_variables[5];
        mask_index = trigger_shape; 
    }
    else {
        mask_index = asset_get("solid_32x32"); 
        image_xscale = trigger_w;
        image_yscale = trigger_h;
    }
    ...
    visible = trigger_shape != 3;
    _init = 1;
}
if trigger_shape > 1 {
    image_index = get_gameplay_time() * img_spd
}
if state == 2 {
    instance_destroy();
    exit;
...
switch trigger_shape {
    ...
    case 1:
        ...
        break;
    case 3:
        if point_distance(x + trigger_w/2, y + trigger_h/2, obj_stage_main.room_manager.follow_point.x, obj_stage_main.room_manager.follow_point.y) <= 128
            collis_obj = obj_stage_main.room_manager.follow_player;
        else
            collis_obj = noone;
        break;
    default:
