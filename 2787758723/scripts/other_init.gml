...
//If the character is in an adventure!
in_adventure = true;
...
dead_pos = [x,y];
death_cooldown = 0;
death_cooldown_max = 3;
old_pos = [x,y];
hit_pos = [x,y];
hit_cooldown_max = 60;
hit_cooldown = 0;

respawn_point = [[0, 0],[16+4, 16-4],1];
...
actually_dead = false; //For training mode purposes.
can_respawn = false;

//#region exclusive to mario
am_mario_health = 1;
am_mario_state = 0; //0 = normal; 1 = death; 2 = grow; 3 = shrink; 4 = warp; 5 = flagple
am_mario_state_timer = 0;
am_mario_mask_prev = mask_index;
am_mario_draw_xsp = 0;
am_mario_draw_ysp = 0;
am_mario_invincible = 0;
am_mario_invincible_max = 540 + 120;
am_mario_dash_speed_old = 0;
am_mario_lives = 3;
am_mario_warp_dir = 0;
am_mario_warp_vars = [[0, 0],[0, 0], 1, 0];
am_mario_warp_pos_s = [x, y]
am_mario_warp_pos_e = [x, y]
am_mario_warp_out = -1;
am_mario_warp_fade = false;
am_mario_flagpole = noone;
am_mario_castle = noone;
//#endregion
...
if !("am_is_guest" in self)         am_is_guest = true;             //Is this character an aetherian local, or are they a strange guest from another dimension?
...
switch p_name {
    ...
    case "Orcane":
        am_is_water = true;
        am_is_noob = true; //Does not mean he's naieve - just that other percieve it
        // am_is_undead = true; //when orcane side specials are we sure it's the same orcane?
        am_is_myth = true;
        am_is_verbal = false;
        // am_is_experienced = true; //Does not mean he's experienced - it means other do not percieve it
        am_is_handed = false;
        am_is_guest = false; 
        // am_is_not = true;
        pronouns = ["he","him","his","is"];
