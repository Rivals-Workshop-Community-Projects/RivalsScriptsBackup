...
sfx_pho_open = sound_get("_pho_open1");
sfx_pho_close = sound_get("_pho_close2");
sfx_pho_move = sound_get("_pho_move");
sfx_pho_move_home = sound_get("_pho_move");
sfx_pho_page = sound_get("_pho_select1");
sfx_pho_open_app = sound_get("_pho_select1");
sfx_pho_close_app = sound_get("_pho_close1");
sfx_pho_power_off = sound_get("_pho_close2");
sfx_pho_power_on = sound_get("_pho_close2");
...
with phone{
	...
	APP_HOME		= pho_initApp("Home Screen",	$3F7935, $7A9B13, apps);
	APP_TIPS		= pho_initApp("Tips",			$3F7935, $7A9B13, tips);
	APP_PATCHES		= pho_initApp("Patches",		$3F7935, $7A9B13, patches);
	APP_DATA		= pho_initApp("Data",			$3F7935, $7A9B13, data);
	APP_CHEATS		= pho_initApp("Cheats",			$3F7935, $7A9B13, cheats);
	APP_UTILS		= pho_initApp("Utilities",		$3F7935, $7A9B13, utils);
	APP_POWER		= pho_initApp("Power",			$3F7935, $7A9B13, []);
}
...
with phone{
	...
	UTIL_MILITARY   = pho_initUtil("Clock Format", [0, 1], ["12-Hour", "24-Hour"], "Change the format of the phone's clock.");
...
initWords_ext("- Muno", fa_right, $4ABAA7, 0, 0);
...
initHeader("Turned back into Munophone 1 by");
initSection("Flophawk");
initHeader("SFX from");
initSection("Super Mario 64");
initHeader("Firmware");
		...
		if phone_offscreen[i] != noone{
			...
			if !instance_exists(phone_offscreen[i]){
				...
				if idx != noone{
					...
					draw_sprite_ext(phone_offscr_sprite, phone_offscr_index, clamp(x_ - view_get_xview(), margin, view_get_wview() - margin) - 32, clamp(y_ - view_get_yview(), margin, view_get_hview() - 52 - margin) - 32, 2, 2, 0, $4ABAA7, 1);
...
if !phone.has_opened_yet && phone_practice{
	...
	textDraw(x_pos, y_pos, "fName", $4ABAA7, 100, 100, fa_left, 1, true, 1, "Taunt!", true);
...
phone.home_scroll = lerp(phone.home_scroll, phone.home_scroll_dest, 0.2);
	...
	var text_y = phone_y + 16;
	
	textDraw(text_x, text_y, "fName", $4ABAA7, 18, phone.screen_width, fa_center, 1, true, 1, "ATTACK:", true);
	text_y += phone.last_text_size[1];
	textDraw(text_x, text_y, "fName", $303B20, 18, phone.screen_width, fa_center, 1, false, 1, "Disable Phone", true);
	text_y += phone.last_text_size[1];
	textDraw(text_x, text_y, "fName", $4ABAA7, 18, phone.screen_width, fa_center, 1, true, 1, "SPECIAL:", true);
	text_y += phone.last_text_size[1];
	textDraw(text_x, text_y, "fName", $303B20, 18, phone.screen_width, fa_center, 1, false, 1, "Cancel", true);
	text_y += phone.last_text_size[1] * 2;
	textDraw(text_x, text_y - 8, "fName", $303B20, 18, phone.screen_width, fa_center, 1, false, 1, "Press F5 to turn back on.", false);
}
	...
	if array_length(arr){
		var text_y = phone_y + 24;
		text_y -= phone.scroll_dist;
		for (var i = 0; i < array_length(arr); i++){
			var sel = phone.cursor == i;
			textDraw(text_x + sel * phone.cursor_change_timer, text_y, "fName", sel ? $4ABAA7 : phone.apps[phone.app].color, 18, phone.screen_width - 8, fa_center, 1, sel, 1, arr[i].name, true);
			text_y += phone.last_text_size[1] + 6;
		}
	}
	else{
		var text_y = phone_y + 65;
		textDraw(text_x, text_y, "fName", $4ABAA7, 18, phone.screen_width, fa_center, 1, true, 1, "No " + phone.apps[phone.app].name + "
		Loaded", true);
		text_y += phone.last_text_size[1];
		textDraw(text_x, text_y, "fName", $4ABAA7, 18, phone.screen_width, fa_center, 1, false, 1, "bruhm oment", false);
	}
...
if phone.app{
	...
	var app_x = phone_x + 2;
	var app_y = phone_y - 32 + ease_backIn(0, 38 * (phone.app - 1), phone.app_icon_slide_timer, phone.app_icon_slide_timer_max, 1);
...
//clock code hi its me floppo again how ya doin just gonna yknow, plonk this in here
if !phone_fast {
drawClock(phone_x + 82, phone_y + 2, current_hour, current_minute, current_second);
}
...
textDraw(app_x + 40, app_y + 13, "fName", app_sel ? $4ABAA7 : phone.apps[i].color, 10000, 200, fa_left, 1, true, 1, phone.apps[i].name, false);
...
if outline {
	for (i = -1; i < 2; i++) {
		for (j = -1; j < 2; j++) {
			draw_text_ext_transformed_color(x1 + i * 2, y1 + j * 2, text, lineb, linew, scale, scale, 0, $303B20, $303B20, $303B20, $303B20, alpha);
		}
	}
}
...
draw_set_alpha(0);
draw_rectangle_color(0,0, room_width, room_height, $303B20, $303B20, $303B20, $303B20, false);
draw_set_alpha(1);
...
if phone.big_screen_pos_offset < 1{
	...
	rectDraw(draw_x, draw_y, draw_w, draw_h, $303B20);
	draw_set_alpha(1);
	...
	if array_length(phone.apps[phone.app].array) > 0{
		...
		if "page_starts" in item && array_length(item.page_starts) > 1{
			textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 175 - phone.extra_top_size, "fName", $4ABAA7, 1000, 1000, fa_center, 1, 0, 1, "Page " + string(phone.page + 1), 0);
			textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 196 - phone.extra_top_size, "fName", $4ABAA7, 1000, 1000, fa_center, 1, 0, 1, "ATTACK: " + (phone.page == array_length(item.page_starts) - 1 ? "First" : "Next"), 1);
		}
		
		else if "options" in item{
			textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 196 - phone.extra_top_size, "fName", $4ABAA7, 1000, 1000, fa_center, 1, 0, 1, "ATTACK: " + (array_length(item.options) > 1 ? (array_length(item.options) > 2 ? "Next Option" : "Toggle") : "Activate"), 1);
			if array_length(item.options) > 2 textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 175 - phone.extra_top_size, "fName", $4ABAA7, 1000, 1000, fa_center, 1, 0, 1, "JUMP: Previous Option", 1);
		}
	}
	...
	if phone.app == phone.APP_TIPS || phone.app == phone.APP_PATCHES{
		...
		switch(item.type){
			case 1: // stats
				...
				for (var i = 0; i < array_length(stats_table); i += 2){
					...
					text_y += phone.last_text_size[1];
					textDraw(text_x + 16, text_y, "fName", $4ABAA7, 18, 100, fa_left, 1, 0, 1, decimalToString(stats_table[i+1]), true);
					text_y += phone.last_text_size[1];
				...
				break;
			case 2: // a move
				textDraw(draw_x + view_get_wview() - 9 + round(118 * 3 * (phone.y / phone.lowered_y)), draw_y + 32, "fName", $4ABAA7, 1000, 1000, fa_right, 1, 0, 1, "JUMP: Refresh", 1);
				...
				textDraw(text_x, text_y, "fName", $4ABAA7, 18, 100, fa_left, 1, 0, 1, item.length, false);
				text_x = orig_x + 240;
				textDraw(text_x, text_y, "fName", app_color, 18, 160, fa_left, 1, 0, 1, "Ending Lag (Whiff):", true);
				text_x += phone.last_text_size[0] + 8;
				textDraw(text_x, text_y, "fName", $4ABAA7, 18, 100, fa_left, 1, 0, 1, item.ending_lag, false);
				text_x = orig_x + 480;
				textDraw(text_x, text_y, "fName", app_color, 18, 160, fa_left, 1, 0, 1, "Landing Lag (Whiff):", true);
				text_x += phone.last_text_size[0] + 8;
				textDraw(text_x, text_y, "fName", $4ABAA7, 18, 100, fa_left, 1, 0, 1, item.landing_lag, false);
				...
				if item.misc != "-"{
					...
					text_x += phone.last_text_size[0] + 8;
					textDraw(text_x, text_y, "fName", $4ABAA7, 18, draw_w - margin_l - margin_r - 64, fa_left, 1, 0, 1, item.misc, true);
					text_y += phone.last_text_size[1];
					text_x = orig_x;
				}
				...
				if array_length(item.hitboxes){
					...
					text_y += ease_sineIn(0, 148, phone.page_change_timer, phone.page_change_timer_max);
					...
					for (var i = item.page_starts[phone.page]; i < array_length(item.hitboxes) && !reached_end; i++){
						...
						for (var j = 0; j < array_length(stats_table); j++){
							textDraw(text_x, text_y, "fName", hb.parent_hbox && hb.parent_hbox != i + 1 && j > 0 ? $139B7A : $4ABAA7, 18, 24, fa_left, 1, 0, 1, stats_table[j], true);
							text_x += add_x;
						}
						...
						if hb.misc != "-"{
							...
							textDraw(text_x, text_y, "fName", $139B7A, 18, draw_w - margin_l - margin_r - 128, fa_left, 1, 0, 1, hb.misc, true);
							text_y += phone.last_text_size[1];
						}
				...
				for (var i = 0; i < array_length(phone.custom_fd_content); i++){
					...
					switch(cur.type){
						case 0: // header
							...
							break;
						case 1: // body
							textDraw(text_x + 16, text_y, "fName", $4ABAA7, 18, 100, fa_left, 1, 0, 1, cur.content, true);
							text_y += phone.last_text_size[1];
							if text_y > draw_y + draw_h - margin_b * 8{
								text_y = orig_y;
								text_x += 160;
		...
		textDraw(this_x, text_y, "fName", app_color, 18, this_w, fa_center, 1, 0, 1, "- " + item.name + " -", true);
		text_y += phone.last_text_size[1] + 10;
		textDraw(this_x, text_y, "fName", $4ABAA7, 18, this_w, fa_center, 1, 0, 1, item.description, true);
		...
		if num_options > 1{
			...
			for (var j = 0; j < num_cols; j++){
				...
				for (var i = j * col_height; i < (j + 1) * col_height && i < num_options; i++){
					...
					textDraw(col_x, col_y, "fName", item.on == i ? $4ABAA7 : $139B7A, 18, this_w, fa_center, 1, 0, 1, to_draw, true);
			...
			textDraw(this_x, text_y, "fName", $4ABAA7, 18, this_w, fa_center, 1, 0, 1, to_draw, true);
		}
	}
}
...
if phone.app > 0{
	...
	for (var i = 0; i < array_length(arr); i++){
		textDraw(0, 0, "fName", $4ABAA7, 18, phone.screen_width - 8, fa_center, 1, 0, 0, arr[i].name, true);
	...
	phone.target_scroll_dist = max(0, min(phone.target_scroll_dist - 68, total_height - 130));
...
//its ya girl flimflam back at it with the c l o n c k
#define drawClock(x1, y1, hr, mn, sc) // Draw the system clock

if !phone.utils_cur[phone.UTIL_MILITARY] == 1{
	if hr > 12 hr -= 12;
	if hr == 0 hr = 12;
}
...
if digit_1 || phone.utils_cur[phone.UTIL_MILITARY] == 1
draw_sprite(sprite_get("_pho_digits"), digit_1, x1, y1);
draw_sprite(sprite_get("_pho_digits"), digit_2, x1 + 8, y1);
draw_sprite(sprite_get("_pho_digits"), digit_3, x1 + 20, y1);
draw_sprite(sprite_get("_pho_digits"), digit_4, x1 + 28, y1);
