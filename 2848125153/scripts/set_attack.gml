...
if (attack == AT_USTRONG){
	...
	if(current_money <= 15000){
		...
		set_window_value(AT_USTRONG, 5, AG_WINDOW_LENGTH, 4);set_window_value(AT_USTRONG, 6, AG_WINDOW_LENGTH, 16);
		...
		reset_window_value(AT_USTRONG, 5, AG_WINDOW_LENGTH);set_window_value(AT_USTRONG, 6, AG_WINDOW_LENGTH, 25);
...
if (attack == AT_DATTACK){
    ...
    //if(!muted){
		...
		var choochooexists = false;
		with (pHitBox){
			if (player_id == other && type == 2 && attack == AT_DATTACK && hbox_num == 4){
				with(other)choochooexists = true;
			}
		}
		if(choochooexists && current_money >= 3000*discount){
...
if (attack == AT_BAIR){
	...
	if(current_money <= 15000){
		...
		set_window_value(AT_BAIR, 3, AG_WINDOW_LENGTH, 13);set_attack_value(AT_BAIR, AG_LANDING_LAG, 5);
		...
		set_window_value(AT_BAIR, 3, AG_WINDOW_LENGTH, 25);set_attack_value(AT_BAIR, AG_LANDING_LAG, 16);
