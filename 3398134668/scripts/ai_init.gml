// ai stuff is going to largely be over looked in this tutorial series, feel free to learn that for yourself if you're interested.

far_down_attacks[0] = AT_DAIR;
far_down_attacks[1] = AT_NAIR;
close_up_attacks[0] = AT_USTRONG;
close_up_attacks[1] = AT_UAIR;
close_up_attacks[2] = AT_DSPECIAL;
tauntOnce = true;
close_down_attacks[0] = AT_DAIR;
close_down_attacks[1] = AT_DSPECIAL;

mid_down_attack[0] = AT_DAIR;

mid_side_attacks[0] = AT_FSPECIAL;
mid_side_attacks[1] = AT_NSPECIAL;
far_side_attacks[0] = AT_FSPECIAL;
far_up_attacks[0] = AT_UTILT;
close_side_attacks[0] = AT_FSTRONG;
close_side_attacks[1] = AT_DSPECIAL;
close_side_attacks[2] = AT_DTILT;
close_side_attacks[3] = AT_NAIR;
close_side_attacks[4] = AT_DSTRONG;

neutral_attacks[0] = AT_NAIR;
neutral_attacks[1] = AT_BAIR;
neutral_attacks[2] = AT_UAIR;
neutral_attacks[3] = AT_DSPECIAL;
waveState = 0;
movementICD = 0;
grabICD = 0;
pickRandom = 0;