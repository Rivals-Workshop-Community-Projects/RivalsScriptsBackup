...
switch(my_hitboxID.attack){
	case AT_FSPECIAL:
		switch(my_hitboxID.hbox_num){
			case 1:
				if (fspecial_grab_id == noone) && (!hit_player_obj.super_armor) && (hit_player_obj.soft_armor == 0 || hit_player_obj.soft_armor >= orig_knock) && (hit_player_obj.cofa_mummy || hit_player_obj.cofa_mummy_omen || phone_cheats[CHEAT_SUDOCURSE]){ // grabby
	...
	break;
	case AT_DSPECIAL:
		sound_play((hit_player_obj.cofa_mummy || hit_player_obj.cofa_mummy_omen || phone_cheats[CHEAT_SUDOCURSE]) ? sound_get("hex_hit_strong") : sound_get("hex_hit_weak"));
		if (hit_player_obj.cofa_mummy) || (hit_player_obj.cofa_mummy_omen) || (phone_cheats[CHEAT_SUDOCURSE]){
