...
// draw_debug_text(x-100,y+85,"url = " + original_url);
if !(st = 5 || st = 8)
{
    /*
    if hit_exists == true
    ...
    var wall3 = draw_sprite_ext(sprite_get("spr_wall"),0,x,stagefloor-255,30,4,0,c_white,wall3a);
    
    if hitstop == 0
    {
        if (spd >= 50)
        {
            var ball1 = draw_sprite_ext(sprite_get("p"+string(player_id.player)+"ball"),subimg,x,y,3.5,0.35,angle,c_white,1);
        }
        else if (spd >= 20) && (spd <= 49)
        {
            var ball2 = draw_sprite_ext(sprite_get("p"+string(player_id.player)+"ball"),subimg,x,y,1.6,0.8,angle,c_white,1);
        }
        else
        {
            var ball3 = draw_sprite(sprite_get("p"+string(player_id.player)+"ball"),subimg,x,y);
        }
    ...
    else if hitstop > 0
    {
        ...
        {
            var ball4 = draw_sprite_ext(sprite_get("p"+string(player_id.player)+"ball"),subimg,x,y,0.8,1.6,0,c_white,1);
        }
        
        else if (y = (stagefloor-250)) || (y = stagefloor) 
        {
            var ball5 = draw_sprite_ext(sprite_get("p"+string(player_id.player)+"ball"),subimg,x,y,1.6,0.8,0,c_white,1);
        }
        else
        {
            var ball6 = draw_sprite_ext(sprite_get("p"+string(player_id.player)+"ball"),subimg,x,y,0.8,1.6,angle,c_white,1);
        }
    }
