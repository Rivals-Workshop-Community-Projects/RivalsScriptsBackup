set_attack_value(AT_TAUNT_2, AG_SPRITE, sprite_get("taunt"));
set_attack_value(AT_TAUNT_2, AG_NUM_WINDOWS, 2);
set_attack_value(AT_TAUNT_2, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_TAUNT_2, AG_OFF_LEDGE, 1);
set_attack_value(AT_TAUNT_2, AG_HURTBOX_SPRITE, asset_get("ex_guy_hurt_box"));


set_window_value(AT_TAUNT_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_LENGTH, 25);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_ANIM_FRAMES, 5);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_ANIM_FRAME_START, 15);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak2"));
set_window_value(AT_TAUNT_2, 1, AG_WINDOW_SFX_FRAME, 19);

set_window_value(AT_TAUNT_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_LENGTH, 90);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_ANIM_FRAMES, 20);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_ANIM_FRAME_START, 20);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy1"));
set_window_value(AT_TAUNT_2, 2, AG_WINDOW_SFX_FRAME, 9);

set_num_hitboxes(AT_TAUNT_2, 1);

set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_WINDOW, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_WINDOW_CREATION_FRAME, 9);
set_hitbox_value(AT_TAUNT_2, 1, HG_LIFETIME, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_X, 32);
set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_Y, -44);
set_hitbox_value(AT_TAUNT_2, 1, HG_WIDTH, 60);
set_hitbox_value(AT_TAUNT_2, 1, HG_HEIGHT, 30);
set_hitbox_value(AT_TAUNT_2, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_TAUNT_2, 1, HG_DAMAGE, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_ANGLE, 60);
set_hitbox_value(AT_TAUNT_2, 1, HG_TECHABLE, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_TAUNT_2, 1, HG_BASE_HITPAUSE, 1);
set_hitbox_value(AT_TAUNT_2, 1, HG_EXTRA_HITPAUSE, 100);
set_hitbox_value(AT_TAUNT_2, 1, HG_VISUAL_EFFECT, 196);
set_hitbox_value(AT_TAUNT_2, 1, HG_VISUAL_EFFECT_X_OFFSET, 24);
set_hitbox_value(AT_TAUNT_2, 1, HG_VISUAL_EFFECT_Y_OFFSET, 15);
set_hitbox_value(AT_TAUNT_2, 1, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_TAUNT_2, 1, HG_HITBOX_GROUP, 1);