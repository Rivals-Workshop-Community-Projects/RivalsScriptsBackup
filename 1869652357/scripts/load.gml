sprite_change_offset("idle", 48, 94);
sprite_change_offset("hurt", 64, 126);
sprite_change_offset("bighurt", 64, 126);
sprite_change_offset("uphurt", 64, 126);
sprite_change_offset("downhurt", 64, 126);
sprite_change_offset("bouncehurt", 64, 126);
sprite_change_offset("crouch", 48, 95);
sprite_change_offset("walk", 48, 95);
sprite_change_offset("walkturn", 48, 95);
sprite_change_offset("dash", 64, 126);
sprite_change_offset("dashstart", 48, 95);
sprite_change_offset("dashstop", 48, 95);
sprite_change_offset("dashturn", 48, 95);

sprite_change_offset("jumpstart", 48, 95);
sprite_change_offset("jump", 48, 95);
sprite_change_offset("doublejump", 48, 95);
sprite_change_offset("walljump", 64, 119);
sprite_change_offset("pratfall", 64, 126);
sprite_change_offset("land", 48, 95);
sprite_change_offset("landinglag", 48, 95);

sprite_change_offset("parry", 48, 124);
sprite_change_offset("roll_forward", 64, 118);
sprite_change_offset("roll_backward", 64, 118);
sprite_change_offset("airdodge", 64, 126);
sprite_change_offset("waveland", 64, 126);
sprite_change_offset("tech", 64, 118);

sprite_change_offset("jab", 62, 123);
sprite_change_offset("dattack", 64, 126);
sprite_change_offset("ftilt", 64, 126);
sprite_change_offset("dtilt", 64, 125);
sprite_change_offset("utilt", 64, 126);
sprite_change_offset("nair", 64, 126);
sprite_change_offset("fair", 64, 126);
sprite_change_offset("bair", 64, 126);
sprite_change_offset("uair", 64, 126);
sprite_change_offset("dair", 64, 100);
sprite_change_offset("fstrong", 64, 126);
sprite_change_offset("ustrong", 64, 126);
sprite_change_offset("dstrong", 64, 126);
sprite_change_offset("nspecial", 64, 126);
sprite_change_offset("fspecial", 64, 126);
sprite_change_offset("uspecial", 64, 126);
sprite_change_offset("dspecial", 64, 126);
sprite_change_offset("taunt", 64, 126);
sprite_change_offset("atkextra3", 48, 95);

    sprite_change_offset("plat", 64, 100);

sprite_change_offset("fspecial_fx", 64, 126);
sprite_change_offset("ash_hurtbox", 64, 126);
sprite_change_offset("ash_crouching_hurtbox", 64, 126);
sprite_change_offset("cancel_chance", 64, 126);
sprite_change_offset("cancel_chance_star", 64, 126);
sprite_change_offset("dizzy", 64, 126);
sprite_change_offset("nspecial_proj", 64, 94);
sprite_change_offset("flame_proj", 64, 94);
sprite_change_offset("star_proj", 64, 94);
sprite_change_offset("taunt3", 64, 126);
sprite_change_offset("rocket_uppercut", 64, 192);
sprite_change_offset("rocket_uppercut_wind", 64, 192);

sprite_change_offset("car", 6, 6);

sprite_change_offset("intro", 64, 126);

set_victory_bg(sprite_get("ashvictorybg"));

set_victory_theme( sound_get( "akb_victory") );