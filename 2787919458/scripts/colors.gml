// DEFAULT COLOR

// Fur
set_color_profile_slot( 0, 0, 63, 71, 203 );
set_color_profile_slot_range( 0, 1, 1, 1 );

// Fur S
set_color_profile_slot( 0, 1, 45, 33, 153 );
set_color_profile_slot_range( 1, 1, 8, 25 );

// Eyes
set_color_profile_slot( 0, 2, 247, 247, 253 );
set_color_profile_slot_range( 2, 23, 5, 32 );

// Iris
set_color_profile_slot( 0, 3, 33, 27, 28 );
set_color_profile_slot_range( 3, 11, 25, 4 );

// Skin
set_color_profile_slot( 0, 4, 239, 229, 175 );
set_color_profile_slot_range( 4, 16, 26, 17 );

// Gloves
set_color_profile_slot( 0, 5, 221, 218, 242 );
set_color_profile_slot_range( 5, 31, 1, 55 );

// Shoes
set_color_profile_slot( 0, 6, 237, 29, 38 );
set_color_profile_slot_range( 6, 1, 9, 21 );

// Smears
set_color_profile_slot( 0, 7, 50, 210, 255 );
set_color_profile_slot_range( 7, 18, 18, 36 );


// ALTERNATE COLORS
set_num_palettes( 28 );

// Super
set_color_profile_slot( 1, 0, 254, 216, 27 ); //Fur
set_color_profile_slot( 1, 1, 219, 151, 23 ); //Fur S
set_color_profile_slot( 1, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 1, 3, 33, 27, 28 ); //Iris
set_color_profile_slot( 1, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 1, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 1, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 1, 7, 254, 216, 27 ); //Smears

// Hyper
set_color_profile_slot( 2, 0, 255, 255, 255 ); //Fur
set_color_profile_slot( 2, 1, 147, 159, 204 ); //Fur S
set_color_profile_slot( 2, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 2, 3, 237, 29, 38 ); //Iris
set_color_profile_slot( 2, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 2, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 2, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 2, 7, 63, 71, 203 ); //Smears

// Dankspine
set_color_profile_slot( 3, 0, 163, 73, 165 ); //Fur
set_color_profile_slot( 3, 1, 96, 55, 125 ); //Fur S
set_color_profile_slot( 3, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 3, 3, 247, 247, 253 ); //Iris
set_color_profile_slot( 3, 4, 217, 210, 238 ); //Skin
set_color_profile_slot( 3, 5, 163, 73, 165 ); //Gloves
set_color_profile_slot( 3, 6, 254, 243, 5 ); //Shoes
set_color_profile_slot( 3, 7, 217, 210, 238 ); //Smears

// Dark
set_color_profile_slot( 4, 0, 39, 38, 46 ); //Fur
set_color_profile_slot( 4, 1, 23, 21, 31 ); //Fur S
set_color_profile_slot( 4, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 4, 3, 247, 247, 253 ); //Iris
set_color_profile_slot( 4, 4, 39, 38, 46 ); //Skin
set_color_profile_slot( 4, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 4, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 4, 7, 39, 38, 46 ); //Smears

// Super Sanic God
set_color_profile_slot( 5, 0, 239, 27, 40 ); //Fur
set_color_profile_slot( 5, 1, 150, 16, 47 ); //Fur S
set_color_profile_slot( 5, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 5, 3, 143, 81, 49 ); //Iris
set_color_profile_slot( 5, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 5, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 5, 6, 239, 27, 40 ); //Shoes
set_color_profile_slot( 5, 7, 239, 27, 40 ); //Smears

// Super Sanic Blue
set_color_profile_slot( 6, 0, 0, 163, 233 ); //Fur
set_color_profile_slot( 6, 1, 0, 76, 191 ); //Fur S
set_color_profile_slot( 6, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 6, 3, 133, 203, 233 ); //Iris
set_color_profile_slot( 6, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 6, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 6, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 6, 7, 133, 203, 233 ); //Smears

// Sanic.EXE
set_color_profile_slot( 7, 0, 0, 1, 100 ); //Fur
set_color_profile_slot( 7, 1, 5, 0, 59 ); //Fur S
set_color_profile_slot( 7, 2, 0, 0, 0 ); //Eyes
set_color_profile_slot( 7, 3, 237, 29, 38 ); //Iris
set_color_profile_slot( 7, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 7, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 7, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 7, 7, 237, 29, 38 ); //Smears

// Taels
set_color_profile_slot( 8, 0, 255, 201, 14 ); //Fur
set_color_profile_slot( 8, 1, 209, 100, 12 ); //Fur S
set_color_profile_slot( 8, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 8, 3, 33, 27, 28 ); //Iris
set_color_profile_slot( 8, 4, 247, 247, 253 ); //Skin
set_color_profile_slot( 8, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 8, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 8, 7, 237, 29, 38 ); //Smears

// Knackles
set_color_profile_slot( 9, 0, 234, 31, 33 ); //Fur
set_color_profile_slot( 9, 1, 130, 18, 25 ); //Fur S
set_color_profile_slot( 9, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 9, 3, 33, 27, 28 ); //Iris
set_color_profile_slot( 9, 4, 240, 234, 188 ); //Skin
set_color_profile_slot( 9, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 9, 6, 76, 126, 103 ); //Shoes
set_color_profile_slot( 9, 7, 255, 201, 14 ); //Smears

// Robuttnik
set_color_profile_slot( 10, 0, 234, 0, 2 ); //Fur
set_color_profile_slot( 10, 1, 140, 19, 20 ); //Fur S
set_color_profile_slot( 10, 2, 99, 157, 255 ); //Eyes
set_color_profile_slot( 10, 3, 99, 157, 255 ); //Iris
set_color_profile_slot( 10, 4, 195, 147, 122 ); //Skin
set_color_profile_slot( 10, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 10, 6, 33, 27, 28 ); //Shoes
set_color_profile_slot( 10, 7, 99, 157, 255 ); //Smears

// Shewdew
set_color_profile_slot( 11, 0, 33, 27, 28 ); //Fur
set_color_profile_slot( 11, 1, 26, 15, 15 ); //Fur S
set_color_profile_slot( 11, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 11, 3, 237, 29, 38 ); //Iris
set_color_profile_slot( 11, 4, 230, 189, 120 ); //Skin
set_color_profile_slot( 11, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 11, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 11, 7, 237, 29, 38 ); //Smears

// Amy
set_color_profile_slot( 12, 0, 255, 133, 222 ); //Fur
set_color_profile_slot( 12, 1, 177, 78, 199 ); //Fur S
set_color_profile_slot( 12, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 12, 3, 0, 194, 0 ); //Iris
set_color_profile_slot( 12, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 12, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 12, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 12, 7, 255, 201, 14 ); //Smears

// Majin Sanic
set_color_profile_slot( 13, 0, 1, 0, 128 ); //Fur
set_color_profile_slot( 13, 1, 0, 1, 32 ); //Fur S
set_color_profile_slot( 13, 2, 0, 1, 32 ); //Eyes
set_color_profile_slot( 13, 3, 96, 64, 161 ); //Iris
set_color_profile_slot( 13, 4, 96, 64, 161 ); //Skin
set_color_profile_slot( 13, 5, 96, 64, 161 ); //Gloves
set_color_profile_slot( 13, 6, 1, 0, 128 ); //Shoes
set_color_profile_slot( 13, 7, 96, 64, 161 ); //Smears

// Obama
set_color_profile_slot( 14, 0, 48, 139, 209 ); //Fur
set_color_profile_slot( 14, 1, 39, 74, 171 ); //Fur S
set_color_profile_slot( 14, 2, 254, 245, 6 ); //Eyes
set_color_profile_slot( 14, 3, 0, 0, 0 ); //Iris
set_color_profile_slot( 14, 4, 254, 245, 6 ); //Skin
set_color_profile_slot( 14, 5, 188, 30, 44 ); //Gloves
set_color_profile_slot( 14, 6, 188, 30, 44 ); //Shoes
set_color_profile_slot( 14, 7, 188, 30, 44 ); //Smears

// Weegee
set_color_profile_slot( 15, 0, 76, 126, 103 ); //Fur
set_color_profile_slot( 15, 1, 50, 82, 75 ); //Fur S
set_color_profile_slot( 15, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 15, 3, 109, 110, 214 ); //Iris
set_color_profile_slot( 15, 4, 255, 237, 223 ); //Skin
set_color_profile_slot( 15, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 15, 6, 150, 104, 76 ); //Shoes
set_color_profile_slot( 15, 7, 249, 221, 77 ); //Smears

// Morshu
set_color_profile_slot( 16, 0, 224, 71, 17 ); //Fur
set_color_profile_slot( 16, 1, 155, 40, 0 ); //Fur S
set_color_profile_slot( 16, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 16, 3, 109, 110, 214 ); //Iris
set_color_profile_slot( 16, 4, 223, 172, 16 ); //Skin
set_color_profile_slot( 16, 5, 16, 128, 32 ); //Gloves
set_color_profile_slot( 16, 6, 164, 80, 16 ); //Shoes
set_color_profile_slot( 16, 7, 237, 29, 38 ); //Smears

// Harambe
set_color_profile_slot( 17, 0, 50, 49, 44 ); //Fur
set_color_profile_slot( 17, 1, 24, 28, 31 ); //Fur S
set_color_profile_slot( 17, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 17, 3, 24, 28, 31 ); //Iris
set_color_profile_slot( 17, 4, 113, 109, 102 ); //Skin
set_color_profile_slot( 17, 5, 113, 109, 102 ); //Gloves
set_color_profile_slot( 17, 6, 113, 109, 102 ); //Shoes
set_color_profile_slot( 17, 7, 237, 29, 38 ); //Smears

// Illuminati
set_color_profile_slot( 18, 0, 78, 107, 79 ); //Fur
set_color_profile_slot( 18, 1, 51, 76, 52 ); //Fur S
set_color_profile_slot( 18, 2, 166, 179, 170 ); //Eyes
set_color_profile_slot( 18, 3, 13, 19, 13 ); //Iris
set_color_profile_slot( 18, 4, 166, 179, 170 ); //Skin
set_color_profile_slot( 18, 5, 255, 242, 193 ); //Gloves
set_color_profile_slot( 18, 6, 255, 242, 193 ); //Shoes
set_color_profile_slot( 18, 7, 51, 76, 52 ); //Smears

// Shrek
set_color_profile_slot( 19, 0, 189, 212, 38 ); //Fur
set_color_profile_slot( 19, 1, 126, 156, 28 ); //Fur S
set_color_profile_slot( 19, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 19, 3, 66, 34, 22 ); //Iris
set_color_profile_slot( 19, 4, 233, 227, 215 ); //Skin
set_color_profile_slot( 19, 5, 121, 95, 68 ); //Gloves
set_color_profile_slot( 19, 6, 121, 95, 68 ); //Shoes
set_color_profile_slot( 19, 7, 209, 235, 42 ); //Smears

// Doge
set_color_profile_slot( 20, 0, 201, 150, 46 ); //Fur
set_color_profile_slot( 20, 1, 161, 101, 37 ); //Fur S
set_color_profile_slot( 20, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 20, 3, 33, 27, 28 ); //Iris
set_color_profile_slot( 20, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 20, 5, 239, 229, 175 ); //Gloves
set_color_profile_slot( 20, 6, 239, 229, 175 ); //Shoes
set_color_profile_slot( 20, 7, 250, 87, 255 ); //Smears

// DatBoi
set_color_profile_slot( 21, 0, 6, 171, 8 ); //Fur
set_color_profile_slot( 21, 1, 5, 90, 5 ); //Fur S
set_color_profile_slot( 21, 2, 33, 27, 28 ); //Eyes
set_color_profile_slot( 21, 3, 33, 27, 28 ); //Iris
set_color_profile_slot( 21, 4, 6, 171, 8 ); //Skin
set_color_profile_slot( 21, 5, 112, 53, 35 ); //Gloves
set_color_profile_slot( 21, 6, 33, 27, 28 ); //Shoes
set_color_profile_slot( 21, 7, 0, 255, 0 ); //Smears

// Sonc
set_color_profile_slot( 22, 0, 107, 106, 213 ); //Fur
set_color_profile_slot( 22, 1, 71, 70, 175 ); //Fur S
set_color_profile_slot( 22, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 22, 3, 0, 0, 0 ); //Iris
set_color_profile_slot( 22, 4, 250, 179, 140 ); //Skin
set_color_profile_slot( 22, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 22, 6, 236, 0, 0 ); //Shoes
set_color_profile_slot( 22, 7, 50, 210, 255 ); //Smears

// SkyCrafter1234
set_color_profile_slot( 23, 0, 0, 168, 243 ); //Fur
set_color_profile_slot( 23, 1, 0, 100, 214 ); //Fur S
set_color_profile_slot( 23, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 23, 3, 0, 168, 243 ); //Iris
set_color_profile_slot( 23, 4, 253, 217, 166 ); //Skin
set_color_profile_slot( 23, 5, 88, 88, 88 ); //Gloves
set_color_profile_slot( 23, 6, 255, 202, 24 ); //Shoes
set_color_profile_slot( 23, 7, 50, 210, 255 ); //Smears

// Alisa(Dons)
set_color_profile_slot( 24, 0, 148, 148, 158 ); //Fur
set_color_profile_slot( 24, 1, 101, 91, 115 ); //Fur S
set_color_profile_slot( 24, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 24, 3, 98, 100, 237 ); //Iris
set_color_profile_slot( 24, 4, 243, 225, 203 ); //Skin
set_color_profile_slot( 24, 5, 221, 218, 242 ); //Gloves
set_color_profile_slot( 24, 6, 237, 29, 38 ); //Shoes
set_color_profile_slot( 24, 7, 98, 100, 237 ); //Smears

// Gee
set_color_profile_slot( 25, 0, 251, 255, 18 ); //Fur
set_color_profile_slot( 25, 1, 214, 168, 15 ); //Fur S
set_color_profile_slot( 25, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 25, 3, 151, 75, 11 ); //Iris
set_color_profile_slot( 25, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 25, 5, 239, 229, 175 ); //Gloves
set_color_profile_slot( 25, 6, 65, 65, 65 ); //Shoes
set_color_profile_slot( 25, 7, 255, 132, 74 ); //Smears

// Oarly
set_color_profile_slot( 26, 0, 193, 228, 237 ); //Fur
set_color_profile_slot( 26, 1, 138, 163, 170 ); //Fur S
set_color_profile_slot( 26, 2, 168, 222, 237 ); //Eyes
set_color_profile_slot( 26, 3, 168, 222, 237 ); //Iris
set_color_profile_slot( 26, 4, 239, 229, 175 ); //Skin
set_color_profile_slot( 26, 5, 72, 111, 78 ); //Gloves
set_color_profile_slot( 26, 6, 150, 104, 76 ); //Shoes
set_color_profile_slot( 26, 7, 255, 143, 169 ); //Smears

// Chip
set_color_profile_slot( 27, 0, 210, 40, 40 ); //Fur
set_color_profile_slot( 27, 1, 155, 0, 0 ); //Fur S
set_color_profile_slot( 27, 2, 247, 247, 253 ); //Eyes
set_color_profile_slot( 27, 3, 189, 114, 81 ); //Iris
set_color_profile_slot( 27, 4, 254, 211, 166 ); //Skin
set_color_profile_slot( 27, 5, 66, 79, 159 ); //Gloves
set_color_profile_slot( 27, 6, 114, 149, 229 ); //Shoes
set_color_profile_slot( 27, 7, 237, 29, 38 ); //Smears


/* This is a comment used by that one RoA colors.gml generator tool to store palette data. You can safely keep it in your colors.gml if you plan to re-use the tool later, or safely remove it if you don't.
=== BEGIN JSON PALETTE ===
{"formatversion":1,"data":[{"name":"Fur","colors":[{"r":63,"g":71,"b":203,"main":true}]},{"name":"Fur S","colors":[{"r":45,"g":33,"b":153,"main":true},{"r":22,"g":14,"b":93}]},{"name":"Eyes","colors":[{"r":247,"g":247,"b":253,"main":true},{"r":218,"g":217,"b":220},{"r":166,"g":162,"b":173}]},{"name":"Iris","colors":[{"r":33,"g":27,"b":28,"main":true},{"r":26,"g":15,"b":15}]},{"name":"Skin","colors":[{"r":239,"g":229,"b":175,"main":true},{"r":200,"g":158,"b":97}]},{"name":"Gloves","colors":[{"r":221,"g":218,"b":242,"main":true},{"r":178,"g":166,"b":185},{"r":101,"g":94,"b":105}]},{"name":"Shoes","colors":[{"r":237,"g":29,"b":38,"main":true},{"r":186,"g":7,"b":15}]},{"name":"Smears","colors":[{"r":50,"g":210,"b":255,"main":true},{"r":94,"g":252,"b":255},{"r":33,"g":99,"b":166},{"r":39,"g":124,"b":194}]}]}
=== END JSON PALETTE ===
*/
