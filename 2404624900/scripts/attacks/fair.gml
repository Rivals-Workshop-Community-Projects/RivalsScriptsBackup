set_attack_value(AT_FAIR, AG_CATEGORY, 1);
set_attack_value(AT_FAIR, AG_SPRITE, sprite_get("fair"));
set_attack_value(AT_FAIR, AG_NUM_WINDOWS, 3);
set_attack_value(AT_FAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_FAIR, AG_LANDING_LAG, 6);
set_attack_value(AT_FAIR, AG_HURTBOX_SPRITE, sprite_get("fair_hurt"));

set_window_value(AT_FAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_FAIR, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_FAIR, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_FAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FAIR, 1, AG_WINDOW_SFX_FRAME, 6);
set_window_value(AT_FAIR, 1, AG_WINDOW_SFX, sound_get("fair"));

set_window_value(AT_FAIR, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_FAIR, 2, AG_WINDOW_LENGTH, 9);
set_window_value(AT_FAIR, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_FAIR, 2, AG_WINDOW_ANIM_FRAME_START, 2);

set_window_value(AT_FAIR, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_FAIR, 3, AG_WINDOW_LENGTH, 6);
set_window_value(AT_FAIR, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_FAIR, 3, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_FAIR, 3, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_FAIR, 3);

set_hitbox_value(AT_FAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_X, -48);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_Y, -67);
set_hitbox_value(AT_FAIR, 1, HG_WIDTH, 30);
set_hitbox_value(AT_FAIR, 1, HG_HEIGHT, 43);
set_hitbox_value(AT_FAIR, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_FAIR, 1, HG_DAMAGE, 5);
set_hitbox_value(AT_FAIR, 1, HG_ANGLE, 140);
set_hitbox_value(AT_FAIR, 1, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_FAIR, 1, HG_KNOCKBACK_SCALING, .25);
set_hitbox_value(AT_FAIR, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FAIR, 1, HG_HITPAUSE_SCALING, .3);
set_hitbox_value(AT_FAIR, 1, HG_HIT_SFX, sound_get("blade_hit4"));
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT, 194);

set_hitbox_value(AT_FAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FAIR, 2, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_X, -24);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_Y, -40);
set_hitbox_value(AT_FAIR, 2, HG_WIDTH, 47);
set_hitbox_value(AT_FAIR, 2, HG_HEIGHT, 45);
set_hitbox_value(AT_FAIR, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_FAIR, 2, HG_DAMAGE, 5);
set_hitbox_value(AT_FAIR, 2, HG_ANGLE, 120);
set_hitbox_value(AT_FAIR, 2, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_FAIR, 2, HG_KNOCKBACK_SCALING, .25);
set_hitbox_value(AT_FAIR, 2, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FAIR, 2, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_FAIR, 2, HG_HIT_SFX, sound_get("blade_hit4"));
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 2, HG_VISUAL_EFFECT, 194);

set_hitbox_value(AT_FAIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FAIR, 3, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_FAIR, 3, HG_HITBOX_X, 22);
set_hitbox_value(AT_FAIR, 3, HG_HITBOX_Y, -31);
set_hitbox_value(AT_FAIR, 3, HG_WIDTH, 84);
set_hitbox_value(AT_FAIR, 3, HG_HEIGHT, 47);
set_hitbox_value(AT_FAIR, 3, HG_PRIORITY, 4);
set_hitbox_value(AT_FAIR, 3, HG_DAMAGE, 5);
set_hitbox_value(AT_FAIR, 3, HG_ANGLE, 45);
set_hitbox_value(AT_FAIR, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 3, HG_KNOCKBACK_SCALING, .55);
set_hitbox_value(AT_FAIR, 3, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 3, HG_HITPAUSE_SCALING, .6);
set_hitbox_value(AT_FAIR, 3, HG_HIT_SFX, sound_get("blade_hit4"));
set_hitbox_value(AT_FAIR, 3, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 3, HG_VISUAL_EFFECT, 194);

set_hitbox_value(AT_FAIR, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_FAIR, 4, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 4, HG_LIFETIME, 4);
set_hitbox_value(AT_FAIR, 4, HG_HITBOX_X, -48);
set_hitbox_value(AT_FAIR, 4, HG_HITBOX_Y, -67);
set_hitbox_value(AT_FAIR, 4, HG_WIDTH, 30);
set_hitbox_value(AT_FAIR, 4, HG_HEIGHT, 43);
set_hitbox_value(AT_FAIR, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_FAIR, 4, HG_DAMAGE, 4);
set_hitbox_value(AT_FAIR, 4, HG_ANGLE, 140);
set_hitbox_value(AT_FAIR, 4, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_FAIR, 4, HG_KNOCKBACK_SCALING, .15);
set_hitbox_value(AT_FAIR, 4, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FAIR, 4, HG_HITPAUSE_SCALING, .3);
set_hitbox_value(AT_FAIR, 4, HG_HIT_SFX, sound_get("clone_sword_hit"));
set_hitbox_value(AT_FAIR, 4, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_FAIR, 5, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_FAIR, 5, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 5, HG_LIFETIME, 4);
set_hitbox_value(AT_FAIR, 5, HG_HITBOX_X, -24);
set_hitbox_value(AT_FAIR, 5, HG_HITBOX_Y, -40);
set_hitbox_value(AT_FAIR, 5, HG_WIDTH, 47);
set_hitbox_value(AT_FAIR, 5, HG_HEIGHT, 45);
set_hitbox_value(AT_FAIR, 5, HG_PRIORITY, 3);
set_hitbox_value(AT_FAIR, 5, HG_DAMAGE, 4);
set_hitbox_value(AT_FAIR, 5, HG_ANGLE, 120);
set_hitbox_value(AT_FAIR, 5, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_FAIR, 5, HG_KNOCKBACK_SCALING, .15);
set_hitbox_value(AT_FAIR, 5, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FAIR, 5, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_FAIR, 5, HG_HIT_SFX, sound_get("clone_sword_hit"));
set_hitbox_value(AT_FAIR, 5, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_FAIR, 6, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_FAIR, 6, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 6, HG_LIFETIME, 4);
set_hitbox_value(AT_FAIR, 6, HG_HITBOX_X, 22);
set_hitbox_value(AT_FAIR, 6, HG_HITBOX_Y, -31);
set_hitbox_value(AT_FAIR, 6, HG_WIDTH, 84);
set_hitbox_value(AT_FAIR, 6, HG_HEIGHT, 47);
set_hitbox_value(AT_FAIR, 6, HG_PRIORITY, 4);
set_hitbox_value(AT_FAIR, 6, HG_DAMAGE, 4);
set_hitbox_value(AT_FAIR, 6, HG_ANGLE, 45);
set_hitbox_value(AT_FAIR, 6, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 6, HG_KNOCKBACK_SCALING, .45);
set_hitbox_value(AT_FAIR, 6, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 6, HG_HITPAUSE_SCALING, .5);
set_hitbox_value(AT_FAIR, 6, HG_HIT_SFX, sound_get("clone_sword_hit"));
set_hitbox_value(AT_FAIR, 6, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_FAIR, 4, HG_PROJECTILE_SPRITE, sprite_get("nothing"));
set_hitbox_value(AT_FAIR, 4, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_FAIR, 4, HG_PROJECTILE_IS_TRANSCENDENT, true);
set_hitbox_value(AT_FAIR, 4, HG_PROJECTILE_PARRY_STUN, true);
set_hitbox_value(AT_FAIR, 4, HG_VISUAL_EFFECT, clone_hit);

set_hitbox_value(AT_FAIR, 5, HG_PROJECTILE_SPRITE, sprite_get("nothing"));
set_hitbox_value(AT_FAIR, 5, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_FAIR, 5, HG_PROJECTILE_IS_TRANSCENDENT, true);
set_hitbox_value(AT_FAIR, 5, HG_PROJECTILE_PARRY_STUN, true);
set_hitbox_value(AT_FAIR, 5, HG_VISUAL_EFFECT, clone_hit);

set_hitbox_value(AT_FAIR, 6, HG_PROJECTILE_SPRITE, sprite_get("nothing"));
set_hitbox_value(AT_FAIR, 6, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_FAIR, 6, HG_PROJECTILE_IS_TRANSCENDENT, true);
set_hitbox_value(AT_FAIR, 6, HG_PROJECTILE_PARRY_STUN, true);
set_hitbox_value(AT_FAIR, 6, HG_VISUAL_EFFECT, clone_hit);

set_hitbox_value(AT_FAIR, 1, HG_HIT_LOCKOUT, 2);
set_hitbox_value(AT_FAIR, 2, HG_HIT_LOCKOUT, 2);
set_hitbox_value(AT_FAIR, 3, HG_HIT_LOCKOUT, 2);
set_hitbox_value(AT_FAIR, 4, HG_HIT_LOCKOUT, 1);
set_hitbox_value(AT_FAIR, 5, HG_HIT_LOCKOUT, 1);
set_hitbox_value(AT_FAIR, 6, HG_HIT_LOCKOUT, 1);

set_hitbox_value(AT_FAIR, 4, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 5, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 6, HG_EFFECT, 0);

set_hitbox_value(AT_FAIR, 4, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_FAIR, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_FAIR, 5, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_FAIR, 5, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_FAIR, 6, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_FAIR, 6, HG_PROJECTILE_GROUND_BEHAVIOR, 1);

set_hitbox_value(AT_FAIR, 4, HG_EXTENDED_PARRY_STUN, 1);
set_hitbox_value(AT_FAIR, 5, HG_EXTENDED_PARRY_STUN, 1);
set_hitbox_value(AT_FAIR, 6, HG_EXTENDED_PARRY_STUN, 1);