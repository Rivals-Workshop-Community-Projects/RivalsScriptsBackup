...
if (hbox.effect != 9 || hitstun > 0) {
	...
	if hbox.force_flinch && !hbox.free orig_knock = 0; //uncomment this line for grounded articles.
	//if hbox.force_flinch orig_knock = 0.3; //comment out this line for grounded articles.
	else orig_knock = get_kb_formula(percent, kb_adj, 1, hbox.damage, hbox.kb_value, hbox.kb_scale)
	if (hitstun == 0 || orig_knock <= soft_armor || super_armor) orig_knock = 0;
