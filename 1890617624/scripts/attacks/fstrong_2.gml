// Mc. Burst

set_attack_value(AT_FSTRONG_2, AG_CATEGORY, 2);
set_attack_value(AT_FSTRONG_2, AG_SPRITE, sprite_get("tech"));
set_attack_value(AT_FSTRONG_2, AG_NUM_WINDOWS, 3);
set_attack_value(AT_FSTRONG_2, AG_HAS_LANDING_LAG, 0);
set_attack_value(AT_FSTRONG_2, AG_AIR_SPRITE, sprite_get("tech"));
set_attack_value(AT_FSTRONG_2, AG_HURTBOX_SPRITE, sprite_get("goldendeath_hurt"));

set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_LENGTH, 12);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_VSPEED_TYPE, 1);

set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_LENGTH, 5);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_VSPEED_TYPE, 1);

set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_LENGTH, 4);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_VSPEED_TYPE, 1);

set_num_hitboxes(AT_FSTRONG_2, 0);

set_hitbox_value(AT_FSTRONG_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_WINDOW, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_LIFETIME, 8);
set_hitbox_value(AT_FSTRONG_2, 1, HG_SHAPE, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITBOX_X, 0);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITBOX_Y, -20);
set_hitbox_value(AT_FSTRONG_2, 1, HG_WIDTH, 250);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HEIGHT, 250);
set_hitbox_value(AT_FSTRONG_2, 1, HG_PRIORITY, 10);
set_hitbox_value(AT_FSTRONG_2, 1, HG_DAMAGE, 10);
set_hitbox_value(AT_FSTRONG_2, 1, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(AT_FSTRONG_2, 1, HG_BASE_HITPAUSE, 2);
set_hitbox_value(AT_FSTRONG_2, 1, HG_EXTRA_HITPAUSE, 20);
set_hitbox_value(AT_FSTRONG_2, 1, HG_VISUAL_EFFECT, hitsparkheavy);
set_hitbox_value(AT_FSTRONG_2, 1, HG_ANGLE, 90);
set_hitbox_value(AT_FSTRONG_2, 1, HG_TECHABLE, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITSTUN_MULTIPLIER, 0.5);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HIT_SFX, sound_get("counterhit"));