set_attack_value(AT_USTRONG_2, AG_SPRITE, sprite_get("ustrong"));
set_attack_value(AT_USTRONG_2, AG_CATEGORY, 2);
set_attack_value(AT_USTRONG_2, AG_NUM_WINDOWS, 6);
set_attack_value(AT_USTRONG_2, AG_OFF_LEDGE, 0);
set_attack_value(AT_USTRONG_2, AG_HURTBOX_SPRITE, sprite_get("ustrong_hurt"));

set_window_value(AT_USTRONG_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_USTRONG_2, 1, AG_WINDOW_LENGTH, 12);
set_window_value(AT_USTRONG_2, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USTRONG_2, 1, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_USTRONG_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USTRONG_2, 1, AG_WINDOW_SFX_FRAME, 11);
set_window_value(AT_USTRONG_2, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy2"));

set_window_value(AT_USTRONG_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_USTRONG_2, 2, AG_WINDOW_LENGTH, 3);
set_window_value(AT_USTRONG_2, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USTRONG_2, 2, AG_WINDOW_ANIM_FRAME_START, 9);

set_window_value(AT_USTRONG_2, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_USTRONG_2, 3, AG_WINDOW_LENGTH, 6);
set_window_value(AT_USTRONG_2, 3, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_USTRONG_2, 3, AG_WINDOW_ANIM_FRAME_START, 10);

set_window_value(AT_USTRONG_2, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_USTRONG_2, 4, AG_WINDOW_LENGTH, 10);
set_window_value(AT_USTRONG_2, 4, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_USTRONG_2, 4, AG_WINDOW_ANIM_FRAME_START, 11);

set_num_hitboxes(AT_USTRONG_2,2);

set_hitbox_value(AT_USTRONG_2, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_USTRONG_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG_2, 1, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG_2, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_USTRONG_2, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_USTRONG_2, 1, HG_HITBOX_X, 20);
set_hitbox_value(AT_USTRONG_2, 1, HG_HITBOX_Y, -60);
set_hitbox_value(AT_USTRONG_2, 1, HG_WIDTH, 70);
set_hitbox_value(AT_USTRONG_2, 1, HG_HEIGHT, 80);
set_hitbox_value(AT_USTRONG_2, 1, HG_SHAPE, 0);
set_hitbox_value(AT_USTRONG_2, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_USTRONG_2, 1, HG_DAMAGE, 9);
set_hitbox_value(AT_USTRONG_2, 1, HG_TECHABLE, 1);
set_hitbox_value(AT_USTRONG_2, 1, HG_ANGLE, -25);
set_hitbox_value(AT_USTRONG_2, 1, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_USTRONG_2, 1, HG_KNOCKBACK_SCALING, .75);
set_hitbox_value(AT_USTRONG_2, 1, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_USTRONG_2, 1, HG_HITPAUSE_SCALING, .9);
set_hitbox_value(AT_USTRONG_2, 1, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));
set_hitbox_value(AT_USTRONG_2, 1, HG_HIT_LOCKOUT, 0);