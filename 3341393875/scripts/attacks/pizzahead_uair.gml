set_attack_value(AT_UTHROW, AG_CATEGORY, 1);
set_attack_value(AT_UTHROW, AG_SPRITE, sprite_get("pizzahead_uair"));
set_attack_value(AT_UTHROW, AG_NUM_WINDOWS, 3);
set_attack_value(AT_UTHROW, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_UTHROW, AG_LANDING_LAG, 8);
set_attack_value(AT_UTHROW, AG_HURTBOX_SPRITE, sprite_get("pizzahead_uair_hurt"));

set_window_value(AT_UTHROW, 1, AG_WINDOW_TYPE, 0);
set_window_value(AT_UTHROW, 1, AG_WINDOW_LENGTH, 9);
set_window_value(AT_UTHROW, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_UTHROW, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UTHROW, 1, AG_WINDOW_SFX_FRAME, 8);
set_window_value(AT_UTHROW, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));

set_window_value(AT_UTHROW, 2, AG_WINDOW_TYPE, 0);
set_window_value(AT_UTHROW, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_UTHROW, 2, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_UTHROW, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_UTHROW, 2, AG_WINDOW_VSPEED, -1);
set_window_value(AT_UTHROW, 2, AG_WINDOW_VSPEED_TYPE, 0);

set_window_value(AT_UTHROW, 3, AG_WINDOW_TYPE, 0);
set_window_value(AT_UTHROW, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_UTHROW, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_UTHROW, 3, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_UTHROW, 3, AG_WINDOW_HAS_WHIFFLAG, 1);


set_num_hitboxes(AT_UTHROW, 2);

//Sour
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTHROW, 1, HG_WINDOW, 2);
set_hitbox_value(AT_UTHROW, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_X, 22);
set_hitbox_value(AT_UTHROW, 1, HG_HITBOX_Y, -68);
set_hitbox_value(AT_UTHROW, 1, HG_WIDTH, 50);
set_hitbox_value(AT_UTHROW, 1, HG_HEIGHT, 78);
set_hitbox_value(AT_UTHROW, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_UTHROW, 1, HG_DAMAGE, 7);
set_hitbox_value(AT_UTHROW, 1, HG_ANGLE, 70);
set_hitbox_value(AT_UTHROW, 1, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_UTHROW, 1, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_UTHROW, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_UTHROW, 1, HG_HITPAUSE_SCALING, 0.5);
set_hitbox_value(AT_UTHROW, 1, HG_HIT_SFX, asset_get("sfx_shovel_hit_light1"));

//Sweet
set_hitbox_value(AT_UTHROW, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_UTHROW, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTHROW, 2, HG_WINDOW, 2);
set_hitbox_value(AT_UTHROW, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_UTHROW, 2, HG_HITBOX_X, 7);
set_hitbox_value(AT_UTHROW, 2, HG_HITBOX_Y, -110);
set_hitbox_value(AT_UTHROW, 2, HG_WIDTH, 54);
set_hitbox_value(AT_UTHROW, 2, HG_HEIGHT, 64);
set_hitbox_value(AT_UTHROW, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_UTHROW, 2, HG_DAMAGE, 9);
set_hitbox_value(AT_UTHROW, 2, HG_ANGLE, 90);
set_hitbox_value(AT_UTHROW, 2, HG_BASE_KNOCKBACK, 7.5);
set_hitbox_value(AT_UTHROW, 2, HG_KNOCKBACK_SCALING, 0.75);
set_hitbox_value(AT_UTHROW, 2, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UTHROW, 2, HG_HITPAUSE_SCALING, 0.75);
set_hitbox_value(AT_UTHROW, 2, HG_HIT_SFX, asset_get("sfx_shovel_hit_med1"));
set_hitbox_value(AT_UTHROW, 2, HG_VISUAL_EFFECT, HFX_GEN_SWEET);