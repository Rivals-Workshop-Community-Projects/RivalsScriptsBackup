set_attack_value(AT_FTHROW, AG_CATEGORY, 1);
set_attack_value(AT_FTHROW, AG_SPRITE, sprite_get("pizzahead_fair"));
set_attack_value(AT_FTHROW, AG_NUM_WINDOWS, 4);
set_attack_value(AT_FTHROW, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_FTHROW, AG_LANDING_LAG, 8);
set_attack_value(AT_FTHROW, AG_HURTBOX_SPRITE, sprite_get("pizzahead_fair_hurt"));

set_window_value(AT_FTHROW, 1, AG_WINDOW_TYPE, 0);
set_window_value(AT_FTHROW, 1, AG_WINDOW_LENGTH, 10);
set_window_value(AT_FTHROW, 1, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_FTHROW, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTHROW, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_FTHROW, 1, AG_WINDOW_SFX_FRAME, 4);

set_window_value(AT_FTHROW, 2, AG_WINDOW_TYPE, 0);
set_window_value(AT_FTHROW, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_FTHROW, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_FTHROW, 2, AG_WINDOW_ANIM_FRAME_START, 4);

set_window_value(AT_FTHROW, 3, AG_WINDOW_TYPE, 0);
set_window_value(AT_FTHROW, 3, AG_WINDOW_LENGTH, 8);
set_window_value(AT_FTHROW, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_FTHROW, 3, AG_WINDOW_ANIM_FRAME_START, 5);

set_window_value(AT_FTHROW, 4, AG_WINDOW_TYPE, 0);
set_window_value(AT_FTHROW, 4, AG_WINDOW_LENGTH, 15);
set_window_value(AT_FTHROW, 4, AG_WINDOW_ANIM_FRAMES, 5);
set_window_value(AT_FTHROW, 4, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_FTHROW, 4, AG_WINDOW_HAS_WHIFFLAG, 1);


set_num_hitboxes(AT_FTHROW, 5);

set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 1, HG_WINDOW, 2);
set_hitbox_value(AT_FTHROW, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FTHROW, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_X, 60);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_Y, -40);
set_hitbox_value(AT_FTHROW, 1, HG_WIDTH, 66);
set_hitbox_value(AT_FTHROW, 1, HG_HEIGHT, 54);
set_hitbox_value(AT_FTHROW, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_FTHROW, 1, HG_DAMAGE, 1);
set_hitbox_value(AT_FTHROW, 1, HG_ANGLE, 80);
set_hitbox_value(AT_FTHROW, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FTHROW, 1, HG_KNOCKBACK_SCALING, 0.5);
set_hitbox_value(AT_FTHROW, 1, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_FTHROW, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FTHROW, 1, HG_HITPAUSE_SCALING, 0.15);
set_hitbox_value(AT_FTHROW, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FTHROW, 1, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_FTHROW, 1, HG_VISUAL_EFFECT, 301);

set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 2, HG_WINDOW, 3);
set_hitbox_value(AT_FTHROW, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FTHROW, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_X, 74);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_Y, -70);
set_hitbox_value(AT_FTHROW, 2, HG_WIDTH, 70);
set_hitbox_value(AT_FTHROW, 2, HG_HEIGHT, 70);
set_hitbox_value(AT_FTHROW, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_FTHROW, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_FTHROW, 2, HG_ANGLE, 125);
set_hitbox_value(AT_FTHROW, 2, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_FTHROW, 2, HG_BASE_KNOCKBACK, 5.5);
set_hitbox_value(AT_FTHROW, 2, HG_KNOCKBACK_SCALING, 0.25);
set_hitbox_value(AT_FTHROW, 2, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_FTHROW, 2, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FTHROW, 2, HG_HITPAUSE_SCALING, 0.15);
set_hitbox_value(AT_FTHROW, 2, HG_HITBOX_GROUP, 2);
set_hitbox_value(AT_FTHROW, 2, HG_HIT_SFX, asset_get("sfx_shovel_hit_light1"));
set_hitbox_value(AT_FTHROW, 2, HG_VISUAL_EFFECT, 302);

set_hitbox_value(AT_FTHROW, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 3, HG_WINDOW, 3);
set_hitbox_value(AT_FTHROW, 3, HG_WINDOW_CREATION_FRAME, 2);
set_hitbox_value(AT_FTHROW, 3, HG_LIFETIME, 2);
set_hitbox_value(AT_FTHROW, 3, HG_HITBOX_X, 74);
set_hitbox_value(AT_FTHROW, 3, HG_HITBOX_Y, -70);
set_hitbox_value(AT_FTHROW, 3, HG_WIDTH, 70);
set_hitbox_value(AT_FTHROW, 3, HG_HEIGHT, 70);
set_hitbox_value(AT_FTHROW, 3, HG_PRIORITY, 2);
set_hitbox_value(AT_FTHROW, 3, HG_DAMAGE, 2);
set_hitbox_value(AT_FTHROW, 3, HG_ANGLE, 125);
set_hitbox_value(AT_FTHROW, 3, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_FTHROW, 3, HG_BASE_KNOCKBACK, 5.5);
set_hitbox_value(AT_FTHROW, 3, HG_KNOCKBACK_SCALING, 0.25);
set_hitbox_value(AT_FTHROW, 3, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_FTHROW, 3, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FTHROW, 3, HG_HITPAUSE_SCALING, 0.15);
set_hitbox_value(AT_FTHROW, 3, HG_HITBOX_GROUP, 3);
set_hitbox_value(AT_FTHROW, 3, HG_HIT_SFX, asset_get("sfx_shovel_hit_light1"));
set_hitbox_value(AT_FTHROW, 3, HG_VISUAL_EFFECT, 302);

set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 4, HG_WINDOW, 3);
set_hitbox_value(AT_FTHROW, 4, HG_WINDOW_CREATION_FRAME, 4);
set_hitbox_value(AT_FTHROW, 4, HG_LIFETIME, 2);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_X, 74);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_Y, -70);
set_hitbox_value(AT_FTHROW, 4, HG_WIDTH, 70);
set_hitbox_value(AT_FTHROW, 4, HG_HEIGHT, 70);
set_hitbox_value(AT_FTHROW, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_FTHROW, 4, HG_DAMAGE, 2);
set_hitbox_value(AT_FTHROW, 4, HG_ANGLE, 125);
set_hitbox_value(AT_FTHROW, 4, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_FTHROW, 4, HG_BASE_KNOCKBACK, 5.5);
set_hitbox_value(AT_FTHROW, 4, HG_KNOCKBACK_SCALING, 0.25);
set_hitbox_value(AT_FTHROW, 4, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_FTHROW, 4, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FTHROW, 4, HG_HITPAUSE_SCALING, 0.15);
set_hitbox_value(AT_FTHROW, 4, HG_HITBOX_GROUP, 4);
set_hitbox_value(AT_FTHROW, 4, HG_HIT_SFX, asset_get("sfx_shovel_hit_light1"));
set_hitbox_value(AT_FTHROW, 4, HG_VISUAL_EFFECT, 302);

set_hitbox_value(AT_FTHROW, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTHROW, 5, HG_WINDOW, 3);
set_hitbox_value(AT_FTHROW, 5, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_FTHROW, 5, HG_LIFETIME, 2);
set_hitbox_value(AT_FTHROW, 5, HG_HITBOX_X, 79);
set_hitbox_value(AT_FTHROW, 5, HG_HITBOX_Y, -75);
set_hitbox_value(AT_FTHROW, 5, HG_WIDTH, 80);
set_hitbox_value(AT_FTHROW, 5, HG_HEIGHT, 80);  
set_hitbox_value(AT_FTHROW, 5, HG_PRIORITY, 1);
set_hitbox_value(AT_FTHROW, 5, HG_DAMAGE, 3);
set_hitbox_value(AT_FTHROW, 5, HG_ANGLE, 75);
set_hitbox_value(AT_FTHROW, 5, HG_BASE_KNOCKBACK, 7);     
set_hitbox_value(AT_FTHROW, 5, HG_KNOCKBACK_SCALING, 0.75);
set_hitbox_value(AT_FTHROW, 5, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_FTHROW, 5, HG_HITPAUSE_SCALING, 0.75);
set_hitbox_value(AT_FTHROW, 5, HG_HITBOX_GROUP, 5);
set_hitbox_value(AT_FTHROW, 5, HG_HIT_SFX, asset_get("sfx_shovel_hit_med1"));
