set_attack_value(AT_DSTRONG_2, AG_SPRITE, sprite_get("pizzahead_dstrong"));
set_attack_value(AT_DSTRONG_2, AG_NUM_WINDOWS, 4);
set_attack_value(AT_DSTRONG_2, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_DSTRONG_2, AG_STRONG_CHARGE_WINDOW, 1);
set_attack_value(AT_DSTRONG_2, AG_HURTBOX_SPRITE, sprite_get("pizzahead_dstrong_hurt"));

set_window_value(AT_DSTRONG_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG_2, 1, AG_WINDOW_LENGTH, 18);
set_window_value(AT_DSTRONG_2, 1, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_DSTRONG_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG_2, 1, AG_WINDOW_SFX, sound_get("dstrongstart")); 

set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_LENGTH, 6);
set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_ANIM_FRAME_START, 7);
set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_SFX, sound_get("ground_hit"));
set_window_value(AT_DSTRONG_2, 2, AG_WINDOW_SFX_FRAME, 2);

set_window_value(AT_DSTRONG_2, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG_2, 3, AG_WINDOW_LENGTH, 18);
set_window_value(AT_DSTRONG_2, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSTRONG_2, 3, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_DSTRONG_2, 3, AG_WINDOW_HAS_WHIFFLAG, 1);

set_window_value(AT_DSTRONG_2, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG_2, 4, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DSTRONG_2, 4, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DSTRONG_2, 4, AG_WINDOW_ANIM_FRAME_START, 12);
set_window_value(AT_DSTRONG_2, 4, AG_WINDOW_HAS_WHIFFLAG, 1);


set_num_hitboxes(AT_DSTRONG_2, 3);

set_hitbox_value(AT_DSTRONG_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSTRONG_2, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DSTRONG_2, 1, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_DSTRONG_2, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HITBOX_X, 44);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HITBOX_Y, -8);
set_hitbox_value(AT_DSTRONG_2, 1, HG_WIDTH, 146);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HEIGHT, 30);
set_hitbox_value(AT_DSTRONG_2, 1, HG_SHAPE, 2);
set_hitbox_value(AT_DSTRONG_2, 1, HG_PRIORITY, 4);
set_hitbox_value(AT_DSTRONG_2, 1, HG_DAMAGE, 10);
set_hitbox_value(AT_DSTRONG_2, 1, HG_ANGLE, 75);
set_hitbox_value(AT_DSTRONG_2, 1, HG_ANGLE_FLIPPER, 3);
set_hitbox_value(AT_DSTRONG_2, 1, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DSTRONG_2, 1, HG_KNOCKBACK_SCALING, 1.1);
set_hitbox_value(AT_DSTRONG_2, 1, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HITPAUSE_SCALING, 1);
set_hitbox_value(AT_DSTRONG_2, 1, HG_FORCE_FLINCH, 1 * has_rune("B"));
set_hitbox_value(AT_DSTRONG_2, 1, HG_EFFECT, 15 * has_rune("B"));
set_hitbox_value(AT_DSTRONG_2, 1, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DSTRONG_2, 1, HG_VISUAL_EFFECT_Y_OFFSET, 36);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));
set_hitbox_value(AT_DSTRONG_2, 1, HG_HITSTUN_MULTIPLIER, 1.0);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSTRONG_2, 1, HG_HIT_LOCKOUT, 4);

//Stomp
set_hitbox_value(AT_DSTRONG_2, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DSTRONG_2, 2, HG_WINDOW, -1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_WINDOW_CREATION_FRAME, -1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HITBOX_X, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HITBOX_Y, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_WIDTH, 50);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HEIGHT, 50);
set_hitbox_value(AT_DSTRONG_2, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DSTRONG_2, 2, HG_DAMAGE, 7);
set_hitbox_value(AT_DSTRONG_2, 2, HG_ANGLE, 90);
set_hitbox_value(AT_DSTRONG_2, 2, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DSTRONG_2, 2, HG_KNOCKBACK_SCALING, 0.8);
set_hitbox_value(AT_DSTRONG_2, 2, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HITPAUSE_SCALING, 0.8);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_GROUNDEDNESS, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_COLLISION_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_DESTROY_EFFECT, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_HSPEED, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_VSPEED, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_GRAVITY, 0);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_PARRY_STUN, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_EXTENDED_PARRY_STUN, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_DOES_NOT_REFLECT, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_PLASMA_SAFE, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_PROJECTILE_UNBASHABLE, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_DSTRONG_2, 2, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));
set_hitbox_value(AT_DSTRONG_2, 2, HG_EXTRA_CAMERA_SHAKE, -1);

//Big Stomp
set_hitbox_value(AT_DSTRONG_2, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSTRONG_2, 3, HG_WINDOW, 2);
set_hitbox_value(AT_DSTRONG_2, 3, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_DSTRONG_2, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HITBOX_X, 0);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HITBOX_Y, 0);
set_hitbox_value(AT_DSTRONG_2, 3, HG_WIDTH, 3000);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HEIGHT, 10);
set_hitbox_value(AT_DSTRONG_2, 3, HG_SHAPE, 1);
set_hitbox_value(AT_DSTRONG_2, 3, HG_PRIORITY, 2);
set_hitbox_value(AT_DSTRONG_2, 3, HG_DAMAGE, 0);
set_hitbox_value(AT_DSTRONG_2, 3, HG_ANGLE, 90);
set_hitbox_value(AT_DSTRONG_2, 3, HG_TECHABLE, 3);
set_hitbox_value(AT_DSTRONG_2, 3, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(AT_DSTRONG_2, 3, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HITSTUN_MULTIPLIER, 0.75);
set_hitbox_value(AT_DSTRONG_2, 3, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DSTRONG_2, 3, HG_EXTRA_HITPAUSE, 14 * has_rune("B"));
set_hitbox_value(AT_DSTRONG_2, 3, HG_GROUNDEDNESS, 1);
set_hitbox_value(AT_DSTRONG_2, 3, HG_FORCE_FLINCH, 1 * has_rune("B"));
set_hitbox_value(AT_DSTRONG_2, 3, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSTRONG_2, 3, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_DSTRONG_2, 3, HG_VISUAL_EFFECT_Y_OFFSET, 36);
set_hitbox_value(AT_DSTRONG_2, 3, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));