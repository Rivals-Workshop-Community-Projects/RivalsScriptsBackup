set_attack_value(AT_FTILT, AG_SPRITE, sprite_get("ftilt"));
set_attack_value(AT_FTILT, AG_NUM_WINDOWS, 4);
set_attack_value(AT_FTILT, AG_HURTBOX_SPRITE, sprite_get("ftilt_hurt"));

set_window_value(AT_FTILT, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTILT, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_FTILT, 1, AG_WINDOW_ANIM_FRAMES, 0);
set_window_value(AT_FTILT, 1, AG_WINDOW_ANIM_FRAME_START, 0);

set_window_value(AT_FTILT, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTILT, 2, AG_WINDOW_LENGTH, 20);
set_window_value(AT_FTILT, 2, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_FTILT, 2, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_FTILT, 2, AG_WINDOW_HSPEED, 3);
set_window_value(AT_FTILT, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTILT, 2, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_FTILT, 2, AG_WINDOW_SFX_FRAME, 2);

set_window_value(AT_FTILT, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTILT, 3, AG_WINDOW_LENGTH, 10);
set_window_value(AT_FTILT, 3, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_FTILT, 3, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_FTILT, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FTILT, 3, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));

set_window_value(AT_FTILT, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_FTILT, 4, AG_WINDOW_LENGTH, 9);
set_window_value(AT_FTILT, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_FTILT, 4, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_FTILT, 4, AG_WINDOW_HAS_WHIFFLAG, 5);

set_num_hitboxes(AT_FTILT,2);

set_hitbox_value(AT_FTILT, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_FTILT, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTILT, 1, HG_WINDOW, 2);
set_hitbox_value(AT_FTILT, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_FTILT, 1, HG_HITBOX_X, 45);
set_hitbox_value(AT_FTILT, 1, HG_HITBOX_Y, -24);
set_hitbox_value(AT_FTILT, 1, HG_WIDTH, 50);
set_hitbox_value(AT_FTILT, 1, HG_HEIGHT, 40);
set_hitbox_value(AT_FTILT, 1, HG_SHAPE, 0);
set_hitbox_value(AT_FTILT, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_FTILT, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_FTILT, 1, HG_ANGLE, 40);
set_hitbox_value(AT_FTILT, 1, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_FTILT, 1, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_FTILT, 1, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_FTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 16);
set_hitbox_value(AT_FTILT, 1, HG_EXTRA_CAMERA_SHAKE, 2);

set_hitbox_value(AT_FTILT, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_FTILT, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FTILT, 2, HG_WINDOW, 3);
set_hitbox_value(AT_FTILT, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_FTILT, 2, HG_HITBOX_X, 55);
set_hitbox_value(AT_FTILT, 2, HG_HITBOX_Y, -24);
set_hitbox_value(AT_FTILT, 2, HG_WIDTH, 60);
set_hitbox_value(AT_FTILT, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FTILT, 2, HG_HEIGHT, 50);
set_hitbox_value(AT_FTILT, 2, HG_SHAPE, 0);
set_hitbox_value(AT_FTILT, 2, HG_PRIORITY, 1);
set_hitbox_value(AT_FTILT, 2, HG_DAMAGE, 4);
set_hitbox_value(AT_FTILT, 2, HG_ANGLE, 361);
set_hitbox_value(AT_FTILT, 2, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FTILT, 2, HG_KNOCKBACK_SCALING, .4);
set_hitbox_value(AT_FTILT, 2, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_FTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 16);
set_hitbox_value(AT_FTILT, 2, HG_EXTRA_CAMERA_SHAKE, 2);

set_hitbox_value(AT_FTILT, 3, HG_MUNO_HITBOX_NAME, "Star 1");
set_hitbox_value(AT_FTILT, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_FTILT, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_FTILT, 3, HG_WINDOW, 2);
set_hitbox_value(AT_FTILT, 3, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_FTILT, 3, HG_LIFETIME, 10);
set_hitbox_value(AT_FTILT, 3, HG_HITBOX_X, 45);
set_hitbox_value(AT_FTILT, 3, HG_HITBOX_Y, -24);
set_hitbox_value(AT_FTILT, 3, HG_WIDTH, 90);
set_hitbox_value(AT_FTILT, 3, HG_HEIGHT, 60);
set_hitbox_value(AT_FTILT, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_FTILT, 3, HG_DAMAGE, star_damage);
set_hitbox_value(AT_FTILT, 3, HG_ANGLE, 50);
set_hitbox_value(AT_FTILT, 3, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_FTILT, 3, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_FTILT, 3, HG_HITSTUN_MULTIPLIER, -1);
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_SPRITE, sprite_get("star_proj"));
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_MASK, sprite_get("star_proj"));
set_hitbox_value(AT_FTILT, 3, HG_VISUAL_EFFECT, stardestroyfx);
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_ANIM_SPEED, .5);
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_HSPEED, 5);
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_AIR_FRICTION, -0.05);
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_FTILT, 3, HG_PROJECTILE_WALL_BEHAVIOR, 2);

set_hitbox_value(AT_FTILT, 4, HG_MUNO_HITBOX_NAME, "Star 2");
set_hitbox_value(AT_FTILT, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_FTILT, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_FTILT, 4, HG_WINDOW, 3);
set_hitbox_value(AT_FTILT, 4, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_FTILT, 4, HG_LIFETIME, 15);
set_hitbox_value(AT_FTILT, 4, HG_HITBOX_X, 55);
set_hitbox_value(AT_FTILT, 4, HG_HITBOX_Y, -24);
set_hitbox_value(AT_FTILT, 4, HG_WIDTH, 90);
set_hitbox_value(AT_FTILT, 4, HG_HEIGHT, 60);
set_hitbox_value(AT_FTILT, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_FTILT, 4, HG_DAMAGE, star_damage);
set_hitbox_value(AT_FTILT, 4, HG_ANGLE, 50);
set_hitbox_value(AT_FTILT, 4, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_FTILT, 4, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_FTILT, 4, HG_HITSTUN_MULTIPLIER, -1);
set_hitbox_value(AT_FTILT, 4, HG_PROJECTILE_SPRITE, sprite_get("star_proj"));
set_hitbox_value(AT_FTILT, 4, HG_PROJECTILE_MASK, sprite_get("star_proj"));
set_hitbox_value(AT_FTILT, 4, HG_VISUAL_EFFECT, stardestroyfx);
set_hitbox_value(AT_FTILT, 4, HG_PROJECTILE_ANIM_SPEED, .5);
set_hitbox_value(AT_FTILT, 4, HG_PROJECTILE_HSPEED, 7);
set_hitbox_value(AT_FTILT, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_FTILT, 4, HG_PROJECTILE_WALL_BEHAVIOR, 2);
