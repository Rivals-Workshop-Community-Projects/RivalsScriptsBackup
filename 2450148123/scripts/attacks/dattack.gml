set_attack_value(AT_DATTACK, AG_SPRITE, sprite_get("dattack"));
set_attack_value(AT_DATTACK, AG_NUM_WINDOWS, 4);
set_attack_value(AT_DATTACK, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_DATTACK, AG_HURTBOX_SPRITE, sprite_get("dattack_hurt"));

set_window_value(AT_DATTACK, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DATTACK, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DATTACK, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DATTACK, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_DATTACK, 1, AG_WINDOW_SFX_FRAME, 7);

set_window_value(AT_DATTACK, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 2, AG_WINDOW_LENGTH, 15);
set_window_value(AT_DATTACK, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DATTACK, 2, AG_WINDOW_ANIM_FRAME_START, 1);


set_window_value(AT_DATTACK, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 3, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DATTACK, 3, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DATTACK, 3, AG_WINDOW_ANIM_FRAME_START, 6);

set_window_value(AT_DATTACK, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 4, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DATTACK, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DATTACK, 4, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_DATTACK, 4, AG_WINDOW_HAS_WHIFFLAG, 10);

set_num_hitboxes(AT_DATTACK, 2);

set_hitbox_value(AT_DATTACK, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DATTACK, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DATTACK, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DATTACK, 1, HG_LIFETIME, 10);
set_hitbox_value(AT_DATTACK, 1, HG_HITBOX_X, 26);
set_hitbox_value(AT_DATTACK, 1, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DATTACK, 1, HG_WIDTH, 60);
set_hitbox_value(AT_DATTACK, 1, HG_HEIGHT, 20);
set_hitbox_value(AT_DATTACK, 1, HG_SHAPE, 1);
set_hitbox_value(AT_DATTACK, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DATTACK, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_DATTACK, 1, HG_ANGLE, 80);
set_hitbox_value(AT_DATTACK, 1, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DATTACK, 1, HG_KNOCKBACK_SCALING, 0.1);
set_hitbox_value(AT_DATTACK, 1, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_DATTACK, 1, HG_EXTRA_HITPAUSE, 10);
set_hitbox_value(AT_DATTACK, 1, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_DATTACK, 1, HG_VISUAL_EFFECT, 13);
set_hitbox_value(AT_DATTACK, 1, HG_VISUAL_EFFECT_X_OFFSET, 10);
set_hitbox_value(AT_DATTACK, 1, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_DATTACK, 1, HG_HITSTUN_MULTIPLIER, 1.0);

set_hitbox_value(AT_DATTACK, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DATTACK, 2, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 2, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_DATTACK, 2, HG_LIFETIME, 5);
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_X, 58);
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DATTACK, 2, HG_WIDTH, 70);
set_hitbox_value(AT_DATTACK, 2, HG_HEIGHT, 46);
set_hitbox_value(AT_DATTACK, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DATTACK, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DATTACK, 2, HG_DAMAGE, 5);
set_hitbox_value(AT_DATTACK, 2, HG_ANGLE, 60);
set_hitbox_value(AT_DATTACK, 2, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DATTACK, 2, HG_KNOCKBACK_SCALING, 0.6);
set_hitbox_value(AT_DATTACK, 2, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_DATTACK, 2, HG_HITSTUN_MULTIPLIER, 0.7);
set_hitbox_value(AT_DATTACK, 1, HG_EXTRA_HITPAUSE, 5);
set_hitbox_value(AT_DATTACK, 2, HG_HITPAUSE_SCALING, .9);
set_hitbox_value(AT_DATTACK, 2, HG_VISUAL_EFFECT, 13);
set_hitbox_value(AT_DATTACK, 2, HG_VISUAL_EFFECT_X_OFFSET, 10);
set_hitbox_value(AT_DATTACK, 2, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_DATTACK, 2, HG_HITSTUN_MULTIPLIER, 1.0);

set_hitbox_value(AT_DATTACK, 3, HG_MUNO_HITBOX_NAME, "Star 1");
set_hitbox_value(AT_DATTACK, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DATTACK, 3, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DATTACK, 3, HG_WINDOW_CREATION_FRAME, 2);
set_hitbox_value(AT_DATTACK, 3, HG_LIFETIME, 15);
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_X, 40);
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DATTACK, 3, HG_WIDTH, 90);
set_hitbox_value(AT_DATTACK, 3, HG_HEIGHT, 60);
set_hitbox_value(AT_DATTACK, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_DATTACK, 3, HG_DAMAGE, star_damage);
set_hitbox_value(AT_DATTACK, 3, HG_ANGLE, 50);
set_hitbox_value(AT_DATTACK, 3, HG_EXTRA_HITPAUSE, 3);
set_hitbox_value(AT_DATTACK, 3, HG_HITSTUN_MULTIPLIER, -1);
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_SPRITE, sprite_get("star_proj"));
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_MASK, sprite_get("star_proj"));
set_hitbox_value(AT_DATTACK, 3, HG_VISUAL_EFFECT, stardestroyfx);
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_ANIM_SPEED, .5);
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_HSPEED, 9);
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_VSPEED, 1);
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DATTACK, 3, HG_PROJECTILE_WALL_BEHAVIOR, 2);

set_hitbox_value(AT_DATTACK, 4, HG_MUNO_HITBOX_NAME, "Star 2");
set_hitbox_value(AT_DATTACK, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DATTACK, 4, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DATTACK, 4, HG_WINDOW_CREATION_FRAME, 2);
set_hitbox_value(AT_DATTACK, 4, HG_LIFETIME, 15);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_X, 20);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DATTACK, 4, HG_WIDTH, 90);
set_hitbox_value(AT_DATTACK, 4, HG_HEIGHT, 60);
set_hitbox_value(AT_DATTACK, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_DATTACK, 4, HG_DAMAGE, star_damage);
set_hitbox_value(AT_DATTACK, 4, HG_ANGLE, 50);
set_hitbox_value(AT_DATTACK, 4, HG_EXTRA_HITPAUSE, 3);
set_hitbox_value(AT_DATTACK, 4, HG_HITSTUN_MULTIPLIER, -1);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_SPRITE, sprite_get("star_proj"));
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_MASK, sprite_get("star_proj"));
set_hitbox_value(AT_DATTACK, 4, HG_VISUAL_EFFECT, stardestroyfx);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_ANIM_SPEED, .5);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_HSPEED, 8);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_VSPEED, 2);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_AIR_FRICTION, -0.05);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_GROUND_FRICTION, 0);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DATTACK, 4, HG_PROJECTILE_WALL_BEHAVIOR, 2);

set_hitbox_value(AT_DATTACK, 5, HG_MUNO_HITBOX_NAME, "Star 3");
set_hitbox_value(AT_DATTACK, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DATTACK, 5, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DATTACK, 5, HG_WINDOW_CREATION_FRAME, 2);
set_hitbox_value(AT_DATTACK, 5, HG_LIFETIME, 15);
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_X, -20);
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_Y, 48)
set_hitbox_value(AT_DATTACK, 5, HG_WIDTH, 90);
set_hitbox_value(AT_DATTACK, 5, HG_HEIGHT, 60);
set_hitbox_value(AT_DATTACK, 5, HG_PRIORITY, 3);
set_hitbox_value(AT_DATTACK, 5, HG_DAMAGE, star_damage);
set_hitbox_value(AT_DATTACK, 5, HG_ANGLE, 50);
set_hitbox_value(AT_DATTACK, 5, HG_EXTRA_HITPAUSE, 3);
set_hitbox_value(AT_DATTACK, 5, HG_HITSTUN_MULTIPLIER, -1);
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_SPRITE, sprite_get("star_proj"));
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_MASK, sprite_get("star_proj"));
set_hitbox_value(AT_DATTACK, 5, HG_VISUAL_EFFECT, stardestroyfx);
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_ANIM_SPEED, .5);
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_HSPEED, 7);
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_VSPEED, 3);
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_GROUND_BEHAVIOR, 2);
set_hitbox_value(AT_DATTACK, 5, HG_PROJECTILE_WALL_BEHAVIOR, 2)
