...
if (attack == AT_NSPECIAL && window == 1 && window_timer == 1 && nspecial_done == 0) {
    ...
    sound_play(sound_get("sfx_switch"));
    move_cooldown[AT_NSPECIAL] = 60;
    //if hit_timer > 0 move_cooldown[AT_NSPECIAL] = 20;
    //else move_cooldown[AT_NSPECIAL] = 60;
    nspec_vfx_timer = nspec_vfx_time_max;
    nspec_vfx_spr = sprite_index;
    nspec_vfx_index = image_index;
    nspec_vfx_x = x;
...
if (attack == AT_NAIR && window == 2 && window_timer == 2) spr_dir = -spr_dir;
if attack == AT_NAIR && attack_down == 0 att_down = 0;
if attack == AT_NAIR && window == 5 && window_timer == 4 && att_down {
    set_attack(AT_NAIR);
}
...
if attack == AT_DSTRONG && window == 1 {
    ...
    set_hitbox_value(attack, 1, HG_BASE_KNOCKBACK, abs(hsp/1.2));
    set_hitbox_value(attack, 1, HG_BASE_HITPAUSE, abs(hsp/1.2));
    ...
    if abs(hsp) > 8 && state_timer > 7 {
        instance_destroy(dstr_hit);
        dstr_hit = create_hitbox(AT_DSTRONG, 1, x,y);
        if !dspec_sound_done {
...
if attack == AT_DAIR {
    if window == 1 && window_timer == 1 {
        set_window_value(AT_DAIR, 2, AG_WINDOW_VSPEED, -6+dair_count);
        dair_count++;
    }
...
if attack == AT_TAUNT && window == 1 && window_timer == 1 && player_color != 3 {
    if random_func(0,40,true) == 1 {
