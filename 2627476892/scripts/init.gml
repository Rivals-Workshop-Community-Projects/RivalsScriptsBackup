...
spr_vfx_roll = sprite_get("roll_vfx");
vfx_roll = hit_fx_create(spr_vfx_roll, 15);
...
// Hurtbox sprites
hurtbox_spr         = sprite_get("lancer_hurtbox");
crouchbox_spr       = sprite_get("lancer_crouchbox");
air_hurtbox_spr     = -1; // -1 = use hurtbox_spr
hitstun_hurtbox_spr = -1; // -1 = use hurtbox_spr

// Victory
set_victory_bg(sprite_get("victory_background")); // victory_background.png
set_victory_theme(sound_get("victory_lancer2")); // victory_theme.ogg
...
air_dodge_sound     = sound_get("snd_arrow");
...
if(alt == 6){ //susie
	lancer_laugh = sound_get("snd_susielaugh");
}if(alt == 10){ //jevil
	lancer_laugh = sound_get("snd_joker_laugh0");
}if(alt == 22){ //wario
	lancer_laugh = sound_get("snd_wariolaugh");
}if(alt == 23){ //robotnik
	lancer_laugh = sound_get("snd_robotniklaugh");
}if(alt == 14){ //spamton
	lancer_laugh = sound_get("snd_spamtonlaugh");
}if(alt == 9){ //king
	lancer_laugh = sound_get("snd_dadlaugh");
}if(alt == 13){ //queen
...
vfx_afterimage = hit_fx_create(spr_vfx_afterimage, 20);
}if(alt != 0){
	spr_vfx_afterimage = sprite_get("afterimage1");
vfx_afterimage = hit_fx_create(spr_vfx_afterimage, 20);
}

arena_title = "The Dark Jack";
battle_text = "*Smells like Salsa.";
teve_death_message = "Steve joined The Lancer Fan Club.";
