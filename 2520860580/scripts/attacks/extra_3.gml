set_attack_value(AT_EXTRA_3, AG_CATEGORY, 2);
set_attack_value(AT_EXTRA_3, AG_SPRITE, sprite_get("extra3"));
set_attack_value(AT_EXTRA_3, AG_NUM_WINDOWS, 3);
set_attack_value(AT_EXTRA_3, AG_HAS_LANDING_LAG, 4);
set_attack_value(AT_EXTRA_3, AG_AIR_SPRITE, sprite_get("extra3"));
set_attack_value(AT_EXTRA_3, AG_HURTBOX_SPRITE, sprite_get("extra3_hurt"));


set_window_value(AT_EXTRA_3, 1, AG_WINDOW_TYPE, 0);
set_window_value(AT_EXTRA_3, 1, AG_WINDOW_LENGTH, 24);
set_window_value(AT_EXTRA_3, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_EXTRA_3, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_EXTRA_3, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_3, 1, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_mega_instant"));
set_window_value(AT_EXTRA_3, 1, AG_WINDOW_SFX_FRAME, 2);

set_window_value(AT_EXTRA_3, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_EXTRA_3, 2, AG_WINDOW_LENGTH, 8);
set_window_value(AT_EXTRA_3, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_EXTRA_3, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_3, 2, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_mega_instant"));
set_window_value(AT_EXTRA_3, 2, AG_WINDOW_SFX_FRAME, 0);

set_window_value(AT_EXTRA_3, 3, AG_WINDOW_ANIM_FRAME_START, 7);
set_window_value(AT_EXTRA_3, 3, AG_WINDOW_LENGTH, 30);
set_window_value(AT_EXTRA_3, 3, AG_WINDOW_ANIM_FRAMES, 5);
set_window_value(AT_EXTRA_3, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_3, 3, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_mega_instant"));
set_window_value(AT_EXTRA_3, 3, AG_WINDOW_SFX_FRAME, 0);

set_window_value(AT_EXTRA_3, 4, AG_WINDOW_ANIM_FRAME_START, 12);
set_window_value(AT_EXTRA_3, 4, AG_WINDOW_LENGTH, 60);
set_window_value(AT_EXTRA_3, 4, AG_WINDOW_ANIM_FRAMES, 11);
set_window_value(AT_EXTRA_3, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_3, 4, AG_WINDOW_SFX, sound_get("Drive2"));
set_window_value(AT_EXTRA_3, 4, AG_WINDOW_SFX_FRAME, 1);


set_num_hitboxes(AT_EXTRA_3, 4);


set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(AT_EXTRA_3, 1, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_EXTRA_3, 1, HG_WINDOW, 2);
set_hitbox_value(AT_EXTRA_3, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_3, 1, HG_LIFETIME, 60);
set_hitbox_value(AT_EXTRA_3, 1, HG_HITBOX_X, 10);
set_hitbox_value(AT_EXTRA_3, 1, HG_HITBOX_Y, -36);
set_hitbox_value(AT_EXTRA_3, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_SPRITE, sprite_get("bladewaveH"));
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_AIR_FRICTION, 0);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_HSPEED, 22);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_VSPEED, -2);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_ANIM_SPEED, 1);
set_hitbox_value(AT_EXTRA_3, 1, HG_PROJECTILE_DESTROY_EFFECT, 306);
set_hitbox_value(AT_EXTRA_3, 1, HG_WIDTH, 45);
set_hitbox_value(AT_EXTRA_3, 1, HG_HEIGHT, 76);
set_hitbox_value(AT_EXTRA_3, 1, HG_SHAPE, 0);
set_hitbox_value(AT_EXTRA_3, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_EXTRA_3, 1, HG_DAMAGE, 4);
set_hitbox_value(AT_EXTRA_3, 1, HG_ANGLE, 60);
set_hitbox_value(AT_EXTRA_3, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_EXTRA_3, 1, HG_KNOCKBACK_SCALING, 0.4);
set_hitbox_value(AT_EXTRA_3, 1, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_EXTRA_3, 1, HG_EXTRA_HITPAUSE, 22);
set_hitbox_value(AT_EXTRA_3, 1, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_EXTRA_3, 1, HG_HIT_SFX, sound_get("SpaceCut"));
set_hitbox_value(AT_EXTRA_3, 1, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_3, 1, HG_VISUAL_EFFECT, slashc);
set_hitbox_value(AT_EXTRA_3, 1, HG_HITSTUN_MULTIPLIER, .7);
set_hitbox_value(AT_EXTRA_2, 1, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(AT_EXTRA_2, 1, HG_EXTENDED_PARRY_STUN, 1);
set_hitbox_value(AT_EXTRA_2, 1, HG_PROJECTILE_DOES_NOT_REFLECT, true);

set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(AT_EXTRA_3, 2, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_EXTRA_3, 2, HG_WINDOW, 3);
set_hitbox_value(AT_EXTRA_3, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_3, 2, HG_LIFETIME, 60);
set_hitbox_value(AT_EXTRA_3, 2, HG_HITBOX_X, 10);
set_hitbox_value(AT_EXTRA_3, 2, HG_HITBOX_Y, -36);
set_hitbox_value(AT_EXTRA_3, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_SPRITE, sprite_get("bladewaveH"));
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_AIR_FRICTION, 0);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_HSPEED, 22);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_VSPEED, 0.5);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_ANIM_SPEED, 1);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_DESTROY_EFFECT, 306);
set_hitbox_value(AT_EXTRA_3, 2, HG_WIDTH, 45);
set_hitbox_value(AT_EXTRA_3, 2, HG_HEIGHT, 76);
set_hitbox_value(AT_EXTRA_3, 2, HG_SHAPE, 0);
set_hitbox_value(AT_EXTRA_3, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_EXTRA_3, 2, HG_DAMAGE, 4);
set_hitbox_value(AT_EXTRA_3, 2, HG_ANGLE, 60);
set_hitbox_value(AT_EXTRA_3, 2, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_EXTRA_3, 2, HG_KNOCKBACK_SCALING, 0.4);
set_hitbox_value(AT_EXTRA_3, 2, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_EXTRA_3, 2, HG_EXTRA_HITPAUSE, 22);
set_hitbox_value(AT_EXTRA_3, 2, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_EXTRA_3, 2, HG_HIT_SFX, sound_get("SpaceCut"));
set_hitbox_value(AT_EXTRA_3, 2, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_3, 2, HG_VISUAL_EFFECT, slashc);
set_hitbox_value(AT_EXTRA_3, 2, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_EXTRA_3, 2, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(AT_EXTRA_3, 2, HG_EXTENDED_PARRY_STUN, 1);
set_hitbox_value(AT_EXTRA_2, 2, HG_PROJECTILE_DOES_NOT_REFLECT, true);

set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_EXTRA_3, 3, HG_WINDOW, 999);
set_hitbox_value(AT_EXTRA_3, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_LIFETIME, 150);
set_hitbox_value(AT_EXTRA_3, 3, HG_HITBOX_X, 40);
set_hitbox_value(AT_EXTRA_3, 3, HG_HITBOX_Y, -36);
set_hitbox_value(AT_EXTRA_3, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_SPRITE, sprite_get("bladewaveH"));
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_AIR_FRICTION, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_HSPEED, 5);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_VSPEED, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_ANIM_SPEED, 0.6);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_DESTROY_EFFECT, 306);
set_hitbox_value(AT_EXTRA_3, 3, HG_WIDTH, 50);
set_hitbox_value(AT_EXTRA_3, 3, HG_HEIGHT, 80);
set_hitbox_value(AT_EXTRA_3, 3, HG_SHAPE, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_EXTRA_3, 3, HG_DAMAGE, 1);
set_hitbox_value(AT_EXTRA_3, 3, HG_ANGLE, 35);
set_hitbox_value(AT_EXTRA_3, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_EXTRA_3, 3, HG_KNOCKBACK_SCALING, 0.1);
set_hitbox_value(AT_EXTRA_3, 3, HG_BASE_HITPAUSE, 2);
set_hitbox_value(AT_EXTRA_3, 3, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_HIT_SFX, sound_get("slice"));
set_hitbox_value(AT_EXTRA_3, 3, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_3, 3, HG_VISUAL_EFFECT, 305);
set_hitbox_value(AT_EXTRA_3, 3, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_PARRY_STUN, 0);
set_hitbox_value(AT_EXTRA_3, 3, HG_EXTENDED_PARRY_STUN, 1);
set_hitbox_value(AT_EXTRA_3, 3, HG_HIT_LOCKOUT, 1);
set_hitbox_value(AT_EXTRA_3, 3, HG_SDI_MULTIPLIER, 0.1);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_IS_TRANSCENDENT, true);
set_hitbox_value(AT_EXTRA_3, 3, HG_PROJECTILE_DOES_NOT_REFLECT, true);