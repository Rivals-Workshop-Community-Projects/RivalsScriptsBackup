...
if phone.state{
	...
	if !(phone.stage_id != noone && phone.state == 5){
		...
		with phone{
			...
			var txt = "Quit Pokédex";
			if app txt = "Home Menu";
			if selected txt = apps[app].name;
			
			drawControls(origin_x - 38, origin_y - 162, "Special:
			   " + txt + "
			Parry or Taunt:
			   Minimize");
			...
			// Wallpaper
				...
				rectDraw(origin_x-20, origin_y, screen_w+20, screen_h+6, apps[app].color);
			...
			switch(app){
					...
					switch(player_id.muno_char_id){
						...
						case 1:
							...
							if !array_length_1d(player_id.codec_handler.active_codecs){
								...
								textDraw(origin_x + screen_w / 2 + 0, origin_y + screen_h / 2, "fName", c_white, 18, screen_w, fa_center, 1, true, phone.image_alpha, "This app is useless!", false);
								textDraw(origin_x + screen_w / 2 + 0, origin_y + screen_h / 2 + 50, "fName", apps[app].color, 18, screen_w, fa_center, 1, true, phone.image_alpha, "Get out and don't come back!", false);
						...
						default:
							...
							var drawn = textDraw(draw_x, draw_y, "fName", c_white, 18, draw_w, fa_center, 1, true, phone.image_alpha, "Welcome to the useless app!", true);
							if !selected changeCursor(draw_x - drawn[0] / 2 - 7, draw_y - drawn[1] / 2 - 4 + sin_thing, drawn[0] + 11, drawn[1] + 1);
							
							draw_set_valign(valign);
							
							textDraw(draw_x, origin_y + screen_h - 58, "fName", apps[app].color, 18, draw_w, fa_center, 1, true, phone.image_alpha, "This app is useless!", false);
				...
				case 9:
					...
					var drawn = textDraw(draw_x, draw_y, "fName", c_white, 18, draw_w, fa_center, 1, true, phone.image_alpha, "Turn Off Dex", true);
					if !selected changeCursor(draw_x - drawn[0] / 2 - 7, draw_y - drawn[1] / 2 - 4 + sin_thing, drawn[0] + 11, drawn[1] + 1);
			...
			if !phone_settings[setting_fast_graphics] && phone.image_alpha > 0.75 drawClock(origin_x + 80, origin_y + 2, other.current_hour, other.current_minute, other.current_second);
...
textDraw(draw_x, origin_y + screen_h - 36, "fName", app_color, 18, draw_w, fa_center, 1, state != 5, phone.image_alpha, "Minimize to
Auto-Scroll", false);
