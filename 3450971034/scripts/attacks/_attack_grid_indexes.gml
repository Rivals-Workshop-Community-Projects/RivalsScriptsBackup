...
/*
    ...
    set_hitbox_value(atk, hitbox_num, HG_MULTIHIT_FLIPPER, [number]);
    //  sets an angle flipper for multihit projectiles
    //  HG_ANGLE_FLIPPER acts as the final hit's angle flipper, and writing -1 will keep HG_ANGLE_FLIPPER on the multihits

    set_hitbox_value(atk, hitbox_num, HG_PROJECTILE_HOMING, [number]);
    //  if true, allow projectile to use the homing code, homing into players

    set_hitbox_value(atk, hitbox_num, HG_PROJECTILE_HOMING_TURN, [number]);
    //  how sharp the angle for the homing is, and only applies if homing is true (make sure it's a number between 0 and 1)
*/