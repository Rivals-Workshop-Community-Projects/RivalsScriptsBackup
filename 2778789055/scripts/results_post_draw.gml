...
if ("pumbo_initialized_victory_screen" not in self)
{
    pumbo_initialized_victory_screen = true;
    //defaults to prevent errors
    pumbo_victory_quote = "Hey Po, is the stove supposed to be shaking like that?";
...
if (winner == player /* && uhc_batteries */)
{
    ...
    var quote_time  =   240;
    //Animate panel
    if ("pumbo_quote_current_pos_x" not in self)
    { pumbo_quote_current_pos_x = hide_pos_x; }
    
    //Must check with timing or if result boxes are open
    var diff = ((results_timer > quote_time && !someone_pressed) ? 
                    quote_pos_x : hide_pos_x) - pumbo_quote_current_pos_x;
    
    pumbo_quote_current_pos_x += sign(diff) 
                                * max(min(abs(diff), 5), abs(diff) * 0.25);
    
    //Draw panel
    if (pumbo_quote_current_pos_x > hide_pos_x)
    {
        draw_sprite(sprite_get("victory_quote_bg"), 0, 
                    pumbo_quote_current_pos_x, quote_pos_y);
        draw_sprite_ext(sprite_get("stockicon_speaker"), pumbo_victory_speaker, 
                    pumbo_quote_current_pos_x + 46, quote_pos_y + 10, 1.5, 1.5, 0, c_white, 1);
        draw_win_quote(pumbo_quote_current_pos_x+135, quote_pos_y+15, 
                        pumbo_victory_quote);
    }
}
...

{
        ...
        switch(random_seed){
            case 0:
                quotes = "Yo, nice fight! Now do you wanna order somethin', or...?";
                pumbo_victory_speaker = 0;
            break;
            case 1:
                quotes = "So, you want fries with that?";
                pumbo_victory_speaker = 1;
            break;
            case 2:
                quotes = "All this fighting has really built up my appetite!";
                pumbo_victory_speaker = 0;
            break;
            case 3:
                quotes = "We don't get paid enough for this... The fight club is THAT way.";
                pumbo_victory_speaker = 1;
            break;            
            case 4:
                quotes = "That was a tough fight.. You're really good at this.";
                pumbo_victory_speaker = 0;
            break;            
            case 5:
                quotes = "Nice fighting Gum, I can take the shift from here.";
                pumbo_victory_speaker = 1;
            break;
        }
...
/*
{
    var quotes = [];
    var i = 0;
    quotes[i++] = "Yo, nice fight! Now do you wanna order somethin', or...?";
    quotes[i++] = "";
    quotes[i++] = "";
    quotes[i++] = "";
    quotes[i++] = "";
...
#define get_victory_screen_data()
{
    ...
    with (asset_get("pHitBox")) if ("pumbo_victory_screen_array" in self)
    {
        data_array = pumbo_victory_screen_array;
    ...
    if (string_length(data_array[winner].status_quote) > 1)
    {
        ...
        pumbo_victory_quote = data_array[winner].status_quote;
        pumbo_victory_speaker = data_array[winner].speaker;
    }
    else
    {
        ...
        pumbo_victory_speaker = data_array[best_player].speaker;
        pumbo_victory_quote = data_array[best_player].quote;
        if (string_length(pumbo_victory_quote) < 1)
        { pumbo_victory_quote = get_random_quote(); }
    }
