sprite_change_offset("gumbo_hurtbox", 56, 94);//
sprite_change_offset("gumbo_crouchbox", 56, 94);//
sprite_change_offset("idle", 28, 47);//
sprite_change_offset("bighurt", 28, 47);//
sprite_change_offset("uphurt", 28, 47);//
sprite_change_offset("downhurt", 28, 47);//
sprite_change_offset("hurt", 28, 47);//
sprite_change_offset("bouncehurt", 31, 47);//
sprite_change_offset("spinhurt", 28, 47);//
sprite_change_offset("crouch", 28, 47);
sprite_change_offset("walk", 28, 47);//
sprite_change_offset("walkturn", 28, 47);//
sprite_change_offset("dash", 28, 47);//
sprite_change_offset("dashstart", 28, 47);//
sprite_change_offset("dashstop", 28, 47);//
sprite_change_offset("dashturn", 28, 47);//

sprite_change_offset("jumpstart", 28, 47);//
sprite_change_offset("jump", 28, 49);//
sprite_change_offset("doublejump", 28, 51);//
sprite_change_offset("walljump", 28, 51);//
sprite_change_offset("pratfall", 28, 51);//
sprite_change_offset("land", 28, 47);//
sprite_change_offset("landinglag", 28, 47);//

sprite_change_offset("parry", 28, 47);//
sprite_change_offset("roll_forward", 28, 47);//
sprite_change_offset("roll_backward", 28, 47);//
sprite_change_offset("airdodge", 28, 49);//
sprite_change_offset("waveland", 28, 47);//
sprite_change_offset("tech", 28, 47);//

sprite_change_offset("jab", 28, 63, true);//
sprite_change_offset("dattack", 28, 47, true);//
sprite_change_offset("ftilt", 38, 47, true);//
sprite_change_offset("dtilt", 28, 47, true);//
sprite_change_offset("utilt", 28, 69, true);//
sprite_change_offset("nair", 34, 51, true);//
sprite_change_offset("fair", 33, 50, true);//
sprite_change_offset("bair", 54, 49, true);//
sprite_change_offset("uair", 34, 61, true);//
sprite_change_offset("dair", 28, 50, true);//
sprite_change_offset("fstrong", 48, 47, true);//
sprite_change_offset("fstrong2", 28, 47, true);//
sprite_change_offset("ustrong", 56, 75, true);//
sprite_change_offset("ustrong2", 56, 75, true);//
sprite_change_offset("dstrong", 94, 47, true);//
sprite_change_offset("dstrong2", 43, 47, true);//
sprite_change_offset("nspecial", 32, 47, true);//
sprite_change_offset("fspecial", 38, 51, true);//
sprite_change_offset("fspecial2", 28, 47, true);//
sprite_change_offset("uspecial", 28, 63, true);//
sprite_change_offset("uspecial2", 28, 49, true);//
sprite_change_offset("dspecial", 28, 47, true);//
sprite_change_offset("dspecial_var", 28, 47, true);//
sprite_change_offset("dspecial2", 28, 47, true);//
sprite_change_offset("taunt", 28, 47, true);//
sprite_change_offset("phone_open", 28, 47, true);//
sprite_change_offset("pot_activation", 28, 47, true);//

sprite_change_offset("spice_vfx", 70, 60);//
sprite_change_offset("corn1", 15, 8);//
sprite_change_offset("corn2", 15, 8);//
sprite_change_offset("spice", 10, 13);//
sprite_change_offset("lemon", 13, 8);//
sprite_change_offset("we_do_a_little_trolling", 45, 45);//
sprite_change_offset("we_do_a_little_trolling_3", 70, 60);//
sprite_change_offset("uspecial_fx", 140, 140);//
sprite_change_offset("bubble", 9, 9);//
sprite_change_offset("bubblepop", 21, 21);//
sprite_change_offset("shrimp_proj", 24, 24);//
sprite_change_offset("shrimphurt", 28, 28);//
sprite_change_offset("shrim_jumpscare", 30, 34);//
sprite_change_offset("dspec_afterimage", 70, 50);//
sprite_change_offset("steam_vfx", 40, 40);//
sprite_change_offset("eat_up", 88, 28);//
sprite_change_offset("uspec_afterimage", 56, 98);//
sprite_change_offset("dtilt_hfx", 68, 70);//

sprite_change_offset("pot_idle", 30, 48);//
sprite_change_offset("pot_hurtbox", 40, 86);//
sprite_change_offset("pot_expandanim", 52, 90);//
sprite_change_offset("pot_idle_maxspice", 30, 58);//
sprite_change_offset("pot_expandanim_shrimp", 52, 110);//
sprite_change_offset("pot_shrimp_stir", 30, 98);//
sprite_change_offset("pot_burst", 52, 150);//
sprite_change_offset("article_offscreen", 33, 33);

sprite_change_offset("nspecial_summer", 32, 47, true);//
sprite_change_offset("pot_idle_summer", 30, 68);//
sprite_change_offset("pot_idle_maxspice_summer", 30, 68);//
sprite_change_offset("pot_expandanim_summer", 52, 90);//
sprite_change_offset("pot_expandanim_shrimp_summer", 52, 110);//
sprite_change_offset("pot_burst_summer", 52, 150);//

sprite_change_offset("plat", 48, 47);//