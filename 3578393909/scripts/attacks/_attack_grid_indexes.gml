...
//  - win_num = the value of the window you are on, make sure to increase it's value every new window
//  - hit_num = same thing as win_num but for hitboxes
...
/*
    ...
    //  fun fact, with how i did the windows, you can also put this at the bottom of the window with "win_num" inside and the game will know
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
    win_num = 1;   //put this at the start of window 1
    win_num ++;    //copy this line above every window that isn't window 1

    set_window_value(atk, win_num, AG_WINDOW_TYPE, [0 | 7 | 8 | 9 | 10]);
    //  0 = normal
    ...
    set_window_value(atk, win_num, AG_WINDOW_LENGTH, [number]);
    //  how long the window is in frames

    set_window_value(atk, win_num, AG_WINDOW_ANIM_FRAMES, [number]);
    //  how many sprites from the strip the window uses

    set_window_value(atk, win_num, AG_WINDOW_ANIM_FRAME_START, [number]);
    //  what's the first sprites from the strip the window uses

    set_window_value(atk, win_num, AG_WINDOW_HSPEED, [number]);
    //  horizontal speed to apply in this window (positive values make the character go forward)

    set_window_value(atk, win_num, AG_WINDOW_HSPEED_TYPE, [0 | 1 | 2]);
    ...
    set_window_value(atk, win_num, AG_WINDOW_HAS_WHIFFLAG, [0 | 1]);
    //  a value of 1 will make the window 1.5x longer if you haven’t hit someone

    set_window_value(atk, win_num, AG_WINDOW_INVINCIBILITY, [0 | 1 | 2]);
    //  0 = no invincibility
    //  1 = Invincible to all attacks
    //  2 = Invincible to projectiles (HG_HITBOX_TYPE = 2)

    set_window_value(atk, win_num, AG_WINDOW_HITPAUSE_FRAME, [number]);
    //  0 = no specific frame
    //  +1 = displays the sprite value written when you hit while in this window

    set_window_value(atk, win_num, AG_WINDOW_CANCEL_TYPE, [0 | 1 | 2]);
    //  0 = normal
    ...
    set_window_value(atk, win_num, AG_WINDOW_CANCEL_FRAME, [number]);
    //  if AG_WINDOW_CANCEL_TYPE is above 0, the attack becomes cancellable on the frame written

    set_window_value(atk, win_num, AG_WINDOW_HAS_SFX, [0 | 1]);
    //  a value of 1 will allows sounds to be played on this window

    set_window_value(atk, win_num, AG_WINDOW_SFX, [sound_get("sound_name") | asset_get("sound_name")]);
    ...
    set_window_value(atk, win_num, AG_WINDOW_GOTO, [number]);
...
/*
    ...
    hit_num = 1;   //put this at the start of hitbox 1
    hit_num ++;    //copy this line above every window that isn't hitbox 1

    ///////////////////////////////////////////////////////// HITBOX TYPE 1 = MELEE /////////////////////////////////////////////////////////
    //  these indexes are used to make pretty much any hitbox you can thing of
    {
        set_hitbox_value(atk, hit_num, HG_PARENT_HITBOX, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_HITBOX_TYPE, [1, 2]);
        //  1 = melee hitbox - goes along with the player
        //  2 = projectile - doesn't move along with the player

        set_hitbox_value(atk, hit_num, HG_WINDOW, [number]);
        //  the window this hitbox should appear on

        set_hitbox_value(atk, hit_num, HG_WINDOW_CREATION_FRAME, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_LIFETIME, [number]);
        //  the amount of time the hitbox stays on

        set_hitbox_value(atk, hit_num, HG_HITBOX_X, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_HITBOX_Y, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_WIDTH, [number]);
        //  the hitbox's width

        set_hitbox_value(atk, hit_num, HG_HEIGHT, [number]);
        //  the hitbox's height

        set_hitbox_value(atk, hit_num, HG_SHAPE, [number]);
        //  0 = circle
        //  1 = rectangle
        //  2 = rectangle with rounded edges

        set_hitbox_value(atk, hit_num, HG_PRIORITY, [1-10]);
        //  when 2 hitboxes from this attack hit at the same frame, this tells the game which hitbox should register the hit

        set_hitbox_value(atk, hit_num, HG_DAMAGE, [number]);
        //  hitbox's damage

        set_hitbox_value(atk, hit_num, HG_ANGLE, [0-361]);
        //  0 = forward
        //  90 = up
        //  180 = back
        //  270 = funny spike
        ...
        set_hitbox_value(atk, hit_num, HG_BASE_KNOCKBACK, [number]);
        //  the minimal amount of knockback the attack does

        set_hitbox_value(atk, hit_num, HG_KNOCKBACK_SCALING, [number]);
        //  the amount of knockback to increase it by according to the target's %

        set_hitbox_value(atk, hit_num, HG_EFFECT, [number]);
        //  0 = normal
        ...
        set_hitbox_value(atk, hit_num, HG_BASE_HITPAUSE, [number]);
        //  the minimal amount of hitpause the attack does

        set_hitbox_value(atk, hit_num, HG_HITPAUSE_SCALING, [number]);
        //  the amount of hitpause to increase it by according to the target's %

        set_hitbox_value(atk, hit_num, HG_VISUAL_EFFECT, [number | string]);
        ...
        set_hitbox_value(atk, hit_num, HG_VISUAL_EFFECT_X_OFFSET, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_VISUAL_EFFECT_Y_OFFSET, [number]);
        //  offset the effect on the vertical axis
        //  a negative value brings it upward
        //  a value of 0 does the hit effect at the exact point where it hit

        set_hitbox_value(atk, hit_num, HG_HIT_PARTICLE_NUM, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_HIT_SFX, [sound_get("sound_name") | asset_get("sound_name")]);
        //  the sound to play when you hit with this hitbox
        //  sound_get("sound_name") is for sounds in your sounds folder
        //  asset_get("sound_name") is for sounds in the game engine (list of in-game sounds: https://www.rivalsofaether.com/workshop/sfx-list/)

        set_hitbox_value(atk, hit_num, HG_ANGLE_FLIPPER, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_EXTENDED_PARRY_STUN, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_HITBOX_GROUP, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_FINAL_BASE_KNOCKBACK, [number]);
        ...
        set_hitbox_value(atk, hit_num, HG_THROWS_ROCK, [number]);
        //  0 = breaks kragg's rock
        //  1 = knocks kragg's rock away
        //  2 = ignores kragg's rock

        set_hitbox_value(atk, hit_num, HG_VISUAL_EFFECT_DISTANCE, [number]);
    ...
    {
        set_hitbox_value(atk, hit_num, HG_PROJECTILE_SPRITE, sprite_get("sprite_name"));
        ...
        set_hitbox_value(atk, hit_num, HG_PROJECTILE_MASK, [sprite_get("sprite_name") | -1]);
        ...
        set_hitbox_value(atk, hit_num, HG_PROJECTILE_ENEMY_BEHAVIOR, [0 | 1]);
        //  0 = stops projectile when it hits an enemy
        //  1 = goes through enemies
        //  lol i wish it could bounce off enemies

        set_hitbox_value(atk, hit_num, HG_PROJECTILE_UNBASHABLE, [0 | 1]);
        //  a value of 1 makes it so ori can't grab the projectile

        set_hitbox_value(atk, hit_num, HG_PROJECTILE_PARRY_STUN, [0 | 1]);
        //  a value of 1 makes the player get parry stunned if the enemy parries the projectile

        set_hitbox_value(atk, hit_num, HG_PROJECTILE_DOES_NOT_REFLECT, [0 | 1]);
        //  a value of 1 makes the projectile not change directions while parried

        set_hitbox_value(atk, hit_num, HG_PROJECTILE_IS_TRANSCENDENT, [0 | 1]);
        //  a value of 1 makes the projectile unbreakable by hitboxes

        set_hitbox_value(atk, hit_num, HG_PROJECTILE_DESTROY_EFFECT, [number | string]);
        //  the visual effect to use when the projectile is destroyed
        //  you can either type in a number from this page https://www.rivalsofaether.com/workshop/visual-effects-list/ for base game hit effects
        //  or you can put in your hit effect if you set it up on init.gml

        set_hitbox_value(atk, hit_num, HG_PROJECTILE_PLASMA_SAFE, [0 | 1]);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
    set_window_value(atk, win_num, AG_WINDOW_LOOP_TIMES, #loops)
    ...
    set_hitbox_value(atk, hit_num, HG_PROJECTILE_MULTIHIT, [number]);
    //  sets the amount of times a projectile would hit

    set_hitbox_value(atk, hit_num, HG_PROJECTILE_HITRATE, [number]);
    //  similarly to HG_HIT_LOCKOUT, it will tell the projectile how frequently it should hit

    set_hitbox_value(atk, hit_num, HG_MULTIHIT_DAMAGE, [number]);
    ...
    set_hitbox_value(atk, hit_num, HG_MULTIHIT_VFX, [number]);
    ...
    set_hitbox_value(atk, hit_num, HG_MULTIHIT_SFX, [number]);
