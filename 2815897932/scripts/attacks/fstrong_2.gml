set_attack_value(AT_FSTRONG_2, AG_CATEGORY, 0);
set_attack_value(AT_FSTRONG_2, AG_SPRITE, sprite_get("fstrong"));
set_attack_value(AT_FSTRONG_2, AG_NUM_WINDOWS, 5);
set_attack_value(AT_FSTRONG_2, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_FSTRONG_2, AG_OFF_LEDGE, 0);
set_attack_value(AT_FSTRONG_2, AG_STRONG_CHARGE_WINDOW, 1);
set_attack_value(AT_FSTRONG_2, AG_HURTBOX_SPRITE, sprite_get("fstrong_hurt"));

set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_LENGTH, 9);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_CUSTOM_GROUND_FRICTION, 1);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_HSPEED, -3);
set_window_value(AT_FSTRONG_2, 1, AG_WINDOW_HSPEED_TYPE, 2);

set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_LENGTH, 10);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_ANIM_FRAME_START, 16);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy2"));
set_window_value(AT_FSTRONG_2, 2, AG_WINDOW_SFX_FRAME, 9);

set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_TYPE, 2);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_LENGTH, 9);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_ANIM_FRAME_START, 19);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_HITPAUSE_FRAME, 0);
set_window_value(AT_FSTRONG_2, 3, 33, 0);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_HSPEED, 18);
set_window_value(AT_FSTRONG_2, 3, AG_WINDOW_HSPEED_TYPE, 2);

set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_TYPE, 2);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_LENGTH, 13);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_ANIM_FRAME_START, 22);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_FSTRONG_2, 4, 33, 0);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_CUSTOM_GROUND_FRICTION, 1.33);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_SFX, sound_get("sfx_bounce_light"));
set_window_value(AT_FSTRONG_2, 4, AG_WINDOW_SFX_FRAME, 0);

set_window_value(AT_FSTRONG_2, 5, AG_WINDOW_TYPE, 2);
set_window_value(AT_FSTRONG_2, 5, AG_WINDOW_LENGTH, 15);
set_window_value(AT_FSTRONG_2, 5, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_FSTRONG_2, 5, AG_WINDOW_ANIM_FRAME_START, 23);
set_window_value(AT_FSTRONG_2, 5, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_FSTRONG_2, 5, 33, 0);

set_num_hitboxes(AT_FSTRONG_2, 3);
set_hitbox_value(AT_FSTRONG_2, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_WINDOW, 3);
set_hitbox_value(AT_FSTRONG_2, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FSTRONG_2, 1, HG_GROUNDEDNESS, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITBOX_X, 10);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITBOX_Y, -52);
set_hitbox_value(AT_FSTRONG_2, 1, HG_WIDTH, 60);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HEIGHT, 104);
set_hitbox_value(AT_FSTRONG_2, 1, HG_SHAPE, 1);
set_hitbox_value(AT_FSTRONG_2, 1, HG_PRIORITY, 5);
set_hitbox_value(AT_FSTRONG_2, 1, HG_DAMAGE, 12);
set_hitbox_value(AT_FSTRONG_2, 1, HG_ANGLE, 38);
set_hitbox_value(AT_FSTRONG_2, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FSTRONG_2, 1, HG_BASE_KNOCKBACK, 6.7);
set_hitbox_value(AT_FSTRONG_2, 1, HG_KNOCKBACK_SCALING, .875);
set_hitbox_value(AT_FSTRONG_2, 1, HG_BASE_HITPAUSE, 12);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HITPAUSE_SCALING, 1.33);
set_hitbox_value(AT_FSTRONG_2, 1, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_FSTRONG_2, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_FSTRONG_2, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_FSTRONG_2, 1, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));
set_hitbox_value(AT_FSTRONG_2, 1, HG_IGNORES_PROJECTILES, 0);

set_hitbox_value(AT_FSTRONG_2, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_FSTRONG_2, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_FSTRONG_2, 2, HG_WINDOW, 3);
set_hitbox_value(AT_FSTRONG_2, 2, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_FSTRONG_2, 2, HG_GROUNDEDNESS, 1);
set_hitbox_value(AT_FSTRONG_2, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_FSTRONG_2, 2, HG_HITBOX_X, 24);
set_hitbox_value(AT_FSTRONG_2, 2, HG_HITBOX_Y, -28);
set_hitbox_value(AT_FSTRONG_2, 2, HG_WIDTH, 96);
set_hitbox_value(AT_FSTRONG_2, 2, HG_HEIGHT, 56);
set_hitbox_value(AT_FSTRONG_2, 2, HG_SHAPE, 1);
set_hitbox_value(AT_FSTRONG_2, 2, HG_PRIORITY, 5);
set_hitbox_value(AT_FSTRONG_2, 2, HG_DAMAGE, 12);
set_hitbox_value(AT_FSTRONG_2, 2, HG_ANGLE, 38);
set_hitbox_value(AT_FSTRONG_2, 2, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FSTRONG_2, 2, HG_BASE_KNOCKBACK, 6.7);
set_hitbox_value(AT_FSTRONG_2, 2, HG_KNOCKBACK_SCALING, .875);
set_hitbox_value(AT_FSTRONG_2, 2, HG_BASE_HITPAUSE, 12);
set_hitbox_value(AT_FSTRONG_2, 2, HG_HITPAUSE_SCALING, 1.33);
set_hitbox_value(AT_FSTRONG_2, 2, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_FSTRONG_2, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_FSTRONG_2, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_FSTRONG_2, 2, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));
set_hitbox_value(AT_FSTRONG_2, 2, HG_IGNORES_PROJECTILES, 0);