...
with phone{
	...
	initTip("NSpecial: Burning Chakram");
	initTipWords("This projectile flies in the air before falling to the abyss. Has burning properties.");
	...
	initTip("Fspecial: Scoville Tornado");
	initTipWords("After a small startup, this whirlwind attack burns the opponent with multiple hits. Travels horizontally.");
	initTipImage(player_id.spr_fspecial, -5, fa_center, 1, c_white, 0);
	
	initTip("USpecial: Ghost Eject");
	initTipWords("Scoville goes into ghost form, shedding his skin to recover.");
	initTipImage(player_id.spr_uspecial, -5, fa_center, 1, c_white, 0);
	
	initTip("Dspecial: Scoville's Malice");
	initTipWords("A slow but deadly attack that does even more damage when the enemy is on fire.");
	initTipImage(player_id.spr_dspecial, -5, fa_center, 1, c_white, 0);
	
	initTip("Fair: Scoville Backhand");
	initTipWords_ext("This far reaching attack is great for edge guarding.", fa_left, c_red, 0, 0);
	initTipImage(other.spr_fair, -5, fa_center, 1, c_white, 0);
	
	initTip("Nair: Scoville Boot");
	initTipWords_ext("This attack can be jump canceled on hit to perform different air attacks.", fa_left, c_red, 0, 0);
	initTipImage(player_id.spr_nair, -5, fa_center, 1, c_white, 0);
	
	initTip("Nair: Scoville Spike");
	initTipWords_ext("This move has a sweetspot that spikes. Use this in conjuction with nair!.", fa_left, c_red, 0, 0);
	initTipImage(player_id.spr_dair, -5, fa_center, 1, c_white, 0);
	
	initTip("Fstrong: Scoville's Surprise");
	initTipWords("This attack knocks backwards after a short announcement from Scoville.");
...
with phone{
	...
	initPatch("1.0", "6 April, 2021");
	initPatchWords_ext("HOLY CRAP THE CHARACTER IS OUT!", fa_center, c_white, 0, 1);
	initPatchImage(other.spr_nspecial, 0, fa_center, 1, c_white, 1);
