...
if (my_hitboxID.attack == AT_FSPECIAL) 
{
    ...
    && hit_player_obj.clone == false) 
    {
        ...
        hit_player_obj.x = floor(lerp(hit_player_obj.x, x+(200*spr_dir), .5));
        hit_player_obj.y = floor(lerp(hit_player_obj.y, y, .5));
        hit_player_obj.spr_dir = -spr_dir;
    }
}
...
if (my_hitboxID.attack == AT_FTHROW) 
{
    ...
    {
        case 1:
            hit_player_obj.y = floor(lerp(hit_player_obj.y, y-120, .5));
        break;
        case 4:
            sound_play(asset_get("sfx_ell_dspecial_explosion_3"), 0, noone, 1, 1.5)

            sound_play(asset_get("sfx_icehit_heavy1"), 0, noone, 1, 1.5)
        break;
        case 2:
            hit_player_obj.x = floor(lerp(hit_player_obj.x, x+(30*spr_dir), .5));
            hit_player_obj.y = floor(lerp(hit_player_obj.y, y-20, .5));
            sound_play(asset_get("sfx_ell_dspecial_explosion_1"), 0, noone, 1, 1.5)
            sound_play(asset_get("sfx_icehit_heavy1"), 0, noone, 1, 1.5)
        break;
        case 3:
            hit_player_obj.x = floor(lerp(hit_player_obj.x, x+(20*spr_dir), .5));
            hit_player_obj.y = floor(lerp(hit_player_obj.y, y, .5));
        break;
    }
}
...
switch (atk) {
	...
	case AT_USPECIAL:
        if(hbox == 1 || hbox == 2 || hbox == 3 ){
		    sound_play(asset_get("sfx_blow_heavy1"))}
		if hbox == 3 {
            sound_play(asset_get("sfx_oly_uspecial_kick"), 0, noone, .7, 1)
        }
	break;
	case AT_FSTRONG:
		    sound_play(asset_get("sfx_blow_heavy1"))
