...
with phone{
	...
	initTip("NSpecial: What to choose...");
	initTipWords("Henry's neutral special gives him three different options on what to do, and a different input will select one.");
	if ("spr_nspecial_proj" in player_id) initTipImage_ext(player_id.spr_nspecial_proj, 0, fa_center, 1, c_white, 0, 120, 120, 120, -40);
	initTipImage_ext(player_id.spr_nspecial, 3, fa_center, 1, c_white, 0, 24, 40, 70, 0);
	initTipWords("Laser, which uses the attack button to select, will make him fire a short thin laser that will deal a decent amount of damage while only dealing knockback if the target is already in hitstun.");
	initTipImage_ext(player_id.spr_nspecial, 8, fa_center, 1, c_white, 0, 24, 40, 70, 0);
	initTipWords("TNT, which uses the special button to select, will make him throw a pack of dynamite that can bounce off anything solid like walls and ground. The TNT will blow up anyone it touches, including Henry.");
	initTipImage_ext(player_id.spr_nspecial, 18, fa_center, 1, c_white, 0, 24, 40, 70, 0);
	initTipWords("Teleporter, which uses the taunt button to select, will make him pull out the teleporter and press a button on it. The catch, however, is that unless the diamond is out, the teleporter has no set destination, so the outcome will be completely random.");
	initTipImage_ext(player_id.spr_nspecial, 29, fa_center, 1, c_white, 0, 24, 40, 70, 0);
	
	initTip("FSpecial's Offensive Options");
	initTipWords("Henry's forward special uses his scooter, which depending on his input can do one of two things to attack. Pressing attack (or running into a wall or ledge) will make him do a wheelie and slow down, and pressing special will make him rush forward while a drill springs out from the front of his scooter. On the ground, Henry can press shield to get off the scooter, and in the air, he can press shield to cancel into an airdodge or press jump to cancel into a double jump or a wall jump.");
	initTipImage(player_id.spr_fspecial, -5, fa_center, 1, c_white, 3);
	
	initTip("The Tunisian Diamond");
	initTipWords("Henry's down special makes him drop none other than the diamond he stole from a museum in Stealing the Diamond. While onstage, he can throw the diamond around with various attacks and hit others with the diamond. However, the diamond has a set number of hits it can give or take before it goes away. A hit point will be added when either Henry hits it with an attack that does 8 bkb or above or when it hits somebody else.");
	if ("spr_nspecial_proj" in player_id) initTipImage_ext(player_id.spr_example, 0, fa_center, 1, c_white, 0, 48, 20, 70, 0);
	
	initTip("Double Edged Jab");
	initTipWords("Henry's jab can hit on both sides of himself, which makes it a good option to catch opponents rushing him from behind off guard!");
	initTipImage(player_id.spr_jab, -5, fa_center, 1, c_white, 0);
	
	initTip("Stickmin Punch!");
	initTipWords("Forward strong makes Henry do the Stickmin Punch: The strongest melee attack in his kit. With a ton of startup to it, it's hard to land right, but its chargeable feature should make it that much easier to time right should you be too early, and perhaps even reward you for your patience since it's stronger with charge.");
	initTipImage(player_id.spr_fstrong, -6, fa_center, 1, c_white, 0);
	
	initTip("Playstyle Synopsis");
	initTipWords("Henry Stickmin isn't too unusual of a fighter, being a more rushdown focused character with lots of moves that combo into one another and some that act as perfect finishers for said combos. He does have some dynamic and versitile tools like the diamond and his TNT, and mix up options like his teleporter and scooter.");
	
	initTip("Henry's Origins");
	initTipWords("Henry Stickmin started out as a nameless character that starred in one of Newgrounds user Puffballs United's earliest games: Breaking the Bank. It was a short game that gave a little stickman multiple options to break into a bank, but little did Puff know that it'd become so much more than that.
	
	After Breaking the Bank gained so much traction, Puffballs made a sequel called Escaping the Prison, which was a much longer and more engaging point and click adventure where Henry could break out of prison in a few different ways while trying to do such in several, many of which methods resulted in him failing miserably. Over time, Puff made more and more of these games, and in summer of 2020 even released a full remaster of all of them alongside the grand finale to the series: Completing the Mission.
	
	The story of Henry Stickmin himself is hard to explain in full without making a whole essay, but to sum things up: He started as a thieving criminal trying to make a quick buck, and depending on where he goes after being taken in by the government to perform a special task for them, he could either remain a rogue thief, become an ally with the military, or even become the leader of a criminal organization. In the end, it's up to you to decide the fate of Henry Stickmin.");
...
if pho_has_steve_dmsg{
	...
	steve_death_message = "Steve failed the mission";
