sprite_change_offset("idle", 20, 64);
sprite_change_offset("hurt", 21, 54);
sprite_change_offset("spinhurt", 21, 54);
sprite_change_offset("uphurt", 21, 54);
sprite_change_offset("crouch", 22, 78);
sprite_change_offset("Koffing_hurt", 22, 60);
sprite_change_offset("Koffing_hurt_air", 22, 56);

//Get updated
sprite_change_offset("walk", 32, 62);
sprite_change_offset("walkturn", 32, 62);
sprite_change_offset("dash", 32, 62);
sprite_change_offset("dashstart", 32, 62);
sprite_change_offset("dashstop", 32, 62);
sprite_change_offset("dashturn", 32, 62);

sprite_change_offset("tech", 21, 78);
sprite_change_offset("jumpstart", 21, 78);
sprite_change_offset("jump", 22, 78);
sprite_change_offset("land", 21, 78);
sprite_change_offset("landinglag", 21, 78);
sprite_change_offset("doublejump", 22, 78);
sprite_change_offset("pratfall", 22, 74);
sprite_change_offset("airdodge", 22, 78);
sprite_change_offset("walljump", 30, 78);
sprite_change_offset("waveland", 36, 78);
sprite_change_offset("roll_forward", 28, 82);
sprite_change_offset("roll_backward", 28, 82);
sprite_change_offset("parry", 31, 68);

sprite_change_offset("nspecial_smoke", 30, 30);
sprite_change_offset("nspecial_smoke2", 30, 30);
sprite_change_offset("nspecial_smoke_detonate", 30, 30);

sprite_change_offset("ustrong", 22, 150);
sprite_change_offset("uspecial", 30, 64);
sprite_change_offset("dspecial", 28, 64);
sprite_change_offset("fspecial", 40, 92);
sprite_change_offset("nspecial", 30, 67);
sprite_change_offset("dtilt", 24, 78);
sprite_change_offset("fair", 30,62);
sprite_change_offset("fstrong", 30, 64);
sprite_change_offset("taunt", 22, 62);
sprite_change_offset("taunt_2", 22, 62);
sprite_change_offset("nair", 46, 76);
sprite_change_offset("nair_proj", 10, 24);
sprite_change_offset("dstrong", 70, 78);
sprite_change_offset("dstrong_air", 70, 78);
sprite_change_offset("jab", 22, 60);
sprite_change_offset("dattack", 29, 62);
sprite_change_offset("dair", 45, 84);
sprite_change_offset("bair", 90, 80);

sprite_change_offset("ftilt", 22, 68);
sprite_change_offset("utilt", 40, 107);
sprite_change_offset("uair", 40, 78);

sprite_change_offset("intro", 43, 538);
sprite_change_offset("intro2", 42, 62);
sprite_change_offset("intro_proj", 22, 27);
sprite_change_offset("plat", 48, 8);
sprite_change_offset("Fcancel", 12, 11);
sprite_change_offset("owner", 7, 14);	

sprite_change_offset("nspecial_proj", 64, 94);

sprite_change_offset("Udir", 0, 35);
sprite_change_offset("boost", 22, 55);
sprite_change_offset("Dpad", 15, 15);	
sprite_change_offset("notice", 27, 22);