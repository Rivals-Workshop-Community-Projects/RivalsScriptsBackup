...
if(get_local_setting(SET_FX_QUALITY) == 3){
	with(pHitBox){
		if(player == other.player and attack == AT_DSPECIAL){
			...
			draw_vertex_color(x-lengthdir_x(6, proj_angle), y-lengthdir_y(6, proj_angle), c_white, 1);
			for(var i = 8; i >= 1; i--){
				var pos = (queue_pos+i)%9;
				var t = trail_array[pos];
				var len = width/2 - ((10-i)/9*width/2);
				if(t.x != trail_array[(pos+1)%9].x or t.y != trail_array[(pos+1)%9].y){
					dir = point_direction(t.x, t.y,trail_array[(pos+1)%9].x, trail_array[(pos+1)%9].y)
				}
				var xx = lengthdir_x(len, dir+90);
				var yy = lengthdir_y(len, dir+90);
			
				draw_vertex_color(t.x+xx, t.y+yy, c_blue, (i)/8);
				draw_vertex_color(t.x-xx, t.y-yy, c_blue, (i)/8);
			}
			draw_primitive_end();
