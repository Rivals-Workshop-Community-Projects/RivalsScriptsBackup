...
with phone{
	...
	initTip("Nair: main combo starter and extender ");
	initTipWords("The nair comes from the level 'The Whirlygig' in the original game. In the original game, it is an obstacle at the very end of the level, that requires a decent amount of skill and luck to get past. In rivals, it's an amazing combo move, it can be used to start and extend combos. Use this a lot!");
	initTipImage(player_id.spr_nair, -5, fa_center, 1, c_white, 0);
	
	initTip("FSpecial use");
	initTipWords("In the original game, this was one of the player's most useful movement options. Here in rivals, it can be used to lead into a combo finisher. Good to use when someone is *just* out of your reach to finish a combo. It can also be used as an approach tool!");
	initTipImage(player_id.spr_fspecial, -5, fa_center, 1, c_white, 3);

	initTip("Jab");
	initTipWords_ext("The jab is based off of the grab from the original game! It was used to pick things up, or to be the most annoying person on this earth and grab other players. Here in rivals, it functions just as it does in the original game, except you can't move while using it. Use this to catch people off guard and maybe even start some simple combos!", fa_left, c_white, 0, 0);
	initTipImage(other.spr_jab, -4, fa_center, 1, c_white, 0);
	
	initTip("DSpecial uses");
	initTipWords("in the original game, this was one of the most used obstacles. It's part of a variation for almost every level! In rivals, you can place it down anywhere, and it'll stay out for a good amount of time. In this time, it'll deal knockback and damage to opponents. It is an amazing stage control option, and it has so many uses! An example is putting it at the ledge for a cheesy edgeguard. Consider it as a hazard you can put anywhere!");
	    initTipImage(player_id.spr_dspecial_proj, -5, fa_center, 1, c_white, 0);
	
	initTip("Uair: the kill move ");
	initTipWords_ext("Just like the nair, this also originated in the level called 'The WhirlyGig'. Uair is an extremely good kill move, and you can even chain it into more up airs for good damage!", fa_center, c_white, 0, 0);
    initTipImage(player_id.spr_uair, -5, fa_center, 1, c_white, 0);


	
	initTip("End");
	initTipWords("As for everything else, his smash attacks have good range and all of tilts are good combo moves. You'll learn the rest as you go :)");
...
with phone{
	...
	initPatch("1.0", "6 August, 2021");
	initPatchWords("Buffs
	- a");
	initPatchWords_ext("This buff released the character to rivals workshop!", fa_left, c_gray, 1, 0);
	initPatchWords("woo");
...
with phone{
	...
	initAbout("About Fall Guy", " Owned by Mediatonic and Epic Games");
