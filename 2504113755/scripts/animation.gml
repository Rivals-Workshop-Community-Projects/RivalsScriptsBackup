...
if (state == PS_WALK_TURN) {
    	 ...
    	 if (walking_backward_counter*spr_dir < 0) {			// if sign(walking_backward_counter) != spr_dir{
        set_state(PS_WALK);
        
       if (has_skin()) {
        sprite_index=sprite_get("two_walk"); }
        else {         sprite_index=sprite_get("walk"); }

        

        image_index= abs(walking_backward_counter)*walk_anim_speed
    	spr_dir = sign(walking_backward_counter)
