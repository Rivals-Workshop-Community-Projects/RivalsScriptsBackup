...
with (asset_get("oPlayer")) if (string_count("sonic", string_lower(get_char_info(player, INFO_STR_NAME))) > 0) && !(string_count("metal sonic", string_lower(get_char_info(player, INFO_STR_NAME))) > 0) && !(string_count("mecha sonic", string_lower(get_char_info(player, INFO_STR_NAME))) > 0) {
    with (other) {
        instance_destroy()
    }
}
