...
//textDraw(x + 220, y + 210, "fName", c_white, 100, 1000, fa_right, 1, false, 0.25, "ver. " + string(get_char_info(player, INFO_VER_MAJOR)) + "." + string(get_char_info(player, INFO_VER_MINOR)));
...
for (i = 0; i < len; i++){
	if displayed[i]{
		if long draw_sprite(sprite_get("_pho_compatibility_badges"), i, x + 192, y + 34 + 18 * (iters % max_col));
		else draw_sprite(sprite_get("_pho_compatibility_badges"), i, x + 192, y + 34 + 18 * (iters % max_col));
		iters++;
	}
}
...
for(i = 0; i < 32; i++){
    ...
    var draw_x = x + 4 * i;
    rectDraw(draw_x + 78, y + 10, 1, 3, draw_color);
}
