	...
	if (heal_player != noone) && heal_player.vsp > 0 {
		...
		if  heal_player.url == player_id.url {
		    if sjump < 5 {
				...
				spawn_hit_fx( x  , y , 109 )
	            player_id.scuttle = -80
	            heal_player.vsp = -8
	            destroyed = true
	            heal_player.mariospeed += 15 + 9*sjump;
   ...
   if hitbox_timer == 299 {
	...
	with oPlayer{
		...
		if (heal_player != noone) {
			if(state == PS_DEAD or state == PS_RESPAWN or invincible or initial_invince) continue;
