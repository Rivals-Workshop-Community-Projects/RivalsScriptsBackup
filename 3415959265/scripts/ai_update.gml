...
if (ai_target.state == PS_RESPAWN){
    // Prevent all actions during respawn
    clear_ai_inputs();
    hold_neutral();
    return; // Exit further processing
}

// Updated ledge logic to account for target's distance offstage
if (ai_target_offstage) {
    var safe_chase_distance = 300; // Maximum horizontal distance AI will chase offstage
    var safe_target_depth = stagey + 100; // Maximum vertical depth for the target before staying at ledge
    var ledge_x = ai_target.x > stage_center ? stage_right - 20 : stage_left + 20; // Approximate ledge position
    var ledge_guard_distance = 50; // Distance from ledge for guarding

    // Check if target is too low offstage
    if (ai_target.y > safe_target_depth) {
        if (abs(x - ledge_x) > ledge_guard_distance) {
            hold_toward_direction(sign(ledge_x - x)); // Move toward ledge
        } else {
            hold_neutral(); // Stay at ledge
        }
        return; // Exit further logic
    }

    // Normal offstage chase logic
    if (abs(ai_target.x - x) > safe_chase_distance) {
        if (abs(x - ledge_x) > ledge_guard_distance) {
            hold_toward_direction(sign(ledge_x - x)); // Move toward ledge
        } else {
            hold_neutral(); // Stay at ledge
...
if(state_cat != SC_HITSTUN)
{
    if (strategy == STRAT_OFFENSE)
    {
        ...
        {
            if (!ai_recovering) // Enter recovery mode if not already recovering
            {
                clear_ai_inputs(); // Clear any offensive inputs once
                ai_recovering = true; // Set recovery flag
            }
    
            try_recover(); // Execute recovery logic
            return; // Skip further offensive logic
        }     
        // NSPECIAL Article Check
        if (ds_list_size(article_list) > 0)
        {
            for (var i = 0; i < ds_list_size(article_list); i++)
            {
                ...
                {
                    var saya_on_article = abs(article.x - x) < 30 && abs(article.y - y) < 30;
                    var target_on_article = abs(article.x - ai_target.x) < 30 && abs(article.y - ai_target.y) < 30;
    
                    if (saya_on_article && target_on_article)
                    {
                        clear_ai_inputs();
        ...
        // Sparking/Guardian Prediction Logic for Strong Attacks and USPECIAL
        if (is_sparking || is_guardian)
        {
            var prediction_frames = (point_distance(x, y, ai_target.x, ai_target.y) > 200) ? 20 : 10;
            var current_distance = point_distance(x, y, ai_target.x, ai_target.y);
            var predicted_target_pos = newPredict(ai_target, prediction_frames, ai_target.hsp, ai_target.vsp, noone, true, false);
            var predicted_x = predicted_target_pos[@0];
            var predicted_y = predicted_target_pos[@1];
            var predicted_distance = point_distance(x, y, predicted_x, predicted_y);
    
            // Check if the target is approaching
            var is_approaching = predicted_distance < current_distance;
    
            // USPECIAL range thresholds
            var uspecial_vertical_range = 100; // Maximum vertical range
            var uspecial_horizontal_range = 60; // Maximum horizontal range
            var uspecial_chance = 30; // Percentage chance to use USPECIAL
    
            // Check if target will be in range for USPECIAL
            var target_in_uspecial_range = abs(predicted_x - x) <= uspecial_horizontal_range &&
                                           abs(predicted_y - y) <= uspecial_vertical_range;
    
            if (is_approaching && target_in_uspecial_range && random_func(0, 100, true) < uspecial_chance)
            {
                clear_ai_inputs();
                hold_towards_target();
                press_special(true, true); // Hold special to ensure grab
                perform_attack(AT_USPECIAL, true);
                return; // Exit after performing USPECIAL
            }
    
            // Strong attack range thresholds
            var fstrong_max_range = 70; // Maximum range for FSTRONG
            var ustrong_max_range = 50;  // Maximum horizontal range for USTRONG
            var article_proximity_check = false;
    
            if (is_approaching && predicted_distance <= fstrong_max_range)
            {
                // Prediction conditions
                var target_in_front = ((predicted_x > x && spr_dir == 1) || (predicted_x < x && spr_dir == -1)) && predicted_y >= y - 50; // In front and not below
                var target_directly_above = abs(predicted_x - x) < 30 && predicted_y < y - 50; // Directly above AI
    
                // Article proximity checks
                for (var i = 0; i < ds_list_size(article_list); i++)
                {
                    var article = article_list[| i];
                    if (instance_exists(article))
                    {
                        var target_near_article = abs(article.x - predicted_x) < 30 && abs(article.y - predicted_y) < 50;
                        var target_above_article = abs(article.x - predicted_x) < 30 && predicted_y < article.y - 50;
    
                        if (target_near_article)
                        {
                            chosenAttack = AT_FSTRONG;
                            article_proximity_check = true;
                            break;
                        }
                        if (target_above_article)
                        {
                            chosenAttack = AT_USTRONG;
                            article_proximity_check = true;
                            break;
                        }
                    }
                ...
                // Default strong attack prediction if no article condition met
                if (!article_proximity_check)
                {
                    if (target_in_front)
                    {
                        chosenAttack = AT_FSTRONG;
                    }
                    else if (target_directly_above)
                    {
                        chosenAttack = AT_USTRONG;
                    }
                }
    ...
    if (strategy == STRAT_SETUP)
    {
        // Ensure AI is far enough from the opponent before setting up
        if (dist < 200 || abs(ai_target.x - x) < 150) // Adjust safe distance as needed
        {
            hold_away_from_target();
            if (!free) press_jump(true); // Jump to create vertical space if grounded
        }
        else
        {
            // Attempt to place an article if not at max articles
            if (artc_count < max_articles && !free)
            {
                var spawn_x = x + 70 * spr_dir; // Intended spawn position
                var spawn_y = y + 16;
                var min_article_dist = 200; // Minimum distance between articles
    
                // Check for existing articles and ensure minimum distance
                var can_spawn = true;
                for (var i = 0; i < ds_list_size(article_list); i++)
                {
                    ...
                    {
                        var article_dist = point_distance(spawn_x, spawn_y, existing_article.x, existing_article.y);
                        if (article_dist < min_article_dist)
                        {
                            can_spawn = false;
                            break;
                        }
                ...
                // Retry placing the article on the next frame
                if (can_spawn && !attacking && !free)
                {
                    clear_ai_inputs();
                    perform_attack(AT_DSTRONG, true);
                }
            }
            else
            {
                set_strategy(STRAT_NEUTRAL); // Exit setup if max articles are reached
            }
        }
    }
...
if (ai_target == self)
{
    if (!free && !attacking)
    {
        if (offstage) // Check if offstage
        {
            print_debug("No target and offstage. Attempting to recover.");
            try_recover(); // Call the recovery logic
        }
        else
        {
            // Perform tilt dance if onstage and not attacking
...
if(strategy == STRAT_NEUTRAL){
    ...
    if (artc_count < max_articles && random_func(500, 1000, true) < 30 && !ai_recovering) // Chance to transition to setup
    {
        set_strategy(STRAT_SETUP);
    }	
}
...
if (strategy == STRAT_SETUP)
{
    if (artc_count >= max_articles || dist < 100 || strategy_timer > 200)
    {
        set_strategy(STRAT_NEUTRAL);
    }
}
...
if(!attacking){
	...
	if(recover_required){
		...
		if(vsp > 0  && !move_cooldown[AT_USPECIAL] && y>stagey+32 && closest_wall_xdist*0.3 < abs(y-stagey) && closest_wall_xdist < 230){
			perform_attack(AT_USPECIAL, true);
			hold_toward_center();
			// print("WOW33")
		}
...
if (attacking)
{
    switch (attack)
    {
        ...
        case AT_FSPECIAL: // Grounded FSPECIAL
        case AT_FSPECIAL_AIR: // Aerial FSPECIAL
            if (ai_recovering)
            {
                press_special(true, true); // Hold special input to ensure straight-line recovery
                hold_toward_center(); // Always hold toward the stage center for recovery
        
                // If wall jump is possible, attempt to jump as well
                if (window > 12 && can_wall_jump)
                {
                    press_jump(true);
                }
            }
            ...
            {
                // Follow-up logic on hit
                var follow_up_window = (attack == AT_FSPECIAL) ? 7 : 12; // Different windows for grounded and aerial
                if (window == follow_up_window && has_hit && !was_parried && targetdamage < 50) // Check for a successful hit
                {
                    hold_towards_target(); // Hold toward the opponent
        
                    // Randomly choose a follow-up move: FAIR or DAIR
                    var follow_up = random_func(0, 2, true); // Random number between 0 and 1
                    switch (follow_up)
                    {
                        case 0:
                            perform_attack(AT_FAIR, true);
                        break;
        
                        case 1:
                            perform_attack(AT_DAIR, true);
                        break;
                    }
                }
            }
        ...
        case AT_USPECIAL:
            ...
            else if ((is_guardian || is_sparking) && !was_parried)
            {
                    press_special(true, true); // Hold special input
        
                    // Check for existing articles and move towards one
                    if (ds_list_size(article_list) > 0)
                    {
                        // Get the x position of the first valid article
                        for (var i = 0; i < ds_list_size(article_list); i++)
                        {
                            var target_article = article_list[| i];
                            if (instance_exists(target_article))
                            {
                                // Move towards the article's x position
                                if (target_article.x > x)
                                {
                                    press_right();
                                }
                                else if (target_article.x < x)
                                {
                                    press_left();
                                }
        
                                // Hold downward to go through platforms
                                press_down();
        
                                // Stop checking after moving toward one article
                                break;
                            }
                        }
                    }
                    ...
                    {
                        // Default behavior if no articles exist
                        hold_towards_target(); // Maintain directional input towards the opponent
                    }
            }
        break;
        case AT_FSTRONG:
            ...
            {
                // Check for the closest article
                if (ds_list_size(article_list) > 0)
                {
                    var closest_article = noone;
                    ...
                    if (instance_exists(closest_article))
                    {
                        // Determine behavior based on article position
                        var article_x_dist = abs(closest_article.x - x);
                        var article_y_dist = abs(closest_article.y - y);
                        var article_distance = point_distance(x, y, closest_article.x, closest_article.y);
        
                        // Slam in place if close enough to the article
                        if (article_distance < 60) 
                        {
                            hold_toward_direction_8way(6); // Hold down
                        }
                        else if (closest_article.x > x + 80 && spr_dir == 1 || closest_article.x < x - 80 && spr_dir == -1)
                        {
                            hold_forwards();
                        }
                        else if (closest_article.x < x && spr_dir == 1 || closest_article.x > x && spr_dir == -1)
                        {
                            hold_backwards();
                        }
                    }
                ...
                // Additional check for proximity to ledge
                var safe_distance_from_ledge = 50;
                if ((spr_dir == 1 && x > stage_right - safe_distance_from_ledge) || 
                    (spr_dir == -1 && x < stage_left + safe_distance_from_ledge))
                {
                    hold_toward_direction_8way(6); // Slam in place
                }
            }
        break;     
    }
...
// DSPECIAL Logic based on meter
if (!attacking && !ai_recovering)
{
    if (meter >= meter_max && (!is_guardian && !is_sparking))
    {
        clear_ai_inputs();
        perform_attack(AT_DSPECIAL, true); // Use DSPECIAL to enter install
    }
    else if (!is_guardian && !is_sparking) // Only consider using DSPECIAL if not already in the form
    {
        // Calculate chance to use DSPECIAL based on meter
        var dspecial_chance = meter / meter_max * 8;
        if (random_func(0, 100000, true) < dspecial_chance)
        {
            clear_ai_inputs();
            perform_attack(AT_DSPECIAL, true);
        }
    }
}
...
switch(type){
	...
	case HBL_STRONGS:
		...
		break;
	case HBL_SPECIALS:
		var attacke = [AT_NSPECIAL, AT_NSPECIAL_AIR, AT_FSPECIAL, AT_FSPECIAL_AIR, AT_DSPECIAL, AT_USPECIAL];
		break;
	case HBL_DACUS:
...
repeat(len){
  ...
  switch(att_idx){ case AT_NSPECIAL: case AT_NSPECIAL_AIR: case AT_FSPECIAL: case AT_FSPECIAL_AIR: case AT_DSPECIAL: case AT_FTILT: case AT_UTILT: case AT_DTILT: case AT_FSTRONG: case AT_DSTRONG: case AT_USTRONG: can_reverse = true; break; }
...
#define is_attack_valid(attack_test)
{
    ...
    if (move_cooldown[attack_test] > 0) return false;

    // General FSPECIAL Logic
    if (attack_test == AT_FSPECIAL || attack_test == AT_FSPECIAL_AIR)
    {
        var base_prediction_frames = 10; // Base number of frames for prediction
        var max_prediction_frames = 20; // Maximum frames to predict
        var min_prediction_frames = 8;  // Minimum frames to predict
    
        // Adjust prediction frames dynamically based on distance
        var target_distance = point_distance(x, y, ai_target.x, ai_target.y);
        var prediction_frames = clamp(floor(base_prediction_frames + (target_distance - 200) / 20), min_prediction_frames, max_prediction_frames);
        var predicted_target_pos = newPredict(ai_target, prediction_frames, ai_target.hsp, ai_target.vsp, noone, true, false);
        var predicted_x = predicted_target_pos[@0];
        var predicted_y = predicted_target_pos[@1];
        var predicted_distance = point_distance(x, y, predicted_x, predicted_y);
    
        var tap_fs_range = 100;  // Range for tap version
        var hold_fs_range = 100; // Range for hold version
    
        // Avoid FSPECIAL when near ledge
        if ((spr_dir == 1 && x > stage_right - 80) || (spr_dir == -1 && x < stage_left + 80))
        {
            return false;
        }
    
        // Grounded FSPECIAL validation
        if (!free)
        {
            if (targetdamage < 50 && abs(predicted_x - x) <= hold_fs_range) 
            {
                return true; // Valid for grounded hold version
            }
            if (abs(predicted_x - x) <= tap_fs_range) 
            {
                return true; // Valid for grounded tap version
            }
            return false; // If no conditions are met, FSPECIAL is invalid
        }
    
        // Aerial FSPECIAL validation
        if (free)
        {
            var tap_threat_x = 150;  // Horizontal range for tap version
            var tap_threat_y = 50;   // Vertical range for tap version
            var hold_threat_x = 200; // Horizontal range for hold version
            var hold_threat_y = 30;  // Vertical range for hold version
            var distance_x = abs(predicted_x - x);
            var distance_y = abs(predicted_y - y);
    
            if (distance_x <= tap_threat_x && predicted_y > y && distance_y <= tap_threat_y) 
            {
                return true; // Valid for aerial tap version
            }
            if (distance_x <= hold_threat_x && distance_y <= hold_threat_y) 
            {
                return true; // Valid for aerial hold version
            }
            return false; // If no conditions are met, FSPECIAL_AIR is invalid
        }
    }

    // Additional validation for DSPECIAL
    if (attack_test == AT_DSPECIAL)
    {
        if (dist > 100 || abs(ai_target.y - y) > 80) // Restrict usage if too far away
        {
            return false;
        }
    }

    // Prevent USPECIAL during recovery
