...
if (meter_ready_timer > 0) { // Only draw when the timer is active
    var text_x = x; // Adjust position
    var text_y = y - 130; 

    if (floor(get_gameplay_time() / 10) % 2 == 0) { // Flashing effect
        textDraw(text_x, text_y, "tinyFont", c_white, -1, -1, fa_center, 2, true, 1, "READY", false);
    }
}
...
if (artc_parried_timer > 0) {
    ...
    var bar_x = x - 28;  // X position 
    var bar_y = y - 130;  // Y position 
    var bar_width = 50;  // Full bar width
    var bar_height = 3;  // Bar thickness

    // Normalize the timer percentage (1 when full, 0 when empty)
    var fill_percentage = artc_parried_timer / 240;

    // Calculate the filled bar width
    var fill_width = fill_percentage * bar_width;

    // **Draw the black outline (full bar)**
    ...
    // **Draw the filling bar (white) that depletes from right to left**
    draw_rectangle_color(
        bar_x, bar_y, 
