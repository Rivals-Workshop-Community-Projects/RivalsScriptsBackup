sprite_change_offset("beegguy_hurt", 42, 82);
sprite_change_offset("beegguycrouch_hurt", 42, 82);

sprite_change_offset("idle", 42, 82);
sprite_change_offset("phone_open", 42, 82);
sprite_change_offset("hurt", 42, 82);
	sprite_change_offset("bighurt", 42, 82);
	sprite_change_offset("hurtground", 42, 82);
sprite_change_offset("crouch", 42, 82);
sprite_change_offset("walk", 42, 82);
sprite_change_offset("walkturn", 42, 82);
sprite_change_offset("dash", 42, 82);
sprite_change_offset("dashstart", 42, 82);
sprite_change_offset("dashstop", 42, 82);
sprite_change_offset("dashturn", 42, 82);

sprite_change_offset("jumpstart", 42, 82);
sprite_change_offset("jump", 42, 82);
sprite_change_offset("doublejump", 42, 82);
	sprite_change_offset("flutter", 42, 82);
sprite_change_offset("walljump", 42, 82);
sprite_change_offset("pratfall", 42, 82);
sprite_change_offset("land", 42, 82);
sprite_change_offset("landinglag", 42, 82);

sprite_change_offset("parry", 52, 82);
sprite_change_offset("roll_forward", 52, 82);
sprite_change_offset("roll_backward", 52, 82);
sprite_change_offset("airdodge", 42, 82);
sprite_change_offset("waveland", 42, 82);
sprite_change_offset("tech", 42, 82);

sprite_change_offset("jab", 28, 82);
sprite_change_offset("dattack", 42, 86);
sprite_change_offset("ftilt", 26, 82);
sprite_change_offset("dtilt", 42, 82);
sprite_change_offset("utilt", 54, 98);
sprite_change_offset("nair", 42, 82);
sprite_change_offset("fair", 38, 82);
sprite_change_offset("bair", 58, 82);
sprite_change_offset("uair", 42, 98);
sprite_change_offset("dair", 42, 82);
sprite_change_offset("nairs", 126, 164);
sprite_change_offset("dairs", 126, 164);
sprite_change_offset("fstrong", 50, 102);
sprite_change_offset("ustrong", 42, 98);
sprite_change_offset("dstrong", 42, 98);
sprite_change_offset("nspecial", 30, 82);
	sprite_change_offset("fthrow", 42, 82);
	sprite_change_offset("bthrow", 42, 82);	
sprite_change_offset("fspecial", 62, 102);
	sprite_change_offset("fspecialproj", 20, 22);
sprite_change_offset("uspecial", 60, 100);
sprite_change_offset("dspecial", 42, 102);
sprite_change_offset("taunt", 42, 82);
	sprite_change_offset("taunt_crouch", 42, 82);
	sprite_change_offset("taunt_sakk", 42, 82);
	
set_victory_bg( sprite_get( "victorybg" ));

sprite_change_offset("plat", 64, 100);

sprite_change_offset("glow", 42, 94);

sprite_change_offset("eggbreak", 60, 100);
sprite_change_offset("eggbreak2", 60, 55);

sprite_change_offset("eggburst", 40, 40);

sprite_change_offset("arrow", 0, 16);
sprite_change_offset("arrow2", 15, 15);

sprite_change_offset("smoke_l", 35, 80);
sprite_change_offset("smoke_r", 19, 80);
sprite_change_offset("hudbobblehead", -6, -2);
sprite_change_offset("miiverse", 60, 30);

sprite_change_offset("yosr", 130, 130);
sprite_change_offset("yosl", 130, 130);
sprite_change_offset("yosrn", 130, 130);
sprite_change_offset("yosln", 126, 130);
sprite_change_offset("fireheal", 30, 30);