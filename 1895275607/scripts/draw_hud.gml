...
if ("canShoot" in self)
{
	...
	{
	    var t = 0;
	    if(recharge1 != 0 && recharge2 != 0){
	        t = 1 + floor(min(recharge1,recharge2) / 60);
	    }
	    else{
	        t = 1 + floor(max(recharge1,recharge2) / 60);
	    }
	    draw_debug_text(temp_x+148+dspecshiver, temp_y - 6, string(t));
	}
}
...
{
	menu[0] = "Careening UI: ";
	menu[1] = "Careening Min: ";
	menu[2] = "Careening Max: ";
	menu[3] = "Unlimited Ammo: ";
	menu[4] = "Unlimited Magnet: ";
	menu[5] = "Bury time: ";
	menu[6] = "Bury cooldown time: ";
	...
	case 1:
		...
		{
			draw_debug_text((tutmenu==i?round(sin(get_gameplay_time()/8))*2+2:0) + temp_x,temp_y - 20 - (noofitems-i-1)*16 + floor(tutalpha*4)*2-8, (tutmenu==i?"> ":"  ")+menu[i]+string(i==0?careeningui?"True":"False":i==1?careeningmin:i==2?careeningmax:i==3?runeM?"True":"False":i==4?puulnocooldown?"True":"False":i==5?buryframes:burycooldown));
		}
		break;
	default:
		break;
