set_attack_value(AT_DATTACK, AG_CATEGORY, 2);
set_attack_value(AT_DATTACK, AG_OFF_LEDGE, 0);
set_attack_value(AT_DATTACK, AG_SPRITE, sprite_get("dattack"));
set_attack_value(AT_DATTACK, AG_NUM_WINDOWS, 4);
set_attack_value(AT_DATTACK, AG_AIR_SPRITE, sprite_get("dattack"));
set_attack_value(AT_DATTACK, AG_HURTBOX_SPRITE, sprite_get("dattack_hurt"));

set_window_value(AT_DATTACK, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 1, AG_WINDOW_LENGTH, 6);
set_window_value(AT_DATTACK, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DATTACK, 1, AG_WINDOW_HSPEED, 3);
set_window_value(AT_DATTACK, 1, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_DATTACK, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DATTACK, 1, AG_WINDOW_SFX, asset_get("sfx_jumpground"));

set_window_value(AT_DATTACK, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 2, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DATTACK, 2, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DATTACK, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DATTACK, 2, AG_WINDOW_HSPEED, 7);
set_window_value(AT_DATTACK, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DATTACK, 2, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_DATTACK, 2, AG_WINDOW_CUSTOM_GROUND_FRICTION, 2);
set_window_value(AT_DATTACK, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DATTACK, 2, AG_WINDOW_SFX, asset_get("sfx_blow_heavy1"));
set_window_value(AT_DATTACK, 2, AG_WINDOW_SFX_FRAME, 3);

set_window_value(AT_DATTACK, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DATTACK, 3, AG_WINDOW_SFX, sound_get("sfx_rise"));
set_window_value(AT_DATTACK, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DATTACK, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DATTACK, 3, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_DATTACK, 3, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_DATTACK, 3, AG_WINDOW_CUSTOM_AIR_FRICTION, .1);
set_window_value(AT_DATTACK, 3, AG_WINDOW_CUSTOM_GROUND_FRICTION, 2);
set_window_value(AT_DATTACK, 3, AG_WINDOW_HAS_WHIFFLAG, 5);

set_window_value(AT_DATTACK, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DATTACK, 4, AG_WINDOW_LENGTH, 16);
set_window_value(AT_DATTACK, 4, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DATTACK, 4, AG_WINDOW_ANIM_FRAME_START, 10);
set_window_value(AT_DATTACK, 4, AG_WINDOW_HAS_WHIFFLAG, 5);

set_num_hitboxes(AT_DATTACK, 5);

set_hitbox_value(AT_DATTACK, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DATTACK, 1, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DATTACK, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DATTACK, 1, HG_LIFETIME, 15);
set_hitbox_value(AT_DATTACK, 1, HG_WIDTH, 30);
set_hitbox_value(AT_DATTACK, 1, HG_HEIGHT, 30);
set_hitbox_value(AT_DATTACK, 1, HG_HITBOX_X, -70);
set_hitbox_value(AT_DATTACK, 1, HG_HITBOX_Y, -16);
set_hitbox_value(AT_DATTACK, 1, HG_SHAPE, 0 );
set_hitbox_value(AT_DATTACK, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_DATTACK, 1, HG_DAMAGE, 4);
set_hitbox_value(AT_DATTACK, 1, HG_ANGLE, 138);
set_hitbox_value(AT_DATTACK, 1, HG_BASE_KNOCKBACK, 12);
set_hitbox_value(AT_DATTACK, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DATTACK, 1, HG_FINAL_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_DATTACK, 1, HG_VISUAL_EFFECT, 143 );
set_hitbox_value(AT_DATTACK, 1, HG_HIT_SFX, asset_get("sfx_abyss_explosion_big"));
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_SPRITE, sprite_get("skateboard"));
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_MASK, sprite_get("skateboard"));
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_ANIM_SPEED, 1);
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_HSPEED, -8);
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_GRAVITY, 1);
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_PARRY_STUN, true);
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_WALL_BEHAVIOR, 2 );
set_hitbox_value(AT_DATTACK, 1, HG_PROJECTILE_ENEMY_BEHAVIOR, 0 );
set_hitbox_value(AT_DATTACK, 1, HG_HIT_LOCKOUT, 5);


set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DATTACK, 2, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 2, HG_LIFETIME, 12);
set_hitbox_value(AT_DATTACK, 2, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_X, 35);
set_hitbox_value(AT_DATTACK, 2, HG_WIDTH, 20);
set_hitbox_value(AT_DATTACK, 2, HG_HEIGHT, 20);
set_hitbox_value(AT_DATTACK, 2, HG_ANGLE, 40);
set_hitbox_value(AT_DATTACK, 2, HG_SHAPE, 1);
set_hitbox_value(AT_DATTACK, 2, HG_PRIORITY, 6);
set_hitbox_value(AT_DATTACK, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_DATTACK, 2, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DATTACK, 2, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DATTACK, 2, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_DATTACK, 2, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_DATTACK, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DATTACK, 2, HG_VISUAL_EFFECT, 305);

set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DATTACK, 3, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 3, HG_LIFETIME, 9);
set_hitbox_value(AT_DATTACK, 3, HG_WINDOW_CREATION_FRAME, 4);
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_X, 55);
set_hitbox_value(AT_DATTACK, 3, HG_WIDTH, 20);
set_hitbox_value(AT_DATTACK, 3, HG_HEIGHT, 40);
set_hitbox_value(AT_DATTACK, 3, HG_ANGLE, 40);
set_hitbox_value(AT_DATTACK, 3, HG_SHAPE, 1);
set_hitbox_value(AT_DATTACK, 3, HG_PRIORITY, 8);
set_hitbox_value(AT_DATTACK, 3, HG_DAMAGE, 3);
set_hitbox_value(AT_DATTACK, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DATTACK, 3, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DATTACK, 3, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_DATTACK, 3, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_DATTACK, 3, HG_HITBOX_GROUP, 2);
set_hitbox_value(AT_DATTACK, 3, HG_VISUAL_EFFECT, 305);

set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DATTACK, 4, HG_WINDOW, 3);
set_hitbox_value(AT_DATTACK, 4, HG_LIFETIME, 6);
set_hitbox_value(AT_DATTACK, 4, HG_WINDOW_CREATION_FRAME, 8);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_Y, -30);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_X, 75);
set_hitbox_value(AT_DATTACK, 4, HG_WIDTH, 20);
set_hitbox_value(AT_DATTACK, 4, HG_HEIGHT, 60);
set_hitbox_value(AT_DATTACK, 4, HG_ANGLE, 60);
set_hitbox_value(AT_DATTACK, 4, HG_SHAPE, 1);
set_hitbox_value(AT_DATTACK, 4, HG_PRIORITY, 6);
set_hitbox_value(AT_DATTACK, 4, HG_DAMAGE, 4);
set_hitbox_value(AT_DATTACK, 4, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DATTACK, 4, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_DATTACK, 4, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_DATTACK, 4, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_DATTACK, 4, HG_HIT_SFX, sound_get("sfx_monsterhit"));
set_hitbox_value(AT_DATTACK, 4, HG_HIT_LOCKOUT, 5);
set_hitbox_value(AT_DATTACK, 4, HG_HITBOX_GROUP, 3);
set_hitbox_value(AT_DATTACK, 4, HG_VISUAL_EFFECT, 305);

set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DATTACK, 5, HG_WINDOW, 2);
set_hitbox_value(AT_DATTACK, 5, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_DATTACK, 5, HG_LIFETIME, 4);
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_X, 0);
set_hitbox_value(AT_DATTACK, 5, HG_WIDTH, 50);
set_hitbox_value(AT_DATTACK, 5, HG_HEIGHT, 20);
set_hitbox_value(AT_DATTACK, 5, HG_ANGLE, 35);
set_hitbox_value(AT_DATTACK, 5, HG_SHAPE, 1);
set_hitbox_value(AT_DATTACK, 5, HG_PRIORITY, 6);
set_hitbox_value(AT_DATTACK, 5, HG_DAMAGE, 3);
set_hitbox_value(AT_DATTACK, 5, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DATTACK, 5, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DATTACK, 5, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_DATTACK, 5, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));
set_hitbox_value(AT_DATTACK, 5, HG_HITBOX_GROUP, 6);
set_hitbox_value(AT_DATTACK, 5, HG_VISUAL_EFFECT, 305);