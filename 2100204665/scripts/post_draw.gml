if (instance_exists(bone_spire) && bone_spire.state > 0){
    with bone_spire draw_indicator(0, 0)
}
...
if (x < camera_x - 30) {
    offX = camera_x + 35;
    if (y - 440 > camera_y + view_get_hview() - 60) {
        offY = camera_y + view_get_hview() - 56;
        offIndex = 1;
        offRot = -90;
    } else if (y - 440 < camera_y + 35) {
        offY = camera_y + 35;
        offIndex = 1;
        offRot = 180;
    } else {
...
} else if (y - 440 > camera_y + view_get_hview() - 60) {
    offX = x;
    offY = camera_y + view_get_hview() - 56;
    offIndex = 0;
    offRot = 0;
