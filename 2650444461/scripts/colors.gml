set_num_palettes(32);

//DEFAULT (R,G,B)
set_color_profile_slot(0, 0, 42, 133, 218); //BLUE
set_color_profile_slot(0, 1, 135, 211, 255); //LIGHT BLUE
set_color_profile_slot(0, 2, 244, 163, 76); //ORANGE
set_color_profile_slot(0, 3, 15, 20, 47); //EYES
set_color_profile_slot(0, 4, 229, 117, 175); //MOUTH

set_color_profile_slot(0, 5, 0, 58, 168); //PANTS
set_color_profile_slot(0, 6, 0, 149, 229); //SHOES
set_color_profile_slot(0, 7, 0, 224, 178); //BRACELET

//COLOR RANGE (H,S,V)
set_color_profile_slot_range(0, 1, 20, 42); //BLUE
set_color_profile_slot_range(1, 3, 50, 40); //LIGHT BLUE
set_color_profile_slot_range(2, 19, 20, 30); //ORANGE
set_color_profile_slot_range(3, 1, 40, 80); //EYES
set_color_profile_slot_range(4, 15, 80, 85); //MOUTH

set_color_profile_slot_range(5, 1, 55, 26); //PANTS
set_color_profile_slot_range(6, 1, 22, 50); //SHOES
set_color_profile_slot_range(7, 1, 30, 23); //BRACELET


//Shiny
set_color_profile_slot(1, 0, 202, 129, 213); //BLUE
set_color_profile_slot(1, 1, 236, 230, 185); //LIGHT BLUE
set_color_profile_slot(1, 2, 244, 163, 76); //ORANGE
set_color_profile_slot(1, 3, 15, 20, 47); //EYES
set_color_profile_slot(1, 4, 229, 117, 175); //MOUTH
set_color_profile_slot(1, 5, 66, 79, 159); //PANTS
set_color_profile_slot(1, 6, 130, 224, 120); //SHOES
set_color_profile_slot(1, 7, 244, 163, 76); //BRACELET

//Marshtomp
set_color_profile_slot(2, 0, 136, 208, 198); //BLUE
set_color_profile_slot(2, 1, 226, 255, 255); //LIGHT BLUE
set_color_profile_slot(2, 2, 244, 163, 76); //ORANGE
set_color_profile_slot(2, 3, 172, 73, 19); //EYES
set_color_profile_slot(2, 4, 229, 117, 175); //MOUTH

set_color_profile_slot(2, 5, 41, 52, 118); //PANTS
set_color_profile_slot(2, 6, 210, 40, 40); //SHOES
set_color_profile_slot(2, 7, 46, 96, 239); //BRACELET

//Swampert
set_color_profile_slot(3, 0, 68, 147, 186); //BLUE
set_color_profile_slot(3, 1, 206, 194, 214); //LIGHT BLUE
set_color_profile_slot(3, 2, 244, 163, 76); //ORANGE
set_color_profile_slot(3, 3, 172, 73, 19); //EYES
set_color_profile_slot(3, 4, 229, 117, 175); //MOUTH

set_color_profile_slot(3, 5, 66, 79, 159); //PANTS
set_color_profile_slot(3, 6, 244, 163, 76); //SHOES
set_color_profile_slot(3, 7, 130, 224, 120); //BRACELET

//Treecko
set_color_profile_slot(4, 0, 202, 225, 132); //BLUE
set_color_profile_slot(4, 1, 237, 154, 157); //LIGHT BLUE
set_color_profile_slot(4, 2, 153, 191, 139); //ORANGE
set_color_profile_slot(4, 3, 232, 186, 27); //EYES
set_color_profile_slot(4, 4, 229, 117, 175); //MOUTH

//Torchic
set_color_profile_slot(5, 0, 240, 160, 65); //BLUE
set_color_profile_slot(5, 1, 248, 212, 85); //LIGHT BLUE
set_color_profile_slot(5, 2, 248, 212, 85); //ORANGE
set_color_profile_slot(5, 3, 73, 67, 99); //EYES
set_color_profile_slot(5, 4, 229, 117, 175); //MOUTH

//Lotad
set_color_profile_slot(6, 0, 63, 130, 193); //BLUE
set_color_profile_slot(6, 1, 248, 224, 151); //LIGHT BLUE
set_color_profile_slot(6, 2, 115, 187, 136); //ORANGE
set_color_profile_slot(6, 3, 44, 50, 131); //EYES
set_color_profile_slot(6, 4, 229, 117, 175); //MOUTH

//Gardevoir
set_color_profile_slot(7, 0, 248, 249, 250); //BLUE
set_color_profile_slot(7, 1, 152, 203, 154); //LIGHT BLUE
set_color_profile_slot(7, 2, 229, 116, 124); //ORANGE
set_color_profile_slot(7, 3, 185, 77, 93); //EYES
set_color_profile_slot(7, 4, 229, 117, 175); //MOUTH

//Aggron
set_color_profile_slot(8, 0, 186, 194, 200); //BLUE
set_color_profile_slot(8, 1, 62, 64, 77); //LIGHT BLUE
set_color_profile_slot(8, 2, 62, 64, 77); //ORANGE
set_color_profile_slot(8, 3, 51, 186, 225); //EYES
set_color_profile_slot(8, 4, 229, 117, 175); //MOUTH

//Torkoal
set_color_profile_slot(9, 0, 223, 146, 70); //BLUE
set_color_profile_slot(9, 1, 88, 71, 71); //LIGHT BLUE
set_color_profile_slot(9, 2, 198, 55, 44); //ORANGE
set_color_profile_slot(9, 3, 198, 55, 44); //EYES
set_color_profile_slot(9, 4, 229, 117, 175); //MOUTH

//Flygon
set_color_profile_slot(10, 0, 168, 208, 122); //BLUE
set_color_profile_slot(10, 1, 69, 150, 82); //LIGHT BLUE
set_color_profile_slot(10, 2, 213, 94, 103); //ORANGE
set_color_profile_slot(10, 3, 132, 59, 52); //EYES
set_color_profile_slot(10, 4, 229, 117, 175); //MOUTH

//Zeviper
set_color_profile_slot(11, 0, 83, 101, 119); //BLUE
set_color_profile_slot(11, 1, 242, 206, 120); //LIGHT BLUE
set_color_profile_slot(11, 2, 153, 131, 182); //ORANGE
set_color_profile_slot(11, 3, 210, 102, 91); //EYES
set_color_profile_slot(11, 4, 229, 117, 175); //MOUTH

//Zangoose
set_color_profile_slot(12, 0, 226, 229, 243); //BLUE
set_color_profile_slot(12, 1, 229, 94, 108); //LIGHT BLUE
set_color_profile_slot(12, 2, 78, 67, 78); //ORANGE
set_color_profile_slot(12, 3, 244, 159, 181); //EYES
set_color_profile_slot(12, 4, 229, 117, 175); //MOUTH

//Salamence
set_color_profile_slot(13, 0, 71, 168, 197); //BLUE
set_color_profile_slot(13, 1, 185, 196, 208); //LIGHT BLUE
set_color_profile_slot(13, 2, 173, 74, 86); //ORANGE
set_color_profile_slot(13, 3, 173, 74, 86); //EYES
set_color_profile_slot(13, 4, 229, 117, 175); //MOUTH

//Metagross
set_color_profile_slot(14, 0, 80, 129, 174); //BLUE
set_color_profile_slot(14, 1, 183, 198, 208); //LIGHT BLUE
set_color_profile_slot(14, 2, 183, 198, 208); //ORANGE
set_color_profile_slot(14, 3, 224, 90, 97); //EYES
set_color_profile_slot(14, 4, 229, 117, 175); //MOUTH

//Regirock
set_color_profile_slot(15, 0, 200, 178, 174); //BLUE
set_color_profile_slot(15, 1, 204, 131, 93); //LIGHT BLUE
set_color_profile_slot(15, 2, 204, 131, 93); //ORANGE
set_color_profile_slot(15, 3, 232, 134, 62); //EYES
set_color_profile_slot(15, 4, 153, 130, 138); //MOUTH

//Regice
set_color_profile_slot(16, 0, 154, 230, 252); //BLUE
set_color_profile_slot(16, 1, 154, 230, 252); //LIGHT BLUE
set_color_profile_slot(16, 2, 237, 223, 69); //ORANGE
set_color_profile_slot(16, 3, 237, 223, 69); //EYES
set_color_profile_slot(16, 4, 85, 149, 214); //MOUTH

//Registeel
set_color_profile_slot(17, 0, 80, 80, 94); //BLUE
set_color_profile_slot(17, 1, 193, 193, 217); //LIGHT BLUE
set_color_profile_slot(17, 2, 193, 193, 217); //ORANGE
set_color_profile_slot(17, 3, 224, 69, 76); //EYES
set_color_profile_slot(17, 4, 115, 105, 131); //MOUTH

//Latias
set_color_profile_slot(18, 0, 181, 48, 55); //BLUE
set_color_profile_slot(18, 1, 201, 211, 233); //LIGHT BLUE
set_color_profile_slot(18, 2, 48, 119, 196); //ORANGE
set_color_profile_slot(18, 3, 225, 169, 61); //EYES
set_color_profile_slot(18, 4, 229, 117, 175); //MOUTH

//Latios
set_color_profile_slot(19, 0, 48, 119, 196); //BLUE
set_color_profile_slot(19, 1, 165, 166, 165); //LIGHT BLUE
set_color_profile_slot(19, 2, 181, 48, 55); //ORANGE
set_color_profile_slot(19, 3, 224, 90, 97); //EYES
set_color_profile_slot(19, 4, 229, 117, 175); //MOUTH

//Groudon
set_color_profile_slot(20, 0, 194, 48, 31); //BLUE
set_color_profile_slot(20, 1, 136, 123, 112); //LIGHT BLUE
set_color_profile_slot(20, 2, 24, 12, 40); //ORANGE
set_color_profile_slot(20, 3, 241, 206, 42); //EYES
set_color_profile_slot(20, 4, 229, 117, 175); //MOUTH

//Kyogre
set_color_profile_slot(21, 0, 45, 81, 155); //BLUE
set_color_profile_slot(21, 1, 214, 219, 235); //LIGHT BLUE
set_color_profile_slot(21, 2, 201, 40, 72); //ORANGE
set_color_profile_slot(21, 3, 241, 206, 42); //EYES
set_color_profile_slot(21, 4, 229, 117, 175); //MOUTH

//Rayquaza
set_color_profile_slot(22, 0, 66, 133, 85); //BLUE
set_color_profile_slot(22, 1, 179, 62, 78); //LIGHT BLUE
set_color_profile_slot(22, 2, 243, 219, 129); //ORANGE
set_color_profile_slot(22, 3, 241, 206, 42); //EYES
set_color_profile_slot(22, 4, 229, 117, 175); //MOUTH

//Jirachi
set_color_profile_slot(23, 0, 238, 210, 137); //BLUE
set_color_profile_slot(23, 1, 244, 245, 249); //LIGHT BLUE
set_color_profile_slot(23, 2, 73, 190, 191); //ORANGE
set_color_profile_slot(23, 3, 4, 27, 29); //EYES
set_color_profile_slot(23, 4, 229, 117, 175); //MOUTH

//Deoxys
set_color_profile_slot(24, 0, 236, 108, 63); //BLUE
set_color_profile_slot(24, 1, 61, 176, 200); //LIGHT BLUE
set_color_profile_slot(24, 2, 157, 97, 148); //ORANGE
set_color_profile_slot(24, 3, 8, 27, 34); //EYES
set_color_profile_slot(24, 4, 229, 117, 175); //MOUTH

//Pikachu
set_color_profile_slot(25, 0, 255, 215, 62); //BLUE
set_color_profile_slot(25, 1, 40, 40, 40); //LIGHT BLUE
set_color_profile_slot(25, 2, 238, 79, 53); //ORANGE
set_color_profile_slot(25, 3, 40, 40, 40); //EYES
set_color_profile_slot(25, 4, 229, 117, 175); //MOUTH

//Ditto
set_color_profile_slot(26, 0, 203, 179, 240); //BLUE
set_color_profile_slot(26, 1, 203, 179, 240); //LIGHT BLUE
set_color_profile_slot(26, 2, 203, 179, 240); //ORANGE
set_color_profile_slot(26, 3, 119, 68, 196); //EYES
set_color_profile_slot(26, 4, 119, 68, 196); //MOUTH

//Matt
set_color_profile_slot(27, 0, 118, 224, 178); //BLUE
set_color_profile_slot(27, 1, 249, 225, 94); //LIGHT BLUE
set_color_profile_slot(27, 2, 210, 56, 40); //ORANGE
set_color_profile_slot(27, 3, 38, 48, 124); //EYES
set_color_profile_slot(27, 4, 229, 117, 175); //MOUTH

//Aria
set_color_profile_slot(28, 0, 244, 163, 76); //BLUE
set_color_profile_slot(28, 1, 209, 107, 215); //LIGHT BLUE
set_color_profile_slot(28, 2, 99, 58, 167); //ORANGE
set_color_profile_slot(28, 3, 0, 58, 14); //EYES
set_color_profile_slot(28, 4, 229, 117, 175); //MOUTH

//Sol
set_color_profile_slot(29, 0, 210, 56, 40); //BLUE
set_color_profile_slot(29, 1, 249, 225, 94); //LIGHT BLUE
set_color_profile_slot(29, 2, 246, 152, 90); //ORANGE
set_color_profile_slot(29, 3, 0, 103, 112); //EYES
set_color_profile_slot(29, 4, 229, 117, 175); //MOUTH

//Toadie
set_color_profile_slot(30, 0, 97, 215, 45); //BLUE
set_color_profile_slot(30, 1, 25, 141, 239); //LIGHT BLUE
set_color_profile_slot(30, 2, 249, 127, 37); //ORANGE
set_color_profile_slot(30, 3, 44, 57, 157); //EYES
set_color_profile_slot(30, 4, 229, 117, 175); //MOUTH

//Michael
set_color_profile_slot(31, 0, 128, 220, 61); //BLUE
set_color_profile_slot(31, 1, 249, 150, 43); //LIGHT BLUE
set_color_profile_slot(31, 2, 73, 81, 124); //ORANGE
set_color_profile_slot(31, 3, 116, 59, 33); //EYES
set_color_profile_slot(31, 4, 229, 117, 175); //MOUTH