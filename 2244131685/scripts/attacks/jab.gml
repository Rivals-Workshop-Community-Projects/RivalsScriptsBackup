// 3 Hit jab, last hit throws out sword
set_attack_value(AT_JAB, AG_SPRITE, sprite_get("jab"));
set_attack_value(AT_JAB, AG_NUM_WINDOWS, 9);
set_attack_value(AT_JAB, AG_HURTBOX_SPRITE, sprite_get("jab_hurt"));

// Startup JAB 1
set_window_value(AT_JAB, 1, AG_WINDOW_LENGTH, 5);
set_window_value(AT_JAB, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_JAB, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_medium1"));
set_window_value(AT_JAB, 1, AG_WINDOW_SFX_FRAME, 3);

// Active Frames JAB 1
set_window_value(AT_JAB, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_JAB, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 2, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_JAB, 2, AG_WINDOW_HSPEED, 1);

// Endlag JAB 1
set_window_value(AT_JAB, 3, AG_WINDOW_LENGTH, 9);
set_window_value(AT_JAB, 3, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_JAB, 3, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_JAB, 3, AG_WINDOW_HSPEED, 3);
set_window_value(AT_JAB, 3, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_JAB, 3, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_JAB, 3, AG_WINDOW_CANCEL_FRAME, 4);

// Startup JAB 2
set_window_value(AT_JAB, 4, AG_WINDOW_LENGTH, 8);
set_window_value(AT_JAB, 4, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_JAB, 4, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_JAB, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_JAB, 4, AG_WINDOW_SFX, asset_get("sfx_swipe_medium2"));
set_window_value(AT_JAB, 4, AG_WINDOW_SFX_FRAME, 7);

// Active Frames JAB 2
set_window_value(AT_JAB, 5, AG_WINDOW_LENGTH, 2);
set_window_value(AT_JAB, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 5, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_JAB, 5, AG_WINDOW_HSPEED, 6);

// Endlag JAB 2
set_window_value(AT_JAB, 6, AG_WINDOW_LENGTH, 12);
set_window_value(AT_JAB, 6, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_JAB, 6, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_JAB, 6, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_JAB, 6, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_JAB, 6, AG_WINDOW_CANCEL_FRAME, 4);

// Startup JAB 3
set_window_value(AT_JAB, 7, AG_WINDOW_LENGTH, 8);
set_window_value(AT_JAB, 7, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_JAB, 7, AG_WINDOW_ANIM_FRAME_START, 7);

// Active Frames JAB 3
set_window_value(AT_JAB, 8, AG_WINDOW_LENGTH, 18);
set_window_value(AT_JAB, 8, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_JAB, 8, AG_WINDOW_HSPEED, -0.50);
set_window_value(AT_JAB, 8, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_JAB, 8, AG_WINDOW_ANIM_FRAME_START, 9);

// Endlag JAB 3
set_window_value(AT_JAB, 9, AG_WINDOW_LENGTH, 9);
set_window_value(AT_JAB, 9, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_JAB, 9, AG_WINDOW_ANIM_FRAME_START, 13);
set_window_value(AT_JAB, 9, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_JAB, 7);

// Hit 1
set_hitbox_value(AT_JAB, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_JAB, 1, HG_WINDOW, 2);
set_hitbox_value(AT_JAB, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_JAB, 1, HG_HITBOX_X, 50);
set_hitbox_value(AT_JAB, 1, HG_HITBOX_Y, -34);
set_hitbox_value(AT_JAB, 1, HG_WIDTH, 80);
set_hitbox_value(AT_JAB, 1, HG_HEIGHT, 60);
set_hitbox_value(AT_JAB, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_JAB, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_JAB, 1, HG_ANGLE, 361);
set_hitbox_value(AT_JAB, 1, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_JAB, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_JAB, 1, HG_FORCE_FLINCH, 1);
set_hitbox_value(AT_JAB, 1, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 1, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
set_hitbox_value(AT_JAB, 1, HG_HIT_PARTICLE_NUM, 1);

// Hit 2
set_hitbox_value(AT_JAB, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_JAB, 2, HG_WINDOW, 5);
set_hitbox_value(AT_JAB, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_JAB, 2, HG_HITBOX_X, 54);
set_hitbox_value(AT_JAB, 2, HG_HITBOX_Y, -30);
set_hitbox_value(AT_JAB, 2, HG_WIDTH, 80);
set_hitbox_value(AT_JAB, 2, HG_HEIGHT, 56);
set_hitbox_value(AT_JAB, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_JAB, 2, HG_DAMAGE, 3);
set_hitbox_value(AT_JAB, 2, HG_ANGLE, 361);
set_hitbox_value(AT_JAB, 2, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_JAB, 2, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_JAB, 2, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 2, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_JAB, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_JAB, 2, HG_HIT_PARTICLE_NUM, 1);

var throw_win_length = get_window_value(AT_JAB, 8, AG_WINDOW_LENGTH) + 1;
set_hitbox_value(AT_JAB, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_JAB, 3, HG_LIFETIME, throw_win_length);
set_hitbox_value(AT_JAB, 3, HG_HITBOX_X, -10);
set_hitbox_value(AT_JAB, 3, HG_HITBOX_Y, -50);
set_hitbox_value(AT_JAB, 3, HG_ANGLE, 100);
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_SPRITE, sprite_get("sword_proj"));
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_ANIM_SPEED, 0.4);
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 3, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(AT_JAB, 3, HG_PROJECTILE_DESTROY_EFFECT, 1);

// Put Multihits on top of projectile
// Multihit 1
set_hitbox_value(AT_JAB, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_JAB, 4, HG_LIFETIME, 5);
set_hitbox_value(AT_JAB, 4, HG_HITBOX_X, 58);
set_hitbox_value(AT_JAB, 4, HG_HITBOX_Y, -32);
set_hitbox_value(AT_JAB, 4, HG_WIDTH, 84);
set_hitbox_value(AT_JAB, 4, HG_HEIGHT, 44);
set_hitbox_value(AT_JAB, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_JAB, 4, HG_DAMAGE, 1);
set_hitbox_value(AT_JAB, 4, HG_ANGLE, 30);
set_hitbox_value(AT_JAB, 4, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_JAB, 4, HG_BASE_KNOCKBACK, 2);
set_hitbox_value(AT_JAB, 4, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_JAB, 4, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 4, HG_TECHABLE, 1);
set_hitbox_value(AT_JAB, 4, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_PARRY_STUN, 1);
set_hitbox_value(AT_JAB, 4, HG_PROJECTILE_DESTROY_EFFECT, 1);
set_hitbox_value(AT_JAB, 4, HG_HIT_PARTICLE_NUM, 1);

// Multihit 2
set_hitbox_value(AT_JAB, 5, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_JAB, 5, HG_WINDOW_CREATION_FRAME, 5);
set_hitbox_value(AT_JAB, 5, HG_LIFETIME, 4);
set_hitbox_value(AT_JAB, 5, HG_HITBOX_X, 54);
set_hitbox_value(AT_JAB, 5, HG_HITBOX_Y, -32);
set_hitbox_value(AT_JAB, 5, HG_WIDTH, 84);
set_hitbox_value(AT_JAB, 5, HG_HEIGHT, 44);
set_hitbox_value(AT_JAB, 5, HG_PRIORITY, 2);
set_hitbox_value(AT_JAB, 5, HG_DAMAGE, 2);
set_hitbox_value(AT_JAB, 5, HG_ANGLE, 30);
set_hitbox_value(AT_JAB, 5, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_JAB, 5, HG_BASE_KNOCKBACK, 2);
set_hitbox_value(AT_JAB, 5, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_JAB, 5, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 5, HG_TECHABLE, 1);
set_hitbox_value(AT_JAB, 5, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_PARRY_STUN, 1);
set_hitbox_value(AT_JAB, 5, HG_PROJECTILE_DESTROY_EFFECT, 1);
set_hitbox_value(AT_JAB, 5, HG_HIT_PARTICLE_NUM, 1);

// Multihit 3
set_hitbox_value(AT_JAB, 6, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_JAB, 6, HG_WINDOW_CREATION_FRAME, 9);
set_hitbox_value(AT_JAB, 6, HG_LIFETIME, 5);
set_hitbox_value(AT_JAB, 6, HG_HITBOX_X, 70);
set_hitbox_value(AT_JAB, 6, HG_HITBOX_Y, -32);
set_hitbox_value(AT_JAB, 6, HG_WIDTH, 80);
set_hitbox_value(AT_JAB, 6, HG_HEIGHT, 44);
set_hitbox_value(AT_JAB, 6, HG_PRIORITY, 2);
set_hitbox_value(AT_JAB, 6, HG_DAMAGE, 2);
set_hitbox_value(AT_JAB, 6, HG_ANGLE, 50);
set_hitbox_value(AT_JAB, 6, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_JAB, 6, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_JAB, 6, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_JAB, 6, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 6, HG_TECHABLE, 1);
set_hitbox_value(AT_JAB, 6, HG_HIT_SFX, asset_get("sfx_blow_medium2"));
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_PARRY_STUN, 1);
set_hitbox_value(AT_JAB, 6, HG_PROJECTILE_DESTROY_EFFECT, 1);
set_hitbox_value(AT_JAB, 6, HG_HIT_PARTICLE_NUM, 1);

// Multihit Finisher
set_hitbox_value(AT_JAB, 7, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_JAB, 7, HG_WINDOW_CREATION_FRAME, 14);
set_hitbox_value(AT_JAB, 7, HG_LIFETIME, 4);
set_hitbox_value(AT_JAB, 7, HG_HITBOX_X, 80);
set_hitbox_value(AT_JAB, 7, HG_HITBOX_Y, -32);
set_hitbox_value(AT_JAB, 7, HG_WIDTH, 82);
set_hitbox_value(AT_JAB, 7, HG_HEIGHT, 44);
set_hitbox_value(AT_JAB, 7, HG_PRIORITY, 2);
set_hitbox_value(AT_JAB, 7, HG_DAMAGE, 3);
set_hitbox_value(AT_JAB, 7, HG_ANGLE, 50);
set_hitbox_value(AT_JAB, 7, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_JAB, 7, HG_KNOCKBACK_SCALING, 0.6);
set_hitbox_value(AT_JAB, 7, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_JAB, 7, HG_HITPAUSE_SCALING, 0.50);
set_hitbox_value(AT_JAB, 7, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_JAB, 7, HG_HIT_SFX, asset_get("sfx_blow_medium3"));
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_IS_TRANSCENDENT, 1);
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_PARRY_STUN, 1);
set_hitbox_value(AT_JAB, 7, HG_PROJECTILE_DESTROY_EFFECT, 1);
set_hitbox_value(AT_JAB, 7, HG_HIT_PARTICLE_NUM, 1);
