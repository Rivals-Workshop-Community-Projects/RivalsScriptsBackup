...
if (get_training_cpu_action() == CPU_FIGHT) {
	...
	if (instance_exists(pod)) {
		if (!pod.attached && ((ai_target.x - pod.x <= 10) || (ai_target.x - pod.x >= -10))  && ((ai_target.y - pod.y <= 10) || (ai_target.y - pod.y >= -10))) {
			joy_pad_idle = true;
		    left_down = false;
		    right_down = false;
			down_down = true;
	...
	if free and can_attack and get_stage_data( SD_Y_POS ) > y+24 and 0 > AIwait {
	    ...
	    if (ai_target.y < y) and (ai_target.y > y - 70) and xdist < 60 and !ai_recovering{ // Uair
	        clear_button_buffer( PC_ATTACK_PRESSED );
	        joy_pad_idle = true;
	        left_down = false;
	        right_down = false;
