...
switch phase {
	...
	switch state {
        ...
        if state_timer == 30 {
        	...
        	phase++;
        	phase_timer = 0;
        	ratings_id = instance_create(room_width/2 - 44000, 35000, "obj_stage_article", 13);
	...
	switch state {
		...
		if button_selected != undefined && state_timer > 20 {
            ...
            switch button_selected {
                ...
                if mtt_damage_timer == 180 {
                	textbox_finished = true;
                	state = 0;
		        	state_timer = 0;
		        	phase++;
		        	phase_timer = 0;
                ...
                with oPlayer {
                	if !is_dead {
	                    if state == PS_PARRY {
	                        ...
	                        other.state++;
	                        other.state_timer = 0;
	                    } else if state == PS_ATTACK_GROUND && (up_strong_pressed || down_strong_pressed || left_strong_pressed || right_strong_pressed || attack_pressed) {
	                        other.act_index = 0;
	                        other.state++;
	                        other.state_timer = 0;
	                        ...
	                        other.state++;
	                        other.state_timer = 0;
	                    } else if (state == PS_ATTACK_GROUND || state == PS_ATTACK_AIR) && special_pressed {
	                        other.act_index = 0;
	                        other.state++;
	                        other.state_timer = 0;
	                    }
		...
		*/
		...
		switch attack_index {
	    	...
	    	if state_timer == 1 {
	    		...
	    		//spawn_mini_mtt(room_width/2 - 160, -200, 0, 3);
	    		//spawn_mini_mtt(room_width/2 - 160, -300, 0, 3);
	    	} else if state_timer == 4000000 {
	    		//remove_box();
	    		//destroy_attacks();
	    	} else if state_timer == 4200000 {
	    		//gotomenu();
	    		//attack_index++
	    	}
...
gold star."];
	    	...
	    	case 7: //disco ball
	    	if state_timer == 1 {
	    		spawn_disco_ball(room_width/2 - 20, 300, 90, 1.5, 75);
	    	} else if state_timer == 600 {
	    		remove_box();
	    		destroy_attacks();
	    	} else if state_timer ==  620 {
	    		attack_index++;
	    	...
	    	case 8: //disco ball 2
	    	if state_timer == 1 {
	    		spawn_disco_ball(room_width/2 - 20, 300, 20, 2, 60);
	    	} else if state_timer == 400 {
	    		remove_box();
	    		destroy_attacks();
	    	} else if state_timer ==  420 {
...
if (endTimer > 40) {
	with oPlayer {
    	if (attack_pressed || special_pressed) && !is_cpu {
    		with other {
    			end_match();
    			//displayStats = false;
    		}
