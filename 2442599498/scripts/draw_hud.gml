...
if mtt_take_damage {
    var meter_width = 500;
    var meter_height = 25;
    var hp_width = ease_sineOut(hp_old, 0, clamp(mtt_damage_timer, 0, 50), 50)*meter_width/hp_total;
    ...
    var length = string_length(string(dmg));
    var scale = 3;
    for (var i = 0; i < length; i++) {
        draw_sprite_ext(sprite_get("numbers"), real(string_char_at(string(dmg), i+1)), 480 + 90*i - (30*scale)*length/2, 180, scale, scale, 0, c_red, 1);
    }
}
