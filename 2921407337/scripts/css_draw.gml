...
switch(draw_menu){
	case DRAW_NO_MENU:
		...
		textDraw(x + 10, y + 140, "fName", c_white, 18, 200, 1, true, 1,"Color#" + string(get_player_color(player)) + mode_indicator_suffix +":" + string(slot_property_array[ get_player_color(player) + (color_shift * 32),0]))
	...
	case DRAW_MENU_CHARACTER_GUIDE:
		...
		//"Google Doc QR Code / Scan with phone / by ");
		//Draw Guide Menu
	...
	case DRAW_MENU_LORE_INFO:
		...
		textDraw(x+14, y+78, "fName", c_white, 15, 195, 1, false, 1, slot_property_array[ get_player_color(player) + (color_shift * 32),2]); //Text for skin description
	...
	switch(toggle_sl_sprite){
		case 0:
			alt_outfit_mode_string = "Default
			Sprite";
		break;
		case 1:
			alt_outfit_mode_string = "SL 
			Sprite";
		break;
	}
		// Draw partial rectangle Underlay
		...
		switch(toggle_sl_sprite){
			case 0:
				...
				break;
			case 1:
				css_idle_sprite_to_use = sprite_get("sl_idle"); // Set sprite to be the alt idle
				break;
		}
		draw_sprite_ext(css_idle_sprite_to_use,timer/6,x + 165, y + 136,1,1,0,c_white,1);
		// Draw Extra Colors Button
		draw_menu_button(BOX_SHIFT_COLOR_ALTS);
		// Draw Alt Outfit Button
		draw_menu_button(BOX_TOGGLE_SL);
		textDraw(x + 22, y + 106, "tinyFont", c_white, 8, 15, 1, false, 1, "SHOW
		SL
		MODE");
		// Draw Exit Button
		draw_menu_button(BOX_CLOSE_INTERAL_MENU);
	...
	// Draw partial rectangle Underlay
		...
		"Creator: Blaner
		Programmer: Opengunner
		Assisted by TLC Team
