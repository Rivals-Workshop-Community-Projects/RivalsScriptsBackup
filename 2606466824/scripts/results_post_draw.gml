...
if ("incin_initialized_victory_screen" not in self)
{
    incin_initialized_victory_screen = true;
    //defaults to prevent errors
    incin_victory_quote = "ermmmm that wasnt supposed to happen";
    incin_victory_icon = 0;
...
if (winner == player /* && uhc_batteries */)
{
    ...
    var quote_time  =   240;
    //Animate panel
    if ("incin_quote_current_pos_x" not in self)
    { incin_quote_current_pos_x = hide_pos_x; }
    
    //Must check with timing or if result boxes are open
    var diff = ((results_timer > quote_time && !someone_pressed) ? 
                    quote_pos_x : hide_pos_x) - incin_quote_current_pos_x;
    
    incin_quote_current_pos_x += sign(diff) 
                                * max(min(abs(diff), 5), abs(diff) * 0.25);
    
    //Draw panel
    if (incin_quote_current_pos_x > hide_pos_x && (string_length(incin_victory_quote) >= 1))
    {
        draw_sprite(sprite_get("victory_quote_bg"), 0, 
                    incin_quote_current_pos_x, quote_pos_y);
        draw_sprite_ext(sprite_get("stockicon"), incin_victory_icon, 
                    incin_quote_current_pos_x + 46, quote_pos_y + 10, 1.5, 1.5, 0, c_white, 1);
        draw_win_quote(incin_quote_current_pos_x+135, quote_pos_y+15, 
                        incin_victory_quote);
    }
}
...
#define get_random_quote()
{
    var quotes = [];
    var i = 0;
    quotes[i++] = "(Haw haw! I was burning for a nice workout!)";
    quotes[i++] = "(Aw man, I was looking for a challenge. Oh well.)";
    quotes[i++] = "(Is THAT what you call putting up a fight?)";
...
#define get_victory_screen_data()
{
    ...
    with (asset_get("pHitBox")) if ("incin_victory_screen_array" in self)
    {
        data_array = incin_victory_screen_array;
    ...
    if (string_length(data_array[winner].status_quote) > 1)
    {
        ...
        incin_victory_quote = data_array[winner].status_quote;
        incin_victory_icon = data_array[best_player].icon;
    }
    else
    {
        ...
        incin_victory_quote = data_array[best_player].quote;
        incin_victory_icon = data_array[best_player].icon;
        if (string_length(incin_victory_quote) < 1)
        { incin_victory_quote = get_random_quote(); 
          incin_victory_icon = 0;    
        }
    }
