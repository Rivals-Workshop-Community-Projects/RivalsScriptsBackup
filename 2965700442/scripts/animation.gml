...
switch (state) {
    case PS_DASH_STOP:
        dashstop_anim_timer++;
        image_index = ease_linear(0, image_number-1, dashstop_anim_timer, dashstop_anim_frames)
    break;
    case PS_IDLE:
        if (dashstop_anim_timer > 0) {
        	...
        	if (impulse_active) {
        		impulse_idle_change = true;
        		if (state_timer <= 4) {
	                sprite_index = sprite_get("idle_2_trans");
