set_attack_value(AT_NAIR, AG_CATEGORY, 1);
set_attack_value(AT_NAIR, AG_SPRITE, sprite_get("nair"));
set_attack_value(AT_NAIR, AG_NUM_WINDOWS, 3);
set_attack_value(AT_NAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_NAIR, AG_LANDING_LAG, 7); // 4
set_attack_value(AT_NAIR, AG_HURTBOX_SPRITE, sprite_get("nair_hurt"));

set_window_value(AT_NAIR, 1, AG_WINDOW_LENGTH, 12); // 9
set_window_value(AT_NAIR, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_NAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_NAIR, 1, AG_WINDOW_SFX, sound_get("lunge1"));
set_window_value(AT_NAIR, 1, AG_WINDOW_SFX_FRAME, 11);
//set_window_value(AT_NAIR, 1, AG_WINDOW_HAS_SFX, 1);
//set_window_value(AT_NAIR, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy2"));
//set_window_value(AT_NAIR, 1, AG_WINDOW_SFX_FRAME, 9);

set_window_value(AT_NAIR, 2, AG_WINDOW_LENGTH, 12);
set_window_value(AT_NAIR, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_NAIR, 2, AG_WINDOW_ANIM_FRAME_START, 3);


set_window_value(AT_NAIR, 3, AG_WINDOW_LENGTH, 9);
set_window_value(AT_NAIR, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_NAIR, 3, AG_WINDOW_ANIM_FRAME_START, 7);
set_window_value(AT_NAIR, 3, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_NAIR, 4);

set_hitbox_value(AT_NAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_X, 15);
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_Y, -8);
set_hitbox_value(AT_NAIR, 1, HG_WIDTH, 116);
set_hitbox_value(AT_NAIR, 1, HG_HEIGHT, 39);
set_hitbox_value(AT_NAIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_NAIR, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_NAIR, 1, HG_DAMAGE, 2); // 18
set_hitbox_value(AT_NAIR, 1, HG_ANGLE, 270);
set_hitbox_value(AT_NAIR, 1, HG_BASE_KNOCKBACK, 4); // 10
set_hitbox_value(AT_NAIR, 1, HG_KNOCKBACK_SCALING, 0); // 1.0
set_hitbox_value(AT_NAIR, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_NAIR, 1, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_NAIR, 1, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_NAIR, 1, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_GROUP, -1);
//set_hitbox_value(AT_NAIR, 1, HG_VISUAL_EFFECT, hitfx3);

set_hitbox_value(AT_NAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 2, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 2, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_NAIR, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_X, 15);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_Y, -8);
set_hitbox_value(AT_NAIR, 2, HG_WIDTH, 116);
set_hitbox_value(AT_NAIR, 2, HG_HEIGHT, 39);
set_hitbox_value(AT_NAIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_NAIR, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_NAIR, 2, HG_DAMAGE, 2); // 18
set_hitbox_value(AT_NAIR, 2, HG_ANGLE, 270);
set_hitbox_value(AT_NAIR, 2, HG_BASE_KNOCKBACK, 4); // 10
set_hitbox_value(AT_NAIR, 2, HG_KNOCKBACK_SCALING, 0); // 1.0
set_hitbox_value(AT_NAIR, 2, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_NAIR, 2, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_NAIR, 2, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_NAIR, 2, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_GROUP, -1);
//set_hitbox_value(AT_NAIR, 2, HG_VISUAL_EFFECT, hitfx3);

set_hitbox_value(AT_NAIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 3, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 3, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_NAIR, 3, HG_LIFETIME, 2);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_X, 15);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_Y, -8);
set_hitbox_value(AT_NAIR, 3, HG_WIDTH, 116);
set_hitbox_value(AT_NAIR, 3, HG_HEIGHT, 39);
set_hitbox_value(AT_NAIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_NAIR, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_NAIR, 3, HG_DAMAGE, 2); // 18
set_hitbox_value(AT_NAIR, 3, HG_ANGLE, 270);
set_hitbox_value(AT_NAIR, 3, HG_BASE_KNOCKBACK, 4); // 10
set_hitbox_value(AT_NAIR, 3, HG_KNOCKBACK_SCALING, 0); // 1.0
set_hitbox_value(AT_NAIR, 3, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_NAIR, 3, HG_HITPAUSE_SCALING, .3);
set_hitbox_value(AT_NAIR, 3, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_NAIR, 3, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_GROUP, -1);
//set_hitbox_value(AT_NAIR, 3, HG_VISUAL_EFFECT, hitfx3);

set_hitbox_value(AT_NAIR, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 4, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 4, HG_WINDOW_CREATION_FRAME, 9);
set_hitbox_value(AT_NAIR, 4, HG_LIFETIME, 2);
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_X, 15);
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_Y, -8);
set_hitbox_value(AT_NAIR, 4, HG_WIDTH, 116);
set_hitbox_value(AT_NAIR, 4, HG_HEIGHT, 39);
set_hitbox_value(AT_NAIR, 4, HG_SHAPE, 0);
set_hitbox_value(AT_NAIR, 4, HG_PRIORITY, 5);
set_hitbox_value(AT_NAIR, 4, HG_DAMAGE, 4); // 18
set_hitbox_value(AT_NAIR, 4, HG_ANGLE, 50);
set_hitbox_value(AT_NAIR, 4, HG_BASE_KNOCKBACK, 6); // 10
set_hitbox_value(AT_NAIR, 4, HG_KNOCKBACK_SCALING, 0.7); // 1.0
set_hitbox_value(AT_NAIR, 4, HG_BASE_HITPAUSE, 11);
set_hitbox_value(AT_NAIR, 4, HG_HITPAUSE_SCALING, .5);
set_hitbox_value(AT_NAIR, 4, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
//set_hitbox_value(AT_NAIR, 4, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_GROUP, -1);
//set_hitbox_value(AT_NAIR, 4, HG_VISUAL_EFFECT, hitfx2);