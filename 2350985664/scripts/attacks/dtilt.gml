set_attack_value(AT_DTILT, AG_SPRITE, sprite_get("dtilt"));
set_attack_value(AT_DTILT, AG_HURTBOX_SPRITE, sprite_get("dtilt_hurt"));

set_attack_value(AT_DTILT, AG_NUM_WINDOWS, 3);

set_window_value(AT_DTILT, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DTILT, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX, sound_get("throw"));
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX_FRAME, 7);

set_window_value(AT_DTILT, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAME_START, 2);

set_window_value(AT_DTILT, 3, AG_WINDOW_LENGTH, 32);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAME_START, 3);
//set_window_value(AT_DTILT, 3, AG_WINDOW_HAS_WHIFFLAG, 1);

if (!has_rune("E"))
{
    set_num_hitboxes(AT_DTILT,3);
}
else
{
    set_num_hitboxes(AT_DTILT,5);
}

var spd = 4;
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 1, HG_LIFETIME, 999998);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_X, -6);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_Y, -44);
set_hitbox_value(AT_DTILT, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 1, HG_WIDTH, 26);
set_hitbox_value(AT_DTILT, 1, HG_HEIGHT, 26);
set_hitbox_value(AT_DTILT, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE, 85);
set_hitbox_value(AT_DTILT, 1, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_DTILT, 1, HG_KNOCKBACK_SCALING, 0.2);
set_hitbox_value(AT_DTILT, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 1, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_HSPEED, lengthdir_x(spd,70));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_VSPEED, lengthdir_y(spd,70));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_GRAVITY, 0.2);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_AIR_FRICTION, 0.01);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT, 303 );
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_crunch"));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_SPRITE, sprite_get("lamprey_spin"));
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_ANIM_SPEED, 0.1);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_DESTROY_EFFECT, lamprey_destroy);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_GROUND_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_ENEMY_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 1, HG_HIT_LOCKOUT, 3);
set_hitbox_value(AT_DTILT, 1, HG_PROJECTILE_PARRY_STUN, true);

set_hitbox_value(AT_DTILT, 2, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 2, HG_LIFETIME, 999998);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_X, 4);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_Y, -38);
set_hitbox_value(AT_DTILT, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 2, HG_WIDTH, 26);
set_hitbox_value(AT_DTILT, 2, HG_HEIGHT, 26);
set_hitbox_value(AT_DTILT, 2, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE, 85);
set_hitbox_value(AT_DTILT, 2, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_DTILT, 2, HG_KNOCKBACK_SCALING, 0.2);
set_hitbox_value(AT_DTILT, 2, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 2, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_HSPEED, lengthdir_x(spd,50));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_VSPEED, lengthdir_y(spd,50));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_GRAVITY, 0.2);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_AIR_FRICTION, 0.01);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT, 303 );
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, asset_get("sfx_crunch"));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_SPRITE, sprite_get("lamprey_spin"));
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_ANIM_SPEED, 0.1);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_DESTROY_EFFECT, lamprey_destroy);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_GROUND_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_ENEMY_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 2, HG_HIT_LOCKOUT, 3);
set_hitbox_value(AT_DTILT, 2, HG_PROJECTILE_PARRY_STUN, true);

set_hitbox_value(AT_DTILT, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 3, HG_LIFETIME, 999998);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_X, 10);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_Y, -32);
set_hitbox_value(AT_DTILT, 3, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 3, HG_WIDTH, 26);
set_hitbox_value(AT_DTILT, 3, HG_HEIGHT, 26);
set_hitbox_value(AT_DTILT, 3, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 3, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE, 85);
set_hitbox_value(AT_DTILT, 3, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_DTILT, 3, HG_KNOCKBACK_SCALING, 0.2);
set_hitbox_value(AT_DTILT, 3, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 3, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_HSPEED, lengthdir_x(spd,25));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_VSPEED, lengthdir_y(spd,25));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_GRAVITY, 0.2);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_AIR_FRICTION, 0.01);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT, 303 );
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_crunch"));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_SPRITE, sprite_get("lamprey_spin"));
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_ANIM_SPEED, 0.1);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_DESTROY_EFFECT, lamprey_destroy);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_GROUND_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_ENEMY_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 3, HG_HIT_LOCKOUT, 3);
set_hitbox_value(AT_DTILT, 3, HG_PROJECTILE_PARRY_STUN, true);

// rune A only 
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 4, HG_LIFETIME, 999998);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_X, -1);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_Y, -39);
set_hitbox_value(AT_DTILT, 4, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 4, HG_WIDTH, 26);
set_hitbox_value(AT_DTILT, 4, HG_HEIGHT, 26);
set_hitbox_value(AT_DTILT, 4, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 4, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 4, HG_ANGLE, 85);
set_hitbox_value(AT_DTILT, 4, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_DTILT, 4, HG_KNOCKBACK_SCALING, 0.2);
set_hitbox_value(AT_DTILT, 4, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 4, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_HSPEED, lengthdir_x(spd,60));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_VSPEED, lengthdir_y(spd,60));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_GRAVITY, 0.2);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_AIR_FRICTION, 0.01);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT, 303 );
set_hitbox_value(AT_DTILT, 4, HG_HIT_SFX, asset_get("sfx_crunch"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_SPRITE, sprite_get("lamprey_spin"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_ANIM_SPEED, 0.1);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_DESTROY_EFFECT, lamprey_destroy);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_GROUND_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_ENEMY_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 4, HG_HIT_LOCKOUT, 3);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_PARRY_STUN, true);

set_hitbox_value(AT_DTILT, 5, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 5, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 5, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 5, HG_LIFETIME, 999998);
set_hitbox_value(AT_DTILT, 5, HG_HITBOX_X, 7);
set_hitbox_value(AT_DTILT, 5, HG_HITBOX_Y, -34);
set_hitbox_value(AT_DTILT, 5, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 5, HG_WIDTH, 26);
set_hitbox_value(AT_DTILT, 5, HG_HEIGHT, 26);
set_hitbox_value(AT_DTILT, 5, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 5, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 5, HG_ANGLE, 85);
set_hitbox_value(AT_DTILT, 5, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_DTILT, 5, HG_KNOCKBACK_SCALING, 0.2);
set_hitbox_value(AT_DTILT, 5, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 5, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_HSPEED, lengthdir_x(spd,37));
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_VSPEED, lengthdir_y(spd,37));
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_GRAVITY, 0.2);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_AIR_FRICTION, 0.01);
set_hitbox_value(AT_DTILT, 5, HG_VISUAL_EFFECT, 303 );
set_hitbox_value(AT_DTILT, 5, HG_HIT_SFX, asset_get("sfx_crunch"));
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_SPRITE, sprite_get("lamprey_spin"));
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_ANIM_SPEED, 0.1);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_DESTROY_EFFECT, lamprey_destroy);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_WALL_BEHAVIOR, 2);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_GROUND_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_ENEMY_BEHAVIOR, 0);
set_hitbox_value(AT_DTILT, 5, HG_HIT_LOCKOUT, 3);
set_hitbox_value(AT_DTILT, 5, HG_PROJECTILE_PARRY_STUN, true);








