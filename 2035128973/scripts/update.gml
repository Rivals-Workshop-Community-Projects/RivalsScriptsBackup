if (get_synced_var(player) == 15005)
{
	...
	switch (state)
	{
		...
		case PS_PARRY:
			if (state_timer == 0) sound_play(sound_get("shoulder_press"));
			break;
		case PS_DASH_START:
		case PS_DASH_TURN:
			if (state_timer == 0) sound_play(sound_get(spr_dir==1?"stick_tap_r":"stick_tap_l"));
			break;
		case PS_DASH_STOP:
			if (state_timer == 3) sound_play(sound_get("stick_release"));
			break;
		case PS_AIR_DODGE:
			if (state_timer == 3) sound_play(sound_get("vc_lucario_006"));
			break;
		case PS_JUMPSQUAT:
			if (state_timer == 0) sound_play(sound_get("vc_lucario_jump01"));
			break;
		case PS_ATTACK_GROUND:
		case PS_ATTACK_AIR:
			if (state_timer == 0) sound_play(sound_get((attack == AT_JAB || attack == AT_NSPECIAL)?"button_x":"c_stick_flick"))
			break;
	}
}
else
