...
switch (my_hitboxID.attack)
{
    ...
    case AT_USTRONG: //strong charge increasing height
        ...
        break;
    case AT_FSTRONG:
        if (hit_player_obj.state == PS_HITSTUN && hit_player_obj.y < my_hitboxID.y && my_hitboxID.hbox_num != 4)
        {
            hit_player_obj.old_vsp = min(abs(hit_player_obj.y - my_hitboxID.y), my_hitboxID.kb_value/3);
        }
        break;
    case AT_FAIR: //single input multihit
        if (my_hitboxID.hbox_num == 2 && !fast_falling) old_vsp = -2;
    ...
    case AT_FSPECIAL: //tether
        if (my_hitboxID.hbox_num == 2)
        {
            ...
            has_hit = false; //since the tether detects has_hit for it stop tethering, i decided to turn it off just in case
        }
        else my_grab_id = noone; //hbox_num 1 is the collision hitbox, it should let go of grabbed players
        break;
}

//applying custom status code
...
//custom hitbox grid values setup
...
#define set_grab_id
{
    ...
    //  - if the hit object is actually a player or not
