set_attack_value(AT_UTILT, AG_CATEGORY, 0); 
set_attack_value(AT_UTILT, AG_SPRITE, sprite_get("utilt"));
set_attack_value(AT_UTILT, AG_NUM_WINDOWS, 6);
set_attack_value(AT_UTILT, AG_HURTBOX_SPRITE, sprite_get("utilt_hurt"));
set_attack_value(AT_UTILT, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_UTILT, AG_LANDING_LAG, 5);
set_attack_value(AT_UTILT, AG_OFF_LEDGE, 0);

//startup
set_window_value(AT_UTILT, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTILT, 1, AG_WINDOW_LENGTH, 10);
set_window_value(AT_UTILT, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_UTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UTILT, 1, AG_WINDOW_SFX, sound_get("flame_by_AslakHostaker_edited"));
set_window_value(AT_UTILT, 1, AG_WINDOW_SFX_FRAME, 9);
set_window_value(AT_UTILT, 1, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_UTILT, 1, AG_WINDOW_CUSTOM_GROUND_FRICTION, 0.05);

//spawn_article
set_window_value(AT_UTILT, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTILT, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_UTILT, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_UTILT, 2, AG_WINDOW_ANIM_FRAME_START, 3);
//set_window_value(AT_UTILT, 2, AG_WINDOW_HAS_SFX, 1);
//set_window_value(AT_UTILT, 2, AG_WINDOW_SFX, sound_get("flame_by_AslakHostaker_edited"));

//pre-hitbox
set_window_value(AT_UTILT, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTILT, 3, AG_WINDOW_LENGTH, 1);
set_window_value(AT_UTILT, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_UTILT, 3, AG_WINDOW_ANIM_FRAME_START, 3);

//active
set_window_value(AT_UTILT, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTILT, 4, AG_WINDOW_LENGTH, 10);
set_window_value(AT_UTILT, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_UTILT, 4, AG_WINDOW_ANIM_FRAME_START, 3);

//recovery 1
set_window_value(AT_UTILT, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTILT, 5, AG_WINDOW_LENGTH, 6);
set_window_value(AT_UTILT, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_UTILT, 5, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_UTILT, 5, AG_WINDOW_HAS_WHIFFLAG, 1);

//recovery 2
set_window_value(AT_UTILT, 6, AG_WINDOW_TYPE, 1);
set_window_value(AT_UTILT, 6, AG_WINDOW_LENGTH, 9);
set_window_value(AT_UTILT, 6, AG_WINDOW_ANIM_FRAMES, -2);
set_window_value(AT_UTILT, 6, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_UTILT, 6, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_UTILT,4);
//main hitbox
set_hitbox_value(AT_UTILT, 1, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_UTILT, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_UTILT, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 1, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 1, HG_LIFETIME, 10);
set_hitbox_value(AT_UTILT, 1, HG_HITBOX_X, 40);
set_hitbox_value(AT_UTILT, 1, HG_HITBOX_Y, -60);
set_hitbox_value(AT_UTILT, 1, HG_WIDTH, 42);
set_hitbox_value(AT_UTILT, 1, HG_HEIGHT, 42);
set_hitbox_value(AT_UTILT, 1, HG_PRIORITY, 7);
set_hitbox_value(AT_UTILT, 1, HG_DAMAGE, 1);
set_hitbox_value(AT_UTILT, 1, HG_ANGLE, 65);
set_hitbox_value(AT_UTILT, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_UTILT, 1, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_UTILT, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_UTILT, 1, HG_HITPAUSE_SCALING, .25);
set_hitbox_value(AT_UTILT, 1, HG_HIT_SFX, asset_get("sfx_zetter_fireball_fire"));
set_hitbox_value(AT_UTILT, 1, HG_VISUAL_EFFECT, 3);
set_hitbox_value(AT_UTILT, 1, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_UTILT, 1, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_UTILT, 1, HG_FORCE_FLINCH, 2); //cannot flinch

//late hitboxes
set_hitbox_value(AT_UTILT, 2, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_UTILT, 2, HG_HITBOX_TYPE, 2); //projectile
set_hitbox_value(AT_UTILT, 2, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 2, HG_LIFETIME, 10);
set_hitbox_value(AT_UTILT, 2, HG_HITBOX_X, 55);
set_hitbox_value(AT_UTILT, 2, HG_HITBOX_Y, -75); 
set_hitbox_value(AT_UTILT, 2, HG_WIDTH, 36);
set_hitbox_value(AT_UTILT, 2, HG_HEIGHT, 36);
set_hitbox_value(AT_UTILT, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_UTILT, 2, HG_DAMAGE, 1);
set_hitbox_value(AT_UTILT, 2, HG_ANGLE, 70);
set_hitbox_value(AT_UTILT, 2, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_UTILT, 2, HG_KNOCKBACK_SCALING, .3);
set_hitbox_value(AT_UTILT, 2, HG_BASE_HITPAUSE, 1);
set_hitbox_value(AT_UTILT, 2, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_UTILT, 2, HG_HITSTUN_MULTIPLIER, 1.5); //has very low hitstun normally...
//set_hitbox_value(AT_UTILT, 2, HG_VISUAL_EFFECT_Y_OFFSET, -16);
set_hitbox_value(AT_UTILT, 2, HG_HIT_SFX, asset_get("sfx_zetter_upb_hit"));
set_hitbox_value(AT_UTILT, 2, HG_VISUAL_EFFECT, 4);
set_hitbox_value(AT_UTILT, 2, HG_HIT_LOCKOUT, 1);
set_hitbox_value(AT_UTILT, 2, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_UTILT, 2, HG_FORCE_FLINCH, 2); //cannot flinch
set_hitbox_value(AT_UTILT, 2, HG_EFFECT, 9); //polite

set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
//set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_DESTROY_EFFECT, 193); //rock small

set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_WALL_BEHAVIOR, 1 ); //ignore walls
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_GROUND_BEHAVIOR, 1 ); //ignore ground
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_ENEMY_BEHAVIOR, 1 ); //ignore enemies
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_UNBASHABLE, 1 );
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_PARRY_STUN, 1 );
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_DOES_NOT_REFLECT, 1 );
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_IS_TRANSCENDENT, 0 );
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_HSPEED, 0);
set_hitbox_value(AT_UTILT, 2, HG_PROJECTILE_VSPEED, 0);

set_hitbox_value(AT_UTILT, 3, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_UTILT, 3, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_UTILT, 3, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_UTILT, 3, HG_HITBOX_X, 70);
set_hitbox_value(AT_UTILT, 3, HG_HITBOX_Y, -90);

//early hitbox
set_hitbox_value(AT_UTILT, 4, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_GROUP, 4);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 4, HG_WINDOW, 2);
set_hitbox_value(AT_UTILT, 4, HG_LIFETIME, 2);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_X, 20);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_Y, -40);
set_hitbox_value(AT_UTILT, 4, HG_WIDTH, 46);
set_hitbox_value(AT_UTILT, 4, HG_HEIGHT, 46);
set_hitbox_value(AT_UTILT, 4, HG_PRIORITY, 7);
set_hitbox_value(AT_UTILT, 4, HG_DAMAGE, 3);
set_hitbox_value(AT_UTILT, 4, HG_ANGLE, 60);
set_hitbox_value(AT_UTILT, 4, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_UTILT, 4, HG_KNOCKBACK_SCALING, 0.5);
set_hitbox_value(AT_UTILT, 4, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_UTILT, 4, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_UTILT, 4, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_UTILT, 4, HG_VISUAL_EFFECT, 3);
set_hitbox_value(AT_UTILT, 4, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_UTILT, 4, HG_SDI_MULTIPLIER, 0.5);
set_hitbox_value(AT_UTILT, 4, HG_FORCE_FLINCH, 2); //cannot flinch

//final hitbox
set_hitbox_value(AT_UTILT, 5, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_UTILT, 5, HG_HITBOX_TYPE, 2); //projectile
set_hitbox_value(AT_UTILT, 5, HG_WINDOW, 10);
set_hitbox_value(AT_UTILT, 5, HG_LIFETIME, 8);
set_hitbox_value(AT_UTILT, 5, HG_HITBOX_X, 70);
set_hitbox_value(AT_UTILT, 5, HG_HITBOX_Y, -90); 
set_hitbox_value(AT_UTILT, 5, HG_WIDTH, 64);
set_hitbox_value(AT_UTILT, 5, HG_HEIGHT, 64);
set_hitbox_value(AT_UTILT, 5, HG_PRIORITY, 9);
set_hitbox_value(AT_UTILT, 5, HG_DAMAGE, 2);
set_hitbox_value(AT_UTILT, 5, HG_ANGLE, 55);
set_hitbox_value(AT_UTILT, 5, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_UTILT, 5, HG_KNOCKBACK_SCALING, 0.85);
set_hitbox_value(AT_UTILT, 5, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UTILT, 5, HG_HITPAUSE_SCALING, .7);
set_hitbox_value(AT_UTILT, 5, HG_HITSTUN_MULTIPLIER, 1); 
set_hitbox_value(AT_UTILT, 5, HG_HIT_SFX, asset_get("sfx_zetter_upb_hit"));
set_hitbox_value(AT_UTILT, 5, HG_VISUAL_EFFECT, 4);
set_hitbox_value(AT_UTILT, 5, HG_HIT_LOCKOUT, 1);
set_hitbox_value(AT_UTILT, 5, HG_EFFECT, 9); //polite hitbox
set_hitbox_value(AT_UTILT, 5, HG_GROUNDEDNESS, 2); //aerial only
set_hitbox_value(AT_UTILT, 5, HG_FORCE_FLINCH, 2); //cannot flinch

set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_SPRITE, asset_get("empty_sprite"));
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_MASK, -1);
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_WALL_BEHAVIOR, 1 ); //ignore walls
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_GROUND_BEHAVIOR, 1 ); //ignore ground
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_ENEMY_BEHAVIOR, 1 ); //ignore enemies
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_UNBASHABLE, 1 );
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_PARRY_STUN, 1 );
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_DOES_NOT_REFLECT, 1 );
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_IS_TRANSCENDENT, 0 );
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_HSPEED, 0);
set_hitbox_value(AT_UTILT, 5, HG_PROJECTILE_VSPEED, 0);

