//Ike
set_attack_value(AT_UAIR, AG_CATEGORY, 1);
set_attack_value(AT_UAIR, AG_SPRITE, sprite_get("uair"));
set_attack_value(AT_UAIR, AG_NUM_WINDOWS, 1);
set_attack_value(AT_UAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_UAIR, AG_LANDING_LAG, 7);
set_attack_value(AT_UAIR, AG_HURTBOX_SPRITE, sprite_get("uair_hurt"));

set_window_value(AT_UAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_UAIR, 1, AG_WINDOW_LENGTH, 24);
set_window_value(AT_UAIR, 1, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_UAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UAIR, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_medium1"));

set_num_hitboxes(AT_UAIR, 5);

set_hitbox_value(AT_UAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_UAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UAIR, 1, HG_WINDOW, 1);
set_hitbox_value(AT_UAIR, 1, HG_WINDOW_CREATION_FRAME, 12);
set_hitbox_value(AT_UAIR, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_UAIR, 1, HG_HITBOX_X, -59);
set_hitbox_value(AT_UAIR, 1, HG_HITBOX_Y, -51);
set_hitbox_value(AT_UAIR, 1, HG_WIDTH, 56);
set_hitbox_value(AT_UAIR, 1, HG_HEIGHT, 56);
set_hitbox_value(AT_UAIR, 1, HG_PRIORITY, 6);
set_hitbox_value(AT_UAIR, 1, HG_DAMAGE, 6);
set_hitbox_value(AT_UAIR, 1, HG_ANGLE, 90);
set_hitbox_value(AT_UAIR, 1, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_UAIR, 1, HG_KNOCKBACK_SCALING, .65);
set_hitbox_value(AT_UAIR, 1, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UAIR, 1, HG_HITPAUSE_SCALING, .45);
set_hitbox_value(AT_UAIR, 1, HG_HIT_SFX, sound_get("slash1"));
set_hitbox_value(AT_UAIR, 1, HG_VISUAL_EFFECT, 6);
set_hitbox_value(AT_UAIR, 1, HG_DRIFT_MULTIPLIER, 0.4);

set_hitbox_value(AT_UAIR, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_UAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UAIR, 2, HG_WINDOW, 1);
set_hitbox_value(AT_UAIR, 2, HG_WINDOW_CREATION_FRAME, 12);
set_hitbox_value(AT_UAIR, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_UAIR, 2, HG_HITBOX_X, -42);
set_hitbox_value(AT_UAIR, 2, HG_HITBOX_Y, -83);
set_hitbox_value(AT_UAIR, 2, HG_WIDTH, 66);
set_hitbox_value(AT_UAIR, 2, HG_HEIGHT, 50);
set_hitbox_value(AT_UAIR, 2, HG_PRIORITY, 8);
set_hitbox_value(AT_UAIR, 2, HG_DAMAGE, 7);
set_hitbox_value(AT_UAIR, 2, HG_ANGLE, 90);
set_hitbox_value(AT_UAIR, 2, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_UAIR, 2, HG_KNOCKBACK_SCALING, .65);
set_hitbox_value(AT_UAIR, 2, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UAIR, 2, HG_HITPAUSE_SCALING, .45);
set_hitbox_value(AT_UAIR, 2, HG_HIT_SFX, sound_get("slash1"));
set_hitbox_value(AT_UAIR, 2, HG_VISUAL_EFFECT, 150);
set_hitbox_value(AT_UAIR, 2, HG_DRIFT_MULTIPLIER, 0.2);

set_hitbox_value(AT_UAIR, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_UAIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UAIR, 3, HG_WINDOW, 1);
set_hitbox_value(AT_UAIR, 3, HG_SHAPE, 2);
set_hitbox_value(AT_UAIR, 3, HG_WINDOW_CREATION_FRAME, 12);
set_hitbox_value(AT_UAIR, 3, HG_LIFETIME, 4);
set_hitbox_value(AT_UAIR, 3, HG_HITBOX_X, 2);
set_hitbox_value(AT_UAIR, 3, HG_HITBOX_Y, -94);
set_hitbox_value(AT_UAIR, 3, HG_WIDTH, 78);
set_hitbox_value(AT_UAIR, 3, HG_HEIGHT, 51);
set_hitbox_value(AT_UAIR, 3, HG_PRIORITY, 9);
set_hitbox_value(AT_UAIR, 3, HG_DAMAGE, 8);
set_hitbox_value(AT_UAIR, 3, HG_ANGLE, 90);
set_hitbox_value(AT_UAIR, 3, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_UAIR, 3, HG_KNOCKBACK_SCALING, .65);
set_hitbox_value(AT_UAIR, 3, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UAIR, 3, HG_HITPAUSE_SCALING, .45);
set_hitbox_value(AT_UAIR, 3, HG_HIT_SFX, sound_get("slash1"));
set_hitbox_value(AT_UAIR, 3, HG_VISUAL_EFFECT, 195);
set_hitbox_value(AT_UAIR, 3, HG_DRIFT_MULTIPLIER, 0.05);

set_hitbox_value(AT_UAIR, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_UAIR, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UAIR, 4, HG_WINDOW, 1);
set_hitbox_value(AT_UAIR, 4, HG_WINDOW_CREATION_FRAME, 12);
set_hitbox_value(AT_UAIR, 4, HG_LIFETIME, 4);
set_hitbox_value(AT_UAIR, 4, HG_HITBOX_X, 47);
set_hitbox_value(AT_UAIR, 4, HG_HITBOX_Y, -80);
set_hitbox_value(AT_UAIR, 4, HG_WIDTH, 49);
set_hitbox_value(AT_UAIR, 4, HG_HEIGHT, 49);
set_hitbox_value(AT_UAIR, 4, HG_PRIORITY, 8);
set_hitbox_value(AT_UAIR, 4, HG_DAMAGE, 7);
set_hitbox_value(AT_UAIR, 4, HG_ANGLE, 90);
set_hitbox_value(AT_UAIR, 4, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_UAIR, 4, HG_KNOCKBACK_SCALING, .65);
set_hitbox_value(AT_UAIR, 4, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UAIR, 4, HG_HITPAUSE_SCALING, .45);
set_hitbox_value(AT_UAIR, 4, HG_HIT_SFX, sound_get("slash1"));
set_hitbox_value(AT_UAIR, 4, HG_VISUAL_EFFECT, 150);
set_hitbox_value(AT_UAIR, 4, HG_DRIFT_MULTIPLIER, 0.2);

set_hitbox_value(AT_UAIR, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_UAIR, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UAIR, 5, HG_WINDOW, 1);
set_hitbox_value(AT_UAIR, 5, HG_WINDOW_CREATION_FRAME, 12);
set_hitbox_value(AT_UAIR, 5, HG_LIFETIME, 4);
set_hitbox_value(AT_UAIR, 5, HG_HITBOX_X, 58);
set_hitbox_value(AT_UAIR, 5, HG_HITBOX_Y, -43);
set_hitbox_value(AT_UAIR, 5, HG_WIDTH, 31);
set_hitbox_value(AT_UAIR, 5, HG_HEIGHT, 60);
set_hitbox_value(AT_UAIR, 5, HG_PRIORITY, 8);
set_hitbox_value(AT_UAIR, 5, HG_DAMAGE, 6);
set_hitbox_value(AT_UAIR, 5, HG_ANGLE, 90);
set_hitbox_value(AT_UAIR, 5, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_UAIR, 5, HG_KNOCKBACK_SCALING, .65);
set_hitbox_value(AT_UAIR, 5, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_UAIR, 5, HG_HITPAUSE_SCALING, .45);
set_hitbox_value(AT_UAIR, 5, HG_HIT_SFX, sound_get("slash1"));
set_hitbox_value(AT_UAIR, 5, HG_VISUAL_EFFECT, 6);
set_hitbox_value(AT_UAIR, 5, HG_DRIFT_MULTIPLIER, 0.4);