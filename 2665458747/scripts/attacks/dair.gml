    set_attack_value(AT_DAIR, AG_CATEGORY, 2);
set_attack_value(AT_DAIR, AG_SPRITE, sprite_get("dair"));
set_attack_value(AT_DAIR, AG_NUM_WINDOWS, 5);
set_attack_value(AT_DAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_DAIR, AG_LANDING_LAG, 12);
set_attack_value(AT_DAIR, AG_HURTBOX_SPRITE, sprite_get("dair_hurt"));

set_window_value(AT_DAIR, 1, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX, sound_get("sm-spin"));
set_window_value(AT_DAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX_FRAME, 9);

set_window_value(AT_DAIR, 2, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAMES, 1);

set_window_value(AT_DAIR, 3, AG_WINDOW_LENGTH, 5);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAMES, 2);

set_window_value(AT_DAIR, 4, AG_WINDOW_LENGTH, 5);
set_window_value(AT_DAIR, 4, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DAIR, 4, AG_WINDOW_ANIM_FRAMES, 2);

set_window_value(AT_DAIR, 5, AG_WINDOW_LENGTH, 13);
set_window_value(AT_DAIR, 5, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_DAIR, 5, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DAIR, 5, AG_WINDOW_HAS_WHIFFLAG, 1);

set_window_value(AT_DAIR, 6, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DAIR, 6, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_DAIR, 6, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 6, AG_WINDOW_SFX, asset_get("sfx_swipe_medium2"));
set_window_value(AT_DAIR, 6, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 6, AG_WINDOW_SFX_FRAME, 2);

set_window_value(AT_DAIR, 7, AG_WINDOW_LENGTH, 5);
set_window_value(AT_DAIR, 7, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_DAIR, 7, AG_WINDOW_ANIM_FRAMES, 1);

set_window_value(AT_DAIR, 8, AG_WINDOW_LENGTH, 13);
set_window_value(AT_DAIR, 8, AG_WINDOW_ANIM_FRAME_START, 10);
set_window_value(AT_DAIR, 8, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 8, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_DAIR, 4);

set_hitbox_value(AT_DAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DAIR, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_X, 15);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_Y, -5);
set_hitbox_value(AT_DAIR, 1, HG_WIDTH, 65);
set_hitbox_value(AT_DAIR, 1, HG_HEIGHT, 65);
set_hitbox_value(AT_DAIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_DAIR, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_DAIR, 1, HG_ANGLE, 90);
set_hitbox_value(AT_DAIR, 1, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_DAIR, 1, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 1, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, fighthit);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DAIR, 1, HG_EXTRA_HITPAUSE, 2);
set_hitbox_value(AT_DAIR, 1, HG_HIT_SFX, sound_get("dp-hit"));
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_GROUP, 0);

set_hitbox_value(AT_DAIR, 2, HG_WINDOW, 3);
set_hitbox_value(AT_DAIR, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_X, 15);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_Y, -5);
set_hitbox_value(AT_DAIR, 2, HG_WIDTH, 65);
set_hitbox_value(AT_DAIR, 2, HG_HEIGHT, 65);
set_hitbox_value(AT_DAIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_DAIR, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_DAIR, 2, HG_ANGLE, 90);
set_hitbox_value(AT_DAIR, 2, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_DAIR, 2, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 2, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT, fighthit);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 2, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DAIR, 2, HG_EXTRA_HITPAUSE, 2);
set_hitbox_value(AT_DAIR, 2, HG_HIT_SFX, sound_get("dp-hit"));
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_DAIR, 3, HG_WINDOW, 4);
set_hitbox_value(AT_DAIR, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_X, 20);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_Y, -0);
set_hitbox_value(AT_DAIR, 3, HG_WIDTH, 60);
set_hitbox_value(AT_DAIR, 3, HG_HEIGHT, 60);
set_hitbox_value(AT_DAIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_DAIR, 3, HG_DAMAGE, 3);
set_hitbox_value(AT_DAIR, 3, HG_ANGLE, 290);
set_hitbox_value(AT_DAIR, 3, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 3, HG_KNOCKBACK_SCALING, .25);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT, fightinghit);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 3, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DAIR, 3, HG_HITPAUSE_SCALING, .25);
set_hitbox_value(AT_DAIR, 3, HG_HIT_SFX, sound_get("dp-hit2"));
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_DAIR, 4, HG_WINDOW, 7);
set_hitbox_value(AT_DAIR, 4, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 4, HG_LIFETIME, 4);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_X, 0);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DAIR, 4, HG_WIDTH, 90);
set_hitbox_value(AT_DAIR, 4, HG_HEIGHT, 40);
set_hitbox_value(AT_DAIR, 4, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_DAIR, 4, HG_DAMAGE, 3);
set_hitbox_value(AT_DAIR, 4, HG_ANGLE, 85);
set_hitbox_value(AT_DAIR, 4, HG_BASE_KNOCKBACK, 11);
set_hitbox_value(AT_DAIR, 4, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT, fightinghit);
set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 4, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DAIR, 4, HG_HITPAUSE_SCALING, .25);
set_hitbox_value(AT_DAIR, 4, HG_HIT_SFX, sound_get("dp-hit2"));
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_GROUP, 3);

set_hitbox_value(AT_DAIR, 5, HG_WINDOW, 2);
set_hitbox_value(AT_DAIR, 5, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 5, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 5, HG_HITBOX_X, 32);
set_hitbox_value(AT_DAIR, 5, HG_HITBOX_Y, 17);
set_hitbox_value(AT_DAIR, 5, HG_WIDTH, 35);
set_hitbox_value(AT_DAIR, 5, HG_HEIGHT, 35);
set_hitbox_value(AT_DAIR, 5, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 5, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 5, HG_DAMAGE, 3);
set_hitbox_value(AT_DAIR, 5, HG_ANGLE, 90);
set_hitbox_value(AT_DAIR, 5, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_DAIR, 5, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 5, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 5, HG_VISUAL_EFFECT, poisonjabhit);
set_hitbox_value(AT_DAIR, 5, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 5, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 5, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DAIR, 5, HG_EXTRA_HITPAUSE, 4);
set_hitbox_value(AT_DAIR, 5, HG_HIT_SFX, sound_get("sm-tipper"));
set_hitbox_value(AT_DAIR, 5, HG_HITBOX_GROUP, 0);

set_hitbox_value(AT_DAIR, 6, HG_WINDOW, 3);
set_hitbox_value(AT_DAIR, 6, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 6, HG_LIFETIME, 3);
set_hitbox_value(AT_DAIR, 6, HG_HITBOX_X, 30);
set_hitbox_value(AT_DAIR, 6, HG_HITBOX_Y, 15);
set_hitbox_value(AT_DAIR, 6, HG_WIDTH, 35);
set_hitbox_value(AT_DAIR, 6, HG_HEIGHT, 35);
set_hitbox_value(AT_DAIR, 6, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 6, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 6, HG_DAMAGE, 3);
set_hitbox_value(AT_DAIR, 6, HG_ANGLE, 90);
set_hitbox_value(AT_DAIR, 6, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_DAIR, 6, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 6, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 6, HG_VISUAL_EFFECT, poisonjabhit);
set_hitbox_value(AT_DAIR, 6, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 6, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 6, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DAIR, 6, HG_EXTRA_HITPAUSE, 4);
set_hitbox_value(AT_DAIR, 6, HG_HIT_SFX, sound_get("sm-tipper"));
set_hitbox_value(AT_DAIR, 6, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_DAIR, 7, HG_WINDOW, 4);
set_hitbox_value(AT_DAIR, 7, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 7, HG_LIFETIME, 3);
set_hitbox_value(AT_DAIR, 7, HG_HITBOX_X, 30);
set_hitbox_value(AT_DAIR, 7, HG_HITBOX_Y, 15);
set_hitbox_value(AT_DAIR, 7, HG_WIDTH, 35);
set_hitbox_value(AT_DAIR, 7, HG_HEIGHT, 35);
set_hitbox_value(AT_DAIR, 7, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 7, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 7, HG_DAMAGE, 5);
set_hitbox_value(AT_DAIR, 7, HG_ANGLE, 300);
set_hitbox_value(AT_DAIR, 7, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_DAIR, 7, HG_KNOCKBACK_SCALING, .4);
set_hitbox_value(AT_DAIR, 7, HG_VISUAL_EFFECT, poisonjabhit);
set_hitbox_value(AT_DAIR, 7, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 7, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 7, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_DAIR, 7, HG_HITPAUSE_SCALING, .5);
set_hitbox_value(AT_DAIR, 7, HG_HIT_SFX, sound_get("sm-tipper"));
set_hitbox_value(AT_DAIR, 7, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_DAIR, 8, HG_WINDOW, 7);
set_hitbox_value(AT_DAIR, 8, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DAIR, 8, HG_LIFETIME, 4);
set_hitbox_value(AT_DAIR, 8, HG_HITBOX_X, 3);
set_hitbox_value(AT_DAIR, 8, HG_HITBOX_Y, -25);
set_hitbox_value(AT_DAIR, 8, HG_WIDTH, 110);
set_hitbox_value(AT_DAIR, 8, HG_HEIGHT, 50);
set_hitbox_value(AT_DAIR, 8, HG_SHAPE, 2);
set_hitbox_value(AT_DAIR, 8, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 8, HG_DAMAGE, 4);
set_hitbox_value(AT_DAIR, 8, HG_ANGLE, 85);
set_hitbox_value(AT_DAIR, 8, HG_BASE_KNOCKBACK, 11);
set_hitbox_value(AT_DAIR, 8, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_DAIR, 8, HG_VISUAL_EFFECT, poisonjabhit);
set_hitbox_value(AT_DAIR, 8, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DAIR, 8, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DAIR, 8, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_DAIR, 8, HG_HITPAUSE_SCALING, .25);
set_hitbox_value(AT_DAIR, 8, HG_HIT_SFX, sound_get("sm-tipper"));
set_hitbox_value(AT_DAIR, 8, HG_HITBOX_GROUP, 3);