...
if ("geno_initialized_victory_screen" not in self)
{
    geno_initialized_victory_screen = true;
    //defaults to prevent errors
    geno_victory_quote = "ermmmm that wasnt supposed to happen";
    geno_victory_icon = 0;
...
if (winner == player /* && uhc_batteries */)
{
    ...
    var quote_time  =   240;
    //Animate panel
    if ("geno_quote_current_pos_x" not in self)
    { geno_quote_current_pos_x = hide_pos_x; }
    
    //Must check with timing or if result boxes are open
    var diff = ((results_timer > quote_time && !someone_pressed) ? 
                    quote_pos_x : hide_pos_x) - geno_quote_current_pos_x;
    
    geno_quote_current_pos_x += sign(diff) 
                                * max(min(abs(diff), 5), abs(diff) * 0.25);
    
    //Draw panel
    if (geno_quote_current_pos_x > hide_pos_x && (string_length(geno_victory_quote) >= 1))
    {
        draw_sprite(sprite_get("victory_quote_bg"), 0, 
                    geno_quote_current_pos_x, quote_pos_y);
        draw_sprite_ext(sprite_get("stockicon"), geno_victory_icon, 
                    geno_quote_current_pos_x + 46, quote_pos_y + 10, 1.5, 1.5, 0, c_white, 1);
        draw_win_quote(geno_quote_current_pos_x+135, quote_pos_y+15, 
                        geno_victory_quote);
    }
}
...
#define get_random_quote()
{
    var quotes = [];
    var i = 0;
    quotes[i++] = "I cannot stay. My power is needed elsewhere.";
    quotes[i++] = "Where will my mission take me next?";
    quotes[i++] = "My journey will not stop here.";
...
#define get_victory_screen_data()
{
    ...
    with (asset_get("pHitBox")) if ("geno_victory_screen_array" in self)
    {
        data_array = geno_victory_screen_array;
    ...
    if (string_length(data_array[winner].status_quote) > 1)
    {
        ...
        geno_victory_quote = data_array[winner].status_quote;
        geno_victory_icon = data_array[best_player].icon;
    }
    else
    {
        ...
        geno_victory_quote = data_array[best_player].quote;
        geno_victory_icon = data_array[best_player].icon;
        if (string_length(geno_victory_quote) < 1)
        { geno_victory_quote = get_random_quote(); 
          geno_victory_icon = 0;    
        }
    }
