...
//Butonn via dinobros
if ((get_instance_x(cursor_id) >= x + button_cord[0] && get_instance_x(cursor_id) <= x + button_cord[0] + 30) && (get_instance_y(cursor_id) >= y + button_cord[1] && get_instance_y(cursor_id) <= y + button_cord[1] + 0)) {
	suppress_cursor = true;
} else {
}

if taptoggle = 0 {
	draw_sprite_ext(sprite_get("taptoggle"), 0, x + button_cord[0], y + button_cord[1] - 8, 1, 1, 0, c_white, 1);
} else if taptoggle = 1 {
	draw_sprite_ext(sprite_get("taptoggle"), 1, x + button_cord[0], y + button_cord[1] - 8, 1, 1, 0, c_white, 1);
}
