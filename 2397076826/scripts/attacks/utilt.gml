set_attack_value(AT_UTILT, AG_SPRITE, sprite_get("utilt"));
set_attack_value(AT_UTILT, AG_NUM_WINDOWS, 5);
set_attack_value(AT_UTILT, AG_CATEGORY, 2);
set_attack_value(AT_UTILT, AG_HURTBOX_SPRITE, sprite_get("utilt_hurt"));

set_window_value(AT_UTILT, 1, AG_WINDOW_LENGTH, 10);
set_window_value(AT_UTILT, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_UTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UTILT, 1, AG_WINDOW_SFX, asset_get("sfx_clairen_spin"));
set_window_value(AT_UTILT, 1, AG_WINDOW_SFX_FRAME, 1);

set_window_value(AT_UTILT, 2, AG_WINDOW_LENGTH, 1);
set_window_value(AT_UTILT, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_UTILT, 2, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_UTILT, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UTILT, 2, AG_WINDOW_SFX, asset_get("sfx_swipe_weak2"));
set_window_value(AT_UTILT, 2, AG_WINDOW_SFX_FRAME, 0);



set_window_value(AT_UTILT, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_UTILT, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_UTILT, 3, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_UTILT, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_UTILT, 3, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_UTILT, 3, AG_WINDOW_SFX_FRAME, 4);


set_window_value(AT_UTILT, 4, AG_WINDOW_LENGTH, 6);
set_window_value(AT_UTILT, 4, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_UTILT, 4, AG_WINDOW_ANIM_FRAME_START, 6);



set_window_value(AT_UTILT, 5, AG_WINDOW_LENGTH, 15);
set_window_value(AT_UTILT, 5, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_UTILT, 5, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_UTILT, 5,  AG_WINDOW_HAS_WHIFFLAG, 1);


set_num_hitboxes(AT_UTILT,6);

set_hitbox_value(AT_UTILT, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 1, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_UTILT, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_UTILT, 1, HG_HITBOX_X, 12);
set_hitbox_value(AT_UTILT, 1, HG_HITBOX_Y, -20);
set_hitbox_value(AT_UTILT, 1, HG_WIDTH, 66);
set_hitbox_value(AT_UTILT, 1, HG_HEIGHT, 46);
set_hitbox_value(AT_UTILT, 1, HG_SHAPE, 0);
set_hitbox_value(AT_UTILT, 1, HG_PRIORITY, 3);
set_hitbox_value(AT_UTILT, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_UTILT, 1, HG_ANGLE, 90);
set_hitbox_value(AT_UTILT, 1, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_UTILT, 1, HG_KNOCKBACK_SCALING, .1);
set_hitbox_value(AT_UTILT, 1, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_UTILT, 1, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_UTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 19);
set_hitbox_value(AT_UTILT, 1, HG_HIT_SFX, sound_get("slicel"));
set_hitbox_value(AT_UTILT, 1, HG_HITSTUN_MULTIPLIER, .76);
set_hitbox_value(AT_UTILT, 1, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_UTILT, 1, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_UTILT, 1, HG_TECHABLE, 1);

set_hitbox_value(AT_UTILT, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 2, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_UTILT, 2, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_UTILT, 2, HG_HITBOX_X, -6);
set_hitbox_value(AT_UTILT, 2, HG_HITBOX_Y, -88);
set_hitbox_value(AT_UTILT, 2, HG_WIDTH, 86);
set_hitbox_value(AT_UTILT, 2, HG_HEIGHT, 60);
set_hitbox_value(AT_UTILT, 2, HG_SHAPE, 0);
set_hitbox_value(AT_UTILT, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_UTILT, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_UTILT, 2, HG_ANGLE, 90);
set_hitbox_value(AT_UTILT, 2, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_UTILT, 2, HG_KNOCKBACK_SCALING, .1);
set_hitbox_value(AT_UTILT, 2, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_UTILT, 2, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_UTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 19);
set_hitbox_value(AT_UTILT, 2, HG_HIT_SFX, sound_get("slicel"));
set_hitbox_value(AT_UTILT, 2, HG_HITSTUN_MULTIPLIER, .76);
set_hitbox_value(AT_UTILT, 2, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_UTILT, 2, HG_HITSTUN_MULTIPLIER, 1);

set_hitbox_value(AT_UTILT, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 3, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_UTILT, 3, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_UTILT, 3, HG_HITBOX_X, 30);
set_hitbox_value(AT_UTILT, 3, HG_HITBOX_Y, -46);
set_hitbox_value(AT_UTILT, 3, HG_WIDTH, 60);
set_hitbox_value(AT_UTILT, 3, HG_HEIGHT, 86);
set_hitbox_value(AT_UTILT, 3, HG_SHAPE, 0);
set_hitbox_value(AT_UTILT, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_UTILT, 3, HG_DAMAGE, 2);
set_hitbox_value(AT_UTILT, 3, HG_ANGLE, 90);
set_hitbox_value(AT_UTILT, 3, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_UTILT, 3, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_UTILT, 3, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_UTILT, 3, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_UTILT, 3, HG_VISUAL_EFFECT_X_OFFSET, 19);
set_hitbox_value(AT_UTILT, 3, HG_HIT_SFX, sound_get("slicel"));
set_hitbox_value(AT_UTILT, 3, HG_HITSTUN_MULTIPLIER, .76);
set_hitbox_value(AT_UTILT, 3, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_UTILT, 3, HG_HITSTUN_MULTIPLIER, 1);

set_hitbox_value(AT_UTILT, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 4, HG_WINDOW, 3);
set_hitbox_value(AT_UTILT, 4, HG_LIFETIME, 3);
set_hitbox_value(AT_UTILT, 4, HG_WINDOW_CREATION_FRAME, 9);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_X, 12);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_Y, -20);
set_hitbox_value(AT_UTILT, 4, HG_WIDTH, 66);
set_hitbox_value(AT_UTILT, 4, HG_HEIGHT, 46);
set_hitbox_value(AT_UTILT, 4, HG_SHAPE, 0);
set_hitbox_value(AT_UTILT, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_UTILT, 4, HG_DAMAGE, 6);
set_hitbox_value(AT_UTILT, 4, HG_ANGLE, 90);
set_hitbox_value(AT_UTILT, 4, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_UTILT, 4, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_UTILT, 4, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_UTILT, 4, HG_HITPAUSE_SCALING, .6);
set_hitbox_value(AT_UTILT, 4, HG_VISUAL_EFFECT_X_OFFSET, 19);
set_hitbox_value(AT_UTILT, 4, HG_HIT_SFX, sound_get("slice"));
set_hitbox_value(AT_UTILT, 4, HG_HITSTUN_MULTIPLIER, .76);
set_hitbox_value(AT_UTILT, 4, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_UTILT, 4, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_UTILT, 4, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_UTILT, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 5, HG_WINDOW, 4);
set_hitbox_value(AT_UTILT, 5, HG_LIFETIME, 3);
set_hitbox_value(AT_UTILT, 5, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_UTILT, 5, HG_HITBOX_X, -6);
set_hitbox_value(AT_UTILT, 5, HG_HITBOX_Y, -88);
set_hitbox_value(AT_UTILT, 5, HG_WIDTH, 86);
set_hitbox_value(AT_UTILT, 5, HG_HEIGHT, 60);
set_hitbox_value(AT_UTILT, 5, HG_SHAPE, 0);
set_hitbox_value(AT_UTILT, 5, HG_PRIORITY, 2);
set_hitbox_value(AT_UTILT, 5, HG_DAMAGE, 6);
set_hitbox_value(AT_UTILT, 5, HG_ANGLE, 90);
set_hitbox_value(AT_UTILT, 5, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_UTILT, 5, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_UTILT, 5, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_UTILT, 5, HG_HITPAUSE_SCALING, .6);
set_hitbox_value(AT_UTILT, 5, HG_VISUAL_EFFECT_X_OFFSET, 19);
set_hitbox_value(AT_UTILT, 5, HG_HIT_SFX, sound_get("slice"));
set_hitbox_value(AT_UTILT, 5, HG_HITSTUN_MULTIPLIER, .76);
set_hitbox_value(AT_UTILT, 5, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_UTILT, 5, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_UTILT, 5, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_UTILT, 6, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_UTILT, 6, HG_WINDOW, 4);
set_hitbox_value(AT_UTILT, 6, HG_LIFETIME, 3);
set_hitbox_value(AT_UTILT, 6, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_UTILT, 6, HG_HITBOX_X, 30);
set_hitbox_value(AT_UTILT, 6, HG_HITBOX_Y, -46);
set_hitbox_value(AT_UTILT, 6, HG_WIDTH, 60);
set_hitbox_value(AT_UTILT, 6, HG_HEIGHT, 86);
set_hitbox_value(AT_UTILT, 6, HG_SHAPE, 0);
set_hitbox_value(AT_UTILT, 6, HG_PRIORITY, 3);
set_hitbox_value(AT_UTILT, 6, HG_DAMAGE, 6);
set_hitbox_value(AT_UTILT, 6, HG_ANGLE, 90);
set_hitbox_value(AT_UTILT, 6, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_UTILT, 6, HG_KNOCKBACK_SCALING, .5);
set_hitbox_value(AT_UTILT, 6, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_UTILT, 6, HG_HITPAUSE_SCALING, .6);
set_hitbox_value(AT_UTILT, 6, HG_VISUAL_EFFECT_X_OFFSET, 19);
set_hitbox_value(AT_UTILT, 6, HG_HIT_SFX, sound_get("slice"));
set_hitbox_value(AT_UTILT, 6, HG_HITSTUN_MULTIPLIER, .76);
set_hitbox_value(AT_UTILT, 6, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_UTILT, 6, HG_HITSTUN_MULTIPLIER, 1);
set_hitbox_value(AT_UTILT, 6, HG_HITBOX_GROUP, 2);