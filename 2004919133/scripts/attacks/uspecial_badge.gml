set_attack_value(AT_USPECIAL_2, AG_CATEGORY, 2);
set_attack_value(AT_USPECIAL_2, AG_SPRITE, sprite_get("uspecial"));
set_attack_value(AT_USPECIAL_2, AG_NUM_WINDOWS, 5);
set_attack_value(AT_USPECIAL_2, AG_HAS_LANDING_LAG, 4);
set_attack_value(AT_USPECIAL_2, AG_OFF_LEDGE, 1);
set_attack_value(AT_USPECIAL_2, AG_AIR_SPRITE, sprite_get("uspecial"));
set_attack_value(AT_USPECIAL_2, AG_HURTBOX_SPRITE, sprite_get("uspecial_hurt"));

set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_LENGTH, 4);
set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_medium2"));
set_window_value(AT_USPECIAL_2, 1, AG_WINDOW_SFX_FRAME, 3);

set_window_value(AT_USPECIAL_2, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_USPECIAL_2, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_USPECIAL_2, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USPECIAL_2, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_USPECIAL_2, 2, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_USPECIAL_2, 2, AG_WINDOW_VSPEED, -3);

set_window_value(AT_USPECIAL_2, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_USPECIAL_2, 3, AG_WINDOW_LENGTH, 14);
set_window_value(AT_USPECIAL_2, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USPECIAL_2, 3, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_USPECIAL_2, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_USPECIAL_2, 3, AG_WINDOW_VSPEED, -10);

set_window_value(AT_USPECIAL_2, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_USPECIAL_2, 4, AG_WINDOW_LENGTH, 6);
set_window_value(AT_USPECIAL_2, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USPECIAL_2, 4, AG_WINDOW_ANIM_FRAME_START, 5);

set_window_value(AT_USPECIAL_2, 5, AG_WINDOW_TYPE, 8);
set_window_value(AT_USPECIAL_2, 5, AG_WINDOW_LENGTH, 150);
set_window_value(AT_USPECIAL_2, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USPECIAL_2, 5, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_USPECIAL_2, 5, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_USPECIAL_2, 5, AG_WINDOW_VSPEED, 2);

set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_TYPE, 7);
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_LENGTH, 2);
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_SFX, asset_get("sfx_ori_glide_featherout"));
set_window_value(AT_USPECIAL_2, 6, AG_WINDOW_SFX_FRAME, 1);

set_num_hitboxes(AT_USPECIAL_2, 5);

set_hitbox_value(AT_USPECIAL_2, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_USPECIAL_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USPECIAL_2, 1, HG_WINDOW, 2);
set_hitbox_value(AT_USPECIAL_2, 1, HG_LIFETIME, 1);
set_hitbox_value(AT_USPECIAL_2, 1, HG_HITBOX_X, 18);
set_hitbox_value(AT_USPECIAL_2, 1, HG_HITBOX_Y, -76);
set_hitbox_value(AT_USPECIAL_2, 1, HG_WIDTH, 42);
set_hitbox_value(AT_USPECIAL_2, 1, HG_HEIGHT, 90);
set_hitbox_value(AT_USPECIAL_2, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_USPECIAL_2, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_USPECIAL_2, 1, HG_ANGLE, 70);
set_hitbox_value(AT_USPECIAL_2, 1, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_USPECIAL_2, 1, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(AT_USPECIAL_2, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_USPECIAL_2, 1, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_USPECIAL_2, 1, HG_HITBOX_GROUP, -1);

set_hitbox_value(AT_USPECIAL_2, 2, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_USPECIAL_2, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USPECIAL_2, 2, HG_WINDOW, 3);
set_hitbox_value(AT_USPECIAL_2, 2, HG_WINDOW_CREATION_FRAME, 4);
set_hitbox_value(AT_USPECIAL_2, 2, HG_LIFETIME, 1);
set_hitbox_value(AT_USPECIAL_2, 2, HG_HITBOX_X, 20);
set_hitbox_value(AT_USPECIAL_2, 2, HG_HITBOX_Y, -57);
set_hitbox_value(AT_USPECIAL_2, 2, HG_WIDTH, 53);
set_hitbox_value(AT_USPECIAL_2, 2, HG_HEIGHT, 80);
set_hitbox_value(AT_USPECIAL_2, 2, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_USPECIAL_2, 2, HG_ANGLE, 90);
set_hitbox_value(AT_USPECIAL_2, 2, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_USPECIAL_2, 2, HG_DAMAGE, 1);
set_hitbox_value(AT_USPECIAL_2, 2, HG_BASE_HITPAUSE, 4);

set_hitbox_value(AT_USPECIAL_2, 3, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_USPECIAL_2, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USPECIAL_2, 3, HG_WINDOW, 3);
set_hitbox_value(AT_USPECIAL_2, 3, HG_WINDOW_CREATION_FRAME, 8);
set_hitbox_value(AT_USPECIAL_2, 3, HG_LIFETIME, 1);
set_hitbox_value(AT_USPECIAL_2, 3, HG_HITBOX_X, 20);
set_hitbox_value(AT_USPECIAL_2, 3, HG_HITBOX_Y, -55);
set_hitbox_value(AT_USPECIAL_2, 3, HG_WIDTH, 48);
set_hitbox_value(AT_USPECIAL_2, 3, HG_HEIGHT, 64);
set_hitbox_value(AT_USPECIAL_2, 3, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_USPECIAL_2, 3, HG_ANGLE, 90);
set_hitbox_value(AT_USPECIAL_2, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_USPECIAL_2, 3, HG_DAMAGE, 1);
set_hitbox_value(AT_USPECIAL_2, 3, HG_BASE_HITPAUSE, 4);

set_hitbox_value(AT_USPECIAL_2, 4, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_USPECIAL_2, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USPECIAL_2, 4, HG_WINDOW, 3);
set_hitbox_value(AT_USPECIAL_2, 4, HG_WINDOW_CREATION_FRAME, 12);
set_hitbox_value(AT_USPECIAL_2, 4, HG_LIFETIME, 1);
set_hitbox_value(AT_USPECIAL_2, 4, HG_HITBOX_X, 20);
set_hitbox_value(AT_USPECIAL_2, 4, HG_HITBOX_Y, -55);
set_hitbox_value(AT_USPECIAL_2, 4, HG_WIDTH, 48);
set_hitbox_value(AT_USPECIAL_2, 4, HG_HEIGHT, 64);
set_hitbox_value(AT_USPECIAL_2, 4, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_USPECIAL_2, 4, HG_ANGLE, 90);
set_hitbox_value(AT_USPECIAL_2, 4, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_USPECIAL_2, 4, HG_DAMAGE, 2);
set_hitbox_value(AT_USPECIAL_2, 4, HG_BASE_HITPAUSE, 4);

set_hitbox_value(AT_USPECIAL_2, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_USPECIAL_2, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USPECIAL_2, 5, HG_WINDOW, 4);
set_hitbox_value(AT_USPECIAL_2, 5, HG_LIFETIME, 2);
set_hitbox_value(AT_USPECIAL_2, 5, HG_HITBOX_X, 20);
set_hitbox_value(AT_USPECIAL_2, 5, HG_HITBOX_Y, -57);
set_hitbox_value(AT_USPECIAL_2, 5, HG_WIDTH, 53);
set_hitbox_value(AT_USPECIAL_2, 5, HG_HEIGHT, 80);
set_hitbox_value(AT_USPECIAL_2, 5, HG_PRIORITY, 1);
set_hitbox_value(AT_USPECIAL_2, 5, HG_DAMAGE, 4);
set_hitbox_value(AT_USPECIAL_2, 5, HG_ANGLE, 90);
set_hitbox_value(AT_USPECIAL_2, 5, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_USPECIAL_2, 5, HG_KNOCKBACK_SCALING, .85);
set_hitbox_value(AT_USPECIAL_2, 5, HG_BASE_HITPAUSE, 15);
set_hitbox_value(AT_USPECIAL_2, 5, HG_HITPAUSE_SCALING, .4);
set_hitbox_value(AT_USPECIAL_2, 5, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_USPECIAL_2, 5, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
set_hitbox_value(AT_USPECIAL_2, 5, HG_HITBOX_GROUP, -1);