...
if (place_meeting(x , y + vsp, asset_get("par_block")) || place_meeting(x , y + vsp, asset_get("par_jumpthrough"))){
	vsp = bounce_vsp;
}
...
//enable collision after enough time has passed if spawned low
if (low_spawn){
	if (timer >= time_before_grabbable/2){
...
//despawning stuff
var despawn_time = 480; //8 seconds to get a player-spawned star
...
//despawn after time is up or if too low
if (timer >= despawn_time || (y > room_height + 20)){
