set_attack_value(AT_TAUNT, AG_SPRITE, sprite_get("taunt"));
set_attack_value(AT_TAUNT, AG_NUM_WINDOWS, 2);
set_attack_value(AT_TAUNT, AG_OFF_LEDGE, 1);
set_attack_value(AT_TAUNT, AG_CATEGORY, 2);
set_attack_value(AT_TAUNT, AG_HURTBOX_SPRITE, asset_get("ex_guy_hurt_box"));

// start
set_window_value(AT_TAUNT, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT, 1, AG_WINDOW_LENGTH, 15);
set_window_value(AT_TAUNT, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_TAUNT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT, 1, AG_WINDOW_SFX, sfx_minecraft_pop);
set_window_value(AT_TAUNT, 1, AG_WINDOW_SFX_FRAME, 14);

// idle
set_window_value(AT_TAUNT, 2, AG_WINDOW_TYPE, 9);
set_window_value(AT_TAUNT, 2, AG_WINDOW_LENGTH, 1);
set_window_value(AT_TAUNT, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_TAUNT, 2, AG_WINDOW_ANIM_FRAME_START, 6);

// walk
set_window_value(AT_TAUNT, 3, AG_WINDOW_TYPE, 9);
set_window_value(AT_TAUNT, 3, AG_WINDOW_LENGTH, 28);
set_window_value(AT_TAUNT, 3, AG_WINDOW_ANIM_FRAMES, 8);
set_window_value(AT_TAUNT, 3, AG_WINDOW_ANIM_FRAME_START, 3);

// run
set_window_value(AT_TAUNT, 4, AG_WINDOW_TYPE, 9);
set_window_value(AT_TAUNT, 4, AG_WINDOW_LENGTH, 20);
set_window_value(AT_TAUNT, 4, AG_WINDOW_ANIM_FRAMES, 8);
set_window_value(AT_TAUNT, 4, AG_WINDOW_ANIM_FRAME_START, 11);

// crouch
set_window_value(AT_TAUNT, 5, AG_WINDOW_TYPE, 9);
set_window_value(AT_TAUNT, 5, AG_WINDOW_LENGTH, 1);
set_window_value(AT_TAUNT, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_TAUNT, 5, AG_WINDOW_ANIM_FRAME_START, 22);

// crouch walk
set_window_value(AT_TAUNT, 6, AG_WINDOW_TYPE, 9);
set_window_value(AT_TAUNT, 6, AG_WINDOW_LENGTH, 28);
set_window_value(AT_TAUNT, 6, AG_WINDOW_ANIM_FRAMES, 8);
set_window_value(AT_TAUNT, 6, AG_WINDOW_ANIM_FRAME_START, 19);

// punch
set_window_value(AT_TAUNT, 7, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT, 7, AG_WINDOW_LENGTH, 16);
set_window_value(AT_TAUNT, 7, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_TAUNT, 7, AG_WINDOW_ANIM_FRAME_START, 27);
set_window_value(AT_TAUNT, 7, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT, 7, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_TAUNT, 7, AG_WINDOW_SFX_FRAME, 4);

// end
set_window_value(AT_TAUNT, 8, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT, 8, AG_WINDOW_LENGTH, 15);
set_window_value(AT_TAUNT, 8, AG_WINDOW_ANIM_FRAMES, -3);
set_window_value(AT_TAUNT, 8, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_TAUNT, 8, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT, 8, AG_WINDOW_SFX, sfx_minecraft_pop);
set_window_value(AT_TAUNT, 8, AG_WINDOW_SFX_FRAME, 14);

// build
set_window_value(AT_TAUNT, 9, AG_WINDOW_TYPE, 1);
set_window_value(AT_TAUNT, 9, AG_WINDOW_LENGTH, 16);
set_window_value(AT_TAUNT, 9, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_TAUNT, 9, AG_WINDOW_ANIM_FRAME_START, 27);
set_window_value(AT_TAUNT, 9, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_TAUNT, 9, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_TAUNT, 9, AG_WINDOW_SFX_FRAME, 4);



set_num_hitboxes(AT_TAUNT, 1);

set_hitbox_value(AT_TAUNT, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_TAUNT, 1, HG_WINDOW, 7);
set_hitbox_value(AT_TAUNT, 1, HG_WINDOW_CREATION_FRAME, 5);
set_hitbox_value(AT_TAUNT, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_TAUNT, 1, HG_HITBOX_X, 20);
set_hitbox_value(AT_TAUNT, 1, HG_HITBOX_Y, -25);
set_hitbox_value(AT_TAUNT, 1, HG_WIDTH, 32);
set_hitbox_value(AT_TAUNT, 1, HG_HEIGHT, 50);
set_hitbox_value(AT_TAUNT, 1, HG_SHAPE, 2);
set_hitbox_value(AT_TAUNT, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_TAUNT, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_TAUNT, 1, HG_ANGLE, 50); // 60
set_hitbox_value(AT_TAUNT, 1, HG_BASE_KNOCKBACK, 6); // 4
set_hitbox_value(AT_TAUNT, 1, HG_KNOCKBACK_SCALING, 0.4);
set_hitbox_value(AT_TAUNT, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_TAUNT, 1, HG_HITPAUSE_SCALING, 0.4);
set_hitbox_value(AT_TAUNT, 1, HG_HITSTUN_MULTIPLIER, 0.8);
set_hitbox_value(AT_TAUNT, 1, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_TAUNT, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_TAUNT, 1, HG_TECHABLE, 1);
set_hitbox_value(AT_TAUNT, 1, HG_MUNO_HITBOX_ACTIVE, "6-8");
set_hitbox_value(AT_TAUNT, 1, HG_MUNO_HITBOX_NAME, "Punch");