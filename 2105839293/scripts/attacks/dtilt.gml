set_attack_value(AT_DTILT, AG_SPRITE, sprite_get("dtilt"));
set_attack_value(AT_DTILT, AG_NUM_WINDOWS, 3);
set_attack_value(AT_DTILT, AG_HURTBOX_SPRITE, sprite_get("dtilt_hurt"));

set_window_value(AT_DTILT, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DTILT, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX, asset_get("sfx_abyss_explosion_big"));
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX_FRAME, 8);

set_window_value(AT_DTILT, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_DTILT, 2, AG_WINDOW_HSPEED, -2);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAME_START, 4);

set_window_value(AT_DTILT, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 3, AG_WINDOW_LENGTH, 15);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_DTILT, 3, AG_WINDOW_HAS_WHIFFLAG, 6);

set_num_hitboxes(AT_DTILT,4);

set_hitbox_value(AT_DTILT, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_X, 48);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_Y, -30);
set_hitbox_value(AT_DTILT, 1, HG_WIDTH, 30);
set_hitbox_value(AT_DTILT, 1, HG_HEIGHT, 70);
set_hitbox_value(AT_DTILT, 1, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 1, HG_PRIORITY, 5);
set_hitbox_value(AT_DTILT, 1, HG_DAMAGE, 7);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE, 100);
set_hitbox_value(AT_DTILT, 1, HG_BASE_KNOCKBACK, 11);
set_hitbox_value(AT_DTILT, 1, HG_KNOCKBACK_SCALING, .05);
set_hitbox_value(AT_DTILT, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DTILT, 1, HG_HITPAUSE_SCALING, .35);
set_hitbox_value(AT_DTILT, 1, HG_HITSTUN_MULTIPLIER, 0.90);
//set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT, 251);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_abyss_hazard_hit"));
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_DTILT, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_X, 55);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DTILT, 2, HG_WIDTH, 40);
set_hitbox_value(AT_DTILT, 2, HG_HEIGHT, 40);
set_hitbox_value(AT_DTILT, 2, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 2, HG_PRIORITY, 5);
set_hitbox_value(AT_DTILT, 2, HG_DAMAGE, 7);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE, 100);
set_hitbox_value(AT_DTILT, 2, HG_BASE_KNOCKBACK, 11);
set_hitbox_value(AT_DTILT, 2, HG_KNOCKBACK_SCALING, .05);
set_hitbox_value(AT_DTILT, 2, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DTILT, 2, HG_HITPAUSE_SCALING, .35);
set_hitbox_value(AT_DTILT, 2, HG_HITSTUN_MULTIPLIER, 0.90);
//set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT, 251);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, asset_get("sfx_abyss_hazard_hit"));
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_DTILT, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_X, 17);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_Y, -20);
set_hitbox_value(AT_DTILT, 3, HG_WIDTH, 35);
set_hitbox_value(AT_DTILT, 3, HG_HEIGHT, 40);
set_hitbox_value(AT_DTILT, 3, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 3, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 3, HG_DAMAGE, 7);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE, 100);
set_hitbox_value(AT_DTILT, 3, HG_BASE_KNOCKBACK, 11);
set_hitbox_value(AT_DTILT, 3, HG_KNOCKBACK_SCALING, .05);
set_hitbox_value(AT_DTILT, 3, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DTILT, 3, HG_HITPAUSE_SCALING, .35);
set_hitbox_value(AT_DTILT, 3, HG_HITSTUN_MULTIPLIER, 0.90);
//set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT, 251);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_Y_OFFSET, 10);
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_abyss_hazard_hit"));
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_GROUP, 1);


set_hitbox_value(AT_DTILT, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_TYPE, 2);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW, 1);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW_CREATION_FRAME, 9);
set_hitbox_value(AT_DTILT, 4, HG_LIFETIME, 14);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_X, 20);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_Y, -15);
set_hitbox_value(AT_DTILT, 4, HG_WIDTH, 90);
set_hitbox_value(AT_DTILT, 4, HG_HEIGHT, 60);
set_hitbox_value(AT_DTILT, 4, HG_PRIORITY, 0);
set_hitbox_value(AT_DTILT, 4, HG_DAMAGE, 0);
set_hitbox_value(AT_DTILT, 4, HG_ANGLE, 50);
set_hitbox_value(AT_DTILT, 4, HG_BASE_KNOCKBACK, 0);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value(AT_DTILT, 4, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT_X_OFFSET, 32);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_DTILT, 4, HG_BASE_HITPAUSE, 2);
set_hitbox_value(AT_DTILT, 4, HG_HITPAUSE_SCALING, .25);
set_hitbox_value(AT_DTILT, 4, HG_HITSTUN_MULTIPLIER, 0.95);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_SPRITE, sprite_get("dtilt_proj"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_MASK, sprite_get("dtilt_proj"));
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_ANIM_SPEED, 0.5);
set_hitbox_value(AT_DTILT, 4, HG_PROJECTILE_HSPEED, 0);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT, 1);