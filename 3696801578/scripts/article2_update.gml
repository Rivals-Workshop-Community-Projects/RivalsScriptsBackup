...
sprite_index = player_id.detached_jake_data[? owner.state].hurtbox
...
if hbox.force_flinch && !owner.free orig_knock = 0; //uncomment this line for grounded articles.
//if hbox.force_flinch orig_knock = 0.3; //comment out this line for grounded articles.
else orig_knock = min(hbox.kb_value + hbox.damage * hbox.kb_scale * 0.12 * kb_adj, 50);
...
owner.vsp = lengthdir_y(orig_knock, kb_dir);
owner.hsp = lengthdir_x(orig_knock, kb_dir);
owner.hitstop = ceil(get_hitstop_formula(50, hbox.hbox_num, hbox.hitpause, hbox.hitpause_growth, hbox.extra_hitpause));
owner.image_index = 0
owner.sprite_index = player_id.detached_jake_data[? owner.state].sprite
