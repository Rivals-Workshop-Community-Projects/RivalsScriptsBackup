set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_ANGLE_FLIPPER, 8);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_SHAPE, 0);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_HITBOX_TYPE, 2);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_LIFETIME, 2);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_DAMAGE, 0);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_BASE_KNOCKBACK, 12);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_MASK, -1);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_SPRITE, sprite_get("empty"));
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_HSPEED, 0);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_VSPEED, 0);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_GRAVITY, 0);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_DOES_NOT_REFLECT, true);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_IS_TRANSCENDENT, true);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_UNBASHABLE, true);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_PARRY_STUN, false);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_PLASMA_SAFE, true);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_GROUND_BEHAVIOR, 1);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_WALL_BEHAVIOR, 1);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_ENEMY_BEHAVIOR, 1);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_VISUAL_EFFECT, 1);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PROJECTILE_DESTROY_EFFECT, empty);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_PRIORITY, 10);

set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_HITBOX_X, 450);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_HITBOX_Y, 249);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_HEIGHT, 60);
set_hitbox_value( AT_USPECIAL_GROUND, 1, HG_WIDTH, 88);