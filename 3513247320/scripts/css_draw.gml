...
*/
if ("css_hovered" not in self) return;
...
if slide_timer < 120 { 
	...
	draw_sprite_part_ext(preview_idle, (css_timer*preview_anim_speed) % sprite_get_number(preview_idle),
			-dist_from_side/2,
			0,
			width,
			height,
...
if (setting_hover_time > 0) {
	draw_set_font(asset_get("fName")) //ensure correct font for text length measurement
	var offs_y = -8;
	var offs_x = 0;
	draw_set_alpha(clamp(setting_hover_time/30, 0, 1));
	draw_debug_text(floor(x)+offs_x,floor(y)+offs_y-8,setting_message);
	draw_set_alpha(1);
}
...
if (info_hover_time > 0) {
	var offs_y = -8;
	var offs_x = 0;
	draw_set_alpha(clamp(info_hover_time/30, 0, 1));
	draw_debug_text(floor(x)+offs_x,floor(y)+offs_y-8,alt_desc[alt]);
	draw_set_alpha(1);
}
