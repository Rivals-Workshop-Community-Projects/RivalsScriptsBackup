...
if (aodev == true){
	...
	draw_debug_text(scrleft+16,scrtop+32,"player "+string(player)+" | hsp "+string(hsp)+" | vsp "+string(vsp)+" | x "+string(x)+" y "+string(y));
	draw_debug_text(scrleft+16,scrtop+48,"state ["+get_state_name(state)+"] ("+string(state)+") - ("+string(clamp(state_timer,0,100))+")");
	...
	draw_debug_text(scrleft+16,scrtop+112,"id: "+string(id)+" dur: "+string(fall_dur));
...
draw_debug_text(x+30,y,string("article 1"));
	if (player_id == other.id){
		draw_debug_text(x+30,y+16,"explorer's");
	}
}*/
with (obj_article2){
