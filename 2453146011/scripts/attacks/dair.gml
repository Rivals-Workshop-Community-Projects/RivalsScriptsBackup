set_attack_value(AT_DAIR, AG_CATEGORY, 1);
set_attack_value(AT_DAIR, AG_SPRITE, sprite_get("dair"));
set_attack_value(AT_DAIR, AG_NUM_WINDOWS, 5);
set_attack_value(AT_DAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_DAIR, AG_LANDING_LAG, 10);
set_attack_value(AT_DAIR, AG_HURTBOX_SPRITE, sprite_get("dair_hurt"));
set_attack_value(AT_DAIR, AG_USES_CUSTOM_GRAVITY, 1);


set_window_value(AT_DAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAMES, 5);
set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED, -1);
set_window_value(AT_DAIR, 1, AG_WINDOW_CUSTOM_GRAVITY, 1.0);
set_window_value(AT_DAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX_FRAME, 5);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX, sound_get("monarch_woosh3"));
set_window_value(AT_DAIR, 1, AG_WINDOW_CUSTOM_GRAVITY, 1);

set_window_value(AT_DAIR, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAME_START, 5);
set_window_value(AT_DAIR, 2, AG_WINDOW_CUSTOM_GRAVITY, .5);
set_window_value(AT_DAIR, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_SFX_FRAME, 3);
set_window_value(AT_DAIR, 2, AG_WINDOW_SFX, sound_get("monarch_woosh2"));

set_window_value(AT_DAIR, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 3, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_DAIR, 3, AG_WINDOW_CUSTOM_GRAVITY, .5);
set_window_value(AT_DAIR, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 3, AG_WINDOW_SFX_FRAME, 2);
set_window_value(AT_DAIR, 3, AG_WINDOW_SFX, sound_get("monarch_gunshot1"));

set_window_value(AT_DAIR, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 4, AG_WINDOW_LENGTH, 2);
set_window_value(AT_DAIR, 4, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DAIR, 4, AG_WINDOW_ANIM_FRAME_START, 11);
set_window_value(AT_DAIR, 4, AG_WINDOW_CUSTOM_GRAVITY, .5);

set_window_value(AT_DAIR, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 5, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DAIR, 5, AG_WINDOW_ANIM_FRAMES, 5);
set_window_value(AT_DAIR, 5, AG_WINDOW_ANIM_FRAME_START, 13);
set_window_value(AT_DAIR, 5, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_DAIR, 5, AG_WINDOW_CUSTOM_GRAVITY, .8);

set_num_hitboxes(AT_DAIR,4);

set_hitbox_value(AT_DAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DAIR, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_Y, -30);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_X, 15);
set_hitbox_value(AT_DAIR, 1, HG_WIDTH, 50);
set_hitbox_value(AT_DAIR, 1, HG_HEIGHT, 80);
set_hitbox_value(AT_DAIR, 1, HG_SHAPE, 2);
set_hitbox_value(AT_DAIR, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_DAIR, 1, HG_ANGLE, 45);
set_hitbox_value(AT_DAIR, 1, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 1, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 1, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_DAIR, 1, HG_HITPAUSE_SCALING, .35);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_DAIR, 1, HG_HIT_SFX, sound_get("monarch_smallhit4"));
set_hitbox_value(AT_DAIR, 1, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DAIR, 1, HG_FORCE_FLINCH, 1);

set_hitbox_value(AT_DAIR, 2, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 2, HG_WINDOW, 3);
set_hitbox_value(AT_DAIR, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_Y, -30);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_X, 15);
set_hitbox_value(AT_DAIR, 2, HG_WIDTH, 50);
set_hitbox_value(AT_DAIR, 2, HG_HEIGHT, 80);
set_hitbox_value(AT_DAIR, 2, HG_SHAPE, 2);
set_hitbox_value(AT_DAIR, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_DAIR, 2, HG_ANGLE, 45);
set_hitbox_value(AT_DAIR, 2, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_DAIR, 2, HG_KNOCKBACK_SCALING, .2);
set_hitbox_value(AT_DAIR, 2, HG_BASE_HITPAUSE, 3);
set_hitbox_value(AT_DAIR, 2, HG_HITPAUSE_SCALING, .35);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT, 1);
set_hitbox_value(AT_DAIR, 2, HG_HIT_SFX, sound_get("monarch_knifehit1"));
set_hitbox_value(AT_DAIR, 2, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DAIR, 2, HG_FORCE_FLINCH, 1);

set_hitbox_value(AT_DAIR, 3, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 3, HG_WINDOW, 5);
set_hitbox_value(AT_DAIR, 3, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_Y, 10);
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_X, 25);
set_hitbox_value(AT_DAIR, 3, HG_WIDTH, 40);
set_hitbox_value(AT_DAIR, 3, HG_HEIGHT, 60);
set_hitbox_value(AT_DAIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 3, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 3, HG_DAMAGE, 6);
set_hitbox_value(AT_DAIR, 3, HG_ANGLE, 285);
set_hitbox_value(AT_DAIR, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DAIR, 3, HG_KNOCKBACK_SCALING, .6);
set_hitbox_value(AT_DAIR, 3, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DAIR, 3, HG_HITPAUSE_SCALING, .8);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT, hitfx4);
set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 3, HG_HIT_SFX, sound_get("monarch_gunhit1"));
set_hitbox_value(AT_DAIR, 3, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_DAIR, 4, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 4, HG_WINDOW, 3);
set_hitbox_value(AT_DAIR, 4, HG_WINDOW_CREATION_FRAME, 2);
set_hitbox_value(AT_DAIR, 4, HG_LIFETIME, 2);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_Y, -55);
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_X, -5);
set_hitbox_value(AT_DAIR, 4, HG_WIDTH, 20);
set_hitbox_value(AT_DAIR, 4, HG_HEIGHT, 50);
set_hitbox_value(AT_DAIR, 4, HG_SHAPE, 0);
set_hitbox_value(AT_DAIR, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_DAIR, 4, HG_DAMAGE, 3);
set_hitbox_value(AT_DAIR, 4, HG_ANGLE, 100);
set_hitbox_value(AT_DAIR, 4, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_DAIR, 4, HG_KNOCKBACK_SCALING, .55);
set_hitbox_value(AT_DAIR, 4, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DAIR, 4, HG_HITPAUSE_SCALING, .5);
set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT, hitfx1);
set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_DAIR, 4, HG_HIT_SFX, sound_get("monarch_smallhit4"));
set_hitbox_value(AT_DAIR, 4, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DAIR, 4, HG_FORCE_FLINCH, 1);
set_hitbox_value(AT_DAIR, 4, HG_ANGLE_FLIPPER, 9);

// set_num_hitboxes(AT_DAIR,4);

// set_hitbox_value(AT_DAIR, 1, HG_PARENT_HITBOX, 1);
// set_hitbox_value(AT_DAIR, 1, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DAIR, 1, HG_WINDOW, 2);
// set_hitbox_value(AT_DAIR, 1, HG_LIFETIME, 3);
// set_hitbox_value(AT_DAIR, 1, HG_HITBOX_Y, -30);
// set_hitbox_value(AT_DAIR, 1, HG_HITBOX_X, 15);
// set_hitbox_value(AT_DAIR, 1, HG_WIDTH, 50);
// set_hitbox_value(AT_DAIR, 1, HG_HEIGHT, 80);
// set_hitbox_value(AT_DAIR, 1, HG_SHAPE, 2);
// set_hitbox_value(AT_DAIR, 1, HG_PRIORITY, 2);
// set_hitbox_value(AT_DAIR, 1, HG_DAMAGE, 2);
// set_hitbox_value(AT_DAIR, 1, HG_ANGLE, 45);
// set_hitbox_value(AT_DAIR, 1, HG_BASE_KNOCKBACK, 3);
// set_hitbox_value(AT_DAIR, 1, HG_KNOCKBACK_SCALING, .2);
// set_hitbox_value(AT_DAIR, 1, HG_BASE_HITPAUSE, 3);
// set_hitbox_value(AT_DAIR, 1, HG_HITPAUSE_SCALING, .35);
// set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, 1);
// set_hitbox_value(AT_DAIR, 1, HG_HIT_SFX, sound_get("monarch_smallhit4"));
// set_hitbox_value(AT_DAIR, 1, HG_ANGLE_FLIPPER, 4);
// set_hitbox_value(AT_DAIR, 1, HG_HITBOX_GROUP, -1);

// set_hitbox_value(AT_DAIR, 2, HG_PARENT_HITBOX, 0);
// set_hitbox_value(AT_DAIR, 2, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DAIR, 2, HG_WINDOW, 3);
// set_hitbox_value(AT_DAIR, 2, HG_LIFETIME, 3);
// set_hitbox_value(AT_DAIR, 2, HG_HITBOX_Y, -30);
// set_hitbox_value(AT_DAIR, 2, HG_HITBOX_X, 15);
// set_hitbox_value(AT_DAIR, 2, HG_WIDTH, 50);
// set_hitbox_value(AT_DAIR, 2, HG_HEIGHT, 80);
// set_hitbox_value(AT_DAIR, 2, HG_SHAPE, 2);
// set_hitbox_value(AT_DAIR, 2, HG_PRIORITY, 2);
// set_hitbox_value(AT_DAIR, 2, HG_DAMAGE, 2);
// set_hitbox_value(AT_DAIR, 2, HG_ANGLE, 45);
// set_hitbox_value(AT_DAIR, 2, HG_BASE_KNOCKBACK, 3);
// set_hitbox_value(AT_DAIR, 2, HG_KNOCKBACK_SCALING, .2);
// set_hitbox_value(AT_DAIR, 2, HG_BASE_HITPAUSE, 3);
// set_hitbox_value(AT_DAIR, 2, HG_HITPAUSE_SCALING, .35);
// set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT, 1);
// set_hitbox_value(AT_DAIR, 2, HG_HIT_SFX, sound_get("monarch_knifehit1"));
// set_hitbox_value(AT_DAIR, 2, HG_ANGLE_FLIPPER, 4);
// set_hitbox_value(AT_DAIR, 2, HG_HITBOX_GROUP, -1);

// set_hitbox_value(AT_DAIR, 3, HG_PARENT_HITBOX, 0);
// set_hitbox_value(AT_DAIR, 3, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DAIR, 3, HG_WINDOW, 5);
// set_hitbox_value(AT_DAIR, 3, HG_LIFETIME, 2);
// set_hitbox_value(AT_DAIR, 3, HG_HITBOX_Y, 10);
// set_hitbox_value(AT_DAIR, 3, HG_HITBOX_X, 25);
// set_hitbox_value(AT_DAIR, 3, HG_WIDTH, 40);
// set_hitbox_value(AT_DAIR, 3, HG_HEIGHT, 60);
// set_hitbox_value(AT_DAIR, 3, HG_SHAPE, 0);
// set_hitbox_value(AT_DAIR, 3, HG_PRIORITY, 2);
// set_hitbox_value(AT_DAIR, 3, HG_DAMAGE, 6);
// set_hitbox_value(AT_DAIR, 3, HG_ANGLE, 290);
// set_hitbox_value(AT_DAIR, 3, HG_BASE_KNOCKBACK, 6);
// set_hitbox_value(AT_DAIR, 3, HG_KNOCKBACK_SCALING, .65);
// set_hitbox_value(AT_DAIR, 3, HG_BASE_HITPAUSE, 8);
// set_hitbox_value(AT_DAIR, 3, HG_HITPAUSE_SCALING, .8);
// set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT, hitfx4);
// set_hitbox_value(AT_DAIR, 3, HG_VISUAL_EFFECT_Y_OFFSET, 20);
// set_hitbox_value(AT_DAIR, 3, HG_HIT_SFX, sound_get("monarch_gunhit1"));
// set_hitbox_value(AT_DAIR, 3, HG_HITBOX_GROUP, -1);

// set_hitbox_value(AT_DAIR, 4, HG_PARENT_HITBOX, 0);
// set_hitbox_value(AT_DAIR, 4, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DAIR, 4, HG_WINDOW, 3);
// set_hitbox_value(AT_DAIR, 4, HG_WINDOW_CREATION_FRAME, 2);
// set_hitbox_value(AT_DAIR, 4, HG_LIFETIME, 2);
// set_hitbox_value(AT_DAIR, 4, HG_HITBOX_Y, -55);
// set_hitbox_value(AT_DAIR, 4, HG_HITBOX_X, -5);
// set_hitbox_value(AT_DAIR, 4, HG_WIDTH, 20);
// set_hitbox_value(AT_DAIR, 4, HG_HEIGHT, 50);
// set_hitbox_value(AT_DAIR, 4, HG_SHAPE, 0);
// set_hitbox_value(AT_DAIR, 4, HG_PRIORITY, 2);
// set_hitbox_value(AT_DAIR, 4, HG_DAMAGE, 3);
// set_hitbox_value(AT_DAIR, 4, HG_ANGLE, 100);
// set_hitbox_value(AT_DAIR, 4, HG_BASE_KNOCKBACK, 4);
// set_hitbox_value(AT_DAIR, 4, HG_KNOCKBACK_SCALING, .55);
// set_hitbox_value(AT_DAIR, 4, HG_BASE_HITPAUSE, 6);
// set_hitbox_value(AT_DAIR, 4, HG_HITPAUSE_SCALING, .5);
// set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT, hitfx1);
// set_hitbox_value(AT_DAIR, 4, HG_VISUAL_EFFECT_Y_OFFSET, 20);
// set_hitbox_value(AT_DAIR, 4, HG_HIT_SFX, sound_get("monarch_smallhit4"));
// set_hitbox_value(AT_DAIR, 4, HG_HITBOX_GROUP, -1);