...
if (custom_clone) {
    ...
    switch (state)
    {
    	...
    	case PS_ATTACK_GROUND: case PS_ATTACK_AIR:
    		...
    		{
    			case AT_FSTRONG:
    			    if (window == get_attack_value(AT_FSTRONG, AG_STRONG_CHARGE_WINDOW) && smash_charging) //check strong charge window
                    {
                		var anim_first = get_window_value(AT_FSTRONG, 2, AG_WINDOW_ANIM_FRAME_START);
                		var anim_length = get_window_value(AT_FSTRONG, 2, AG_WINDOW_ANIM_FRAMES);
    		...
    		//looping strong charge animation - put this in fstrong
    		/*
