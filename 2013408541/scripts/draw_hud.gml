//draw_debug_text( temp_x + 26 , temp_y - 26, "HIGHWIND");
//draw_debug_text( temp_x + 48 , temp_y - 46, string( knockback_adj ));
if ("waryowl" in self)
{
	shader_start();
	draw_sprite(sprite_get("highwind_meter"), waryowl/50, temp_x - 4, temp_y - 22);
	draw_sprite(sprite_get("highwind_title"), waryowl/500, temp_x + 26, temp_y - 26)
	shader_end();
}

if ("helmet" in self && helmet == 1)
{
	draw_sprite(sprite_get("helmet_off_icon"),1, temp_x +172, temp_y - 30);
}

if attack == AT_NSPECIAL and (special_down && (window_timer >= 20))
{
	draw_sprite(sprite_get("highwind_hud_vfx"), get_gameplay_time() * .5, temp_x + 20, temp_y - 4);
}

if has_hit == true and (state == PS_ATTACK_AIR or state == PS_ATTACK_GROUND)
{
	draw_sprite(sprite_get("highwind_hud_vfx"), get_gameplay_time() * .5, temp_x + 20, temp_y - 4);
}
