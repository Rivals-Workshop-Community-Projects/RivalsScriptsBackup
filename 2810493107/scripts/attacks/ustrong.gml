set_attack_value(AT_USTRONG, AG_SPRITE, sprite_get("ustrong"));
set_attack_value(AT_USTRONG, AG_NUM_WINDOWS, 4);
set_attack_value(AT_USTRONG, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_USTRONG, AG_STRONG_CHARGE_WINDOW, 1);
set_attack_value(AT_USTRONG, AG_HURTBOX_SPRITE, sprite_get("ustrong_hurt"));

set_window_value(AT_USTRONG, 1, AG_WINDOW_LENGTH, 16);
set_window_value(AT_USTRONG, 1, AG_WINDOW_ANIM_FRAMES, 4);

set_window_value(AT_USTRONG, 2, AG_WINDOW_LENGTH, 20);
set_window_value(AT_USTRONG, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_USTRONG, 2, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_USTRONG, 2, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_USTRONG, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_USTRONG, 2, AG_WINDOW_SFX, sound_get("ustrongweakhits"));

set_window_value(AT_USTRONG, 3, AG_WINDOW_LENGTH, 3);
set_window_value(AT_USTRONG, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_USTRONG, 3, AG_WINDOW_ANIM_FRAME_START, 8);
//set_window_value(AT_USTRONG, 3, AG_WINDOW_HAS_SFX, 1);
//set_window_value(AT_USTRONG, 3, AG_WINDOW_SFX, sound_get("ustrongclap"));

set_window_value(AT_USTRONG, 4, AG_WINDOW_LENGTH, 12);
set_window_value(AT_USTRONG, 4, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_USTRONG, 4, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_USTRONG, 4, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_USTRONG, 13);

set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 1, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_Y, -32);
set_hitbox_value(AT_USTRONG, 1, HG_WIDTH, 150);
set_hitbox_value(AT_USTRONG, 1, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 1, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 1, HG_ANGLE, 120);
set_hitbox_value(AT_USTRONG, 1, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 1, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 1, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 1, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 1, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 1, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 2, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_Y, -85);
set_hitbox_value(AT_USTRONG, 2, HG_WIDTH, 75);
set_hitbox_value(AT_USTRONG, 2, HG_HEIGHT, 65);
set_hitbox_value(AT_USTRONG, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 2, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 2, HG_ANGLE, 90);
set_hitbox_value(AT_USTRONG, 2, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 2, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 2, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 2, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 2, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 2, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 2, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_USTRONG, 2, HG_FORCE_FLINCH, 1);
set_hitbox_value(AT_USTRONG, 2, HG_TECHABLE, 2);
set_hitbox_value(AT_USTRONG, 2, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 3, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 3, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 3, HG_WINDOW_CREATION_FRAME, 5);
set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_Y, -32);
set_hitbox_value(AT_USTRONG, 3, HG_WIDTH, 150);
set_hitbox_value(AT_USTRONG, 3, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 3, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 3, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 3, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 3, HG_ANGLE, 120);
set_hitbox_value(AT_USTRONG, 3, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 3, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 3, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 3, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 3, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 3, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 3, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 4, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 4, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 4, HG_WINDOW_CREATION_FRAME, 5);
set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_Y, -90);
set_hitbox_value(AT_USTRONG, 4, HG_WIDTH, 75);
set_hitbox_value(AT_USTRONG, 4, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 4, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 4, HG_ANGLE, 90);
set_hitbox_value(AT_USTRONG, 4, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 4, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 4, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 4, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 4, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 4, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 4, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_USTRONG, 4, HG_FORCE_FLINCH, 1);
set_hitbox_value(AT_USTRONG, 4, HG_TECHABLE, 2);
set_hitbox_value(AT_USTRONG, 4, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 5, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 5, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 5, HG_WINDOW_CREATION_FRAME, 10);
set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_Y, -32);
set_hitbox_value(AT_USTRONG, 5, HG_WIDTH, 150);
set_hitbox_value(AT_USTRONG, 5, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 5, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 5, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 5, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 5, HG_ANGLE, 60);
set_hitbox_value(AT_USTRONG, 5, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 5, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 5, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 5, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 5, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 5, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 5, HG_HITBOX_GROUP, 3);

set_hitbox_value(AT_USTRONG, 6, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 6, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 6, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 6, HG_WINDOW_CREATION_FRAME, 10);
set_hitbox_value(AT_USTRONG, 6, HG_HITBOX_Y, -90);
set_hitbox_value(AT_USTRONG, 6, HG_WIDTH, 75);
set_hitbox_value(AT_USTRONG, 6, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 6, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 6, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 6, HG_ANGLE, 90);
set_hitbox_value(AT_USTRONG, 6, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 6, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 6, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 6, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 6, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 6, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 6, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_USTRONG, 6, HG_FORCE_FLINCH, 1);
set_hitbox_value(AT_USTRONG, 6, HG_TECHABLE, 2);
set_hitbox_value(AT_USTRONG, 6, HG_HITBOX_GROUP, 3);

set_hitbox_value(AT_USTRONG, 7, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 7, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 7, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 7, HG_WINDOW_CREATION_FRAME, 15);
set_hitbox_value(AT_USTRONG, 7, HG_HITBOX_Y, -32);
set_hitbox_value(AT_USTRONG, 7, HG_WIDTH, 150);
set_hitbox_value(AT_USTRONG, 7, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 7, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 7, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 7, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 7, HG_ANGLE, 60);
set_hitbox_value(AT_USTRONG, 7, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 7, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 7, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 7, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 7, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 7, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 7, HG_HITBOX_GROUP, 4);

set_hitbox_value(AT_USTRONG, 8, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 8, HG_WINDOW, 2);
set_hitbox_value(AT_USTRONG, 8, HG_LIFETIME, 4);
set_hitbox_value(AT_USTRONG, 8, HG_WINDOW_CREATION_FRAME, 15);
set_hitbox_value(AT_USTRONG, 8, HG_HITBOX_Y, -90);
set_hitbox_value(AT_USTRONG, 8, HG_WIDTH, 75);
set_hitbox_value(AT_USTRONG, 8, HG_HEIGHT, 50);
set_hitbox_value(AT_USTRONG, 8, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 8, HG_DAMAGE, 3);
set_hitbox_value(AT_USTRONG, 8, HG_ANGLE, 90);
set_hitbox_value(AT_USTRONG, 8, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_USTRONG, 8, HG_KNOCKBACK_SCALING, 0.3);
set_hitbox_value(AT_USTRONG, 8, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_USTRONG, 8, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostweakfx"), 20));
set_hitbox_value(AT_USTRONG, 8, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 8, HG_HIT_SFX, asset_get("sfx_absa_singlezap1"));
set_hitbox_value(AT_USTRONG, 8, HG_ANGLE_FLIPPER, 9);
set_hitbox_value(AT_USTRONG, 8, HG_FORCE_FLINCH, 1);
set_hitbox_value(AT_USTRONG, 8, HG_TECHABLE, 2);
set_hitbox_value(AT_USTRONG, 8, HG_HITBOX_GROUP, 4);

set_hitbox_value(AT_USTRONG, 9, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_USTRONG, 9, HG_WINDOW, 3);
set_hitbox_value(AT_USTRONG, 9, HG_LIFETIME, 3);
set_hitbox_value(AT_USTRONG, 9, HG_HITBOX_X, -5);
set_hitbox_value(AT_USTRONG, 9, HG_HITBOX_Y, -97);
set_hitbox_value(AT_USTRONG, 9, HG_WIDTH, 75);
set_hitbox_value(AT_USTRONG, 9, HG_HEIGHT, 60);
set_hitbox_value(AT_USTRONG, 9, HG_SHAPE, 2);
set_hitbox_value(AT_USTRONG, 9, HG_PRIORITY, 2);
set_hitbox_value(AT_USTRONG, 9, HG_DAMAGE, 4);
set_hitbox_value(AT_USTRONG, 9, HG_ANGLE, 90);
set_hitbox_value(AT_USTRONG, 9, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_USTRONG, 9, HG_KNOCKBACK_SCALING, 1.1);
set_hitbox_value(AT_USTRONG, 9, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_USTRONG, 9, HG_HIT_LOCKOUT, 10);
set_hitbox_value(AT_USTRONG, 9, HG_VISUAL_EFFECT, hit_fx_create(sprite_get("ghostfx"), 20));
set_hitbox_value(AT_USTRONG, 9, HG_HITPAUSE_SCALING, .5);
set_hitbox_value(AT_USTRONG, 9, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_USTRONG, 9, HG_HIT_SFX, sound_get("ustrongstronghit"));