...
muno_char_name = "Mario Kart (ft. Shy Guy)";
...
initTip("Having no landing lag while running");

initWords("When you jump, if you're fast enough, there's no landing lag when you hit the ground.");
initWords("Basically, you can keep hopping while running as much as you like!");
initWords("However, if you do anything while in the air (such as attacking or double jumping), this isn't the case.");
initImage_ext(sprite_get("jump"), -1, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("dash"), -1, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("Using speed boosts");

initWords("There's plenty of little speed boosts you can have while playing as Kart.");
initWords("For starters, you can do this right at the start of the match!");
initWords("Hold the attack button when the '2' is on screen for a speed boost. If you do this too early, you'll get damaged. Do it too late, and nothing happens");
initImage_ext(sprite_get("idle"), 0, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("lakitu_spawn"), 5, fa_center, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("nair"), 29, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);initWords("");
initWords("");
initWords("");
initWords("");
initWords("");
...
initWords("After using Nair, keep the attack button pressed for a speed boost when you land.");
initWords("If you let go of the button then press it again, it won't work!");
initImage_ext(sprite_get("nair"), 4, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("nair"), 29, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);
initWords("");
initWords("");
initWords("");
initWords("");
...
initWords("Lastly, when you use the Dash Attack, you can keep the attack button pressed for another speed boost!");
initWords("This time, it only works if you hit the move! Like hitting another kart in anti-gravity mode in Mario Kart 8!");
initWords("The direction you'll dash into depends on the direction being held, so you can go left or right!");
initWords("So basically, you can either follow the opponent to continue a string, or do a classic hit-and-run!");
initImage_ext(sprite_get("dattack"), 7, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("nair"), 29, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("Cancelling Dair into Uspecial");

initWords("As the title implies, you can cancel the Dair into the Uspecial.");
initWords("If you do that, the glider gets some increased horizontal speed, as it carried momentum from your downwards trajectory.");
initWords("Use this near the ground for, like, a super wavedash.");
initImage_ext(sprite_get("dair"), 12, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("uspecialair"), 16, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("Using bananas effectively");

initWords("Bananas are versatile little things.");
initWords("Just like in Mario Kart, you can use them to mess with opponents or to protect yourself.");
initWords("If you use the move, the banana will initially be behind you.");
initWords("This banana can protect you from projectiles and other moves, but it only takes a hit.");
initWords("It's possible that you might hit your own banana with moves such as NAir, so always that this into consideration.");
initImage_ext(sprite_get("green_shell"), 1, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
...
initWords("The banana, as expected, trips up opponents when they touch it. But unlike in Smash, the opponents slide!");
initWords("If they touch the banana when they're not moving, however, the opponent still gets stunned, but doesn't slide.");
initImage_ext(sprite_get("banana"), 1, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("hurt"), 0, fa_center, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);
initWords("");
initWords("");
initWords("");
initWords("");
...
initWords("When you try to use DSpecial while the banana is behind you, you will drop it on the ground. Again, just like in Mario Kart.");
initWords("The same properties apply. They can shield hits, and be used as traps to opponents.");
initImage_ext(sprite_get("green_shell"), 1, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("banana"), 0, fa_center, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("dspecial"), 11, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initWords("Just be aware that (surprise!) just like in Mario Kart, you are NOT immune to your own banana.");
initWords("Which also means: don't drop the banana while driving backwards. What do you think will happen?");
initWords("One last thing! There's a 3 second cooldown before spawning a banana again, starting from when the previous banana disappeared.");
...
initPatch("Check the steam page", "???"); // (replace the date lol)
initHeader("No patches here");
initSection("Uh... Check the steam page to see other patches. If there's enough demand, the patches will go here as well.");

initPatch("About Mario Kart (ft. Shy Guy)", "");
initHeader("Character by Ghostabo");
initSection("I make stage commissions :)");
initHeader("Sfx from:");
initSection("The Sounds Resouce, mostly");
initHeader("Compatible with");
initSection("Whatever the steam page says!");
...
CHEAT_ULTRA_SHORTCUTS	= initCheat("Allow Ultra Shortcuts", [0], ["Sorry, this was patched."], "Sadly, this was patched by Nintendo itself.");
...
initCodecPage(SPK_TRUM, 0, 0, "oh my god a kart");
initCodecPage(SPK_ALTO, 0, 1, "But is the kart mightier than the cloud?");
initCodecPage(SPK_OTTO, 0, 6, "I know for a fact it isn't mightier than the bike.");
...
initCodecPit();
initCodecPagePit(SPK_PIT, 	3,	0,	"Is that a Mario Kart?");
initCodecPagePit(SPK_PALU, 	0,	1,	"Apparently, it can fight as well as it can race.");
initCodecPagePit(SPK_VIR, 	1,	1,	"So... chaotically?");
initCodecPagePit(SPK_PALU, 	2,	2,	"This pretty much sums it. This Kart can use plenty of items to mess you up.");
initCodecPagePit(SPK_PALU, 	2,	-3,	"And as expected, it has great mobility.");
initCodecPagePit(SPK_VIR, 	1,	6,	"But like a certain someone, it's still lacking in the air department.");
initCodecPagePit(SPK_VIR,	6,	-1,	"Still, just like you have the power of flight, the Kart has its glider.");
initCodecPagePit(SPK_PALU,	0,	3,	"By the way, Pit, a Shy Guy may be behind the wheels, but the Kart is the one you're fighting.");
initCodecPagePit(SPK_PALU,	4,	4,	"Not that I'd know if there's really a Shy Guy there. I'm not even looking at the fight.");
initCodecPagePit(SPK_PIT,	9,	9,	"What?! But I need your assistance!");
initCodecPagePit(SPK_PALU,	4,	5,	"Don't worry, Pit. Just scream my name if you need to fly or something.");
initCodecPagePit(SPK_PALU,	5,	4,	"I'm, uh... kinda.. busy.");
initCodecPagePit(SPK_VIR,	5,	1,	"Yeah, Pit. Us goddesses have better things to do than to watch your childish quarrels.");
initCodecPagePit(SPK_PALU,	2,	5,	"... Hey, Viridi. Can I pick Rainbow Road?");
initCodecPagePit(SPK_VIR,	6,	6,	"Only if it's 200cc!");
