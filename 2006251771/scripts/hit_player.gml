...
if (btt && my_hitboxID.attack == AT_EXTRA_1) {
	hit_player_obj.invincible = true;
	hit_player_obj.invince_time = 120;
}

// Stale move calculation
if (!my_hitboxID.orig_player_id.balanced_falco) {
	if (fox) {
	...
	if (fox || (my_hitboxID.attack != AT_DAIR && my_hitboxID.attack != AT_DATTACK && my_hitboxID.attack != AT_DSPECIAL && my_hitboxID.attack != AT_DTILT && my_hitboxID.attack != AT_FAIR && my_hitboxID.attack != AT_FSTRONG && my_hitboxID.attack != AT_UAIR && my_hitboxID.attack != AT_USPECIAL)) {
	...
	if (!my_hitboxID.orig_player_id.shovel_knight_exists) {
