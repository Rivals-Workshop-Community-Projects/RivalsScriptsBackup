...
mask_index = sprite_get("ground_uspecial_shield_mask");
...
replacedcount_shield = 0;                                  //This gets incremented whenever a new article is created. If this reaches maxarticles, the article is destroyed
maxarticles = 1;                                    //The maximum number of this article that should exist at once.

with(asset_get("obj_article2")){
    if (player_id == other.player_id){
        replacedcount_shield++;                            //If there are any other article1s owned by the same person, tell them to increment their replacedcount by 1
    }
}
...
hitbox_hit = noone;
hit_cooldown = 0;
ice_sfx = sound_get("se_zelda_attackair_L01")
