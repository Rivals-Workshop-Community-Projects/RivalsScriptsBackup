set_attack_value(AT_EXTRA_1, AG_MUNO_ATTACK_NAME, "Crouch Dash");

set_attack_value(AT_EXTRA_1, AG_CATEGORY, 0);
set_attack_value(AT_EXTRA_1, AG_SPRITE, sprite_get("crouchdash"));
set_attack_value(AT_EXTRA_1, AG_NUM_WINDOWS, 2);
set_attack_value(AT_EXTRA_1, AG_OFF_LEDGE, 1);
set_attack_value(AT_EXTRA_1, AG_HURTBOX_SPRITE, asset_get("cat_crouchbox"));


set_window_value(AT_EXTRA_1, 1, AG_WINDOW_LENGTH, 5);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 1, AG_MUNO_WINDOW_INVUL, -1);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_HSPEED, 4);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_SFX, asset_get("sfx_dash_start"));


set_window_value(AT_EXTRA_1, 2, AG_WINDOW_LENGTH, 12);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_ANIM_FRAME_START, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_ANIM_FRAMES, 5);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HSPEED, 5);


set_attack_value(AT_EXTRA_1, AG_MUNO_ATTACK_NAME, "Crouch Dash");
set_attack_value(AT_EXTRA_1, AG_MUNO_ATTACK_MISC, "Accessed using: f,d,df
-----------------------
during this move press:
A for Wind God Fist
B for Spinning Demon Kick
Strong for Dragon Uppercut");