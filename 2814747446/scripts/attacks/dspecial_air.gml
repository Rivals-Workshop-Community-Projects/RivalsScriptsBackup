set_attack_value(AT_DSPECIAL_AIR, AG_CATEGORY, 2);
set_attack_value(AT_DSPECIAL_AIR, AG_SPRITE, sprite_get("dspecial_air"));
set_attack_value(AT_DSPECIAL_AIR, AG_NUM_WINDOWS, 8);
set_attack_value(AT_DSPECIAL_AIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_DSPECIAL_AIR, AG_AIR_SPRITE, sprite_get("dspecial_air"));
set_attack_value(AT_DSPECIAL_AIR, AG_HURTBOX_SPRITE, sprite_get("dspecial_air_hurt"));

set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_LENGTH, 21);
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_VSPEED, -1)
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy"));
set_window_value(AT_DSPECIAL_AIR, 1, AG_WINDOW_SFX_FRAME, 1);

set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_TYPE, 10);
set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_LENGTH, 5);
set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_VSPEED, 25);
set_window_value(AT_DSPECIAL_AIR, 2, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);

set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_LENGTH, 16);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_SFX, asset_get("sfx_kragg_rock_shatter"));
set_window_value(AT_DSPECIAL_AIR, 3, AG_WINDOW_SFX_FRAME, 4);

set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_LENGTH, 50);
set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_ANIM_FRAMES, 7);
set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_ANIM_FRAME_START, 11);
set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_HSPEED, 8.3);
set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSPECIAL_AIR, 4, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);

set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_ANIM_FRAME_START, 18);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_CUSTOM_GROUND_FRICTION, 5);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_VSPEED, 9999);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_SFX, asset_get("sfx_kragg_rock_pillar"));
set_window_value(AT_DSPECIAL_AIR, 5, AG_WINDOW_SFX_FRAME, 2);

set_window_value(AT_DSPECIAL_AIR, 6, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 6, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DSPECIAL_AIR, 6, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSPECIAL_AIR, 6, AG_WINDOW_ANIM_FRAME_START, 21);
set_window_value(AT_DSPECIAL_AIR, 6, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSPECIAL_AIR, 6, AG_WINDOW_SFX, asset_get("sfx_dust_knuckle"));

set_window_value(AT_DSPECIAL_AIR, 7, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 7, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DSPECIAL_AIR, 7, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSPECIAL_AIR, 7, AG_WINDOW_ANIM_FRAME_START, 24);


set_window_value(AT_DSPECIAL_AIR, 8, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 8, AG_WINDOW_LENGTH, 15);
set_window_value(AT_DSPECIAL_AIR, 8, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSPECIAL_AIR, 8, AG_WINDOW_ANIM_FRAME_START, 27);

set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_LENGTH, 26);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_ANIM_FRAMES, 8);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_ANIM_FRAME_START, 31);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_CUSTOM_GROUND_FRICTION, 0.35);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_SFX, asset_get("sfx_rag_axe_hitsground"));
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_SFX_FRAME, 2);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_DSPECIAL_AIR, 9, AG_WINDOW_CANCEL_FRAME, 35);

set_num_hitboxes(AT_DSPECIAL_AIR, 5);

set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HITBOX_GROUP, 5);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_WINDOW,2);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HITBOX_X, 0);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HITBOX_Y, -19);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_WIDTH, 100);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HEIGHT, 100);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_DAMAGE, 8);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_ANGLE, 80);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_KNOCKBACK_SCALING, .9);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HITPAUSE_SCALING, .25);
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));
set_hitbox_value(AT_DSPECIAL_AIR, 1, HG_IGNORES_PROJECTILES, 1);

set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_WINDOW, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_HITBOX_X, 50);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_HITBOX_Y, -19);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_WIDTH, 90);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_HEIGHT, 80);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_DAMAGE, 3);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_ANGLE, 100);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_HIT_SFX, asset_get("sfx_kragg_roll_land"));
set_hitbox_value(AT_DSPECIAL_AIR, 2, HG_IGNORES_PROJECTILES, 1);


set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_WINDOW, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_LIFETIME, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HITBOX_X, 30);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HITBOX_Y, -90);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_WIDTH, 108);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HEIGHT, 133);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_PRIORITY, 8);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_DAMAGE, 13);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_ANGLE, 93);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_KNOCKBACK_SCALING, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HITPAUSE_SCALING, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 3, HG_HIT_LOCKOUT, 0);

set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_WINDOW, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_LIFETIME, 5);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_SHAPE, 0);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HITBOX_X, 15);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HITBOX_Y, -155);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_WIDTH, 74);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HEIGHT, 64);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_DAMAGE, 13);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_ANGLE, 75);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_KNOCKBACK_SCALING, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HITPAUSE_SCALING, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 4, HG_HIT_LOCKOUT, 10);

set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_WINDOW, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_LIFETIME, 4);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_SHAPE, 0);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HITBOX_X, -20);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HITBOX_Y, -150);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_WIDTH, 74);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HEIGHT, 64);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_PRIORITY, 2);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_DAMAGE, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_ANGLE, 120);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_KNOCKBACK_SCALING, 0.7);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HITPAUSE_SCALING, .6);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HIT_SFX, asset_get("sfx_blow_medium1"));
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSPECIAL_AIR, 5, HG_HIT_LOCKOUT, 10);