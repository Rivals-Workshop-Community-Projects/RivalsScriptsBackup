...
var hurtboxWidth = fn("HurtboxWidth", ai_target);
...
var voat = ceil((max_fall_spd - vvsp)/ac_y);


if(frame > voat){
	var new_y = yy + vvsp*voat + ac_y*voat*voat/2;
	new_y += vvsp*(frame-voat);
}else{
	var new_y = yy + vvsp*frame + ac_y*frame*frame/2;
}
...
for(var i = 0; i < len; i++){
  ...
  lastPos = newPredict(self, cad[@ AD_FRAME]);
  estOPos = newPredict(ai_target, cad[@ AD_FRAME]);
