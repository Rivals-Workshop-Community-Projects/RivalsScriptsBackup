...
initPatch("2.0", "25 September, 2021");
initHeader("Girlfriend - Adjustments, Bugfixes");
initSection("Added hitpause.
Added a hurt frame.
Added a hud indicating HP.");
initHeader("DSpecial");
initSection("DSpecial's timer is .5% faster.
DSpecial's timer no longer gets reset on hit.
Notes: Hitpause 6 > 10.
Notes: Knockback 5 > 6.
Notes: Hitpause scaling .7 > 1.
Notes: Can't tech or bounce.
Explosion: Knockback scaling .7 > 1.1.
Explosion: Hitpause 7 > 15.
Explosion: Extra hitpause 0 > 5.
Explosion: Hitpause scaling .7 > 1.1.");
initHeader("DAttack");
initSection("Made DAttack's back to dash window two frames later.
DAttack's ground friction on window 3 .4 > .1.
DAttack's endlag 11 > 13
DAttack whifflag 6 > 12.");
initHeader("DStrong");
initSection("Window 1 length 13 > 10.
Window 3 length 13 > 10.");
initHeader("FSpecial");
initSection("Made FSpecial no longer put you into pratfall.
Gave FSpecial anti stall (first use in the air it stalls you, and further uses until landing will not stall you).");
initHeader("DTilt");
initSection("Sweetspot priority 7 > 8.
Sweetspot sound changed.");
initHeader("NAir");
initSection("Hbox 1/2: Damage 7 > 5.
Hbox 1/2: SFX changed.
Hbox 3: Damage 6 > 8.
Hbox 3: Hitpause 7 > 9.
Hbox 3: SFX changed.
You bounce higher vsp -5 > -6.");
initHeader("NSpecial");
initSection("Full charge: Hitpause .6 > 1.
Full charge: Transcendent.
Notes now have despawn effects.");
initHeader("UAir");
initSection("Sweetspot SFX changed.");
initHeader("UTilt");
initSection("Sweetspot SFX changed.
Sourspot extra hitpause 0 > 4.
Sourspot hitpause scaling .35 > 5.
Sweetspot knockback 7 > 8.
Sweetspot knockback scaling .5 > .6.
Sweetspot hitpause 8 > 10.
Sweetspot extra hitpause 0 > 6.
Sweetspot hitpause scaling .5 > 7.");
initHeader("USpecial");
initSection("Initial SFX changed.
All arrows: KB scaling .1 > .2.
All arrows: Hitpause 0 > 6.
All arrows: Hitpause scaling 0 > .2.");
initHeader("FTilt");
initSection("Sourspot angle 70 > 60.
Sweetspot angle 361 > 30.");
initHeader("Misc");
initSection("Added the blue balls SFX when losing the final stock.
Added MunoPhone.
Pit compat.
Trummel 2 compat.
Fixed an error where BF used the old jump sprites.
Fixed the airdodge frames.");
...
SPK_BOOP = initSpeaker(1, "Boyfriend", sprite_get("_pho_bf_speaker"));
SPK_GIRL = initSpeaker(2, "Girlfriend", sprite_get("_pho_example_speaker"));

initCodec(0); // this should just always be 0, because there are no codec gimmicks
initCodecPage(SPK_TRUM, 0, 0, "wow is that boyfriend friday night funkin");
initCodecPage(SPK_BOOP, 0, 0, "beep?"); // this page uses the custom speaker
...
initCodecPit();
initCodecPagePit(SPK_PIT,	3,	9,	"Hey, it's Boyfriend!");
initCodecPagePit(SPK_PALU,	0,	5,	"Indeed it is Pit.");
initCodecPagePit(SPK_PIT,	0,	0,	"I can easily beat him. Look at how small he is");
initCodecPagePit(SPK_PALU,	1,	0,	"I'm not so sure about that.");
initCodecPagePit(SPK_PALU,	0,	5,	"BF's got a lot of tricks up his albeit small sleeves. He can hit you with a barrage of beeps and boops racking up damage");
initCodecPagePit(SPK_PIT,	6,	0,	"Pssh, sounds like a pushover to me.");
initCodecPagePit(SPK_PALU,	0,	5,	"Don't be so sure - BF has his love GF on the stage cheering him on.");
initCodecPagePit(SPK_PALU,	4,	2,	"She can be used for stage control as if BF builds up a meter she'll teleport away leaving a massive explosion behind!");
initCodecPagePit(SPK_PIT,	1,	1,	"What else can he do?");
initCodecPagePit(SPK_VIR,	1,	1,	"Get a girl that's what.");
initCodecPagePit(SPK_PIT,	2,	2,	"....");
initCodecPagePit(SPK_PALU,	4,	5,	"Oh, stop it. BF can also deploy an arrow to launch you. So be careful when recovering or you may be sent hurling to your death.");
initCodecPagePit(SPK_PIT,	4,  6,	"....Can we not do this fight?");
initCodecPagePit(SPK_PALU,	2,	5,	"Nope! Now get out there and see if you can match his rapping skills!");
initCodecPagePit(SPK_PIT,	10,	10,	"....yes Lady Palutena..");
/*
╔═══════════════════════════════════════════════════════════════════════════╗
║																			║
║ Behind-The-Scenes															║
