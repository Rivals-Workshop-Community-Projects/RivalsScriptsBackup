...
if phone.state{
	...
	if !(phone.stage_id != noone && phone.state == 5){
		...
		with phone{
			...
			rectDraw(origin_x, origin_y, screen_w, screen_h, c_black);
			...
			switch(app){
				...
				case 7:
					...
					switch(player_id.muno_char_id){
						...
						default:
							...
							var drawn = textDraw(draw_x, draw_y, "fName", c_lime, 18, draw_w, fa_center, 1, true, phone.image_alpha, "Trummel-Only", true);
							if !selected changeCursor(draw_x - drawn[0] / 2 - 7, draw_y - drawn[1] / 2 - 4 + sin_thing, drawn[0] + 11, drawn[1] + 1);
							
							draw_set_valign(valign);
							
							textDraw(draw_x, origin_y + screen_h - 58, "fName", c_green, 18, draw_w, fa_center, 1, true, phone.image_alpha, "This app is only for Trummel.", false);
				...
				case 9:
					...
					var drawn = textDraw(draw_x, draw_y, "fName", c_lime, 18, draw_w, fa_center, 1, true, phone.image_alpha, "Turn Off MunoPhone", true);
					if !selected changeCursor(draw_x - drawn[0] / 2 - 7, draw_y - drawn[1] / 2 - 4 + sin_thing, drawn[0] + 11, drawn[1] + 1);
					
					draw_set_valign(valign);
					
					textDraw(draw_x, origin_y + screen_h - 40, "fName", c_green, 18, draw_w, fa_center, 1, true, phone.image_alpha, "Press F5 to re-enable.", false);
				...
				case 6:
					...
					if stage_id == noone{
						...
						var drawn = textDraw(draw_x, draw_y, "fName", c_lime, 18, draw_w, fa_center, 1, true, phone.image_alpha, "NOT CONNECTED", true);
						if !selected changeCursor(draw_x - drawn[0] / 2 - 7, draw_y - drawn[1] / 2 - 4 + sin_thing, drawn[0] + 11, drawn[1] + 1);
						
						draw_set_valign(valign);
						
						textDraw(draw_x, origin_y + screen_h - 40, "fName", c_green, 18, draw_w, fa_center, 1, true, phone.image_alpha, "See big screen.", false);
...
if phone.state{
	...
	with phone if (side_bar.state == 2 && app){
		...
		if (state != 2 && state != 4 && stage_id != noone && stage_id.screen.low_fps = true){
			...
			textDraw(draw_x, draw_y, "fName", c_lime, 100, 1000, fa_center, 1, 0, 1, "- Hidden due to Performance Considerations -", false)
...
textDraw(x1 + 12, y1 + 10, "fName", c_lime, 16, 300, fa_left, 1, false, phone.image_alpha * 2 - 1, text, false);
...
textDraw(origin_x + screen_w / 2 + 2, origin_y + 15 - sin_thing, "fName", c_lime, 1000, 1000, fa_center, 1, true, phone.image_alpha, apps[cursor+1].name, false);
...
for (i = 0; i < array_length_1d(apps); i++){
	...
	if (i == 2){
		...
		textDraw(draw_x, draw_y, "fName", c_lime, 1000, 1000, fa_right, 1, true, 1, "v" + string(get_char_info(player_id.player, INFO_VER_MAJOR)) + "." + string(get_char_info(player_id.player, INFO_VER_MINOR)), false);
	}
}
...
if false && (phone_settings[setting_fast_graphics] || array_length_1d(arr) > 50 || (app == 4 && apps[app].array[cursor] == 2 && array_length_1d(apps[app].array[cursor].hitboxes) > 5)){
	...
	if array_length_1d(arr) for (var i = 0; i < array_length_1d(arr); i++){
		...
		var drawn = textDraw(origin_x + 6, draw_y - crs * sin_thing, "fName", crs ? c_lime : c_green, 18, screen_w - 6 * 3, fa_left, 1, false, do_opac, txt, true);
