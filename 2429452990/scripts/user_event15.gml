...
phone.taunt_hint_y = -20;									// Vertical offset
...
spr_mine = sprite_get("duckmine");
spr_rock = sprite_get("duckrock");
spr_jumptv = sprite_get("hudtvjump");
spr_speedtv = sprite_get("hudtvspeed");
...
with phone{
	...
	initTip("NSpecial");
	initTipWords("NSpecial throws out a mine that blows up when anyone that goes near it, including you.");
	initTipImage(player_id.spr_nspecial, -5, fa_left, 1, c_white, 0);
	initTipImage(player_id.spr_mine, -5, fa_right, 1, c_white, 0);
	initTipWords("If you already have a mine out or hold down the Shield button during the start of the move, you'll throw out a rock instead.");
	initTipImage(player_id.spr_rock, -5, fa_center, 1, c_white, 0);

	initTip("FSpecial");
	initTipWords("FSpecial is the WAGNUS, an instant teleporting laser gun. Anyone you pass through when using it gets damaged. Be careful; it has a limited number of charges, and if you try to use it when you've run out, it'll blow up in your face.");
	initTipImage(player_id.spr_fspecial, -5, fa_center, 1, c_white, 0);
	
	initTip("DSpecial");
	initTipWords("DSpecial is the TV. When you use the move, you'll switch between two channels. The green arrow gives you extra midair jumps, while the yellow arrow gives you greatly increased movement speed. Which channel you're currently on is displayed on your HUD.");
	initTipImage(player_id.spr_jumptv, -5, fa_center, 1, c_white, 0);
	initTipImage(player_id.spr_speedtv, -5, fa_center, 1, c_white, 0);
	
	initTip("USpecial");
	initTipWords("USpecial is the jetpack. It's a simple upwards recovery move, and you can cancel out of it at any time.");
	initTipImage(player_id.spr_uspecial, 5, fa_center, 1, c_white, 1);
...
with phone{
	 ...
	 */
	initPatch("1.0.1", "March 19th, 2021");
	initPatchWords_ext("-Increased ground friction (0.3 -> 0.4)
	-Decreased Forward-Air horizontal speed (6 -> 3)
	-Neutral-Air no longer halts your current vertical speed
	-Parrying Neutral-Special explosion now puts the move on cooldown", fa_left, c_white, 0, 0);

	initPatch("1.0", "March 19th, 2021");
	initPatchWords_ext("quack", fa_center, c_white, 0, 1);
	initPatchImage(other.spr_idle, 0, fa_center, 1, c_white, 1);
...
with phone{
	...
	initAbout("About Duck", "Duck Game created by:
	Landon Podbielski (@superjoebob)");
