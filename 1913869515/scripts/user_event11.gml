...
if my_hitboxID.sound_effect == sound_get("tstrong") {
slashdraw = 10
maxdraw = 10
hitdmg = 15
angledraw = random_func(1,360,true)  
...
if dmhit >= 3 {
sound_play(sound_get("stackfinish"),false,noone,0.8);
...
with hit_player_obj {
	...
	dmhit = 0
	if my_hitboxID.attack == AT_JAB jabrp = 6
	if halox < 8 halox += 1
...
if get_player_color(player) == 3{
    if( attack != AT_NSPECIAL){
    	if spr_dir == -1  {
	      create_hitbox(AT_FSPECIAL, 9 ,  round(hit_player_obj.x - 30 + random_func(1, 60, true)), round(hit_player_obj.y - 60 + random_func(2, 30, true)) );
    	} else {
    	 create_hitbox(AT_FSPECIAL, 10 ,  round(hit_player_obj.x - 30 + random_func(1, 60, true)), round(hit_player_obj.y - 60 + random_func(2, 30, true)) );
    	}
    }
}
...
if ( my_hitboxID.attack == AT_NSPECIAL && my_hitboxID.type == 2 && state_cat != SC_HITSTUN && jabrp <= 0){
   hitstop = 2 
   hitpause = true 
   old_hsp = hsp 
   old_vsp = vsp 
