...
if (!"hh_currAlt" in self)
{
	image_alpha = 3;
	hh_currAlt = alt_new;
}
else if (alt_new != hh_currAlt)
{
	image_alpha = 1.5;
	offset = sign(alt_new - hh_currAlt)*16;
	hh_currAlt = alt_new;
	sound_play(sound_get("menu_select_new"))
	
	if alt_new == 31 {
		sound_play(sound_get("voice_mj_hoo"))
	}
}
...
if "hh_alt_name" not in self or ("hh_alt_name" in self and hh_alt_name != names) 
{
	hh_alt_name = names
}


if ("hh_numAlts" not in self or ("hh_numAlts" in self and hh_numAlts != array_length(names))) {
	hh_numAlts = array_length(hh_alt_name);
}
...
if (hh_currAlt != 0)			textDraw(temp_x + 2 + min(image_alpha,1) * 8 + ((max(image_alpha*2,2.5)-1)*4)-6, temp_y + 76 + offset, "fName", c_gray, 0, 1000, 1, true, image_alpha, hh_alt_name[hh_currAlt-1]);
textLen =		 (textLen + textDraw(temp_x + 2 + min(image_alpha,1) * 16 + ((max(image_alpha*2,2.5)-1)*5)-6, temp_y + 96 + offset, "fName", c_white, 0, 1000, 1, true, image_alpha, hh_alt_name[hh_currAlt]))/2;
if (hh_currAlt != hh_numAlts-1)	textDraw(temp_x + 2 + min(image_alpha,1) * 8 + ((max(image_alpha*2,2.5)-1)*4)-6, temp_y + 116 + offset, "fName", c_gray, 0, 1000, 1, true, image_alpha, hh_alt_name[hh_currAlt+1]);
