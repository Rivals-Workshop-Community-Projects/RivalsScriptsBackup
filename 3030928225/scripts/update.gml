...
// Needed to make the charge-done flash fade out
if (charge_flash_cooldown > 0) {
	charge_flash_cooldown--;
}
...
// Stun opponents who were grabbed and released next to us
if (stun_counter > 0) {
	//print_debug("stun countdown " + string(stun_counter));
	stun_counter--;
    with (oPlayer) {
        if (SQUIGLY_HOLDER_ID == other.id) {
			//print_debug("held id =  " + string(id));
        	vsp = 0;
        	hsp = 0;
        	hitstop = 1;
        }
...
with hit_fx_obj if player_id == other {
    if hit_fx == other.fx_pow_hit[1] || hit_fx == other.fx_pow_hit[2] || hit_fx == other.fx_pow_sparks || hit_fx == other.fx_pow_hit[0] {
