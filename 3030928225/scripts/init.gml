...
jabtimer                = 0;                    // 0                used to determine how long rapid jab has been rapid jabbing (local vars hate me)
...
c_fire = make_color_rgb(get_color_profile_slot_r(selected_player_color, 7),
						get_color_profile_slot_g(selected_player_color, 7),
						get_color_profile_slot_b(selected_player_color, 7));

// Strong charge
stored_strong_charge = 0;
strong_full_charge_time = 60; // How long the player flashes yellow, do not modify
// Special charge
special_charge = 0;
special_full_charge_time = 60; // Time to full charge
// Shared for both types of charge
fully_charged = false;
...
grab_hitbox = noone;
rope_image = sprite_get("tail");
SQUIGLY_HOLDER_ID = noone;
holding_someone = false;
fspecial_caught_one = false;
// This is how long we lock the user out from landing fspecial again, in an effort to prevent chain grabbing
//fspecial_lockout_time = fspecial_stun_time + 160;
end_reeling_immediately = false;
returning_empty_handed_in_last_frames = false;
harpoon_parried = false;
// Ok to change these:
//fspecial_uncharged_initial_speed = 11;
fspecial_uncharged_max_distance = 200;
...
dspecial_air_initial_duration = 9;
dspecial_air_looping_duration = 12;
...
fx_pow_sparks = hit_fx_create(sprite_get("fx_pow_sparks"), 30); //this effect spawns when someone is affected by tester's u-air status aswell as the d-spec teleport
