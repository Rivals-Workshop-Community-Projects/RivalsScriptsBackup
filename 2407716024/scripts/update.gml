...
with (hit_fx_obj)
{
	if (player_id == other.id && spr_dir == -1 &&
		(hit_fx == other.bigpow || hit_fx == other.bigboom || hit_fx == other.boom)) 
	{
		spr_dir = 1;
	}
}
...
switch (state) 
{
    ...
    if (has_hit_player && attack_pressed) set_state(PS_IDLE);
    break;
...
with hit_fx_obj if player_id == other {
    if hit_fx == other.pow || hit_fx == other.bigpow || hit_fx == other.boom || hit_fx == other.bigboom {
...
if (attempt_air_dash > 0 && peacock_has_airdash) 
{
	if (state != PS_ATTACK_AIR)
	{
		if (dash_toggle == 0)
		{
			var _trigger = (right_down || right_hard_pressed) - (left_down || left_hard_pressed);
			if (_trigger == 0) _trigger = spr_dir;
			if ((taunt_down || taunt_pressed) && _trigger == spr_dir && 
				(state == PS_TUMBLE || state == PS_IDLE_AIR || state == PS_DOUBLE_JUMP || (state == PS_FIRST_JUMP && state_timer > 2)))
			{
		        ...
		        //had_airdodge_pre_dash = has_airdodge;
		        //has_airdodge = false;
		    }
		    else if ((taunt_down || taunt_pressed) && _trigger != spr_dir) 
		    {
		        if (free) 
		        {
		            clear_button_buffers_after_dashing();
    ...
    if (_triggered && state == PS_ATTACK_AIR && attack != AT_EXTRA_1 && attack != AT_EXTRA_2 && has_hit)
	{
		if (hitpause)
		{
			hitpause = false;
...
if ((free && state != PS_ATTACK_AIR && state != PS_ATTACK_GROUND) || (state == PS_FIRST_JUMP && state_timer == 0) || 
	(free && state == PS_ATTACK_AIR))
{
	var dash_dir = 
		(dash_toggle == 0)
			// TAUNT
			? 0
			// SHIELD
			: (right_down || right_hard_pressed) - (left_down || left_hard_pressed);
    ...
    switch (dash_dir) 
    {
        ...
        if (dash_dir == sign(peacock_airdash_button_main_buffer)) {
            ...
            if (spr_dir == dash_dir) peacock_airdash_forward_dash_buffer = max(7, airdash_buffer_abs);
            else peacock_airdash_backward_dash_buffer = max(7, airdash_buffer_abs);
        }
        else peacock_airdash_button_main_buffer = dash_dir * 12;
        break;
    }
...
if (easter_egg_timer <= 90) 
{
	...
	else if (taunt_down) 
	{
		...
		else
		{
			switch (oPlayer.url)
			{
				case 2135192216: sound_play(sound_get("intro_filia")); break;
				
				case CH_ZETTERBURN: sound_play(sound_get("intro_zet")); break;
				
				case CH_ELLIANA: sound_play(sound_get("intro_elli")); break;
				
				case CH_CLAIREN: sound_play(sound_get("intro_clar")); break;
				
				case CH_ORI: sound_play(sound_get("intro_ori")); break;
				
				case 2229862078: sound_play(sound_get("intro_ori")); break;
				
				case 2261916872: 
				case 1870630263: sound_play(sound_get("intro_peacock")); break;
				
				case 1890617624:
				case 1871153050:
				case 1970731843:
				case 2190448871: sound_play(sound_get("intro_ronald")); break;
				
				default: sound_play(sound_get("intro" + string(1 + random_func(0, 4, true)))); break;
			}
		}
	}
	with (oPlayer) 
...
if (instance_exists(peacock_article_doom_id))
{
	if (time_frozen)
	{
	    ...
	    with (oPlayer) 
	    {
	        if (player != other.player) 
	        {
				...
				if (!place_meeting(x, y, other.peacock_article_doom_id) && state != PS_PRATFALL)
				{
					if (state != PS_ATTACK_GROUND && !free) state = PS_HITSTUN;
			        if (state != PS_ATTACK_AIR && free) state = PS_HITSTUN;
				}
				
				// Attacks handling.
				for (var i = 0; i < array_length_1d(attacks); ++i)
				{
					if (move_cooldown[@attacks[@i]] < 2) move_cooldown[@attacks[@i]] = 2;
				}
	        }
	        ...
	        {
	            with (asset_get(articles[@i])) hitstop = 2;
	        }
	    }
	}
	...
	if (peacock_article_doom_id.roller_freeze = true)
	{
        ...
        if (!place_meeting (x, y, peacock_article_doom_id) && 
        	(state != PS_ATTACK_GROUND && !free && state != PS_PRATFALL ||
        	 state != PS_ATTACK_AIR && free && state != PS_PRATFALL))
        {
        	state = PS_PRATLAND;
        }
	}
...
if (easter_egg_set == true && state_timer == 1)
{
	switch (state)
	{
		case PS_TECH_GROUND:
		sound_play(sound_get("tech" + string(1 + random_func(0, 6, true))));
		break;
		
		case PS_WALL_TECH:
		case PS_TECH_FORWARD:
		case PS_TECH_BACKWARD:
		sound_play(sound_get("tech" + string(1 + random_func(0, 3, true))));
		break;
		
		case PS_FIRST_JUMP:
		case PS_DOUBLE_JUMP:
		sound_play(sound_get("jump" + string(1 + random_func(0, 3, true))));
		break;
	}
}

if (got_hit_num == 1) got_hit_ticks++;
...
if (state_cat == SC_HITSTUN && got_hit_ticks == 16)
{
	...
	if (easter_egg_set == true && got_hit_num == 2)
	{
	    ...
	    sound_play(sound_get("hurt" + string(1 + random_func(0, 6, true))));
	    if (snd_rng == 0) {
