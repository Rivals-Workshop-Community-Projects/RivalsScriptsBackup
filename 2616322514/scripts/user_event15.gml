...
#define CORE_tips
initTip("Moveset Overview");
initWords("The guardian deity of Melemele Island is naturally one of Alola's strongest pokemon, and bears an interest and tendency in fighting anyone it deems worthy enough. In Rivals of Aether, Tapu Koko plays as a glass canon, being very powerful and agile while not hard to knock out when given the right opening.");
initWords("Tapu Koko's ability, Electric Surge, allows it to harness the full potential of its electric abilities, and with it can the guardian truly be a formidable foe. However, such an ability only works to boost Koko itself, whereas its vulnerability doesn't differ in the slightest, showing it most in the immense lag that comes with some of its stronger attacks.");
initWords("All in all, it's Koko's job to come at its foe with everything its got and find the right openings for a powerful attack, and it's up to those fighting it to stand their ground and punish any slip ups Koko might make.");
initImage_ext(sprite_get("taunt"), -4, fa_center, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("Gimmick: Electric Surge");
initWords("Tapu Koko's ability, Electric Surge, makes it create an electric terrain under various conditions. This can be achieved when landing sweetspots on FTilt, FAir, UAir, or DAir, or landing a parry or a counter with DSpecial. In practice mode, holding down the taunt button while taunting will also make an electric terrain.");
initWords("The electric terrain makes any electric based moves stronger in some shape or form. Sweetspots on the moves aforementioned, along with DTilt and UTilt, will stun anyone it hits, whereas tilts can be cancelled into strongs or specials after hitting. NSpecial, FSpecial, and all strongs will consume the terrain to supercharge the attack. In addition, Koko's wavedash will become significantly faster as well, and can even be used to keep a charge outside of the terrain.");
initWords("Keep in mind, the terrain will automatically disappear after 10 seconds, upon being KO'd, or when parried.");
initImage_ext(sprite_get("terrain_idle"), -4, fa_center, -1, 1, true, c_white, 1, true, noone, noone, noone, noone);

initTip("NSpecial: Discharge");
initWords("Tapu Koko used Discharge!");
initWords("During this attack, Koko will let loose a brief burst of electricity that hits anyone within range multiple times before launching them upwards. A brief cooldown will be put on the attack after use.");
initWords("When the attack consumes the electric terrain, however, the shock will reach much farther than normal, whereas the finishing blow deals a little more damage as well.");
initImage_ext(sprite_get("nspecial2"), -4, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("nspecial"), -4, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("FSpecial: Quick Attack/Wild Charge");
initWords("A very simple yet effective move when it comes to both recovery and combos when used normally! Koko will charge forth a good distance before stopping itself with a backflip.");
initWords("Upon hitting anyone in the way, Koko will stop early, although in turn won't go into pratfall after flipping. Koko can also cancel the endlag about halfway through with an aerial attack.");
initWords("When supercharged by electric terrain, Koko uses another move entirely: Wild Charge. This is a much faster and stronger variant of quick attack that holds much more killpower, although Koko will still go into free fall if the move doesn't hit.");
initImage_ext(sprite_get("fspecial2"), -4, fa_left, 1, 1, true, c_white, 1, true, noone, noone, noone, noone);
initImage_ext(sprite_get("fspecial"), -4, fa_right, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("USpecial: Guardian Flight");
initWords("Tapu Koko's USpecial launches it into the air at full throttle, whereas the move can be steered left and right. It can turn sharper by holding the stick diagonally downward. The flight can be cancelled early by pressing shield or special.");
initWords("When in the electric terrain, Koko's flight will pick up in speed, and when cancelled with the special button, instead of cancelling normally, it will use a supercharged NSpecial.");
initImage_ext(sprite_get("uspecial_flying"), 0, fa_center, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("DSpecial: Nature's Madness");
initWords("Nature's Madness is the signature move for the guardian deities of Alola, and in the core Pokemon games, it will typically half the HP of another pokemon.");
initWords("In Rivals of Aether, the move works a bit differently, but similarly enough to earn its title with pride. The move works as a counter attack, whereas being hit within a certain frame of time will cause the opponent to be stuck while Koko performs the attack (unless it's hit by a projectile).");
initWords("The attack itself is big, and can hurt anyone even close by if they're not careful. This is where the true adaptation comes in: Unless countering a projectile, the attack will deal half as much as the target's current damage to their damage gage. If countering a projectile, the damage that the counter deals will equal 1.5x as much as the projectile's damage output.");
...
initSection("Super Smash Bros. Ultimate, Pokemon Sun and Moon");
initWords("
Tapu Koko belongs to Game Freak, Nintendo, and The Pokemon Company.");
