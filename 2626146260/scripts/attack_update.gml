...
if attack == AT_JAB {
    ...
    if window = 1  && window_timer == 1 && !hitpause {
        sound_play(sound_get("cnoise2"))
         sound_play(sound_get("vef4"),false,noone,2,1.05 - (random_func(1,10,true))/100)   
    }
    
    if window = 4  && triggered == 0 && !hitpause {
        sound_play(sound_get("cnoise2"))
         sound_play(sound_get("vef4"),false,noone,2,1.05 - (random_func(1,10,true))/100)   
    }
    ...
    if window == 6 && window_timer > 4 && attack_pressed {
       ...
       if (left_down && spr_dir == 1) or (right_down && spr_dir == -1){
           hsp = 8*spr_dir
           vsp = -6
           sound_play(sound_get("vef1"),false,noone,2,1.05 - (random_func(1,10,true))/100)     
           set_attack (AT_BAIR)
...
if attack == AT_NAIR {
    ...
    if window = 1 && window_timer == 1{
    	sound_play(sound_get("vef2"),false,noone,2,1.05 - (random_func(1,10,true))/100)  
    ...
    if rank < 4 && state_timer == 2 && window == 1{
    	switch get_gameplay_time() % 6 {
    		...
    		break;
     		...
     		case 3:
    		sound_play(sound_get("taunt1"),false,noone,1.8)
    		break;
    		
    		case 4:
    		sound_play(sound_get("taunt2"),false,noone,1.8)
    		break;
    		
    	    case 5:
    		sound_play(sound_get("taunt3"),false,noone,1.8)
    		break;
    	}
    	shake_camera(2,4)
    	set_state(PS_CROUCH)
