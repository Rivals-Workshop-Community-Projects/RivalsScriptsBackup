...
if(instance_exists(other_player_id.clone_object_ID)){
	//print("in clone loop");
	if(other_player_id.clone_object_ID.state == 22 && other_player_id.clone_object_ID.state_timer > 2) // Fixes sprite render on frame 1
	{
		//print("in grab loop");
		// Draw Section
		with(other_player_id){
			//print("in draw loop");
			shader_start();
			draw_sprite_ext(sprite_get("dthrow_behind"),clone_object_ID.image_index,clone_object_ID.x,clone_object_ID.y,clone_object_ID.spr_dir,1,0,c_white,1);
			shader_end();
		}
	}
}