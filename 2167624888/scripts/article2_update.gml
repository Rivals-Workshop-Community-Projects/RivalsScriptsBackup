...
switch (state) {
	case PS_SPAWN:
	if !instance_exists(hitbox_small) break;
			...
			switch attack {
				...
				case AT_NSPECIAL:
					if hbox_num == 1 {
			        	...
			        	var effect, hitbox;
			        	if (other.isRed) {
			        		effect = player_id.should_red_arrow ? player_id.red_explosion : player_id.mixed_hit
			        		hitbox = player_id.should_red_arrow ? 4 : 5;
			        	}
			        	else {
			        		effect = player_id.should_red_arrow ? player_id.mixed_hit : player_id.fspec_effect
			        		hitbox = player_id.should_red_arrow ? 5 : 6;
			        	}
			        	print(hitbox)
