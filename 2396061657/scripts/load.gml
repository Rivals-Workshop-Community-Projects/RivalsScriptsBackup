// load.gml
// called right after the item is loaded into the game. 
// This is where you would normally set sprites’ origins and bounding boxes.

sprite_change_offset("portrait", 350, 350);
sprite_change_offset("cursor", 20, 10);
sprite_change_offset("name tag", 0, 51);
sprite_change_offset("display", 0, 45);
sprite_change_offset("recharged", 25, 68);
sprite_change_offset("grabrecharged", 41, 41);

sprite_change_offset("callie_fireball", 30, 16);
sprite_change_offset("callie_fireball_hitfx", 26, 20);

sprite_change_offset("crouch_hurtbox", 20, 70);
sprite_change_offset("date_logo",  65, 65);
sprite_change_offset("stage_logo",  65, 65);
sprite_change_offset("taunt...",  72, 96);
sprite_change_offset("magnetism", 205, 205);

sprite_change_offset("focuseffectsfront", 42, 88);
sprite_change_offset("focuseffectsback", 40, 88);
sprite_change_offset("focuseffectssmoke", 44, 102);
sprite_change_offset("crumple", 60, 98);

sprite_change_offset("idle", 50, 98);
sprite_change_offset("hurt", 32, 62);
sprite_change_offset("crouch", 50, 98);
sprite_change_offset("walk", 50, 98);
sprite_change_offset("walkturn", 50, 98);
sprite_change_offset("dash", 30, 74);
sprite_change_offset("dashstart", 50, 98);
sprite_change_offset("dashstop", 50, 98);
sprite_change_offset("dashturn", 50, 98);

sprite_change_offset("jumpstart", 50, 98);
sprite_change_offset("jump", 50, 98);
sprite_change_offset("doublejump", 50, 98);
sprite_change_offset("walljump", 50, 98);
sprite_change_offset("pratfall", 50, 98);
sprite_change_offset("land", 50, 98);
sprite_change_offset("landinglag", 50, 98);

sprite_change_offset("parry", 54, 118);
sprite_change_offset("roll_forward", 68, 118);
sprite_change_offset("roll_backward", 68, 118);
sprite_change_offset("airdodge", 82, 118);
sprite_change_offset("waveland", 24, 80);
sprite_change_offset("tech", 24, 74);

sprite_change_offset("jab", 38, 82);
sprite_change_offset("dattack", 50, 118);
sprite_change_offset("ftilt", 30, 72);
sprite_change_offset("dtilt", 42, 64);
sprite_change_offset("utilt", 30, 100);
sprite_change_offset("nair", 74, 100);
sprite_change_offset("fair", 70, 100);
sprite_change_offset("bair", 60, 90);
sprite_change_offset("uair", 88, 148);
sprite_change_offset("dair", 50, 98);
sprite_change_offset("fstrong", 70, 118);
sprite_change_offset("ustrong", 32, 140);
sprite_change_offset("dstrong", 74, 118);
sprite_change_offset("nspecial", 40, 84);
sprite_change_offset("fspecial", 50, 86);
sprite_change_offset("fspecialair", 58, 86);
sprite_change_offset("uspecial", 50, 98);
sprite_change_offset("dspecial", 50, 98);
sprite_change_offset("taunt", 68, 120);

sprite_change_offset("plat", 64, 62);
sprite_change_offset("mini_meter", 31, 11);

sprite_change_offset("nthrow", 68, 102);
sprite_change_offset("fthrow", 30, 98);
sprite_change_offset("uthrow", 50, 98);
sprite_change_offset("dthrow", 50, 98);
sprite_change_offset("balloon", 38, 38);
sprite_change_offset("stickygoo", 50, 98);
sprite_change_offset("string", 10, 16);

sprite_change_offset("nspecialfx", 0, 88);
sprite_change_offset("dspecialmenu", 65, 65);
sprite_change_offset("dspecialmenu2", 81, 81);
sprite_change_offset("gifts", 18, 22);
sprite_change_offset("heart", 10, 10);
sprite_change_offset("heartoutline", 12, 12);
sprite_change_offset("meter", 30, 4);

sprite_change_offset("hurt", 74, 118);
sprite_change_offset("bighurt", 74, 118);
sprite_change_offset("spinhurt", 74, 118);
sprite_change_offset("uphurt", 74, 118);

sprite_change_offset("Callie_VFX_focusatk", 154, 154);
sprite_change_offset("Callie_VFX_Big_Heart", 92, 102);
sprite_change_offset("Callie_VFX_Small_Heart", 70, 70);
sprite_change_offset("Callie_VFX_Bat_Hit", 90, 90);
sprite_change_offset("Callie_VFX_Fire", 58, 54);
sprite_change_offset("Callie_VFX_Balloon", 134, 90);

sprite_change_offset("vfx_surprise", 0,72);
sprite_change_offset("vfx_ribbon", 90,90);
sprite_change_offset("vfx_scatter", 58, 46);
sprite_change_offset("vfx_roses", 68, 64);
sprite_change_offset("vfx_goohit", 68,68);
sprite_change_offset("vfx_ribbonslam", 120, 120);
sprite_change_offset("vfx_beeg", 176, 184);

sprite_change_offset("date_bg", 800, 500);
sprite_change_offset("event_idle", 28, 78);
sprite_change_offset("costume_fall", 48, 98);

sprite_change_offset("hit_particle1", 4, 16);
sprite_change_offset("hit_particle2", 8, 6);
sprite_change_offset("hit_particle3", 6, 4);
sprite_change_offset("hit_particle4", 6, 6);
sprite_change_offset("hit_particle6", 16, 10);