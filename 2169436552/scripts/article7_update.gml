...
if !_init {
    ...
    display_inside = spawn_variables[6];
    _init = 1;
...
if state == 0 { //trigger active
    ...
    if collis_obj != noone  {
        if display_inside {
            if !(collision_point(collis_obj.x+view_get_wview()/2,collis_obj.y,id,false,false) ||
                 collision_point(collis_obj.x-view_get_wview()/2,collis_obj.y,id,false,false)) scroll_horiz = false;
            if !(collision_point(collis_obj.x,collis_obj.y-view_get_hview()/2,id,false,false) ||
                 collision_point(collis_obj.x,collis_obj.y-view_get_hview()/2,id,false,false)) scroll_vert = true;
        } 
