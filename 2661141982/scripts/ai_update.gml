	...
	if (state == PS_ATTACK_AIR || state == PS_ATTACK_GROUND)
	{
		...
		switch (attack)
		{
			case AT_DATTACK:
				...
				break;
			case AT_UTILT:
				if (has_hit_player) DoAttack(AT_UAIR);
				break;
			case AT_JAB:
				if (has_hit_player) DoAttack(AT_DTILT);
				break;
			case AT_DTILT:
				if (has_hit_player) DoAttack(AT_UTILT);
				break;
			case AT_EXTRA_1:
				if (has_hit_player) DoAttack(get_player_damage(hit_player_obj.player)>=120?AT_USTRONG:ydist>20?AT_USPECIAL:AT_UTILT);
				break;
			case AT_USPECIAL_2:
				HoldTowardsTarget();
			...
			case AT_USPECIAL:
				if (ai_state != AS_RECOVER)
				{
					...
					else {DoAttack(AT_FSPECIAL);}//special_pressed = true; down_down = true; DoAttack(AT_FSPECIAL);}
				}
