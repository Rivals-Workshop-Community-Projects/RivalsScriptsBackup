...
if (state == 0){
	sprite_index = sprite_get("dummyspawn");
	image_index += 12 / 60;
	if free == true{
		instance_destroy();
		exit;
...
if (state == 3){
	sprite_index = sprite_get("dummydespawn");
	image_index += 12 / 60;
	if (state_timer == 16){
		instance_destroy();
		exit;
...
if (state == 4){
	sprite_index = sprite_get("dummy_fly");
	if (state_timer == 36){
		state = 1;
		state_timer = 0;
	}
	...
	idle_timer ++;
	if (state_timer == 1){
	create_hitbox( AT_USPECIAL, 1, x + 20*spr_dir, y - 16 );
	}
}
