set_attack_value(AT_EXTRA_2, AG_CATEGORY, 1);
set_attack_value(AT_EXTRA_2, AG_SPRITE, sprite_get("bair_hilt"));
set_attack_value(AT_EXTRA_2, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_EXTRA_2, AG_LANDING_LAG, 8);
set_attack_value(AT_EXTRA_2, AG_HURTBOX_SPRITE, sprite_get("bair_hurt"));

set_attack_value(AT_EXTRA_2, AG_NUM_WINDOWS, 3);

set_window_value(AT_EXTRA_2, 1, AG_WINDOW_LENGTH, 4);
set_window_value(AT_EXTRA_2, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_EXTRA_2, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_EXTRA_2, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_2, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak2"));
set_window_value(AT_EXTRA_2, 1, AG_WINDOW_SFX_FRAME, 3);

set_window_value(AT_EXTRA_2, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_EXTRA_2, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_EXTRA_2, 2, AG_WINDOW_ANIM_FRAME_START, 2);

set_window_value(AT_EXTRA_2, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_EXTRA_2, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_EXTRA_2, 3, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_EXTRA_2, 3, AG_WINDOW_HAS_WHIFFLAG, 1);

set_num_hitboxes(AT_EXTRA_2, 1);

set_hitbox_value(AT_EXTRA_2, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_2, 1, HG_WINDOW, 2);
set_hitbox_value(AT_EXTRA_2, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_2, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_EXTRA_2, 1, HG_HITBOX_X, -30);
set_hitbox_value(AT_EXTRA_2, 1, HG_HITBOX_Y, -32);
set_hitbox_value(AT_EXTRA_2, 1, HG_WIDTH, 70);
set_hitbox_value(AT_EXTRA_2, 1, HG_HEIGHT, 40);
set_hitbox_value(AT_EXTRA_2, 1, HG_PRIORITY, 4);
set_hitbox_value(AT_EXTRA_2, 1, HG_DAMAGE, 6);
set_hitbox_value(AT_EXTRA_2, 1, HG_ANGLE, 70);
set_hitbox_value(AT_EXTRA_2, 1, HG_ANGLE_FLIPPER, 5);
set_hitbox_value(AT_EXTRA_2, 1, HG_SHAPE, 0);
set_hitbox_value(AT_EXTRA_2, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_EXTRA_2, 1, HG_KNOCKBACK_SCALING, 0.4);
set_hitbox_value(AT_EXTRA_2, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_EXTRA_2, 1, HG_HITPAUSE_SCALING, 0.6);
set_hitbox_value(AT_EXTRA_2, 1, HG_HIT_SFX, asset_get("sfx_shovel_hit_med2"));
set_hitbox_value(AT_EXTRA_2, 1, HG_VISUAL_EFFECT, vfx_small_sword);
set_hitbox_value(AT_EXTRA_2, 1, HG_HITBOX_GROUP, 1);