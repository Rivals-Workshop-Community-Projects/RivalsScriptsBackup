...
case AT_DTILT:
	...
	{
		if (!my_hitboxID.hasHitCrystal) ApplyStack();
		var hbox = create_hitbox(AT_DTILT, 3, hit_player_obj.x, hit_player_obj.y-14);
...
case AT_JAB:
	if (my_hitboxID.hbox_num == 1) ForceBreak();
	break;
...
if (hit_player_obj != self && hit_player_obj.noelleFrostgrave > 0)
{
	if (hit_player_obj.noelleFrostgrave <= hit_player_obj.hitstun_full/3) // early pop
	{
		...
		hit_player_obj.noelleEarlyPop = true;
	}
	else
	{
		hit_player_obj.should_make_shockwave = false;
...
#define ApplyFrostGrave()
{
	if (hit_player_obj != self && hit_player_obj.noelleFrostbite >= frostbiteMax)
	{
		...
		hit_player_obj.noelleFrostgraveTime = frostgraveTime;
		if (has_rune("F")) hit_player_obj.noelleFrostgraveTime*=3;
		hit_player_obj.noelleFrostgrave = hit_player_obj.noelleFrostgraveTime;
		hit_player_obj.noelleFrostgraveSpeed = {hsp:hit_player_obj.old_hsp, vsp:hit_player_obj.old_vsp};
		hit_player_obj.noelleFrostbiteID = self;
		hit_player_obj.noelleEarlyPop = false;
...
#define ForceBreak()
{
	if (hit_player_obj != self && hit_player_obj.noelleFrostgrave > 0) hit_player_obj.noelleFrostgrave = 1;
}