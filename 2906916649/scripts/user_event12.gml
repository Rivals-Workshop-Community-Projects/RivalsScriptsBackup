...
*/
    add_named_alt("Deku", 32);
    add_named_alt("LOWIEE", 33);
    add_named_alt("succ", 34);
    add_named_alt("jojo", 35);
    //add_named_alt("versandt", 36);
    add_named_alt("MX", 37);
    add_named_alt("PMINER", 38);
    add_named_alt("seesaw", 39);
    add_named_alt("soup", 40);
...
if ("ssnk_tagpal" not in self) {
    if (room == 69 || room == 113) && object_index != oTestPlayer {
        return
    }
    __init(arg);
};
