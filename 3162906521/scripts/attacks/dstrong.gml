set_attack_value(AT_DSTRONG, AG_SPRITE, sprite_get("dstrong"));
set_attack_value(AT_DSTRONG, AG_NUM_WINDOWS, 4);
set_attack_value(AT_DSTRONG, AG_STRONG_CHARGE_WINDOW, 2);
set_attack_value(AT_DSTRONG, AG_HURTBOX_SPRITE, sprite_get("dstrong_hurt"));

//starup
set_window_value(AT_DSTRONG, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_CUSTOM_GROUND_FRICTION, 0.4);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_SFX, asset_get("sfx_charge_blade_ready"));
set_window_value(AT_DSTRONG, 1, AG_WINDOW_SFX_FRAME, 4); 

//charge
set_window_value(AT_DSTRONG, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_LENGTH, 6);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_SFX, sound_get("gunslinger_triple"));
set_window_value(AT_DSTRONG, 2, AG_WINDOW_SFX_FRAME, 0); 

//active
set_window_value(AT_DSTRONG, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_SFX, asset_get("sfx_shovel_swing_heavy2"));

//englag
set_window_value(AT_DSTRONG, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_LENGTH, 16);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_HAS_WHIFFLAG, 1);


set_num_hitboxes(AT_DSTRONG, 4);

//sourspot front
set_hitbox_value(AT_DSTRONG, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DSTRONG, 1, HG_WINDOW, 3);
set_hitbox_value(AT_DSTRONG, 1, HG_WINDOW_CREATION_FRAME, 4);
set_hitbox_value(AT_DSTRONG, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG, 1, HG_HITBOX_X, 8);
set_hitbox_value(AT_DSTRONG, 1, HG_HITBOX_Y, -27);
set_hitbox_value(AT_DSTRONG, 1, HG_WIDTH, 195);
set_hitbox_value(AT_DSTRONG, 1, HG_HEIGHT, 60);
set_hitbox_value(AT_DSTRONG, 1, HG_SHAPE, 1);
set_hitbox_value(AT_DSTRONG, 1, HG_PRIORITY, 6);
set_hitbox_value(AT_DSTRONG, 1, HG_DAMAGE, 11);
set_hitbox_value(AT_DSTRONG, 1, HG_ANGLE, 361);
set_hitbox_value(AT_DSTRONG, 1, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_DSTRONG, 1, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DSTRONG, 1, HG_KNOCKBACK_SCALING, 1.05);
set_hitbox_value(AT_DSTRONG, 1, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_DSTRONG, 1, HG_HITPAUSE_SCALING, 1.05);
set_hitbox_value(AT_DSTRONG, 1, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DSTRONG, 1, HG_VISUAL_EFFECT_X_OFFSET, 10);
set_hitbox_value(AT_DSTRONG, 1, HG_HIT_SFX, sound_get("smashbros_sword"));
set_hitbox_value(AT_DSTRONG, 1, HG_HIT_LOCKOUT, 10);

// //sourpsot back
// set_hitbox_value(AT_DSTRONG, 2, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DSTRONG, 2, HG_WINDOW, 3);
// set_hitbox_value(AT_DSTRONG, 2, HG_WINDOW_CREATION_FRAME, 4);
// set_hitbox_value(AT_DSTRONG, 2, HG_LIFETIME, 3);
// set_hitbox_value(AT_DSTRONG, 2, HG_HITBOX_X, -34);
// set_hitbox_value(AT_DSTRONG, 2, HG_HITBOX_Y, -15);
// set_hitbox_value(AT_DSTRONG, 2, HG_WIDTH, 41);
// set_hitbox_value(AT_DSTRONG, 2, HG_HEIGHT, 34);
// set_hitbox_value(AT_DSTRONG, 2, HG_PRIORITY, 5);
// set_hitbox_value(AT_DSTRONG, 2, HG_SHAPE, 2);
// set_hitbox_value(AT_DSTRONG, 2, HG_DAMAGE, 11);
// set_hitbox_value(AT_DSTRONG, 2, HG_ANGLE, 361);
// set_hitbox_value(AT_DSTRONG, 2, HG_BASE_KNOCKBACK, 8.5);
// set_hitbox_value(AT_DSTRONG, 2, HG_KNOCKBACK_SCALING, 1.05);
// set_hitbox_value(AT_DSTRONG, 2, HG_BASE_HITPAUSE, 8);
// set_hitbox_value(AT_DSTRONG, 2, HG_HITPAUSE_SCALING, 1.05);
// set_hitbox_value(AT_DSTRONG, 2, HG_VISUAL_EFFECT, 304);
// set_hitbox_value(AT_DSTRONG, 2, HG_VISUAL_EFFECT_X_OFFSET, 10);
// set_hitbox_value(AT_DSTRONG, 2, HG_HIT_SFX, asset_get("sfx_buzzsaw_hit"));
// set_hitbox_value(AT_DSTRONG, 2, HG_HIT_LOCKOUT, 10);
// set_hitbox_value(AT_DSTRONG, 2, HG_ANGLE_FLIPPER, 5);

// //sweetspot front
// set_hitbox_value(AT_DSTRONG, 3, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DSTRONG, 3, HG_WINDOW, 3);
// set_hitbox_value(AT_DSTRONG, 3, HG_WINDOW_CREATION_FRAME, 4);
// set_hitbox_value(AT_DSTRONG, 3, HG_LIFETIME, 3);
// set_hitbox_value(AT_DSTRONG, 3, HG_HITBOX_X, 82);
// set_hitbox_value(AT_DSTRONG, 3, HG_HITBOX_Y, -28);
// set_hitbox_value(AT_DSTRONG, 3, HG_WIDTH, 60);
// set_hitbox_value(AT_DSTRONG, 3, HG_HEIGHT, 60);
// set_hitbox_value(AT_DSTRONG, 3, HG_PRIORITY, 3);
// set_hitbox_value(AT_DSTRONG, 3, HG_SHAPE, 0);
// set_hitbox_value(AT_DSTRONG, 3, HG_DAMAGE, 11);
// set_hitbox_value(AT_DSTRONG, 3, HG_ANGLE, 361);
// set_hitbox_value(AT_DSTRONG, 3, HG_BASE_KNOCKBACK, 9.25);
// set_hitbox_value(AT_DSTRONG, 3, HG_KNOCKBACK_SCALING, 1.05);
// set_hitbox_value(AT_DSTRONG, 3, HG_BASE_HITPAUSE, 10);
// set_hitbox_value(AT_DSTRONG, 3, HG_HITPAUSE_SCALING, 1.05);
// set_hitbox_value(AT_DSTRONG, 3, HG_VISUAL_EFFECT, 305);
// set_hitbox_value(AT_DSTRONG, 3, HG_VISUAL_EFFECT_X_OFFSET, 10);
// set_hitbox_value(AT_DSTRONG, 3, HG_HIT_SFX, sound_get("smashbros_sword"));
// set_hitbox_value(AT_DSTRONG, 3, HG_HIT_LOCKOUT, 10);
// // set_hitbox_value(AT_DSTRONG, 3, HG_ANGLE_FLIPPER, 5);

// //sweetspot back
// set_hitbox_value(AT_DSTRONG, 4, HG_HITBOX_TYPE, 1);
// set_hitbox_value(AT_DSTRONG, 4, HG_WINDOW, 3);
// set_hitbox_value(AT_DSTRONG, 4, HG_WINDOW_CREATION_FRAME, 4);
// set_hitbox_value(AT_DSTRONG, 4, HG_LIFETIME, 3);
// set_hitbox_value(AT_DSTRONG, 4, HG_HITBOX_X, -60);
// set_hitbox_value(AT_DSTRONG, 4, HG_HITBOX_Y, -18);
// set_hitbox_value(AT_DSTRONG, 4, HG_WIDTH, 60);
// set_hitbox_value(AT_DSTRONG, 4, HG_HEIGHT, 50);
// set_hitbox_value(AT_DSTRONG, 4, HG_PRIORITY, 2);
// set_hitbox_value(AT_DSTRONG, 4, HG_SHAPE, 0);
// set_hitbox_value(AT_DSTRONG, 4, HG_DAMAGE, 11);
// set_hitbox_value(AT_DSTRONG, 4, HG_ANGLE, 361);
// set_hitbox_value(AT_DSTRONG, 4, HG_BASE_KNOCKBACK, 9.25);
// set_hitbox_value(AT_DSTRONG, 4, HG_KNOCKBACK_SCALING, 1.05);
// set_hitbox_value(AT_DSTRONG, 4, HG_BASE_HITPAUSE, 10);
// set_hitbox_value(AT_DSTRONG, 4, HG_HITPAUSE_SCALING, 1.05);
// set_hitbox_value(AT_DSTRONG, 4, HG_VISUAL_EFFECT, 305);
// set_hitbox_value(AT_DSTRONG, 4, HG_VISUAL_EFFECT_X_OFFSET, 10);
// set_hitbox_value(AT_DSTRONG, 4, HG_HIT_SFX, asset_get("sfx_shovel_hit_heavy2"));
// set_hitbox_value(AT_DSTRONG, 4, HG_HIT_LOCKOUT, 10);
// set_hitbox_value(AT_DSTRONG, 4, HG_ANGLE_FLIPPER, 5);