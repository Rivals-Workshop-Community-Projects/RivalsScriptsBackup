//The Suitcase

sprite_change_offset("idle_hurt_box", 20, 44);
sprite_change_offset("idle", 20, 50);
sprite_change_offset("idle1", 20, 50);
sprite_change_offset("hurt", 22, 50);
sprite_change_offset("bighurt", 23, 48);
sprite_change_offset("hurtground", 22, 48);
sprite_change_offset("bouncehurt", 30, 38);
sprite_change_offset("spinhurt", 25, 48);
sprite_change_offset("uphurt", 20, 48);
sprite_change_offset("downhurt", 27, 36);
sprite_change_offset("crouch", 20, 42);
sprite_change_offset("crouchKT", 20, 42);
sprite_change_offset("walk", 20, 52);
sprite_change_offset("walkturn", 20, 50);
sprite_change_offset("dash", 20, 58);
sprite_change_offset("dashstart", 22, 54);
sprite_change_offset("dashstop", 23, 56);
sprite_change_offset("dashturn", 20, 50);
sprite_change_offset("intro", 110, 68);
sprite_change_offset("intro1", 32, 394);
sprite_change_offset("intro2", 24, 354);
sprite_change_offset("intro3", 26, 52);
sprite_change_offset("intro4", 24, 112);
sprite_change_offset("intro5", 110, 68);

sprite_change_offset("jumpstart", 22, 46);
sprite_change_offset("jump", 20, 52);
sprite_change_offset("doublejump", 24, 52);
sprite_change_offset("walljump", 22, 54);
sprite_change_offset("pratfall", 21, 56);
sprite_change_offset("land", 24, 48);
sprite_change_offset("landinglag", 22, 48);

sprite_change_offset("parry", 20, 50);
sprite_change_offset("roll_forward", 20, 50);
sprite_change_offset("roll_backward", 22, 52);
sprite_change_offset("airdodge", 27, 54);
sprite_change_offset("waveland", 27, 52);
sprite_change_offset("waveland1", 27, 52);
sprite_change_offset("tech", 27, 52);

sprite_change_offset("jab", 22, 58);
sprite_change_offset("dattack", 32, 56);
sprite_change_offset("ftilt", 24, 58);
sprite_change_offset("ftiltKT", 24, 58);
sprite_change_offset("dtilt", 46, 42);
sprite_change_offset("dtiltKT", 42, 42);
sprite_change_offset("utilt", 24, 112);
sprite_change_offset("utilt1", 24, 112);
sprite_change_offset("utilt2", 24, 112);
sprite_change_offset("nair", 31, 64);
sprite_change_offset("nair1", 31, 64);
sprite_change_offset("fair", 24, 56);
sprite_change_offset("bair", 60, 60);
sprite_change_offset("bair1", 65, 50);
sprite_change_offset("uair", 25, 78);
sprite_change_offset("dair", 24, 56);
sprite_change_offset("fstrong", 77, 110);
sprite_change_offset("ustrong", 26, 102);
sprite_change_offset("ustrong1", 26, 102);
sprite_change_offset("ustrong1L", 26, 102);
sprite_change_offset("ustrongKT", 120, 206);
sprite_change_offset("dstrong", 36, 84);
sprite_change_offset("dstrongKT", 36, 84);
sprite_change_offset("nspecial", 22, 64);
sprite_change_offset("nspecial1", 22, 64);
sprite_change_offset("fspecial", 48, 94);
sprite_change_offset("uspecial", 22, 90);
sprite_change_offset("uspecial1", 22, 90);
sprite_change_offset("dspecial", 20, 52);
sprite_change_offset("dspecial1", 26, 56);
sprite_change_offset("taunt", 22, 52);
sprite_change_offset("taunt1", 26, 52);
sprite_change_offset("taunt2", 26, 54);
sprite_change_offset("taunt3", 22, 76);

sprite_change_offset("plat", 100, 138);

sprite_change_offset("nspecial_proj", 18, 18);
sprite_change_offset("nspecial2_proj", 28, 22);
sprite_change_offset("nspecial3_proj", 18, 18);
sprite_change_offset("xpshards", 14, 14);
sprite_change_offset("xpshards1", 14, 14);
sprite_change_offset("xpshards2", 14, 14);

sprite_change_offset("coldsnap", 76, 72);

//Powerups
sprite_change_offset("hud_shop", 200, 80);
sprite_change_offset("hud_shop1", 200, 80);
sprite_change_offset("hud_shop2", 200, 80);
sprite_change_offset("achievement", 9, 32);
sprite_change_offset("achievement1", 9, 32);
sprite_change_offset("item_mcdouble", 32, 32);
sprite_change_offset("item_poolparty", 32, 32);
sprite_change_offset("item_creatorsmark", 32, 32);
sprite_change_offset("item_creatorsmark1", 32, 32);
sprite_change_offset("item_boardingpass", 32, 32);
sprite_change_offset("item_badge0", 32, 32);
sprite_change_offset("item_badge1", 32, 32);
sprite_change_offset("item_badge2", 32, 32);
sprite_change_offset("item_badge3", 32, 32);
sprite_change_offset("item_prizepurse", 32, 32);
sprite_change_offset("item_carrot", 42, 32);
sprite_change_offset("item_bamboo", 38, 32);
sprite_change_offset("item_pocketcactus", 32, 32);
sprite_change_offset("item_gradcap", 32, 32);
sprite_change_offset("item_glitch", 32, 32);
sprite_change_offset("item_glitchm", 32, 32);
sprite_change_offset("item_godhead", 32, 32);
sprite_change_offset("item_zetplush", 32, 32);
sprite_change_offset("item_belt", 40, 32);
sprite_change_offset("item_milestone", 32, 32);
sprite_change_offset("item_downb", 32, 32);
sprite_change_offset("item_toothpaste", 32, 32);
sprite_change_offset("item_jacket", 39, 32);
sprite_change_offset("item_pandora", 32, 32);
sprite_change_offset("item_controller", 32, 32);
sprite_change_offset("item_aldor", 36, 36);
sprite_change_offset("item_buuuurger", 32, 32);
sprite_change_offset("item_collar", 32, 32);
sprite_change_offset("item_faucet", 32, 32);
sprite_change_offset("item_key", 34, 32);
sprite_change_offset("item_switch", 32, 32);
sprite_change_offset("item_enternt", 32, 32);

//Item HUD
sprite_change_offset("mini_gradcap", 16, 16);
sprite_change_offset("mini_prizepurse", 16, 16);
sprite_change_offset("mini_pocketcactus", 16, 16);
sprite_change_offset("mini_glitch", 16, 16);
sprite_change_offset("mini_glitchm", 16, 16);
sprite_change_offset("mini_badge0", 16, 16);
sprite_change_offset("mini_badge1", 16, 16);
sprite_change_offset("mini_badge2", 16, 16);
sprite_change_offset("mini_badge3", 16, 16);
sprite_change_offset("mini_creatorsmark", 16, 16);
sprite_change_offset("mini_creatorsmark1", 16, 16);
sprite_change_offset("mini_boardingpass", 16, 16);
sprite_change_offset("mini_mcdouble", 16, 16);
sprite_change_offset("mini_buuuurger", 16, 16);
sprite_change_offset("mini_poolparty", 16, 16);
sprite_change_offset("mini_zetplush", 16, 16);
sprite_change_offset("mini_godhead", 16, 16);
sprite_change_offset("mini_belt", 16, 16);
sprite_change_offset("mini_milestone", 16, 16);
sprite_change_offset("mini_downb", 16, 16);
sprite_change_offset("mini_jacket", 16, 16);
sprite_change_offset("mini_toothpaste", 16, 16);
sprite_change_offset("mini_pandora", 16, 16);
sprite_change_offset("mini_controller", 16, 16);
sprite_change_offset("mini_aldor", 16, 16);
sprite_change_offset("mini_collar", 16, 16);
sprite_change_offset("mini_key", 16, 16);
sprite_change_offset("mini_rune", 16, 16);
sprite_change_offset("mini_switch", 16, 16);
sprite_change_offset("mini_faucet", 16, 16);
sprite_change_offset("mini_enternt", 16, 16);
sprite_change_offset("mini_f", 16, 16);

//Hit FX

sprite_change_offset("hit_fx_sandair", 80, 80);
sprite_change_offset("hit_fx_sandair2", 80, 80);
sprite_change_offset("hit_fx_symbiote", 43, 42);
sprite_change_offset("hit_fx_crysSmall", 31, 31);
sprite_change_offset("hit_fx_poolparty", 27, 28);
sprite_change_offset("hit_fx_tada", 67, 85);
sprite_change_offset("hit_fx_shine", 18, 27); 
sprite_change_offset("hit_fx_wheelspin", 47, 49);
sprite_change_offset("hit_fx_taxiDir", 39, 42); 
sprite_change_offset("sparkle", 7, 7); 
sprite_change_offset("smokeXP", 7, 9); 

//Katie Hit FX & VFX - Used with permission from Sydery
sprite_change_offset("smokeKT1", 32, 32);
sprite_change_offset("smokeKT2", 16, 16);
sprite_change_offset("smokeKT3", 16, 16);
sprite_change_offset("dashfxKT", 30, 16);
sprite_change_offset("exp1KT", 80, 80);
sprite_change_offset("exp2KT", 82, 152);
sprite_change_offset("blast1KT", 76, 98);
sprite_change_offset("blast2KT", 76, 98);

//Other Support
sprite_change_offset("taxiPINGED", 0, 0);
sprite_change_offset("otto_bobblehead", 0, 0);
sprite_change_offset("suitkirb", 32, 112);
//sprite_change_offset("dracula_portrait", 96, 96);

#define updateItempool
{
	IDs_available = 0;
	f = 0;
	for(e = 0; e < items_length; e++){
		if(item[e, 7] = true){	//check if each item is allowed to spawn
			IDs_available[f] = item[e, 5];
			f++;
		}
	}
	numItemsAvailable = f;
	if(numItemsAvailable == 0){
		item[6, 7] = true;
		updateItempool();
	} else {
		item[6, 7] = false;
	}
	legendaryPinged = false;
}

updateItempool();