// DEFAULT COLOR

// Light Fur
set_color_profile_slot( 0, 0, 197, 207, 227 );
set_color_profile_slot_range( 0, 2, 3, 25 );

// Dark Fur
set_color_profile_slot( 0, 1, 46, 41, 60 );
set_color_profile_slot_range( 1, 2, 2, 12 );

// Red Tights
set_color_profile_slot( 0, 2, 141, 0, 25 );
set_color_profile_slot_range( 2, 1, 1, 46 );

// Yellow Tights
set_color_profile_slot( 0, 3, 165, 103, 0 );
set_color_profile_slot_range( 3, 2, 1, 36 );

// Kneepads
set_color_profile_slot( 0, 4, 116, 110, 108 );
set_color_profile_slot_range( 4, 1, 1, 27 );

// Black
set_color_profile_slot( 0, 5, 47, 44, 43 );
set_color_profile_slot_range( 5, 1, 1, 1 );

// Eyes
set_color_profile_slot( 0, 6, 238, 226, 33 );
set_color_profile_slot_range( 6, 4, 3, 31 );

// Table
set_color_profile_slot( 0, 7, 56, 69, 235 );
set_color_profile_slot_range( 7, 1, 23, 18 );


// ALTERNATE COLORS
set_num_palettes( 32 );

// Air
set_color_profile_slot( 1, 0, 227, 236, 255 ); //Light Fur
set_color_profile_slot( 1, 1, 46, 41, 60 ); //Dark Fur
set_color_profile_slot( 1, 2, 109, 79, 171 ); //Red Tights
set_color_profile_slot( 1, 3, 98, 139, 166 ); //Yellow Tights
set_color_profile_slot( 1, 4, 116, 110, 108 ); //Kneepads
set_color_profile_slot( 1, 5, 47, 44, 43 ); //Black
set_color_profile_slot( 1, 6, 140, 91, 0 ); //Eyes
set_color_profile_slot( 1, 7, 0, 255, 0 ); //Table

// Ocean
set_color_profile_slot( 2, 0, 232, 232, 232 ); //Light Fur
set_color_profile_slot( 2, 1, 36, 34, 33 ); //Dark Fur
set_color_profile_slot( 2, 2, 36, 50, 54 ); //Red Tights
set_color_profile_slot( 2, 3, 9, 105, 85 ); //Yellow Tights
set_color_profile_slot( 2, 4, 113, 161, 113 ); //Kneepads
set_color_profile_slot( 2, 5, 0, 20, 0 ); //Black
set_color_profile_slot( 2, 6, 0, 157, 255 ); //Eyes
set_color_profile_slot( 2, 7, 0, 255, 0 ); //Table

// Fiery
set_color_profile_slot( 3, 0, 217, 214, 214 ); //Light Fur
set_color_profile_slot( 3, 1, 26, 19, 19 ); //Dark Fur
set_color_profile_slot( 3, 2, 61, 21, 22 ); //Red Tights
set_color_profile_slot( 3, 3, 26, 16, 13 ); //Yellow Tights
set_color_profile_slot( 3, 4, 135, 52, 52 ); //Kneepads
set_color_profile_slot( 3, 5, 36, 0, 0 ); //Black
set_color_profile_slot( 3, 6, 255, 187, 0 ); //Eyes
set_color_profile_slot( 3, 7, 0, 255, 0 ); //Table

// Forest
set_color_profile_slot( 4, 0, 214, 200, 188 ); //Light Fur
set_color_profile_slot( 4, 1, 26, 36, 31 ); //Dark Fur
set_color_profile_slot( 4, 2, 30, 48, 0 ); //Red Tights
set_color_profile_slot( 4, 3, 26, 19, 14 ); //Yellow Tights
set_color_profile_slot( 4, 4, 17, 31, 17 ); //Kneepads
set_color_profile_slot( 4, 5, 51, 38, 38 ); //Black
set_color_profile_slot( 4, 6, 62, 140, 62 ); //Eyes
set_color_profile_slot( 4, 7, 0, 255, 0 ); //Table

// Magenta
set_color_profile_slot( 5, 0, 224, 255, 236 ); //Light Fur
set_color_profile_slot( 5, 1, 35, 43, 48 ); //Dark Fur
set_color_profile_slot( 5, 2, 71, 14, 46 ); //Red Tights
set_color_profile_slot( 5, 3, 19, 28, 28 ); //Yellow Tights
set_color_profile_slot( 5, 4, 133, 196, 196 ); //Kneepads
set_color_profile_slot( 5, 5, 57, 62, 69 ); //Black
set_color_profile_slot( 5, 6, 166, 155, 0 ); //Eyes
set_color_profile_slot( 5, 7, 0, 255, 0 ); //Table

// Sunlight
set_color_profile_slot( 6, 0, 252, 251, 244 ); //Light Fur
set_color_profile_slot( 6, 1, 31, 31, 31 ); //Dark Fur
set_color_profile_slot( 6, 2, 181, 142, 0 ); //Red Tights
set_color_profile_slot( 6, 3, 0, 70, 184 ); //Yellow Tights
set_color_profile_slot( 6, 4, 217, 217, 217 ); //Kneepads
set_color_profile_slot( 6, 5, 29, 31, 29 ); //Black
set_color_profile_slot( 6, 6, 255, 166, 0 ); //Eyes
set_color_profile_slot( 6, 7, 0, 255, 0 ); //Table

// Night Sky
set_color_profile_slot( 7, 0, 255, 238, 217 ); //Light Fur
set_color_profile_slot( 7, 1, 34, 26, 54 ); //Dark Fur
set_color_profile_slot( 7, 2, 14, 1, 82 ); //Red Tights
set_color_profile_slot( 7, 3, 82, 22, 115 ); //Yellow Tights
set_color_profile_slot( 7, 4, 163, 139, 181 ); //Kneepads
set_color_profile_slot( 7, 5, 186, 186, 186 ); //Black
set_color_profile_slot( 7, 6, 115, 50, 0 ); //Eyes
set_color_profile_slot( 7, 7, 0, 255, 0 ); //Table

// Ciclope
set_color_profile_slot( 8, 0, 219, 127, 98 ); //Light Fur
set_color_profile_slot( 8, 1, 28, 24, 18 ); //Dark Fur
set_color_profile_slot( 8, 2, 43, 43, 43 ); //Red Tights
set_color_profile_slot( 8, 3, 156, 62, 33 ); //Yellow Tights
set_color_profile_slot( 8, 4, 38, 38, 38 ); //Kneepads
set_color_profile_slot( 8, 5, 41, 41, 41 ); //Black
set_color_profile_slot( 8, 6, 120, 120, 120 ); //Eyes
set_color_profile_slot( 8, 7, 0, 255, 0 ); //Table

// Akira Taue
set_color_profile_slot( 9, 0, 242, 194, 194 ); //Light Fur
set_color_profile_slot( 9, 1, 48, 14, 14 ); //Dark Fur
set_color_profile_slot( 9, 2, 84, 5, 5 ); //Red Tights
set_color_profile_slot( 9, 3, 247, 247, 247 ); //Yellow Tights
set_color_profile_slot( 9, 4, 204, 204, 204 ); //Kneepads
set_color_profile_slot( 9, 5, 125, 18, 18 ); //Black
set_color_profile_slot( 9, 6, 122, 39, 21 ); //Eyes
set_color_profile_slot( 9, 7, 0, 255, 0 ); //Table

// Toshiaki Kawada
set_color_profile_slot( 10, 0, 250, 234, 158 ); //Light Fur
set_color_profile_slot( 10, 1, 23, 22, 17 ); //Dark Fur
set_color_profile_slot( 10, 2, 15, 15, 15 ); //Red Tights
set_color_profile_slot( 10, 3, 255, 188, 0 ); //Yellow Tights
set_color_profile_slot( 10, 4, 201, 201, 201 ); //Kneepads
set_color_profile_slot( 10, 5, 33, 33, 33 ); //Black
set_color_profile_slot( 10, 6, 217, 182, 20 ); //Eyes
set_color_profile_slot( 10, 7, 0, 255, 0 ); //Table

// Kenta Kobashi
set_color_profile_slot( 11, 0, 222, 182, 149 ); //Light Fur
set_color_profile_slot( 11, 1, 33, 31, 29 ); //Dark Fur
set_color_profile_slot( 11, 2, 179, 70, 0 ); //Red Tights
set_color_profile_slot( 11, 3, 179, 70, 0 ); //Yellow Tights
set_color_profile_slot( 11, 4, 191, 71, 19 ); //Kneepads
set_color_profile_slot( 11, 5, 232, 232, 232 ); //Black
set_color_profile_slot( 11, 6, 104, 76, 150 ); //Eyes
set_color_profile_slot( 11, 7, 0, 255, 0 ); //Table

// Mitsuharu Misawa
set_color_profile_slot( 12, 0, 195, 217, 212 ); //Light Fur
set_color_profile_slot( 12, 1, 43, 41, 46 ); //Dark Fur
set_color_profile_slot( 12, 2, 0, 56, 31 ); //Red Tights
set_color_profile_slot( 12, 3, 255, 255, 255 ); //Yellow Tights
set_color_profile_slot( 12, 4, 0, 61, 34 ); //Kneepads
set_color_profile_slot( 12, 5, 255, 255, 255 ); //Black
set_color_profile_slot( 12, 6, 0, 131, 72 ); //Eyes
set_color_profile_slot( 12, 7, 0, 255, 0 ); //Table

// Sting
set_color_profile_slot( 13, 0, 255, 250, 245 ); //Light Fur
set_color_profile_slot( 13, 1, 31, 26, 26 ); //Dark Fur
set_color_profile_slot( 13, 2, 28, 28, 28 ); //Red Tights
set_color_profile_slot( 13, 3, 255, 255, 252 ); //Yellow Tights
set_color_profile_slot( 13, 4, 69, 68, 66 ); //Kneepads
set_color_profile_slot( 13, 5, 28, 28, 28 ); //Black
set_color_profile_slot( 13, 6, 250, 250, 250 ); //Eyes
set_color_profile_slot( 13, 7, 0, 255, 0 ); //Table

// Undertaker
set_color_profile_slot( 14, 0, 114, 118, 130 ); //Light Fur
set_color_profile_slot( 14, 1, 16, 12, 28 ); //Dark Fur
set_color_profile_slot( 14, 2, 23, 23, 23 ); //Red Tights
set_color_profile_slot( 14, 3, 49, 15, 130 ); //Yellow Tights
set_color_profile_slot( 14, 4, 49, 15, 130 ); //Kneepads
set_color_profile_slot( 14, 5, 19, 17, 31 ); //Black
set_color_profile_slot( 14, 6, 115, 0, 255 ); //Eyes
set_color_profile_slot( 14, 7, 0, 255, 0 ); //Table

// Bret Hart
set_color_profile_slot( 15, 0, 227, 136, 227 ); //Light Fur
set_color_profile_slot( 15, 1, 26, 26, 26 ); //Dark Fur
set_color_profile_slot( 15, 2, 158, 41, 134 ); //Red Tights
set_color_profile_slot( 15, 3, 31, 31, 31 ); //Yellow Tights
set_color_profile_slot( 15, 4, 163, 42, 138 ); //Kneepads
set_color_profile_slot( 15, 5, 26, 26, 26 ); //Black
set_color_profile_slot( 15, 6, 201, 123, 164 ); //Eyes
set_color_profile_slot( 15, 7, 0, 255, 0 ); //Table

// Tizoc (D Palette)
set_color_profile_slot( 16, 0, 114, 122, 140 ); //Light Fur
set_color_profile_slot( 16, 1, 21, 22, 26 ); //Dark Fur
set_color_profile_slot( 16, 2, 25, 31, 33 ); //Red Tights
set_color_profile_slot( 16, 3, 51, 6, 6 ); //Yellow Tights
set_color_profile_slot( 16, 4, 36, 59, 66 ); //Kneepads
set_color_profile_slot( 16, 5, 48, 60, 64 ); //Black
set_color_profile_slot( 16, 6, 129, 199, 199 ); //Eyes
set_color_profile_slot( 16, 7, 0, 255, 0 ); //Table

// Beowulf
set_color_profile_slot( 17, 0, 186, 186, 186 ); //Light Fur
set_color_profile_slot( 17, 1, 135, 131, 116 ); //Dark Fur
set_color_profile_slot( 17, 2, 46, 35, 35 ); //Red Tights
set_color_profile_slot( 17, 3, 163, 171, 152 ); //Yellow Tights
set_color_profile_slot( 17, 4, 0, 0, 0 ); //Kneepads
set_color_profile_slot( 17, 5, 43, 43, 43 ); //Black
set_color_profile_slot( 17, 6, 64, 64, 64 ); //Eyes
set_color_profile_slot( 17, 7, 0, 255, 0 ); //Table

// Goldust
set_color_profile_slot( 18, 0, 217, 183, 32 ); //Light Fur
set_color_profile_slot( 18, 1, 28, 28, 24 ); //Dark Fur
set_color_profile_slot( 18, 2, 168, 142, 25 ); //Red Tights
set_color_profile_slot( 18, 3, 18, 18, 18 ); //Yellow Tights
set_color_profile_slot( 18, 4, 140, 118, 24 ); //Kneepads
set_color_profile_slot( 18, 5, 145, 114, 0 ); //Black
set_color_profile_slot( 18, 6, 140, 118, 24 ); //Eyes
set_color_profile_slot( 18, 7, 0, 255, 0 ); //Table

// Terryman
set_color_profile_slot( 19, 0, 186, 186, 164 ); //Light Fur
set_color_profile_slot( 19, 1, 2, 19, 56 ); //Dark Fur
set_color_profile_slot( 19, 2, 8, 25, 66 ); //Red Tights
set_color_profile_slot( 19, 3, 255, 207, 43 ); //Yellow Tights
set_color_profile_slot( 19, 4, 87, 0, 0 ); //Kneepads
set_color_profile_slot( 19, 5, 8, 25, 66 ); //Black
set_color_profile_slot( 19, 6, 141, 192, 214 ); //Eyes
set_color_profile_slot( 19, 7, 0, 255, 0 ); //Table

// Chris Hayabusa
set_color_profile_slot( 20, 0, 156, 156, 156 ); //Light Fur
set_color_profile_slot( 20, 1, 22, 22, 28 ); //Dark Fur
set_color_profile_slot( 20, 2, 16, 22, 31 ); //Red Tights
set_color_profile_slot( 20, 3, 248, 212, 72 ); //Yellow Tights
set_color_profile_slot( 20, 4, 230, 187, 29 ); //Kneepads
set_color_profile_slot( 20, 5, 53, 15, 69 ); //Black
set_color_profile_slot( 20, 6, 0, 183, 255 ); //Eyes
set_color_profile_slot( 20, 7, 0, 255, 0 ); //Table

// Anglara
set_color_profile_slot( 21, 0, 74, 201, 198 ); //Light Fur
set_color_profile_slot( 21, 1, 214, 208, 118 ); //Dark Fur
set_color_profile_slot( 21, 2, 74, 201, 198 ); //Red Tights
set_color_profile_slot( 21, 3, 255, 0, 0 ); //Yellow Tights
set_color_profile_slot( 21, 4, 255, 1, 1 ); //Kneepads
set_color_profile_slot( 21, 5, 36, 91, 97 ); //Black
set_color_profile_slot( 21, 6, 214, 183, 124 ); //Eyes
set_color_profile_slot( 21, 7, 15, 31, 112 ); //Table

// Olympia
set_color_profile_slot( 22, 0, 184, 128, 83 ); //Light Fur
set_color_profile_slot( 22, 1, 219, 70, 201 ); //Dark Fur
set_color_profile_slot( 22, 2, 242, 233, 224 ); //Red Tights
set_color_profile_slot( 22, 3, 158, 142, 142 ); //Yellow Tights
set_color_profile_slot( 22, 4, 26, 26, 26 ); //Kneepads
set_color_profile_slot( 22, 5, 201, 134, 64 ); //Black
set_color_profile_slot( 22, 6, 0, 88, 150 ); //Eyes
set_color_profile_slot( 22, 7, 0, 255, 0 ); //Table

// Fernet
set_color_profile_slot( 23, 0, 247, 168, 225 ); //Light Fur
set_color_profile_slot( 23, 1, 74, 50, 67 ); //Dark Fur
set_color_profile_slot( 23, 2, 65, 0, 122 ); //Red Tights
set_color_profile_slot( 23, 3, 196, 140, 0 ); //Yellow Tights
set_color_profile_slot( 23, 4, 41, 0, 54 ); //Kneepads
set_color_profile_slot( 23, 5, 30, 14, 36 ); //Black
set_color_profile_slot( 23, 6, 33, 184, 61 ); //Eyes
set_color_profile_slot( 23, 7, 0, 255, 0 ); //Table

// TuQiu
set_color_profile_slot( 24, 0, 255, 255, 255 ); //Light Fur
set_color_profile_slot( 24, 1, 0, 33, 66 ); //Dark Fur
set_color_profile_slot( 24, 2, 0, 7, 28 ); //Red Tights
set_color_profile_slot( 24, 3, 255, 23, 62 ); //Yellow Tights
set_color_profile_slot( 24, 4, 0, 255, 238 ); //Kneepads
set_color_profile_slot( 24, 5, 255, 255, 255 ); //Black
set_color_profile_slot( 24, 6, 89, 125, 255 ); //Eyes
set_color_profile_slot( 24, 7, 0, 255, 0 ); //Table

// Incineroar
set_color_profile_slot( 25, 0, 191, 58, 58 ); //Light Fur
set_color_profile_slot( 25, 1, 41, 33, 33 ); //Dark Fur
set_color_profile_slot( 25, 2, 191, 58, 58 ); //Red Tights
set_color_profile_slot( 25, 3, 41, 33, 33 ); //Yellow Tights
set_color_profile_slot( 25, 4, 41, 33, 33 ); //Kneepads
set_color_profile_slot( 25, 5, 191, 58, 58 ); //Black
set_color_profile_slot( 25, 6, 0, 156, 10 ); //Eyes
set_color_profile_slot( 25, 7, 0, 255, 0 ); //Table

// Snow
set_color_profile_slot( 26, 0, 102, 143, 179 ); //Light Fur
set_color_profile_slot( 26, 1, 13, 26, 38 ); //Dark Fur
set_color_profile_slot( 26, 2, 156, 156, 156 ); //Red Tights
set_color_profile_slot( 26, 3, 4, 10, 20 ); //Yellow Tights
set_color_profile_slot( 26, 4, 2, 17, 79 ); //Kneepads
set_color_profile_slot( 26, 5, 186, 185, 235 ); //Black
set_color_profile_slot( 26, 6, 117, 43, 0 ); //Eyes
set_color_profile_slot( 26, 7, 0, 255, 0 ); //Table

// Cheetara
set_color_profile_slot( 27, 0, 255, 201, 54 ); //Light Fur
set_color_profile_slot( 27, 1, 255, 228, 189 ); //Dark Fur
set_color_profile_slot( 27, 2, 255, 104, 38 ); //Red Tights
set_color_profile_slot( 27, 3, 122, 51, 18 ); //Yellow Tights
set_color_profile_slot( 27, 4, 163, 118, 44 ); //Kneepads
set_color_profile_slot( 27, 5, 163, 118, 44 ); //Black
set_color_profile_slot( 27, 6, 171, 10, 10 ); //Eyes
set_color_profile_slot( 27, 7, 0, 255, 0 ); //Table

// She-Hulk
set_color_profile_slot( 28, 0, 72, 171, 59 ); //Light Fur
set_color_profile_slot( 28, 1, 0, 77, 0 ); //Dark Fur
set_color_profile_slot( 28, 2, 186, 186, 186 ); //Red Tights
set_color_profile_slot( 28, 3, 102, 0, 255 ); //Yellow Tights
set_color_profile_slot( 28, 4, 102, 0, 255 ); //Kneepads
set_color_profile_slot( 28, 5, 242, 255, 242 ); //Black
set_color_profile_slot( 28, 6, 0, 255, 0 ); //Eyes
set_color_profile_slot( 28, 7, 0, 255, 0 ); //Table

// R. Mika
set_color_profile_slot( 29, 0, 255, 254, 37 ); //Light Fur
set_color_profile_slot( 29, 1, 54, 139, 227 ); //Dark Fur
set_color_profile_slot( 29, 2, 46, 118, 191 ); //Red Tights
set_color_profile_slot( 29, 3, 197, 212, 227 ); //Yellow Tights
set_color_profile_slot( 29, 4, 255, 255, 255 ); //Kneepads
set_color_profile_slot( 29, 5, 153, 205, 255 ); //Black
set_color_profile_slot( 29, 6, 107, 255, 236 ); //Eyes
set_color_profile_slot( 29, 7, 0, 255, 0 ); //Table

// Orochi Shermie
set_color_profile_slot( 30, 0, 151, 71, 255 ); //Light Fur
set_color_profile_slot( 30, 1, 45, 21, 77 ); //Dark Fur
set_color_profile_slot( 30, 2, 150, 0, 42 ); //Red Tights
set_color_profile_slot( 30, 3, 15, 15, 15 ); //Yellow Tights
set_color_profile_slot( 30, 4, 20, 20, 20 ); //Kneepads
set_color_profile_slot( 30, 5, 150, 0, 42 ); //Black
set_color_profile_slot( 30, 6, 51, 51, 51 ); //Eyes
set_color_profile_slot( 30, 7, 0, 255, 0 ); //Table

// Abyss
set_color_profile_slot( 31, 0, 95, 84, 110 ); //Light Fur
set_color_profile_slot( 31, 1, 19, 16, 20 ); //Dark Fur
set_color_profile_slot( 31, 2, 32, 32, 46 ); //Red Tights
set_color_profile_slot( 31, 3, 97, 80, 94 ); //Yellow Tights
set_color_profile_slot( 31, 4, 73, 57, 79 ); //Kneepads
set_color_profile_slot( 31, 5, 20, 20, 20 ); //Black
set_color_profile_slot( 31, 6, 179, 147, 173 ); //Eyes
set_color_profile_slot( 31, 7, 0, 255, 0 ); //Table


/* This is a comment used by that one RoA colors.gml generator tool to store palette data. You can safely keep it in your colors.gml if you plan to re-use the tool later, or safely remove it if you don't.
=== BEGIN JSON PALETTE ===
{"formatversion":1,"data":[{"name":"Light Fur","colors":[{"r":144,"g":151,"b":166},{"r":197,"g":207,"b":227,"main":true},{"r":227,"g":236,"b":255}]},{"name":"Dark Fur","colors":[{"r":46,"g":41,"b":60,"main":true},{"r":69,"g":62,"b":90}]},{"name":"Red Tights","colors":[{"r":141,"g":0,"b":25,"main":true},{"r":192,"g":0,"b":34},{"r":255,"g":0,"b":45}]},{"name":"Yellow Tights","colors":[{"r":165,"g":103,"b":0,"main":true},{"r":255,"g":160,"b":0}]},{"name":"Kneepads","colors":[{"r":116,"g":110,"b":108,"main":true},{"r":180,"g":171,"b":168}]},{"name":"Black","colors":[{"r":47,"g":44,"b":43,"main":true}]},{"name":"Eyes","colors":[{"r":238,"g":226,"b":33,"main":true},{"r":160,"g":143,"b":20}]},{"name":"Table","colors":[{"r":56,"g":69,"b":235,"main":true},{"r":3,"g":17,"b":192}]}]}
=== END JSON PALETTE ===
*/
