...
if(state == 0){ //launch state with active attack frames
    ...
    if(state_timer <= 50 && !dial_adjust){
        vsp = 0;
        hsp = ease_cubeOut(6 * start_dir, 0, state_timer, 50);    
    }
    
    if(state_timer <= 70 && dial_adjust){
        vsp = 0;
        hsp = ease_linear(8 * player_id.spr_dir, 0, state_timer, 70);    
    }
...
if(state = 3){
	...
	// if floating downwards due to weird collision interaction prepare to destroy
	if (vsp >= 2 && !position_meeting(x, y, asset_get("par_jumpthrough"))){
		destroy_counter++;
	}
	else {
		destroy_counter = 0;
	...
	// counts up to 8 frames, and if it's still awkward falling after 8 frames destroys itself (note I had this at 5 frames but that's just enough time for it to awkward jitter)
	if (destroy_counter >= 8){
		destroy = true;
	}
	else {
		destroy = false
	...
	// Moves the puddle outside of the stage range and then make it fall
	if (destroy){
