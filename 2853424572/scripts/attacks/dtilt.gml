set_attack_value(AT_DTILT, AG_CATEGORY, 0);
set_attack_value(AT_DTILT, AG_SPRITE, sprite_get("dtilt"));
set_attack_value(AT_DTILT, AG_NUM_WINDOWS, 9);
set_attack_value(AT_DTILT, AG_HAS_LANDING_LAG, 3);
set_attack_value(AT_DTILT, AG_OFF_LEDGE, 0);
set_attack_value(AT_DTILT, AG_HURTBOX_SPRITE, sprite_get("dtilt_hurt"));


set_window_value(AT_DTILT, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_LENGTH, 5);
set_window_value(AT_DTILT, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak2"));
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX_FRAME, 2);


set_window_value(AT_DTILT, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 2, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 2, AG_WINDOW_ANIM_FRAME_START, 1);

set_window_value(AT_DTILT, 3, AG_WINDOW_TYPE, 3);
set_window_value(AT_DTILT, 3, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DTILT, 3, AG_WINDOW_ANIM_FRAME_START, 2);
set_window_value(AT_DTILT, 3, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_DTILT, 3, AG_WINDOW_CANCEL_FRAME, 3);


set_window_value(AT_DTILT, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DTILT, 4, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DTILT, 4, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DTILT, 4, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_DTILT, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 4, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy1"));
set_window_value(AT_DTILT, 4, AG_WINDOW_SFX_FRAME, 5);


set_window_value(AT_DTILT, 5, AG_WINDOW_TYPE, 2);
set_window_value(AT_DTILT, 5, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DTILT, 5, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DTILT, 5, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_DTILT, 5, 27, .1);
set_window_value(AT_DTILT, 5, 28, 0);

set_window_value(AT_DTILT, 6, AG_WINDOW_TYPE, 3);
set_window_value(AT_DTILT, 6, AG_WINDOW_LENGTH, 15);
set_window_value(AT_DTILT, 6, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DTILT, 6, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_DTILT, 6, AG_WINDOW_CANCEL_TYPE, 1);
set_window_value(AT_DTILT, 6, AG_WINDOW_CANCEL_FRAME, 6);


set_window_value(AT_DTILT, 7, AG_WINDOW_TYPE, 3);
set_window_value(AT_DTILT, 7, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DTILT, 7, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DTILT, 7, AG_WINDOW_ANIM_FRAME_START, 11);
set_window_value(AT_DTILT, 7, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 7, AG_WINDOW_SFX, asset_get("sfx_zetter_downb"));
set_window_value(AT_DTILT, 7, AG_WINDOW_SFX_FRAME, 10);

set_window_value(AT_DTILT, 8, AG_WINDOW_TYPE, 2);
set_window_value(AT_DTILT, 8, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DTILT, 8, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DTILT, 8, AG_WINDOW_ANIM_FRAME_START, 14);
set_window_value(AT_DTILT, 8, AG_WINDOW_HITPAUSE_FRAME, 0);


set_window_value(AT_DTILT, 9, AG_WINDOW_TYPE, 5);
set_window_value(AT_DTILT, 9, AG_WINDOW_LENGTH, 15);
set_window_value(AT_DTILT, 9, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_DTILT, 9, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DTILT, 9, AG_WINDOW_ANIM_FRAME_START, 15);


set_num_hitboxes(AT_DTILT, 3);
set_hitbox_value(AT_DTILT, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_X, 30);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_Y, -15);
set_hitbox_value(AT_DTILT, 1, HG_WIDTH, 70);
set_hitbox_value(AT_DTILT, 1, HG_HEIGHT, 30);
set_hitbox_value(AT_DTILT, 1, HG_SHAPE, 2);
set_hitbox_value(AT_DTILT, 1, HG_PRIORITY, 2);
set_hitbox_value(AT_DTILT, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE, 361);
set_hitbox_value(AT_DTILT, 1, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_DTILT, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 1, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_DTILT, 1, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_DTILT, 1, HG_IGNORES_PROJECTILES, 0);
set_hitbox_value(AT_DTILT, 1, HG_FORCE_FLINCH, 1);

set_hitbox_value(AT_DTILT, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW, 5);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 2, HG_LIFETIME, 4);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_X, 25);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_Y, -35);
set_hitbox_value(AT_DTILT, 2, HG_WIDTH, 96);
set_hitbox_value(AT_DTILT, 2, HG_HEIGHT, 64);
set_hitbox_value(AT_DTILT, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 2, HG_PRIORITY, 2);
set_hitbox_value(AT_DTILT, 2, HG_DAMAGE, 3);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE, 128);
set_hitbox_value(AT_DTILT, 2, HG_BASE_KNOCKBACK, 4.2);
set_hitbox_value(AT_DTILT, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 2, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DTILT, 2, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, sound_get("sfx_blow_heavyish"));
set_hitbox_value(AT_DTILT, 2, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_DTILT, 2, HG_IGNORES_PROJECTILES, 0);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DTILT, 2, HG_TECHABLE, 1);

set_hitbox_value(AT_DTILT, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW, 8);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 3, HG_LIFETIME, 2);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_X, 16);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_Y, -30);
set_hitbox_value(AT_DTILT, 3, HG_WIDTH, 120);
set_hitbox_value(AT_DTILT, 3, HG_HEIGHT, 60);
set_hitbox_value(AT_DTILT, 3, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_DTILT, 3, HG_DAMAGE, 6);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE, 90);
set_hitbox_value(AT_DTILT, 3, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DTILT, 3, HG_KNOCKBACK_SCALING, .67);
set_hitbox_value(AT_DTILT, 3, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DTILT, 3, HG_HITPAUSE_SCALING, .7);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_X_OFFSET, 10);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));
set_hitbox_value(AT_DTILT, 3, HG_IGNORES_PROJECTILES, 0);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_GROUP, 2);