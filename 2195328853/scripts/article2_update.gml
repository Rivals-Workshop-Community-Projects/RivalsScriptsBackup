...
if(player != orig_player){
	if(player_id.steam_cloud == self){
		player_id.steam_cloud = noone;
	}else if(player_id.steam_cloud2 == self){
		player_id.steam_cloud2 = noone;
	}else if(player_id.steam_cloud3 == self){
		player_id.steam_cloud3 = noone;
	}else if(player_id.steam_cloud4 == self){
		player_id.steam_cloud4 = noone;
	}
}

if(!Pocketed){
	steam_timer += 1;
	if (get_player_color( orig_player ) != 11){
	...
	}else if (get_player_color( orig_player ) == 11){
	...
	if (instance_exists(steamhitbox)) { //hitbox
		...
		if(steam_timer < steamlifetime-1){
			...
			if(!waspocketed){
				steamhitbox.player = orig_player;
				//steamhitbox.player = current_player;
			}else{
				steamhitbox.player = player;
			}
		}
	}
	...
	waspocketed = true;
	player_id.steam_cooldown = min(60,player_id.steam_cooldown);
	print(player_id.steam_cooldown);
}