...
if state != PS_HITSTUN && (special_down || use_nspec) && state != PS_PRATFALL && state != PS_PRATLAND && !((state == PS_ATTACK_AIR || state == PS_ATTACK_GROUND) && (attack == AT_USPECIAL || attack == AT_DSPECIAL)) && prev_attack != AT_FSPECIAL && prev_attack != AT_NSPECIAL && shadow_cooldown == 0 {
    if !instance_exists(shadow) && !(get_attack_value(prev_attack,AG_CATEGORY) == 1 && !free) {
    	...
    	if (state != 5 && state != 6) || has_hit {
    		if state == PS_AIR_DODGE || state == PS_TECH_GROUND || state == PS_TECH_FORWARD || state == PS_TECH_BACKWARD {
	    		was_airdodging = true;
	    	}
	        x = shadow.x;
	        y = shadow.y;
...
//Code to correct for uspecial
if state == PS_LAND || state == PS_LANDING_LAG || state == PS_WAVELAND {
	if spr_dir < 0 spr_dir = -1;
    if spr_dir >= 0 spr_dir = 1;
}

//After an attack ends, reset invuln stuff just to be safe (need to force an airdodge in order to reset internal invince aerials)
if was_attacking && (state != 5 && state != 6) {
	was_attacking = false;
	if was_airdodging {
		set_state(PS_AIR_DODGE);
		was_airdodging = false;
		print("HALT DEBOM");
	}
