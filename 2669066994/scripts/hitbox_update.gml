...
if (attack == AT_UAIR){
    ...
    //(place_meeting(x, y-2, asset_get("par_block"))){ destroyed = true; }
    if (hitbox_timer <= 15){
    //air_friction = -1;
	var fly_speed = 2;
	...
	var drag_speed = max(sqrt(crad_dist) * fly_speed / 2, fly_speed);
	hsp = lengthdir_x(drag_speed, crad_dir + 180);
	vsp = lengthdir_y(drag_speed, crad_dir - 1);
    }
    if (hitbox_timer > 15 && hitbox_timer < 25){
    //air_friction = 0;
	var fly_speed = 2;
	var crad_dir = point_direction(player_id.x, player_id.y - player_id.char_height * 0.1, x, y);
	var crad_dist = point_distance(player_id.x, player_id.y - player_id.char_height * 0.1, x, y);
	var drag_speed = max(sqrt(crad_dist) * fly_speed / 200, fly_speed);
	hsp += lengthdir_x(drag_speed, crad_dir + 180);
...
if (attack == AT_FAIR){
	...
	ignores_walls = true;
	through_platforms = 2;
    //if (place_meeting(x, y+2, asset_get("par_block"))){ destroyed = true; }
    if (hitbox_timer <= 19){
    //air_friction = -1;
    hsp = 5 * spr_dir;
	var fly_speed = 1;
	...
	if (hitbox_timer > 19 && hitbox_timer < 25){
    //air_friction = 0;
	var fly_speed = 3;
	var crad_dir = point_direction(player_id.x, player_id.y - player_id.char_height * 0.1, x, y);
	var crad_dist = point_distance(player_id.x, player_id.y - player_id.char_height * 0.1, x, y);
...
if (attack == AT_DAIR){
	...
	var crad_dist = point_distance(player_id.x, player_id.y - player_id.char_height * 0.1, x, y);
	var drag_speed = max(sqrt(crad_dist) * fly_speed / 1, fly_speed);
	hsp = lengthdir_x(drag_speed, crad_dir - 99 * player_id.spr_dir);
	vsp = lengthdir_y(drag_speed, crad_dir - 99 * player_id.spr_dir);
    }
    if (hitbox_timer > 15){
    //air_friction = 0;
	var fly_speed = 2;
	...
	var crad_dist = point_distance(player_id.x, player_id.y - player_id.char_height * 0.1, x, y);
	var drag_speed = max(sqrt(crad_dist) * fly_speed / 1, fly_speed);
	hsp = lengthdir_x(drag_speed, crad_dir - 101 * player_id.spr_dir);
	vsp = lengthdir_y(drag_speed, crad_dir - 101 * player_id.spr_dir);
	if (place_meeting(x, y, player_id)){ instance_destroy(); }
    }
}
...
if (attack == AT_FSPECIAL){
	...
	if (place_meeting(x, y, player_id)){
		var healplayer = take_damage( player, -1, -5 );
