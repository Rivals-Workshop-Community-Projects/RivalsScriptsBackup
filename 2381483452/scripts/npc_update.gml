...
switch(sequence) {
	case 0:
	if state == PS_IDLE && state_timer > 60 {
	...
	break;
	case 2:
		if state == PS_IDLE && state_timer > 15 {
			set_attack(AT_TAUNT);
			state_timer = 0;
			
		}
		if state == PS_ATTACK_GROUND && state_timer > 10 {
			sequence++;
		}
	break;
	case 3:
		if state == PS_IDLE && state_timer > 2{
			sequence++;
			spawn_dust_fx(x,y,sprite_get("airdodge_fx"), 20);
	...
	case 5:
		...
		sequence++;
		//state_timer = 0;
		print("let's go "+string(sequence))
		if sequence == 5 sound_play(sound_get("patashu"), 0, 0);
		}
	case 6:
		y = 400;
		visible = false;
