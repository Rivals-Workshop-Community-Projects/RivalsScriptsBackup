...
if (attack == AT_DTILT && window > 1 && gas_dtilt_cont < 5 && (state == PS_ATTACK_GROUND || state == PS_ATTACK_AIR)){
    draw_sprite_ext(sprite_get("gas_DTILT"), gas_dtilt_cont, gas_dtilt_posx, gas_dtilt_posy, gas_dir, 1, 0, c_white, 1 );
}

if (attack == AT_USTRONG && window > 2 && gas_ustrong_cont < 5 && (state == PS_ATTACK_GROUND || state == PS_ATTACK_AIR)){
    draw_sprite_ext(sprite_get("gas_USTRONG"), gas_ustrong_cont, gas_ustrong_posx, gas_ustrong_posy, gas_dir, 1, 0, c_white, 1 );
}
...
if (gas_nspecial_cont > 0){
    ...
    if (gas_nspecial_cont < 6){
    	...
    	gas_nspecial_posy, gas_dir, 1, uspecial_rot, c_white, 1 );	
	}
}

if (gas_djump == 1){
    draw_sprite_ext(sprite_get("gas_DOUBLEJUMP"), gas_djump_cont, gas_djumpx, gas_djumpy, gas_dir, 1, 0, c_white, 1 );
}
