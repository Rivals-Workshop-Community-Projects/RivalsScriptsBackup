far_down_attacks[0] = AT_DAIR;
far_down_attacks[1] = AT_USPECIAL;
far_down_attacks[2] = AT_NSPECIAL;

far_up_attacks[0] = AT_UAIR;
far_up_attacks[1] = AT_FSPECIAL;
far_up_attacks[2] = AT_NSPECIAL;

far_side_attacks[0] = AT_NSPECIAL;
far_side_attacks[1] = AT_FSPECIAL;
far_side_attacks[2] = AT_FAIR;
far_side_attacks[3] = AT_DATTACK;

mid_side_attacks[0] = AT_DTILT;
mid_side_attacks[1] = AT_FTILT;
mid_side_attacks[2] = AT_DATTACK;
mid_side_attacks[3] = AT_FSTRONG;
mid_side_attacks[4] = AT_FSPECIAL;
mid_side_attacks[5] = AT_DSTRONG;
mid_side_attacks[6] = AT_FAIR;
mid_side_attacks[7] = AT_BAIR;
mid_side_attacks[8] = AT_DATTACK;
mid_side_attacks[9] = AT_NSPECIAL;
mid_side_attacks[10] = AT_JAB;
mid_side_attacks[11] = AT_UAIR;
mid_side_attacks[12] = AT_NAIR;
mid_side_attacks[13] = AT_UTILT;
mid_side_attacks[14] = AT_DSPECIAL;

close_up_attacks[0] = AT_USTRONG;
close_up_attacks[1] = AT_UAIR;
close_up_attacks[2] = AT_UTILT;
close_up_attacks[3] = AT_NSPECIAL;
close_up_attacks[4] = AT_FSPECIAL;
close_up_attacks[5] = AT_DSPECIAL;

close_down_attacks[0] = AT_DSTRONG;
close_down_attacks[1] = AT_DAIR;
close_down_attacks[2] = AT_DTILT;
close_down_attacks[3] = AT_FSPECIAL;
close_down_attacks[4] = AT_NSPECIAL;
close_down_attacks[5] = AT_DSPECIAL;

close_side_attacks[0] = AT_FTILT;
close_side_attacks[1] = AT_JAB;
close_side_attacks[2] = AT_DTILT;
close_side_attacks[3] = AT_FAIR;
close_side_attacks[4] = AT_NAIR;
close_side_attacks[5] = AT_BAIR;
close_side_attacks[6] = AT_UAIR;
close_side_attacks[7] = AT_DATTACK;
close_side_attacks[8] = AT_FSTRONG;
close_side_attacks[9] = AT_DSTRONG;
close_side_attacks[10] = AT_FSPECIAL;
close_side_attacks[11] = AT_NSPECIAL;
close_side_attacks[12] = AT_UTILT;
close_side_attacks[13] = AT_DSPECIAL;

neutral_attacks[0] = AT_JAB;
neutral_attacks[1] = AT_NAIR;
neutral_attacks[2] = AT_NSPECIAL;
neutral_attacks[3] = AT_FSPECIAL;
neutral_attacks[4] = AT_DSPECIAL;