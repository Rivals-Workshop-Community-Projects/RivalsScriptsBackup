...
#define CheckCollision()
{
	...
	if (canMove && state == AS_IDLE && !IsMoving() && safeZone)
	{
		...
		if (currentHbox != noone) with (currentHbox)
		{
			var hitAdj = 14;
			with (other) moveAngle = get_hitbox_angle(other);
			other.old_hsp = lengthdir_x(hitAdj, other.moveAngle);
			other.old_vsp = lengthdir_y(hitAdj, other.moveAngle);
			if (type == 1)
			{
