// DEFAULT COLOR

// white
set_color_profile_slot( 0, 0, 251, 250, 255 );
set_color_profile_slot_range( 0, 1, 8, 11 );

// scarf
set_color_profile_slot( 0, 1, 51, 51, 53 );
set_color_profile_slot_range( 1, 1, 19, 7 );

// hair
set_color_profile_slot( 0, 2, 97, 97, 99 );
set_color_profile_slot_range( 2, 1, 9, 13 );

// hoodie
set_color_profile_slot( 0, 3, 153, 153, 156 );
set_color_profile_slot_range( 3, 1, 11, 15 );

// wood
set_color_profile_slot( 0, 4, 102, 57, 49 );
set_color_profile_slot_range( 4, 2, 11, 12 );

// blood
set_color_profile_slot( 0, 5, 166, 0, 0 );
set_color_profile_slot_range( 5, 1, 24, 34 );

// eyes
set_color_profile_slot( 0, 6, 225, 225, 234 );
set_color_profile_slot_range( 6, 1, 1, 1 );

// cleaver
set_color_profile_slot( 0, 7, 127, 127, 127 );
set_color_profile_slot_range( 7, 1, 1, 9 );


// ALTERNATE COLORS
set_num_palettes( 31 );

// Ice Blue
set_color_profile_slot( 1, 0, 245, 255, 255 ); //white
set_color_profile_slot( 1, 1, 0, 66, 69 ); //scarf
set_color_profile_slot( 1, 2, 84, 114, 117 ); //hair
set_color_profile_slot( 1, 3, 168, 199, 207 ); //hoodie
set_color_profile_slot( 1, 4, 132, 173, 163 ); //wood
set_color_profile_slot( 1, 5, 255, 255, 255 ); //blood
set_color_profile_slot( 1, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 1, 7, 255, 255, 255 ); //cleaver

// Caleb from Blood
set_color_profile_slot( 2, 0, 255, 242, 232 ); //white
set_color_profile_slot( 2, 1, 128, 128, 128 ); //scarf
set_color_profile_slot( 2, 2, 77, 44, 19 ); //hair
set_color_profile_slot( 2, 3, 51, 51, 51 ); //hoodie
set_color_profile_slot( 2, 4, 77, 40, 0 ); //wood
set_color_profile_slot( 2, 5, 219, 0, 0 ); //blood
set_color_profile_slot( 2, 6, 255, 0, 0 ); //eyes
set_color_profile_slot( 2, 7, 74, 74, 74 ); //cleaver

// The Intruder
set_color_profile_slot( 3, 0, 255, 225, 201 ); //white
set_color_profile_slot( 3, 1, 115, 115, 115 ); //scarf
set_color_profile_slot( 3, 2, 43, 43, 43 ); //hair
set_color_profile_slot( 3, 3, 54, 54, 54 ); //hoodie
set_color_profile_slot( 3, 4, 74, 47, 18 ); //wood
set_color_profile_slot( 3, 5, 23, 186, 7 ); //blood
set_color_profile_slot( 3, 6, 38, 255, 0 ); //eyes
set_color_profile_slot( 3, 7, 59, 59, 59 ); //cleaver

// Katie
set_color_profile_slot( 4, 0, 255, 255, 255 ); //white
set_color_profile_slot( 4, 1, 29, 29, 29 ); //scarf
set_color_profile_slot( 4, 2, 197, 20, 36 ); //hair
set_color_profile_slot( 4, 3, 226, 116, 132 ); //hoodie
set_color_profile_slot( 4, 4, 148, 4, 15 ); //wood
set_color_profile_slot( 4, 5, 255, 0, 0 ); //blood
set_color_profile_slot( 4, 6, 255, 72, 0 ); //eyes
set_color_profile_slot( 4, 7, 36, 36, 36 ); //cleaver

// Acid Rainbows
set_color_profile_slot( 5, 0, 255, 231, 222 ); //white
set_color_profile_slot( 5, 1, 176, 0, 129 ); //scarf
set_color_profile_slot( 5, 2, 171, 58, 181 ); //hair
set_color_profile_slot( 5, 3, 113, 222, 98 ); //hoodie
set_color_profile_slot( 5, 4, 0, 69, 0 ); //wood
set_color_profile_slot( 5, 5, 173, 0, 0 ); //blood
set_color_profile_slot( 5, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 5, 7, 219, 44, 44 ); //cleaver

// Liz
set_color_profile_slot( 6, 0, 245, 203, 191 ); //white
set_color_profile_slot( 6, 1, 113, 196, 207 ); //scarf
set_color_profile_slot( 6, 2, 250, 67, 171 ); //hair
set_color_profile_slot( 6, 3, 87, 87, 122 ); //hoodie
set_color_profile_slot( 6, 4, 0, 0, 0 ); //wood
set_color_profile_slot( 6, 5, 54, 168, 249 ); //blood
set_color_profile_slot( 6, 6, 144, 56, 231 ); //eyes
set_color_profile_slot( 6, 7, 255, 255, 255 ); //cleaver

// Hikaru
set_color_profile_slot( 7, 0, 243, 232, 232 ); //white
set_color_profile_slot( 7, 1, 212, 34, 34 ); //scarf
set_color_profile_slot( 7, 2, 97, 41, 0 ); //hair
set_color_profile_slot( 7, 3, 214, 109, 212 ); //hoodie
set_color_profile_slot( 7, 4, 255, 0, 179 ); //wood
set_color_profile_slot( 7, 5, 255, 0, 0 ); //blood
set_color_profile_slot( 7, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 7, 7, 255, 28, 28 ); //cleaver

// Yuuto Ichika
set_color_profile_slot( 8, 0, 255, 231, 222 ); //white
set_color_profile_slot( 8, 1, 109, 174, 209 ); //scarf
set_color_profile_slot( 8, 2, 186, 209, 255 ); //hair
set_color_profile_slot( 8, 3, 72, 85, 156 ); //hoodie
set_color_profile_slot( 8, 4, 255, 238, 153 ); //wood
set_color_profile_slot( 8, 5, 255, 218, 97 ); //blood
set_color_profile_slot( 8, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 8, 7, 112, 67, 25 ); //cleaver

// Cupcake
set_color_profile_slot( 9, 0, 255, 248, 253 ); //white
set_color_profile_slot( 9, 1, 134, 4, 194 ); //scarf
set_color_profile_slot( 9, 2, 107, 82, 106 ); //hair
set_color_profile_slot( 9, 3, 255, 105, 188 ); //hoodie
set_color_profile_slot( 9, 4, 225, 0, 255 ); //wood
set_color_profile_slot( 9, 5, 163, 39, 155 ); //blood
set_color_profile_slot( 9, 6, 255, 99, 199 ); //eyes
set_color_profile_slot( 9, 7, 155, 0, 166 ); //cleaver

// Squigly
set_color_profile_slot( 10, 0, 212, 207, 255 ); //white
set_color_profile_slot( 10, 1, 255, 250, 189 ); //scarf
set_color_profile_slot( 10, 2, 88, 69, 130 ); //hair
set_color_profile_slot( 10, 3, 94, 0, 46 ); //hoodie
set_color_profile_slot( 10, 4, 253, 255, 222 ); //wood
set_color_profile_slot( 10, 5, 255, 87, 230 ); //blood
set_color_profile_slot( 10, 6, 196, 0, 0 ); //eyes
set_color_profile_slot( 10, 7, 255, 87, 230 ); //cleaver

// Pico
set_color_profile_slot( 11, 0, 255, 217, 189 ); //white
set_color_profile_slot( 11, 1, 44, 107, 44 ); //scarf
set_color_profile_slot( 11, 2, 255, 129, 18 ); //hair
set_color_profile_slot( 11, 3, 113, 222, 121 ); //hoodie
set_color_profile_slot( 11, 4, 71, 71, 71 ); //wood
set_color_profile_slot( 11, 5, 209, 0, 63 ); //blood
set_color_profile_slot( 11, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 11, 7, 87, 87, 87 ); //cleaver

// Machina Kunagiri
set_color_profile_slot( 12, 0, 255, 248, 242 ); //white
set_color_profile_slot( 12, 1, 184, 29, 29 ); //scarf
set_color_profile_slot( 12, 2, 77, 83, 92 ); //hair
set_color_profile_slot( 12, 3, 41, 41, 41 ); //hoodie
set_color_profile_slot( 12, 4, 145, 145, 145 ); //wood
set_color_profile_slot( 12, 5, 196, 42, 42 ); //blood
set_color_profile_slot( 12, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 12, 7, 82, 82, 82 ); //cleaver

// Deadweight
set_color_profile_slot( 13, 0, 255, 217, 189 ); //white
set_color_profile_slot( 13, 1, 94, 30, 30 ); //scarf
set_color_profile_slot( 13, 2, 214, 214, 214 ); //hair
set_color_profile_slot( 13, 3, 31, 38, 74 ); //hoodie
set_color_profile_slot( 13, 4, 217, 217, 217 ); //wood
set_color_profile_slot( 13, 5, 120, 120, 120 ); //blood
set_color_profile_slot( 13, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 13, 7, 97, 97, 97 ); //cleaver

// Dante
set_color_profile_slot( 14, 0, 255, 255, 255 ); //white
set_color_profile_slot( 14, 1, 64, 15, 15 ); //scarf
set_color_profile_slot( 14, 2, 224, 224, 224 ); //hair
set_color_profile_slot( 14, 3, 143, 0, 0 ); //hoodie
set_color_profile_slot( 14, 4, 71, 44, 19 ); //wood
set_color_profile_slot( 14, 5, 173, 0, 0 ); //blood
set_color_profile_slot( 14, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 14, 7, 130, 130, 130 ); //cleaver

// Yuri (FFCC RoF)
set_color_profile_slot( 15, 0, 255, 231, 222 ); //white
set_color_profile_slot( 15, 1, 120, 120, 120 ); //scarf
set_color_profile_slot( 15, 2, 110, 57, 43 ); //hair
set_color_profile_slot( 15, 3, 101, 171, 85 ); //hoodie
set_color_profile_slot( 15, 4, 200, 204, 159 ); //wood
set_color_profile_slot( 15, 5, 247, 240, 74 ); //blood
set_color_profile_slot( 15, 6, 145, 209, 181 ); //eyes
set_color_profile_slot( 15, 7, 120, 76, 54 ); //cleaver

// Firion (FFII)
set_color_profile_slot( 16, 0, 255, 217, 189 ); //white
set_color_profile_slot( 16, 1, 128, 162, 184 ); //scarf
set_color_profile_slot( 16, 2, 205, 210, 214 ); //hair
set_color_profile_slot( 16, 3, 66, 71, 84 ); //hoodie
set_color_profile_slot( 16, 4, 194, 155, 164 ); //wood
set_color_profile_slot( 16, 5, 171, 12, 12 ); //blood
set_color_profile_slot( 16, 6, 245, 134, 53 ); //eyes
set_color_profile_slot( 16, 7, 168, 42, 42 ); //cleaver

// Prompto (FFXV)
set_color_profile_slot( 17, 0, 255, 236, 222 ); //white
set_color_profile_slot( 17, 1, 115, 43, 43 ); //scarf
set_color_profile_slot( 17, 2, 240, 223, 134 ); //hair
set_color_profile_slot( 17, 3, 52, 61, 44 ); //hoodie
set_color_profile_slot( 17, 4, 107, 107, 107 ); //wood
set_color_profile_slot( 17, 5, 148, 148, 148 ); //blood
set_color_profile_slot( 17, 6, 221, 226, 225 ); //eyes
set_color_profile_slot( 17, 7, 178, 217, 212 ); //cleaver

// Ardyn (FFXV)
set_color_profile_slot( 18, 0, 255, 196, 153 ); //white
set_color_profile_slot( 18, 1, 191, 72, 53 ); //scarf
set_color_profile_slot( 18, 2, 107, 23, 34 ); //hair
set_color_profile_slot( 18, 3, 66, 66, 66 ); //hoodie
set_color_profile_slot( 18, 4, 66, 71, 66 ); //wood
set_color_profile_slot( 18, 5, 255, 71, 71 ); //blood
set_color_profile_slot( 18, 6, 255, 181, 54 ); //eyes
set_color_profile_slot( 18, 7, 255, 71, 71 ); //cleaver

// Travis Touchdown
set_color_profile_slot( 19, 0, 255, 255, 255 ); //white
set_color_profile_slot( 19, 1, 74, 84, 97 ); //scarf
set_color_profile_slot( 19, 2, 52, 46, 56 ); //hair
set_color_profile_slot( 19, 3, 97, 21, 21 ); //hoodie
set_color_profile_slot( 19, 4, 87, 87, 87 ); //wood
set_color_profile_slot( 19, 5, 87, 149, 222 ); //blood
set_color_profile_slot( 19, 6, 255, 201, 84 ); //eyes
set_color_profile_slot( 19, 7, 87, 149, 222 ); //cleaver

// Metapuns
set_color_profile_slot( 20, 0, 224, 209, 255 ); //white
set_color_profile_slot( 20, 1, 73, 197, 222 ); //scarf
set_color_profile_slot( 20, 2, 255, 165, 174 ); //hair
set_color_profile_slot( 20, 3, 207, 134, 240 ); //hoodie
set_color_profile_slot( 20, 4, 209, 255, 209 ); //wood
set_color_profile_slot( 20, 5, 0, 192, 227 ); //blood
set_color_profile_slot( 20, 6, 255, 255, 255 ); //eyes
set_color_profile_slot( 20, 7, 255, 207, 252 ); //cleaver

// sir will's stitch
set_color_profile_slot( 21, 0, 250, 251, 255 ); //white
set_color_profile_slot( 21, 1, 107, 121, 214 ); //scarf
set_color_profile_slot( 21, 2, 70, 72, 94 ); //hair
set_color_profile_slot( 21, 3, 213, 212, 214 ); //hoodie
set_color_profile_slot( 21, 4, 70, 72, 94 ); //wood
set_color_profile_slot( 21, 5, 74, 110, 133 ); //blood
set_color_profile_slot( 21, 6, 63, 136, 196 ); //eyes
set_color_profile_slot( 21, 7, 255, 255, 255 ); //cleaver

// Gooshi
set_color_profile_slot( 22, 0, 255, 255, 255 ); //white
set_color_profile_slot( 22, 1, 158, 226, 255 ); //scarf
set_color_profile_slot( 22, 2, 237, 237, 237 ); //hair
set_color_profile_slot( 22, 3, 40, 50, 56 ); //hoodie
set_color_profile_slot( 22, 4, 255, 173, 247 ); //wood
set_color_profile_slot( 22, 5, 255, 138, 226 ); //blood
set_color_profile_slot( 22, 6, 255, 173, 234 ); //eyes
set_color_profile_slot( 22, 7, 179, 244, 255 ); //cleaver

// Zoe
set_color_profile_slot( 23, 0, 255, 145, 206 ); //white
set_color_profile_slot( 23, 1, 124, 0, 173 ); //scarf
set_color_profile_slot( 23, 2, 184, 0, 111 ); //hair
set_color_profile_slot( 23, 3, 74, 66, 72 ); //hoodie
set_color_profile_slot( 23, 4, 255, 132, 0 ); //wood
set_color_profile_slot( 23, 5, 255, 255, 255 ); //blood
set_color_profile_slot( 23, 6, 0, 132, 255 ); //eyes
set_color_profile_slot( 23, 7, 255, 77, 184 ); //cleaver

// Arizona
set_color_profile_slot( 24, 0, 255, 217, 189 ); //white
set_color_profile_slot( 24, 1, 209, 21, 68 ); //scarf
set_color_profile_slot( 24, 2, 148, 73, 15 ); //hair
set_color_profile_slot( 24, 3, 255, 214, 186 ); //hoodie
set_color_profile_slot( 24, 4, 122, 72, 32 ); //wood
set_color_profile_slot( 24, 5, 209, 110, 21 ); //blood
set_color_profile_slot( 24, 6, 0, 179, 0 ); //eyes
set_color_profile_slot( 24, 7, 214, 214, 214 ); //cleaver

// Oleander
set_color_profile_slot( 25, 0, 156, 143, 161 ); //white
set_color_profile_slot( 25, 1, 232, 202, 167 ); //scarf
set_color_profile_slot( 25, 2, 71, 31, 68 ); //hair
set_color_profile_slot( 25, 3, 150, 114, 148 ); //hoodie
set_color_profile_slot( 25, 4, 156, 0, 98 ); //wood
set_color_profile_slot( 25, 5, 196, 0, 0 ); //blood
set_color_profile_slot( 25, 6, 0, 195, 255 ); //eyes
set_color_profile_slot( 25, 7, 38, 28, 28 ); //cleaver

// Alternate life
set_color_profile_slot( 26, 0, 255, 232, 214 ); //white
set_color_profile_slot( 26, 1, 179, 16, 65 ); //scarf
set_color_profile_slot( 26, 2, 112, 81, 109 ); //hair
set_color_profile_slot( 26, 3, 58, 56, 61 ); //hoodie
set_color_profile_slot( 26, 4, 140, 70, 50 ); //wood
set_color_profile_slot( 26, 5, 212, 0, 0 ); //blood
set_color_profile_slot( 26, 6, 0, 0, 0 ); //eyes
set_color_profile_slot( 26, 7, 168, 168, 168 ); //cleaver

// Margaret
set_color_profile_slot( 27, 0, 229, 222, 214 ); //white
set_color_profile_slot( 27, 1, 247, 247, 242 ); //scarf
set_color_profile_slot( 27, 2, 238, 224, 176 ); //hair
set_color_profile_slot( 27, 3, 54, 47, 41 ); //hoodie
set_color_profile_slot( 27, 4, 93, 113, 123 ); //wood
set_color_profile_slot( 27, 5, 102, 90, 77 ); //blood
set_color_profile_slot( 27, 6, 54, 158, 119 ); //eyes
set_color_profile_slot( 27, 7, 225, 193, 115 ); //cleaver

// Bomb Ghost
set_color_profile_slot( 28, 0, 10, 1, 23 ); //white
set_color_profile_slot( 28, 1, 0, 65, 123 ); //scarf
set_color_profile_slot( 28, 2, 32, 4, 71 ); //hair
set_color_profile_slot( 28, 3, 0, 65, 123 ); //hoodie
set_color_profile_slot( 28, 4, 255, 198, 186 ); //wood
set_color_profile_slot( 28, 5, 255, 47, 0 ); //blood
set_color_profile_slot( 28, 6, 254, 87, 34 ); //eyes
set_color_profile_slot( 28, 7, 37, 6, 79 ); //cleaver

// JJ
set_color_profile_slot( 29, 0, 90, 78, 96 ); //white
set_color_profile_slot( 29, 1, 179, 11, 255 ); //scarf
set_color_profile_slot( 29, 2, 49, 37, 56 ); //hair
set_color_profile_slot( 29, 3, 49, 37, 56 ); //hoodie
set_color_profile_slot( 29, 4, 212, 101, 255 ); //wood
set_color_profile_slot( 29, 5, 179, 11, 255 ); //blood
set_color_profile_slot( 29, 6, 94, 94, 94 ); //eyes
set_color_profile_slot( 29, 7, 134, 11, 255 ); //cleaver

// Doom Slayer
set_color_profile_slot( 30, 0, 252, 199, 179 ); //white
set_color_profile_slot( 30, 1, 160, 105, 65 ); //scarf
set_color_profile_slot( 30, 2, 130, 107, 84 ); //hair
set_color_profile_slot( 30, 3, 150, 158, 58 ); //hoodie
set_color_profile_slot( 30, 4, 99, 69, 54 ); //wood
set_color_profile_slot( 30, 5, 130, 135, 140 ); //blood
set_color_profile_slot( 30, 6, 117, 164, 171 ); //eyes
set_color_profile_slot( 30, 7, 120, 134, 133 ); //cleaver


/* This is a comment used by that one RoA colors.gml generator tool to store palette data. You can safely keep it in your colors.gml if you plan to re-use the tool later, or safely remove it if you don't.
=== BEGIN JSON PALETTE ===
{"formatversion":1,"data":[{"name":"white","colors":[{"r":251,"g":250,"b":255,"main":true},{"r":214,"g":210,"b":230}]},{"name":"scarf","colors":[{"r":51,"g":51,"b":53,"main":true},{"r":29,"g":29,"b":37}]},{"name":"hair","colors":[{"r":97,"g":97,"b":99,"main":true},{"r":63,"g":63,"b":70}]},{"name":"hoodie","colors":[{"r":153,"g":153,"b":156,"main":true},{"r":105,"g":105,"b":119}]},{"name":"wood","colors":[{"r":102,"g":57,"b":49,"main":true},{"r":74,"g":34,"b":28}]},{"name":"blood","colors":[{"r":166,"g":0,"b":0,"main":true},{"r":81,"g":19,"b":19},{"r":109,"g":14,"b":14}]},{"name":"eyes","colors":[{"r":225,"g":225,"b":234,"main":true}]},{"name":"cleaver","colors":[{"r":127,"g":127,"b":127,"main":true},{"r":107,"g":107,"b":107}]}]}
=== END JSON PALETTE ===
*/
