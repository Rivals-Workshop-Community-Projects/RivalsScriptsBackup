...
with phone{
	...
	initTip("Butterfly, blocks, and you");
	initTipWords("All of Eid’s specials connect to the same central gimmick. Your goal as Eid is to spread out all of your resources and control as much space as you can. 

The Blocks and Butterfly serve the same purpose. When activated with Dspecial, they become powerful hitboxes capable of KOing opponents at later percentages.

So make your opponents play some Tower of Heaven™ and obey the rules you lay out for them.");
initTipImage(player_id.spr_dspecial, -5, fa_center, 1, c_white, 0);

	initTip("Advanced block techniques ");
	initTipWords("Unlike the butterfly, blocks are a much more versatile tool that can do a lot more things.

Block cancel Uair :
If an opponent trips a block as you are standing on it you can follow it up with a Uair. this can lead to a kill from Uairs launcher hitbox OR: if close to the ground, can be followed up with a dragdown Uair followed up by a grounded attack (Ustrong comes in handy to snag some sneaky KOs.) 

BlockChaining
Haha funny cryptocurrency. 

If you place blocks in a vertical line from each other and an opponent trips the bottom block, the remaining blocks will chain together and the opponent will launch from the top block.

However Block chaining has a downside. Each block does a total of 10% of damage, if blocks are chained the total damage dealt to the opponent will STAY AS 10%. This however also has a workaround. if the opponent manages to somehow hit multiple blocks in a non chain method (I. E. getting hit by one and DI-ing to another)  the damage will add up for 10% per block.
");
	initTipImage(player_id.spr_nspecial, -5, fa_center, 1, c_white, 3);
	
	initTip("How to recover");
	initTipWords("Eids recovery is pretty convoluted but easy to pull off when you get the hang of it.

much like fspecial, USpecial throws your butterfly friend. While the butterfly is on screen Eid can teleport to it from wherever he wants with Uspecial. As long its on screen you can teleport to it, but be careful, it disappears after a short while.
While in the teleport startup animation, you can press the Parry button to cancel out of it and launch upward, much like Kraggs Fspecial.

Blocks can also be used to recover, however eid MUST be on the ground for them to recharge.
...
Dair, and Dash attack can be jump canceled
...
initTipWords("Tower of Heaven is a cool game, 


you should play it.")
initTipImage(player_id.spr_dtilt, -1, fa_center, 1, c_white, 3);
...
with phone{
	...
	initAbout("About EID", "
	Art and Normals code: FuZZ
	@fuzzertz
	
	Specials code: VVizard
	@VVizardX
");
...
if pho_has_steve_dmsg{
	...
	steve_death_message = "Steve broke the rules";
...
if !phone_lightweight{
	...
	if pho_has_hikaru_fak{
		...
		Hikaru_Title = "Heavenly Traveler";
