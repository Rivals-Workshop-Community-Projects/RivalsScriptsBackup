...
if attack == AT_NSPECIAL {
    if window == get_window_index("active") {
        if window_time_is(1) {
            var projectile = create_hitbox(AT_NSPECIAL, 1, x+spr_dir*50, y-10)
            projectile.hsp = spr_dir*1.5
            projectile.vsp = 0;
            
            // Resettle stars into earliest empty slots
            var empty_slots = 0
            for (var slot_i=0; slot_i<max_stars; slot_i+=1) {
                var star_in_slot = stars[slot_i];
                if star_in_slot == undefined or !instance_exists(star_in_slot) {
                    empty_slots += 1
                    // Slide back other slots towards here
                    for (var slot_j=slot_i+1; slot_j<max_stars; slot_j+=1) {
                        stars[slot_j-1] = stars[slot_j]
                        stars[slot_j] =  undefined
                    }
                }
            }
            if empty_slots == 0 {
                // delete oldest
                stars[0].destroyed = true
                for (var slot_i=0; slot_i<max_stars; slot_i+=1) {
                    if slot_i > 0 {
                        stars[slot_i-1] = stars[slot_i]
                    }
                    stars[slot_i] =  undefined
                }
                empty_slots = 1
            }
            stars[max_stars-empty_slots] = projectile
        }
    }
} 
