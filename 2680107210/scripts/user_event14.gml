...
var col = /*alt_ui_recolor == noone*/ true ? c_white : make_color_rgb(get_color_profile_slot_r(alt_cur, alt_ui_recolor), get_color_profile_slot_g(alt_cur, alt_ui_recolor), get_color_profile_slot_b(alt_cur, alt_ui_recolor));
...
if use_alt_names{
	if alt_cur < array_length(alt_names) && alt_names[alt_cur] != ""{
		...
		textDraw(x + 10, y + 141 - is_online * 12, "fName", c_white, 1000, 200, fa_left, 1, true, 1, alt_names[alt_cur], false); //Doesn't properly adjust for online play
	}
}
...
if phone.big_screen_pos_offset < 1{
	...
	else if phone.app == phone.APP_CHEATS{
...
if(muno_cooldowns_landing == noone)
{
	muno_cooldowns_landing = [];
	for (var checked_move = 0; checked_move < 50; checked_move++)
	{
		if (get_attack_value(checked_move, AG_MUNO_ATTACK_COOLDOWN) < 0)
		{
			muno_cooldowns_landing[array_length(muno_cooldowns_landing)] = checked_move;
		}
	}
...
{
	var amt = get_attack_value(attack, AG_MUNO_ATTACK_COOLDOWN);
	var set_amt = abs(amt);
	if amt != 0 && move_cooldown[attack] == 0
	{
...
if phone_landing
{
	for (var checked_move = 0; checked_move < array_length(muno_cooldowns_landing); checked_move++)
	{
		move_cooldown[muno_cooldowns_landing[checked_move]] = 0;
	}
}
...
if phone_practice phoneBigScreen(phone.app == clamp(phone.app, phone.APP_DATA, phone.APP_CHEATS) && array_length(phone.apps[phone.app].array));
...
for (j = 0; j < array_length(phone.move_ordering); j++){ // j = index in array of ordered attack indexes
