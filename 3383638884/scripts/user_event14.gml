...
switch(muno_event_type){
	case 0: // init
		CORE_init();
		print_debug("NOISE IS CALLING PICK UP PHONE " + string(get_gameplay_time()));
		if(phone_practice && get_gameplay_time() < 5)
			noise_calling = sound_play(sound_get("noise_calling"), true, 0, 1);
		break;
	case 1: // update
		CORE_update();
		break;
...
sfx_pho_open[0] = sound_get("_pho_switch1");
sfx_pho_open[1] = sound_get("_pho_switch2");
sfx_pho_close = sound_get("sfx_comboend");
sfx_pho_move = sound_get("_pho_menu_move");
sfx_pho_move_home = sound_get("_pho_menu_move_ghost");
sfx_pho_page = sound_get("_pho_menu_chomp");
sfx_pho_open_app[0] = sound_get("_pho_menu_select1");
sfx_pho_open_app[1] = sound_get("_pho_menu_select2");
sfx_pho_open_app[2] = sound_get("_pho_menu_select3");
sfx_pho_close_app = sound_get("_pho_menu_back");
sfx_pho_power_off = sound_get("_pho_file_select" + string(1 + random_func(2, 3, true)));
sfx_pho_power_on = sound_get("_pho_collect_toppin");
...
rectDraw(x + 10, y + 10, 8, 142, c_black);

var col = alt_ui_recolor == noone ? c_white : make_color_rgb(get_color_profile_slot_r(alt_cur, 0), get_color_profile_slot_g(alt_cur, 0), get_color_profile_slot_b(alt_cur, 0));
var col_shade = alt_ui_recolor == noone ? c_white : make_color_rgb(get_color_profile_slot_r(alt_cur, 1), get_color_profile_slot_g(alt_cur, 1), get_color_profile_slot_b(alt_cur, 1));
switch(alt_cur){
	case 0:
		col = c_white;
		col_shade = $989898;
...
rectDraw(x + 10, y + 10, 6, 30, c_gray * 0.5);

var draw_y = y + 42;
for (i = 0; i < num_alts; i++){
	if(i == alt_cur)
	{
		rectDraw(x + 10, draw_y - 2, 10, 8, c_black);
		rectDraw(x + 10, draw_y, 8, 4, col);
		rectDraw(x + 10, draw_y, 2, 4, col_shade);
		draw_y += 6;
	}
	else
	{
		draw_color = c_gray * 0.5;
		rectDraw(x + 10, draw_y, 6, 4, draw_color);
		draw_y += 6;
	}
}
...
rectDraw(x + 8, y + 126, 22, 20, c_black);
textDraw(x + 10, y + 127, "fName", c_white, 20, 1000, fa_left, 1, false, 1, txt, false);
...
if use_alt_names{
	if alt_cur < array_length(alt_names) && alt_names[alt_cur] != ""{
		...
		textDraw(x + 10, y + 139 - is_online * 12, "fName", col, 1000, 200, fa_left, 1, true, 1, alt_names[alt_cur], false);
	}
}
...
if !phone.has_opened_yet && phone_practice{
	...
	draw_sprite_ext(sprite_get("_pho_icon"), get_player_color(player), x_pos+phone.last_text_size[0]-2+2*(random_func(0, 3, true)-1)*(("noise_calling" in self && noise_calling != noone)?1:0), y_pos-14+2*(random_func(1, 3, true)-1)*(("noise_calling" in self && noise_calling != noone)?1:0),2,2,0,c_white,1);
}
...
maskHeader();
rectDraw(phone_x - 14, phone_y - 10, phone.screen_width + 28, phone.screen_height + 20, phone.apps[phone.app].color_dark);
maskMidder();
rectDraw(phone_x - 14, phone_y - 10, phone.screen_width + 28, phone.screen_height + 20, phone.apps[phone.app].color_dark);
...
if phone.big_screen_pos_offset < 1{
	...
	if array_length(phone.apps[phone.app].array) > 0{
		...
		if "page_starts" in item && array_length(item.page_starts) > 1{
			textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 151 - phone.extra_top_size, "fName", c_white, 1000, 1000, fa_center, 1, 0, 1, "Page " + string(phone.page + 1), 0);
			textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 172 - phone.extra_top_size, "fName", c_white, 1000, 1000, fa_center, 1, 0, 1, "ATTACK: " + (phone.page == array_length(item.page_starts) - 1 ? "First" : "Next"), 1);
		}
		
		else if "options" in item{
			textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 172 - phone.extra_top_size, "fName", c_white, 1000, 1000, fa_center, 1, 0, 1, "ATTACK: " + (array_length(item.options) > 1 ? (array_length(item.options) > 2 ? "Next Option" : "Toggle") : "Activate"), 1);
			if array_length(item.options) > 2 textDraw(draw_x + 140 - round(118 * (phone.y / phone.lowered_y)), draw_y + 151 - phone.extra_top_size, "fName", c_white, 1000, 1000, fa_center, 1, 0, 1, "JUMP: Previous Option", 1);
		}
	}
...
if phone_practice switch(phone.state){
	...
	case 2: // opened
		...
		if phone.app == phone.APP_HOME{
			...
			if sel != -1{
				setPhoneApp(sel);
				sound_play(sfx_pho_open_app[random_func(3, 3, true)], false, 0);
				phoneAppIconSlide();
				phone.cursor = 0;
			}
		}
...
if (!joy_pad_idle && "held_timer" in self) && array_length(arr) > 1{
	...
	if (down_pressed || (down_down && held)){
		...
		phone.page = 0;
		phoneCursorChange();
		sound_play(sfx_pho_move, false, 0);
		if(!phone.app)
			sound_play(sfx_pho_move_home, false, 0, 1, 1.25 - 0.5*random_func(5, 1, false));
	}
...
if (attack == AT_ENTRANCE && joy_pad_idle && phone_practice) || attack == AT_PHONE{
	if("noise_calling" in self && instance_exists(noise_calling))
		sound_stop(noise_calling);
	attack = AT_PHONE;
	with phone setPhoneState(1);
	sound_play(phone.has_opened_yet ? sfx_pho_open[random_func(4, 2, true)] : sfx_pho_power_on, false, 0);
}
