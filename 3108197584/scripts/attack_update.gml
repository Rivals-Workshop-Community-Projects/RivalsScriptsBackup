	...
	switch window{
		...
		if window_timer == 6 && !hitpause{
			...
			while(fire_end[0] == clamp(fire_end[0], 0, room_width) && fire_end[1] == clamp(fire_end[1], 0, room_height) && hit_count < 2 && !position_meeting(fire_end[0], fire_end[1], asset_get("par_block"))){
				...
				if instance_exists(colart) && colart.is_hittable && array_find_index(hit_list, colart) < 0 && colart != drone{
					array_push(hit_list, colart);
					array_push(hit_pos, fire_end[0]);
					array_push(hit_pos, fire_end[1]);
					hit_count++;
...
shot_angle = shot_angle - angle_difference(shot_angle, (joy_pad_idle? spr_dir*.1: joy_dir + 90 - 90*spr_dir))*(using_controller? .16: .06); //point_direction(x + ground_body_pos[image_index - 5][0], y + ground_body_pos[image_index - 5][1], mouse_x, mouse_y-3)
