if(player_id.player == orig_player){
	if(attack == AT_FSTRONG){
		...
		if(abs(hsp) > 2 || abs(vsp) > 2){
			hsp*=.95;
			vsp*=.95;
		}
		for(var i = 0, objs = ["pHitBox","obj_article1","obj_article2","obj_article3"], len = array_length_1d(objs); i < len; i++)with(asset_get(objs[i]))if(self != other)nspec_stun();
		with(player_id)if(place_meeting(x,y,other)){
			if(phone_attacking && attack == AT_USPECIAL && window == 2 && window_timer == 1){vsp*=1.1;hsp*=1.2;}
		}
...
#define nspec_stun
Frozen = (place_meeting(x,y,other) && ((("UnReflectable" in self && !UnReflectable || "UnReflectable" not in self || sprite_index != asset_get("empty_sprite")) && "Freezable" not in self) || "Freezable" in self && Freezable));
if(other.hitbox_timer == other.length || other.destroyed || other.KoB_destroy)Frozen = false;
if("in_hitpause" in self)in_hitpause = Frozen;
if("hitstop" in self)hitstop = Frozen*2;