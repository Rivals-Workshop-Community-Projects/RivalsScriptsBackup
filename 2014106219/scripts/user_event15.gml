...
muno_char_name = "Cinnamon";							// Here I replace the name, because "Sandbert with a Phone" is awkwardly long
...
spr_dash_bluy = sprite_get("_pho_dash_bluy");
...
with phone{
	...
	initTip("Echo?");
	initTipWords("Being a moveset derivative of her brother Bluey, she wields her own variant of the Omniwand and does many of the same things he does, such as their magic meter mechanic."); 
	initTipWords("She does, however, have some differences, such as a couple different attacks. As such, this guide will remain similar to Bluey's, but adjusted accordingly with Cinna's unique traits.");
	initTipImage(player_id.spr_dash, -4, fa_right, 1, c_white, 3);
	initTipImage(player_id.spr_dash_bluy, -3, fa_left, 1, c_white, 3);
	initTipWords_ext("They may still be similar enough so that mains of either character can use the other nearly as efficiently. Of course, you can also simply go for whoever is more your cup of tea.", fa_center, c_white, 0, 0);
	...
	initTipWords("But wait, there's more! You can also use magic charge to power up regular attacks that use Cinna's magic wand.");
	initTipWords("DTilt inflicts a hefty stun on the opponent.");
	initTipImage(player_id.spr_dtilt, -3, fa_left, 1, c_white, 1);
	
	initTipWords("FAir increases the power and size of the blasts. The final blast has more kill power, too.");
	initTipImage(player_id.spr_fair, -3, fa_left, 1, c_white, 1);
	...
	initTipWords("After throwing a FSpecial, you can relaunch it with hammer attacks - Jab3 and FStrong. These moves throw it at different angles.");
	...
	initTipWords(" ");	
	initTipWords(" ");
	initTipImage(player_id.spr_hurtground, -3, fa_left, 1, c_white, 1);
	initTipWords("haha gottem't");
}
...
with phone{
	...
	initPatchWords("Stats");
	initPatchWords_ext("Air acceleration slightly boosted. (3.5->3.8)", fa_left, c_gray, 1, 0);
	initPatchWords("Forward Aerial (Powered Up)");
	initPatchWords_ext("The final hit has more knockback (6->7BKB, .6->.75KBS), being potentially more effective as a kill move.", fa_left, c_gray, 1, 0);
	initPatchWords("Down Special");
	initPatchWords_ext("Readjusted SFX.", fa_left, c_gray, 1, 0);
	...
	initPatchWords_ext("When having a Bowling Ball connect with another one, be it from another Cinnamon or Bluey, they should both properly ricochet back at their owners.", fa_left, c_gray, 1, 0);

	initPatch("1.0", "5 March, 2020");
...
with self{
	...
	initCheat("Be blue.", "cheat_funny_snail", [1], ["kinda", "maybe"], "You are already (kind of) blue. Definitely more of a red person, though.");
}
...
with phone{
	...
	initAbout("About Cinnamon", "Character designed and developed by SB-2749. @SB2749
...
if pho_has_trum_codec{
	...
	SPK_CINN = initSpeaker(1, "Cinnamon", sprite_get("_pho_cdc_char_cinna"));
	SPK_BLUY = initSpeaker(2, "Bluey", sprite_get("_pho_cdc_char_bluy"));
	
	trummel_codecs = [];
	
	initCodec(0);
	initCodecPage(SPK_TRUM, 0, 0, "woag it's");
	initCodecPage(SPK_TRUM, 0, 0, "...not bluy");
	initCodecPage(SPK_ALTO, 11, 0, "WHAT!!!!!!!!!");
	initCodecPage(SPK_CINN, 0, 0, "...what?")
...
if pho_has_otto_bhead{
	steve_death_message = "Steve wasn't spicy enough.";
}
