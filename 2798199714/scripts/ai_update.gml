...
if (get_training_cpu_action() == CPU_FIGHT && ai_target != self){
	...
	if(!phone_attacking && can_attack && ready_to_attack && (temp_level >= 5 || BossMode)){
		if(!free){
	...
	if(phone_attacking && temp_level > 5){
		//if((attack == AT_DATTACK) && has_hit)jump_pressed = true;
		//if((attack == AT_DTILT || attack == AT_DSTRONG || attack == AT_USPECIAL) && has_hit){attack_pressed = random_func(0,2,true) == 0;jump_pressed = !attack_pressed;}
	}
