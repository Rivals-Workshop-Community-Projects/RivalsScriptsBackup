//====> ANIMATION OFFSETS #######################################################   

//Idle Animations
sprite_change_offset("idle", 48, 94);
sprite_change_offset("idle_alt", 48, 94);
sprite_change_offset("idle_wait", 48, 94);
sprite_change_offset("idle_halloween", 48, 94);
sprite_change_offset("idle_genesis", 48, 94);
sprite_change_offset("taunt", 48, 94);
sprite_change_offset("intro", 80, 94);
sprite_change_offset("intro2", 48, 94);

//Ground Movement
sprite_change_offset("walk", 48, 94);
sprite_change_offset("walkturn", 48, 94);
sprite_change_offset("dash", 48, 94);
sprite_change_offset("dashstart", 48, 94);
sprite_change_offset("dashstop", 48, 94);
sprite_change_offset("dashturn", 48, 94);

//Non-attack Moveset
sprite_change_offset("crouch", 48, 94);
sprite_change_offset("parry", 48, 94);
sprite_change_offset("roll_forward", 48, 94);
sprite_change_offset("roll_backward", 48, 94);
sprite_change_offset("airdodge", 48, 94);
sprite_change_offset("waveland", 35, 60); //!!!!!
sprite_change_offset("tech", 35, 59); //!!!!!

//Hurt Animations
sprite_change_offset("hurt", 48, 94);
sprite_change_offset("bighurt", 48, 94);
sprite_change_offset("hurtground", 48, 94);
sprite_change_offset("hurtground_2", 48, 94);
sprite_change_offset("bouncehurt", 48, 94);
sprite_change_offset("spinhurt", 48, 94);
sprite_change_offset("uphurt", 48, 94);
sprite_change_offset("downhurt", 48, 94);

//Jump Animations 
sprite_change_offset("jumpstart", 48, 94);
sprite_change_offset("jump", 48, 94);
sprite_change_offset("doublejump", 48, 94);
sprite_change_offset("doublejump_back", 48, 94);
sprite_change_offset("walljump", 48, 94);
sprite_change_offset("pratfall", 48, 94);
sprite_change_offset("land", 48, 94);
sprite_change_offset("landinglag", 48, 94);

//Normal Ground Attacks
sprite_change_offset("jab", 48, 94);
sprite_change_offset("dattack", 48, 94);

//Tilt Attacks
sprite_change_offset("ftilt", 48, 94);
sprite_change_offset("dtilt", 80, 94);
sprite_change_offset("utilt", 48, 126);

//Aerial Attacks
sprite_change_offset("nair", 48, 94);
sprite_change_offset("fair", 48, 94);
sprite_change_offset("bair", 80, 94);
sprite_change_offset("uair", 48, 94);
sprite_change_offset("dair", 48, 94);

//Strong Attacks
sprite_change_offset("fstrong", 80, 94);
sprite_change_offset("ustrong", 80, 126);
sprite_change_offset("dstrong", 48, 126);

//Special Moves
sprite_change_offset("nspecial", 80, 94);
sprite_change_offset("nspecial_air", 80, 94);
sprite_change_offset("nspecial_down", 47, 91); //!!!!!
sprite_change_offset("nspecial_2", 48, 94);
sprite_change_offset("nspecial_2_air", 35, 91); //!!!!!
sprite_change_offset("nspecial_3", 48, 94);
sprite_change_offset("nspecial_3_air", 48, 94);
sprite_change_offset("final_smash", 112, 190);
sprite_change_offset("fail", 80, 94);
sprite_change_offset("fspecial", 80, 126);
sprite_change_offset("fspecial_2", 80, 94);
sprite_change_offset("fspecial_2_air", 80, 94);
sprite_change_offset("uspecial", 80, 126);
sprite_change_offset("dspecial", 48, 94);

//Miscellaneous
sprite_change_offset("plat", 51, 41);

sprite_change_offset("fspecial_proj", 103, 103);
sprite_change_offset("ball_form0", 18, 18);
//sprite_change_offset("ball_dust", 17, 17);
//sprite_change_offset("ball_dust_ol", 17, 17);
sprite_change_offset("ballstun", 41, 70);
sprite_change_offset("electricity", 48, 94);
sprite_change_offset("nspecial_dust1", 91, 64);
sprite_change_offset("nspecial_dust2", 63, 77);
sprite_change_offset("ballRegen", 80, 62);
sprite_change_offset("ballLeft", 16, 32);

sprite_change_offset("ball_formA_1", 65, 65);
sprite_change_offset("ball_formA_2", 65, 65);
sprite_change_offset("ball_formA_3", 65, 65);
sprite_change_offset("ball_formA_4", 65, 65);
sprite_change_offset("ball_formA_1H", 65, 65);
sprite_change_offset("ball_formA_2H", 65, 65);
sprite_change_offset("ball_formA_3H", 65, 65);
sprite_change_offset("ball_formA_4H", 65, 65);
sprite_change_offset("ball_formA_1_hurt", 65, 65);
sprite_change_offset("ball_formA_2_hurt", 65, 65);
sprite_change_offset("ball_formA_3_hurt", 65, 65);
sprite_change_offset("ball_formA_4_hurt", 65, 65);

sprite_change_offset("ball_formRip_1", 65, 65);
sprite_change_offset("ball_formRip_2", 65, 65);
sprite_change_offset("ball_formRip_3", 65, 65);
sprite_change_offset("ball_formRip_4", 65, 65);
sprite_change_offset("ball_formRip_1H", 65, 65);
sprite_change_offset("ball_formRip_2H", 65, 65);
sprite_change_offset("ball_formRip_3H", 65, 65);
sprite_change_offset("ball_formRip_4H", 65, 65);

sprite_change_offset("ball_formB_1", 36, 47);
sprite_change_offset("ball_formB_2", 36, 47);
sprite_change_offset("ball_formB_3", 36, 47);
sprite_change_offset("ball_formB_4", 36, 47);
sprite_change_offset("ball_explode", 5, 5);

sprite_change_offset("ball_formC_1", 53, 47);
sprite_change_offset("ball_formC_2", 53, 47);
sprite_change_offset("ball_formC_3", 53, 47);
sprite_change_offset("ball_formC_4", 53, 47);
sprite_change_offset("ball_formC_1T", 53, 47);
sprite_change_offset("ball_formC_2T", 53, 47);
sprite_change_offset("ball_formC_3T", 53, 47);
sprite_change_offset("ball_formC_4T", 53, 47);
sprite_change_offset("ball_formFS", 94, 86);
sprite_change_offset("ball_formC_End", 53, 47);
sprite_change_offset("ball_formC_mask", 53, 47);

sprite_change_offset("ball_shine_1", 16, 16);
sprite_change_offset("ball_shine_2", 20, 20);
sprite_change_offset("ball_shine_3", 24, 24);
sprite_change_offset("ball_shine_4", 28, 28);

sprite_change_offset("ballBounce", 48, 45);
sprite_change_offset("ballBlast", 112, 62);
sprite_change_offset("ballBlast_down", 112, 62);
sprite_change_offset("ballBlast_up", 112, 62);

sprite_change_offset("fspecial_shine", 45, 66);
sprite_change_offset("shine", 80, 94);
sprite_change_offset("bling", 112, 94);
sprite_change_offset("miniSpark", 48, 32);

sprite_change_offset("MPModeMid", 13, 17);
sprite_change_offset("Offscreen", 80, 62);
sprite_change_offset("Offscreen2", 80, 62);
sprite_change_offset("catooken_ind", 16, 32);

sprite_change_offset("brand", 0, 0);
sprite_change_offset("halloween_drop", 48, 94);
sprite_change_offset("funnyg8referencelmao", 48, 94);
sprite_change_offset("gen8background", 48, 62);
sprite_change_offset("introGenesis", 80, 158);

//COMP
sprite_change_offset("kirby_icon", 16, 32);
sprite_change_offset("kirby_icon_B", 16, 32);
sprite_change_offset("nspecial_kirby", 80, 94);
sprite_change_offset("nspecial_kirby_air", 80, 94);
sprite_change_offset("nspecial_kirby_fail", 80, 94);

sprite_change_offset("kart", 6, 6);
sprite_change_offset("regigigas_kart", 7, 7);

//V-SOUND
set_victory_bg(sprite_get("victory_bg"));

//BLUE EXCEPTION
try
{
    //---> ALT RELATED CODE
    alt_arr = 
        get_player_color(player) == 18 ? [sprite_get( "portrait2" ), sprite_get( "result_small2" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 19 ? [sprite_get( "portrait2" ), sprite_get( "result_small2" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 20 ? [sprite_get( "portrait2" ), sprite_get( "result_small2" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 21 ? [sprite_get( "portrait3" ), sprite_get( "result_small3" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 22 ? [sprite_get( "portrait2" ), sprite_get( "result_small2" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 23 ? [sprite_get( "portrait2" ), sprite_get( "result_small2" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 24 ? [sprite_get( "portrait2" ), sprite_get( "result_small2" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 25 ? [sprite_get( "portrait4" ), sprite_get( "result_small4" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 26 ? [sprite_get( "portrait4" ), sprite_get( "result_small4" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 27 ? [sprite_get( "portrait5" ), sprite_get( "result_small5" ), sound_get("fernet_victory"), 40, 100] :
        get_player_color(player) == 28 ? [sprite_get( "portrait5" ), sprite_get( "result_small5" ), sound_get("fernet_victory"), 40, 100] :
        [sprite_get( "portrait" ), sprite_get( "result_small" ), sound_get("OGG_Fernet_Victory_Theme"), 0, 136];
    
    set_victory_portrait( alt_arr[0] );
    set_victory_sidebar( alt_arr[1] );
    set_victory_theme( alt_arr[2] );
    
    fs_char_portrait_override = alt_arr[0];
    fs_portrait_x = alt_arr[3];
    fs_char_portrait_y = alt_arr[4];
    
    /*
    if (get_player_color(player) == alt_kiera)
    {
        set_victory_portrait( sprite_get( "portrait3" ));
        set_victory_sidebar( sprite_get( "result_small3" ));
        set_victory_theme(sound_get("fernet_victory"));
        
        fs_char_portrait_override = sprite_get( "portrait3" );
        fs_char_portrait_y = 100;
        fs_portrait_x = 40;
    }
    
    else if (get_player_color(player) >= alt_split)
    {
        set_victory_portrait( sprite_get( "portrait2" ));
        set_victory_sidebar( sprite_get( "result_small2" ));
        set_victory_theme(sound_get("fernet_victory"));
        
        fs_char_portrait_override = sprite_get( "portrait2" );
        fs_char_portrait_y = 100;
        fs_portrait_x = 40;
    }
    else
    {
        set_victory_portrait( sprite_get( "portrait" ));
        set_victory_sidebar( sprite_get( "result_small" ));
        set_victory_theme(sound_get("OGG_Fernet_Victory_Theme"));
        
        fs_char_portrait_y = 136;
    }
    */
}
catch (er)
{
    print_debug("caught")
    print_debug(er)
    set_victory_portrait( sprite_get( "portrait" ));
    set_victory_sidebar( sprite_get( "result_small" ));
    set_victory_theme(sound_get("OGG_Fernet_Victory_Theme"));
}

//set_victory_theme(sound_get("fernet_victory"));

//END