...
#define AttackUpdate()
{
	...
	if (state == PS_ATTACK_AIR || state == PS_ATTACK_GROUND)
	{
		switch (attack)
		{
			...
			case AT_FSPECIAL:
				if (has_hit_player) DoAttack(AT_USPECIAL);
				break;

			case AT_USPECIAL:
				if (has_hit_player) HoldTowardsStage();
				break;
		}
	}
}
	...
	switch (ai_state)
	{
		...
		case AS_ADVANTAGE:
			if (state != PS_ATTACK_AIR && state != PS_ATTACK_GROUND && can_attack && get_training_cpu_action() == CPU_FIGHT)
			{
				...
				{
					case TSK_NONE:
						if (xdist < 30 && ydist < 100 && ai_target.y < y /*&& get_player_damage(ai_target.player) >= 100*/)
						{
							DoAttack(AT_USPECIAL);
							HoldTowardsTarget();
						}
						else if (xdist > 200 && !move_cooldown[AT_DSPECIAL])
						{
							HoldTowardsTarget();
							DoAttack(AT_DSPECIAL);
						}
						else if (xdist > 100 && ydist < 10)
						{
							HoldTowardsTarget();
							DoAttack(AT_FSPECIAL);
						}
