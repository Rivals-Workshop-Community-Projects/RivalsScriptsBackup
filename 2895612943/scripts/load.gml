sprite_change_offset("sb_hurtbox", 32, 66);
sprite_change_offset("sb_crouchbox", 18, 66);

sprite_change_offset("idle", 32, 63);
sprite_change_offset("hurt", 32, 63);
sprite_change_offset("hfx_hurt_s",  70, 110);
sprite_change_offset("hfx_hurt_b", 70, 110);


sprite_change_offset("hurtground", 32, 63);

sprite_change_offset("spinhurt", 32, 63);

sprite_change_offset("bighurt", 32, 63);
sprite_change_offset("downhurt", 32, 63);
sprite_change_offset("uphurt", 32, 63);
sprite_change_offset("crouch", 32, 63);
sprite_change_offset("crouch_hurt", 64, 126);

sprite_change_offset("walk", 32, 63);
sprite_change_offset("walkturn", 32, 63);
sprite_change_offset("dash", 32, 63);
sprite_change_offset("dashstart", 32, 63);
sprite_change_offset("dashstop", 32, 63);
sprite_change_offset("dashturn", 32, 63);

sprite_change_offset("jumpstart", 32, 63);
sprite_change_offset("jump", 32, 55);
sprite_change_offset("doublejump", 32, 55);
sprite_change_offset("walljump", 32, 55);
sprite_change_offset("pratfall", 32, 55);
sprite_change_offset("land", 32, 63);
sprite_change_offset("landinglag", 32, 63);

sprite_change_offset("parry", 32, 63);
sprite_change_offset("roll_forward", 32, 63);
sprite_change_offset("roll_backward", 32, 63);
sprite_change_offset("airdodge", 32, 55);
sprite_change_offset("airdodge_forward", 32, 55);
sprite_change_offset("waveland", 32, 63);
sprite_change_offset("tech", 32, 63);

sprite_change_offset("jab", 32, 63, true);
sprite_change_offset("dattack", 32, 63, true);
sprite_change_offset("ftilt", 22, 63, true);
sprite_change_offset("dtilt", 32, 63, true);
sprite_change_offset("utilt", 32, 63, true);
sprite_change_offset("nair", 32, 55, true);
sprite_change_offset("fair", 32, 55, true);
sprite_change_offset("bair", 32, 55, true);
sprite_change_offset("uair", 32, 55, true);
sprite_change_offset("dair", 32, 55, true);
sprite_change_offset("fstrong", 32, 63, true);
sprite_change_offset("ustrong", 32, 63, true);
sprite_change_offset("dstrong", 32, 63, true);
sprite_change_offset("nspecial", 32, 63, true);
sprite_change_offset("fspecial", 32, 63, true);
sprite_change_offset("uspecial", 32, 63, true);
sprite_change_offset("dspecialold", 32, 63, true);
sprite_change_offset("dspecial", 32, 63, true);
sprite_change_offset("taunt", 32, 63);
sprite_change_offset("dspecproj", 64, 90);
sprite_change_offset("b_dspecproj", 64, 90);

sprite_change_offset("dspecpose", 64, 126);
sprite_change_offset("bite_dspecpose", 64, 126);
sprite_change_offset("ohno", 32, 63);
sprite_change_offset("bite_ohno", 32, 63);
sprite_change_offset("dspec_mask", 64, 126);

sprite_change_offset("disc", 14, 14);

sprite_change_offset("vfx_scratch", 128, 128);
sprite_change_offset("vfx_bite", 128, 128);

sprite_change_offset("plat", 16, 10, true);
sprite_change_offset("gravcancel", 32, 63);

sprite_change_offset("stronghit", 300, 540, true);

sprite_change_offset("bite_hurt", 32, 63);
sprite_change_offset("bite_bighurt", 32, 63);
sprite_change_offset("bite_uphurt", 32, 63);

sprite_change_offset("arrow", 3, 2);
sprite_change_offset("arrow_white", 4, 3);
sprite_change_offset("offscreen", 16, 18);
sprite_change_offset("offscreenb", 16, 18);
sprite_change_offset("hud_offscreen_arrow", 16, 16);

sprite_change_offset("hud", 0, -2);
sprite_change_offset("hudb", 0, -2);

user_event(15);