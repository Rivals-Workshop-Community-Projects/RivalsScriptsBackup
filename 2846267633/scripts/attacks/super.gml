set_attack_value(AT_EXTRA_1, AG_CATEGORY, 2);
set_attack_value(AT_EXTRA_1, AG_SPRITE, sprite_get("super"));
set_attack_value(AT_EXTRA_1, AG_NUM_WINDOWS, 6);
set_attack_value(AT_EXTRA_1, AG_STRONG_CHARGE_WINDOW, 1);
set_attack_value(AT_EXTRA_1, AG_HAS_LANDING_LAG, 0);
set_attack_value(AT_EXTRA_1, AG_OFF_LEDGE, 1);
set_attack_value(AT_EXTRA_1, AG_USES_CUSTOM_GRAVITY, 1);
set_attack_value(AT_EXTRA_1, AG_AIR_SPRITE, sprite_get("super"));
set_attack_value(AT_EXTRA_1, AG_HURTBOX_SPRITE, sprite_get("super_hurt"));

set_window_value(AT_EXTRA_1, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_LENGTH, 18);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_ANIM_FRAME_START, 0);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_VSPEED, 0);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_SFX, asset_get("sfx_rag_axe_swing"));
set_window_value(AT_EXTRA_1, 1, AG_WINDOW_SFX_FRAME, 0);

set_window_value(AT_EXTRA_1, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_LENGTH, 11);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_ANIM_FRAME_START, 3);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 2, AG_WINDOW_HSPEED, 20);

set_window_value(AT_EXTRA_1, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_LENGTH, 18);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_ANIM_FRAME_START, 4);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 3, AG_WINDOW_HSPEED, 0);

set_window_value(AT_EXTRA_1, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_LENGTH, 8);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_ANIM_FRAME_START, 8);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_INVINCIBILITY, 1);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_EXTRA_1, 4, AG_WINDOW_HSPEED, 0);

set_window_value(AT_EXTRA_1, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 5, AG_WINDOW_LENGTH, 9);
set_window_value(AT_EXTRA_1, 5, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_EXTRA_1, 5, AG_WINDOW_ANIM_FRAME_START, 10);
set_window_value(AT_EXTRA_1, 5, AG_WINDOW_INVINCIBILITY, 1);

set_window_value(AT_EXTRA_1, 6, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_LENGTH, 18);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_ANIM_FRAMES, 6);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_ANIM_FRAME_START, 12);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_CUSTOM_GRAVITY, 0.8);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_CUSTOM_AIR_FRICTION, 0.1);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_CUSTOM_GROUND_FRICTION, 0.2);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_HSPEED, -8);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_HSPEED_TYPE, 2);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_VSPEED, -12);
set_window_value(AT_EXTRA_1, 6, AG_WINDOW_VSPEED_TYPE, 2);

set_window_value(AT_EXTRA_1, 7, AG_WINDOW_TYPE, 1);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_LENGTH, 40);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_ANIM_FRAME_START, 18);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_HAS_CUSTOM_FRICTION, 1);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_CUSTOM_GROUND_FRICTION, 0.4);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_CUSTOM_AIR_FRICTION, 0.35);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_SFX, asset_get("sfx_land_heavy"));
set_window_value(AT_EXTRA_1, 7, AG_WINDOW_SFX_FRAME, 0);

set_num_hitboxes(AT_EXTRA_1, 9);

set_hitbox_value(AT_EXTRA_1, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_EXTRA_1, 1, HG_WINDOW, 2);
set_hitbox_value(AT_EXTRA_1, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_LIFETIME, 8);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITBOX_X, 15);
set_hitbox_value(AT_EXTRA_1, 1, HG_HITBOX_Y, -10);
set_hitbox_value(AT_EXTRA_1, 1, HG_WIDTH, 30);
set_hitbox_value(AT_EXTRA_1, 1, HG_HEIGHT, 25);
set_hitbox_value(AT_EXTRA_1, 1, HG_PRIORITY, 10);
set_hitbox_value(AT_EXTRA_1, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_EXTRA_1, 1, HG_BASE_KNOCKBACK, 4);
set_hitbox_value(AT_EXTRA_1, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_EXTRA_1, 1, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_EXTRA_1, 1, HG_HIT_SFX, asset_get("sfx_crunch_water"));
set_hitbox_value(AT_EXTRA_1, 1, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_EXTRA_1, 1, HG_ANGLE, 60);

set_hitbox_value(AT_EXTRA_1, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_EXTRA_1, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 2, HG_WINDOW, 5);
set_hitbox_value(AT_EXTRA_1, 2, HG_WINDOW_CREATION_FRAME, 1);
set_hitbox_value(AT_EXTRA_1, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_EXTRA_1, 2, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_EXTRA_1, 2, HG_HITBOX_X, 40);
set_hitbox_value(AT_EXTRA_1, 2, HG_HITBOX_Y, -35);
set_hitbox_value(AT_EXTRA_1, 2, HG_WIDTH, 85);
set_hitbox_value(AT_EXTRA_1, 2, HG_HEIGHT, 85);
set_hitbox_value(AT_EXTRA_1, 2, HG_PRIORITY, 10);
set_hitbox_value(AT_EXTRA_1, 2, HG_DAMAGE, 21);
set_hitbox_value(AT_EXTRA_1, 2, HG_ANGLE, 70);
set_hitbox_value(AT_EXTRA_1, 2, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_EXTRA_1, 2, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_EXTRA_1, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_EXTRA_1, 2, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_EXTRA_1, 2, HG_VISUAL_EFFECT, 4);
set_hitbox_value(AT_EXTRA_1, 2, HG_VISUAL_EFFECT_X_OFFSET, -10);
set_hitbox_value(AT_EXTRA_1, 2, HG_VISUAL_EFFECT_Y_OFFSET, 14);
set_hitbox_value(AT_EXTRA_1, 2, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));

set_hitbox_value(AT_EXTRA_1, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_EXTRA_1, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 3, HG_WINDOW, 5);
set_hitbox_value(AT_EXTRA_1, 3, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_EXTRA_1, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_EXTRA_1, 3, HG_HITBOX_GROUP, 2);
set_hitbox_value(AT_EXTRA_1, 3, HG_HITBOX_X, 40);
set_hitbox_value(AT_EXTRA_1, 3, HG_HITBOX_Y, -35);
set_hitbox_value(AT_EXTRA_1, 3, HG_WIDTH, 85);
set_hitbox_value(AT_EXTRA_1, 3, HG_HEIGHT, 85);
set_hitbox_value(AT_EXTRA_1, 3, HG_PRIORITY, 10);
set_hitbox_value(AT_EXTRA_1, 3, HG_DAMAGE, 21);
set_hitbox_value(AT_EXTRA_1, 3, HG_ANGLE, 70);
set_hitbox_value(AT_EXTRA_1, 3, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_EXTRA_1, 3, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_EXTRA_1, 3, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_EXTRA_1, 3, HG_BASE_HITPAUSE, 9);
set_hitbox_value(AT_EXTRA_1, 3, HG_VISUAL_EFFECT, 4);
set_hitbox_value(AT_EXTRA_1, 3, HG_VISUAL_EFFECT_X_OFFSET, -5);
set_hitbox_value(AT_EXTRA_1, 3, HG_VISUAL_EFFECT_Y_OFFSET, 20);
set_hitbox_value(AT_EXTRA_1, 3, HG_HIT_SFX, asset_get("sfx_blow_heavy2"));

set_hitbox_value(AT_EXTRA_1, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_EXTRA_1, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 4, HG_WINDOW, 5);
set_hitbox_value(AT_EXTRA_1, 4, HG_WINDOW_CREATION_FRAME, 5);
set_hitbox_value(AT_EXTRA_1, 4, HG_LIFETIME, 3);
set_hitbox_value(AT_EXTRA_1, 4, HG_HITBOX_GROUP, 3);
set_hitbox_value(AT_EXTRA_1, 4, HG_HITBOX_X, 40);
set_hitbox_value(AT_EXTRA_1, 4, HG_HITBOX_Y, -35);
set_hitbox_value(AT_EXTRA_1, 4, HG_WIDTH, 85);
set_hitbox_value(AT_EXTRA_1, 4, HG_HEIGHT, 85);
set_hitbox_value(AT_EXTRA_1, 4, HG_PRIORITY, 10);
set_hitbox_value(AT_EXTRA_1, 4, HG_DAMAGE, 24);
set_hitbox_value(AT_EXTRA_1, 4, HG_ANGLE, 50);
set_hitbox_value(AT_EXTRA_1, 4, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_EXTRA_1, 4, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_EXTRA_1, 4, HG_KNOCKBACK_SCALING, .85);
set_hitbox_value(AT_EXTRA_1, 4, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_EXTRA_1, 4, HG_HITPAUSE_SCALING, 0.45);
set_hitbox_value(AT_EXTRA_1, 4, HG_VISUAL_EFFECT, 148);
set_hitbox_value(AT_EXTRA_1, 4, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_EXTRA_1, 4, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_EXTRA_1, 4, HG_HIT_SFX, asset_get("sfx_blow_heavy1"));

set_hitbox_value(AT_EXTRA_1, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_EXTRA_1, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 5, HG_WINDOW, 7);
set_hitbox_value(AT_EXTRA_1, 5, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_1, 5, HG_LIFETIME, 7);
set_hitbox_value(AT_EXTRA_1, 5, HG_SHAPE, 2);
set_hitbox_value(AT_EXTRA_1, 5, HG_HITBOX_X, -15);
set_hitbox_value(AT_EXTRA_1, 5, HG_HITBOX_Y, -5);
set_hitbox_value(AT_EXTRA_1, 5, HG_WIDTH, 30);
set_hitbox_value(AT_EXTRA_1, 5, HG_HEIGHT, 15);
set_hitbox_value(AT_EXTRA_1, 5, HG_PRIORITY, 1);
set_hitbox_value(AT_EXTRA_1, 5, HG_DAMAGE, 5);
set_hitbox_value(AT_EXTRA_1, 5, HG_ANGLE, 110);
set_hitbox_value(AT_EXTRA_1, 5, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_EXTRA_1, 5, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_EXTRA_1, 5, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_EXTRA_1, 5, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_EXTRA_1, 5, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_EXTRA_1, 5, HG_EXTRA_HITPAUSE, 11);
set_hitbox_value(AT_EXTRA_1, 5, HG_VISUAL_EFFECT, 303);
set_hitbox_value(AT_EXTRA_1, 5, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_EXTRA_1, 5, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_EXTRA_1, 5, HG_HIT_SFX, asset_get("sfx_blow_weak1"));

set_hitbox_value(AT_EXTRA_1, 6, HG_PARENT_HITBOX, 6);
set_hitbox_value(AT_EXTRA_1, 6, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 6, HG_WINDOW, 6);
set_hitbox_value(AT_EXTRA_1, 6, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_EXTRA_1, 6, HG_LIFETIME, 7);
set_hitbox_value(AT_EXTRA_1, 6, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_1, 6, HG_HITBOX_X, 0);
set_hitbox_value(AT_EXTRA_1, 6, HG_HITBOX_Y, -28);
set_hitbox_value(AT_EXTRA_1, 6, HG_WIDTH, 45);
set_hitbox_value(AT_EXTRA_1, 6, HG_HEIGHT, 45);
set_hitbox_value(AT_EXTRA_1, 6, HG_PRIORITY, 1);
set_hitbox_value(AT_EXTRA_1, 6, HG_DAMAGE, 3);
set_hitbox_value(AT_EXTRA_1, 6, HG_ANGLE, 50);
set_hitbox_value(AT_EXTRA_1, 6, HG_ANGLE_FLIPPER, 5);
set_hitbox_value(AT_EXTRA_1, 6, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_EXTRA_1, 6, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_EXTRA_1, 6, HG_BASE_HITPAUSE, 2);
set_hitbox_value(AT_EXTRA_1, 6, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_EXTRA_1, 6, HG_VISUAL_EFFECT, 302);
set_hitbox_value(AT_EXTRA_1, 6, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_EXTRA_1, 6, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_EXTRA_1, 6, HG_HIT_SFX, asset_get("sfx_blow_weak1"));

set_hitbox_value(AT_EXTRA_1, 7, HG_PARENT_HITBOX, 6);
set_hitbox_value(AT_EXTRA_1, 7, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 7, HG_WINDOW, 6);
set_hitbox_value(AT_EXTRA_1, 7, HG_WINDOW_CREATION_FRAME, 5);
set_hitbox_value(AT_EXTRA_1, 7, HG_LIFETIME, 7);
set_hitbox_value(AT_EXTRA_1, 7, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_1, 7, HG_HITBOX_X, 0);
set_hitbox_value(AT_EXTRA_1, 7, HG_HITBOX_Y, -28);

set_hitbox_value(AT_EXTRA_1, 8, HG_PARENT_HITBOX, 6);
set_hitbox_value(AT_EXTRA_1, 8, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_EXTRA_1, 8, HG_WINDOW, 6);
set_hitbox_value(AT_EXTRA_1, 8, HG_WINDOW_CREATION_FRAME, 10);
set_hitbox_value(AT_EXTRA_1, 8, HG_LIFETIME, 7);
set_hitbox_value(AT_EXTRA_1, 8, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_EXTRA_1, 8, HG_HITBOX_X, 0);
set_hitbox_value(AT_EXTRA_1, 8, HG_HITBOX_Y, -28);
