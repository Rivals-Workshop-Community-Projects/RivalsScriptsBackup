set_attack_value(AT_DAIR, AG_CATEGORY, 1);
set_attack_value(AT_DAIR, AG_SPRITE, sprite_get("dair"));
set_attack_value(AT_DAIR, AG_NUM_WINDOWS, 10);
set_attack_value(AT_DAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_DAIR, AG_LANDING_LAG, 10);
set_attack_value(AT_DAIR, AG_HURTBOX_SPRITE, sprite_get("dair_hurt"));

set_window_value(AT_DAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_LENGTH, 14);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED, -8);
set_window_value(AT_DAIR, 1, AG_WINDOW_VSPEED_TYPE, 2);
set_window_value(AT_DAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy2"));
set_window_value(AT_DAIR, 1, AG_WINDOW_SFX_FRAME, 1);
set_window_value(AT_DAIR, 1, AG_WINDOW_ANIM_FRAME_START, 1);

set_window_value(AT_DAIR, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_SFX, asset_get("sfx_swipe_medium1"));
set_window_value(AT_DAIR, 2, AG_WINDOW_SFX_FRAME, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 2, AG_WINDOW_ANIM_FRAME_START, 4);

set_window_value(AT_DAIR, 3, AG_WINDOW_TYPE, 9);
set_window_value(AT_DAIR, 3, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DAIR, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DAIR, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 3, AG_WINDOW_ANIM_FRAME_START, 4);


set_window_value(AT_DAIR, 4, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 4, AG_WINDOW_LENGTH, 6);
set_window_value(AT_DAIR, 4, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DAIR, 4, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DAIR, 4, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 4, AG_WINDOW_ANIM_FRAME_START, 5);

set_window_value(AT_DAIR, 5, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 5, AG_WINDOW_LENGTH, 1);
set_window_value(AT_DAIR, 5, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DAIR, 5, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DAIR, 5, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 5, AG_WINDOW_ANIM_FRAME_START, 5);

set_window_value(AT_DAIR, 6, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 6, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DAIR, 6, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DAIR, 6, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DAIR, 6, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 6, AG_WINDOW_ANIM_FRAME_START, 5);

set_window_value(AT_DAIR, 7, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 7, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DAIR, 7, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DAIR, 7, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DAIR, 7, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 7, AG_WINDOW_ANIM_FRAME_START, 5);



set_window_value(AT_DAIR, 8, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 8, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DAIR, 8, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DAIR, 8, AG_WINDOW_ANIM_FRAME_START, 6);
set_window_value(AT_DAIR, 8, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 8, AG_WINDOW_HAS_WHIFFLAG, 1);
set_window_value(AT_DAIR, 8, AG_WINDOW_SFX, asset_get("sfx_jumpground"));


set_window_value(AT_DAIR, 9, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 9, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DAIR, 9, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_DAIR, 9, AG_WINDOW_ANIM_FRAME_START, 9);
set_window_value(AT_DAIR, 9, AG_WINDOW_VSPEED, -8);
set_window_value(AT_DAIR, 9, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DAIR, 9, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 9, AG_WINDOW_SFX, asset_get("sfx_boss_shine"));
set_window_value(AT_DAIR, 9, AG_WINDOW_SFX_FRAME, -1);

set_window_value(AT_DAIR, 10, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 10, AG_WINDOW_LENGTH, 10);
set_window_value(AT_DAIR, 10, AG_WINDOW_VSPEED, -8);
set_window_value(AT_DAIR, 10, AG_WINDOW_VSPEED_TYPE, 1);
set_window_value(AT_DAIR, 10, AG_WINDOW_ANIM_FRAMES, 2);
set_window_value(AT_DAIR, 10, AG_WINDOW_ANIM_FRAME_START, 4);

set_window_value(AT_DAIR, 11, AG_WINDOW_TYPE, 1);
set_window_value(AT_DAIR, 11, AG_WINDOW_LENGTH, 20);
set_window_value(AT_DAIR, 11, AG_WINDOW_ANIM_FRAMES, 3);
set_window_value(AT_DAIR, 11, AG_WINDOW_VSPEED, -8);
set_window_value(AT_DAIR, 11, AG_WINDOW_VSPEED_TYPE, 2);
set_window_value(AT_DAIR, 11, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DAIR, 11, AG_WINDOW_SFX, asset_get("sfx_swipe_heavy2"));
set_window_value(AT_DAIR, 11, AG_WINDOW_SFX_FRAME, 1);
set_window_value(AT_DAIR, 11, AG_WINDOW_ANIM_FRAME_START, 1);


set_num_hitboxes(AT_DAIR, 2);

set_hitbox_value(AT_DAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DAIR, 1, HG_LIFETIME, 4);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_X, 21);
set_hitbox_value(AT_DAIR, 1, HG_HITBOX_Y, -22);
set_hitbox_value(AT_DAIR, 1, HG_WIDTH, 45);
set_hitbox_value(AT_DAIR, 1, HG_HEIGHT, 76);
set_hitbox_value(AT_DAIR, 1, HG_PRIORITY, 9);
set_hitbox_value(AT_DAIR, 1, HG_DAMAGE, 12);
set_hitbox_value(AT_DAIR, 1, HG_ANGLE, 272);
set_hitbox_value(AT_DAIR, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DAIR, 1, HG_KNOCKBACK_SCALING, 0.6);
set_hitbox_value(AT_DAIR, 1, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_DAIR, 1, HG_HITPAUSE_SCALING, 1);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT_X_OFFSET, 32);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_DAIR, 1, HG_VISUAL_EFFECT, 304);
set_hitbox_value(AT_DAIR, 1, HG_HIT_SFX, sound_get("RI2"));
set_hitbox_value(AT_DAIR, 1, HG_SHAPE, 2);

set_hitbox_value(AT_DAIR, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_DAIR, 2, HG_WINDOW, 3);
set_hitbox_value(AT_DAIR, 2, HG_LIFETIME, 17);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_X, 21);
set_hitbox_value(AT_DAIR, 2, HG_HITBOX_Y, -22);
set_hitbox_value(AT_DAIR, 2, HG_WIDTH, 45);
set_hitbox_value(AT_DAIR, 2, HG_HEIGHT, 76);
set_hitbox_value(AT_DAIR, 2, HG_PRIORITY, 6);
set_hitbox_value(AT_DAIR, 2, HG_DAMAGE, 7);
set_hitbox_value(AT_DAIR, 2, HG_SHAPE, 2);
set_hitbox_value(AT_DAIR, 2, HG_ANGLE, 65);
set_hitbox_value(AT_DAIR, 2, HG_BASE_KNOCKBACK, 10);
set_hitbox_value(AT_DAIR, 2, HG_KNOCKBACK_SCALING, .05);
set_hitbox_value(AT_DAIR, 2, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_DAIR, 2, HG_HITPAUSE_SCALING, 0.1);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT_X_OFFSET, 32);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_DAIR, 2, HG_VISUAL_EFFECT, 305);
set_hitbox_value(AT_DAIR, 2, HG_HIT_SFX, sound_get("slice"));
set_hitbox_value(AT_DAIR, 2, HG_HITSTUN_MULTIPLIER, 1);