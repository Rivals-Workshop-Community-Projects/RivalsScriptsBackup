/*
//Old data left to preserve lore, new code uses colors.gml generator.
// Hair
set_color_profile_slot( 0, 0, 76, 50, 39 );
set_color_profile_slot_range( 0, 1, 1, 1 );

// Hair2
set_color_profile_slot( 0, 1, 50, 38, 34 );
set_color_profile_slot_range( 1, 1, 1, 1 );

// Skin
set_color_profile_slot( 0, 2, 253, 233, 185 );
set_color_profile_slot_range( 2, 18, 24, 22 );

// Energy
set_color_profile_slot( 0, 3, 249, 202, 53 );
set_color_profile_slot_range( 3, 1, 1, 1 );

// Energy2
set_color_profile_slot( 0, 4, 225, 160, 76 );
set_color_profile_slot_range( 4, 1, 1, 1 );

// Coat
set_color_profile_slot( 0, 5, 39, 70, 94 );
set_color_profile_slot_range( 5, 7, 9, 26 );

// Gun
set_color_profile_slot( 0, 6, 41, 32, 38 );
set_color_profile_slot_range( 6, 9, 6, 10 );

// Pants
set_color_profile_slot( 0, 7, 33, 33, 38 );
set_color_profile_slot_range( 7, 1, 1, 1 );


// ALTERNATE COLORS
set_num_palettes( 16 );

// Hime
// You know, I used to have one of the headbands like what she wore.
// By now, it's too burnt to recognize.
set_color_profile_slot( 1, 0, 109, 140, 161 ); //Hair
set_color_profile_slot( 1, 1, 74, 95, 110 ); //Hair2
set_color_profile_slot( 1, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 1, 3, 116, 184, 229 ); //Energy
set_color_profile_slot( 1, 4, 81, 153, 209 ); //Energy2
set_color_profile_slot( 1, 5, 110, 74, 61 ); //Coat
set_color_profile_slot( 1, 6, 186, 121, 78 ); //Gun
set_color_profile_slot( 1, 7, 22, 28, 61 ); //Pants

// Sprite
set_color_profile_slot( 2, 0, 46, 42, 42 ); //Hair
set_color_profile_slot( 2, 1, 30, 27, 26 ); //Hair2
set_color_profile_slot( 2, 2, 155, 125, 95 ); //Skin
set_color_profile_slot( 2, 3, 245, 243, 248 ); //Energy
set_color_profile_slot( 2, 4, 198, 194, 204 ); //Energy2
set_color_profile_slot( 2, 5, 186, 8, 47 ); //Coat
set_color_profile_slot( 2, 6, 90, 89, 88 ); //Gun
set_color_profile_slot( 2, 7, 39, 38, 37 ); //Pants

// CC
//set_color_profile_slot( 3, 0, 220, 233, 144 ); //Hair
//set_color_profile_slot( 3, 1, 133, 156, 77 ); //Hair2
//set_color_profile_slot( 3, 2, 255, 228, 209 ); //Skin
//set_color_profile_slot( 3, 3, 2, 227, 2 ); //Energy
//set_color_profile_slot( 3, 4, 41, 156, 41 ); //Energy2
//set_color_profile_slot( 3, 5, 226, 226, 226 ); //Coat
//set_color_profile_slot( 3, 6, 93, 88, 109 ); //Gun
//set_color_profile_slot( 3, 7, 194, 194, 194 ); //Pants

// gummy
set_color_profile_slot( 3, 0, 179, 224, 101 ); //Hair
set_color_profile_slot( 3, 1, 130, 145, 48 ); //Hair2
set_color_profile_slot( 3, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 3, 3, 200, 255, 0 ); //Energy
set_color_profile_slot( 3, 4, 176, 217, 74 ); //Energy2
set_color_profile_slot( 3, 5, 240, 155, 78 ); //Coat
set_color_profile_slot( 3, 6, 52, 74, 53 ); //Gun
set_color_profile_slot( 3, 7, 33, 33, 38 ); //Pants

// GoldenGun
//set_color_profile_slot( 4, 0, 249, 229, 189 ); //Hair
//set_color_profile_slot( 4, 1, 196, 174, 151 ); //Hair2
//set_color_profile_slot( 4, 2, 255, 230, 210 ); //Skin
//set_color_profile_slot( 4, 3, 197, 45, 70 ); //Energy
//set_color_profile_slot( 4, 4, 156, 27, 49 ); //Energy2
//set_color_profile_slot( 4, 5, 92, 92, 92 ); //Coat
//set_color_profile_slot( 4, 6, 213, 174, 55 ); //Gun
//set_color_profile_slot( 4, 7, 66, 53, 47 ); //Pants

// Santiana
set_color_profile_slot( 4, 0, 51, 51, 51 ); //Hair
set_color_profile_slot( 4, 1, 28, 28, 28 ); //Hair2
set_color_profile_slot( 4, 2, 207, 171, 155 ); //Skin
set_color_profile_slot( 4, 3, 255, 243, 69 ); //Energy
set_color_profile_slot( 4, 4, 204, 191, 22 ); //Energy2
set_color_profile_slot( 4, 5, 130, 101, 69 ); //Coat
set_color_profile_slot( 4, 6, 145, 145, 145 ); //Gun
set_color_profile_slot( 4, 7, 33, 33, 38 ); //Pants

// Misato
// I wouldn't want any of the others to do what I do.
// They all still have hope that *she's* going to be coming to save them.
// Why can't they just accept that she's abandoned us.
set_color_profile_slot( 5, 0, 81, 65, 107 ); //Hair
set_color_profile_slot( 5, 1, 57, 43, 87 ); //Hair2
set_color_profile_slot( 5, 2, 255, 218, 192 ); //Skin
set_color_profile_slot( 5, 3, 224, 95, 49 ); //Energy
set_color_profile_slot( 5, 4, 206, 68, 22 ); //Energy2
set_color_profile_slot( 5, 5, 138, 30, 30 ); //Coat
set_color_profile_slot( 5, 6, 138, 150, 150 ); //Gun
set_color_profile_slot( 5, 7, 57, 37, 37 ); //Pants

// Leona
//set_color_profile_slot( 6, 0, 108, 195, 239 ); //Hair
//set_color_profile_slot( 6, 1, 74, 141, 203 ); //Hair2
//set_color_profile_slot( 6, 2, 253, 233, 185 ); //Skin
//set_color_profile_slot( 6, 3, 237, 250, 128 ); //Energy
//set_color_profile_slot( 6, 4, 241, 221, 58 ); //Energy2
//set_color_profile_slot( 6, 5, 98, 174, 72 ); //Coat
//set_color_profile_slot( 6, 6, 41, 32, 38 ); //Gun
//set_color_profile_slot( 6, 7, 25, 82, 38 ); //Pants

// rox
set_color_profile_slot( 6, 0, 88, 111, 127 ); //Hair
set_color_profile_slot( 6, 1, 49, 66, 74 ); //Hair2
set_color_profile_slot( 6, 2, 255, 230, 210 ); //Skin
set_color_profile_slot( 6, 3, 224, 93, 187 ); //Energy
set_color_profile_slot( 6, 4, 226, 211, 232 ); //Energy2
set_color_profile_slot( 6, 5, 65, 72, 80 ); //Coat
set_color_profile_slot( 6, 6, 169, 169, 169 ); //Gun
set_color_profile_slot( 6, 7, 33, 33, 38 ); //Pants


// Ольга
// 'Maybe because it's because Rokesha is a human, but... You know, she actually has no Talent.'
// 'They like to call it intelligence. Careful planning. Though, really, all of those are just pretty ways of saying...'
// 'She lucks out.'
set_color_profile_slot( 7, 0, 249, 237, 194 ); //Hair
set_color_profile_slot( 7, 1, 199, 190, 155 ); //Hair2
set_color_profile_slot( 7, 2, 255, 230, 212 ); //Skin
set_color_profile_slot( 7, 3, 189, 184, 122 ); //Energy
set_color_profile_slot( 7, 4, 127, 130, 80 ); //Energy2
set_color_profile_slot( 7, 5, 231, 228, 224 ); //Coat
set_color_profile_slot( 7, 6, 74, 74, 74 ); //Gun
set_color_profile_slot( 7, 7, 155, 178, 179 ); //Pants

// Анна
// The loser looks in the eyes of the girl and
// The room is caving in, 'It's okay, It's okay,' I say
// And just like that, she turns into a monster
// There's nobody to show up and save the day.
set_color_profile_slot( 8, 0, 178, 136, 112 ); //Hair
set_color_profile_slot( 8, 1, 125, 105, 91 ); //Hair2
set_color_profile_slot( 8, 2, 243, 222, 210 ); //Skin
set_color_profile_slot( 8, 3, 90, 162, 181 ); //Energy
set_color_profile_slot( 8, 4, 152, 58, 46 ); //Energy2
set_color_profile_slot( 8, 5, 49, 50, 49 ); //Coat
set_color_profile_slot( 8, 6, 74, 77, 74 ); //Gun
set_color_profile_slot( 8, 7, 85, 92, 96 ); //Pants

// persona 5 royal herself
//set_color_profile_slot( 9, 0, 161, 38, 43 ); //Hair
//set_color_profile_slot( 9, 1, 118, 21, 30 ); //Hair2
//set_color_profile_slot( 9, 2, 251, 230, 213 ); //Skin
//set_color_profile_slot( 9, 3, 252, 44, 44 ); //Energy
//set_color_profile_slot( 9, 4, 176, 21, 21 ); //Energy2
//set_color_profile_slot( 9, 5, 95, 89, 87 ); //Coat
//set_color_profile_slot( 9, 6, 201, 201, 200 ); //Gun
//set_color_profile_slot( 9, 7, 32, 32, 32 ); //Pants

// maki roll
set_color_profile_slot( 9, 0, 64, 64, 53 ); //Hair
set_color_profile_slot( 9, 1, 25, 30, 26 ); //Hair2
set_color_profile_slot( 9, 2, 255, 218, 192 ); //Skin
set_color_profile_slot( 9, 3, 252, 44, 44 ); //Energy
set_color_profile_slot( 9, 4, 176, 21, 21 ); //Energy2
set_color_profile_slot( 9, 5, 127, 58, 54 ); //Coat
set_color_profile_slot( 9, 6, 201, 201, 200 ); //Gun
set_color_profile_slot( 9, 7, 33, 33, 38 ); //Pants

// zetta?
// If you call her Dai-san one more time, your white fur is going to get singed black.
// That's my final warning.
set_color_profile_slot( 10, 0, 245, 245, 245 ); //Hair
set_color_profile_slot( 10, 1, 217, 210, 224 ); //Hair2
set_color_profile_slot( 10, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 10, 3, 83, 224, 35 ); //Energy
set_color_profile_slot( 10, 4, 61, 163, 25 ); //Energy2
set_color_profile_slot( 10, 5, 66, 91, 26 ); //Coat
set_color_profile_slot( 10, 6, 37, 48, 41 ); //Gun
set_color_profile_slot( 10, 7, 32, 32, 32 ); //Pants

// Reisen
// MAD RED EYES AND A FULL INVERTED MOON
// ONE WHICH WATCHES OVER AND ONE WHICH PIERCES THROUGH
set_color_profile_slot( 11, 0, 220, 168, 251 ); //Hair
set_color_profile_slot( 11, 1, 149, 102, 222 ); //Hair2
set_color_profile_slot( 11, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 11, 3, 237, 36, 72 ); //Energy
set_color_profile_slot( 11, 4, 194, 30, 59 ); //Energy2
set_color_profile_slot( 11, 5, 255, 160, 156 ); //Coat
set_color_profile_slot( 11, 6, 248, 197, 104 ); //Gun
set_color_profile_slot( 11, 7, 56, 16, 21 ); //Pants

// Liz
set_color_profile_slot( 12, 0, 255, 72, 135 ); //Hair
set_color_profile_slot( 12, 1, 155, 43, 162 ); //Hair2
set_color_profile_slot( 12, 2, 255, 203, 160 ); //Skin
set_color_profile_slot( 12, 3, 199, 237, 255 ); //Energy
set_color_profile_slot( 12, 4, 56, 186, 255 ); //Energy2
set_color_profile_slot( 12, 5, 38, 27, 110 ); //Coat
set_color_profile_slot( 12, 6, 254, 227, 95 ); //Gun
set_color_profile_slot( 12, 7, 255, 72, 135 ); //Pants

// supersonico
set_color_profile_slot( 13, 0, 225, 237, 255 ); //Hair
set_color_profile_slot( 13, 1, 176, 192, 240 ); //Hair2
set_color_profile_slot( 13, 2, 240, 201, 161 ); //Skin
set_color_profile_slot( 13, 3, 164, 225, 233 ); //Energy
set_color_profile_slot( 13, 4, 78, 203, 229 ); //Energy2
set_color_profile_slot( 13, 5, 65, 71, 120 ); //Coat
set_color_profile_slot( 13, 6, 153, 177, 217 ); //Gun
set_color_profile_slot( 13, 7, 50, 49, 81 ); //Pants

// Abyss
// They've been doing experiments recently, finding more volatile powers than plasma to start working with.
// I feel like I'm about to throw up.
set_color_profile_slot( 14, 0, 93, 54, 127 ); //Hair
set_color_profile_slot( 14, 1, 64, 37, 87 ); //Hair2
set_color_profile_slot( 14, 2, 207, 185, 247 ); //Skin
set_color_profile_slot( 14, 3, 183, 5, 227 ); //Energy
set_color_profile_slot( 14, 4, 140, 4, 173 ); //Energy2
set_color_profile_slot( 14, 5, 125, 39, 204 ); //Coat
set_color_profile_slot( 14, 6, 92, 50, 102 ); //Gun
set_color_profile_slot( 14, 7, 46, 18, 71 ); //Pants

//GUN SUPREME VICTORY
set_color_profile_slot( 15, 0, 93, 54, 127 ); //Hair
set_color_profile_slot( 15, 1, 64, 37, 87 ); //Hair2
set_color_profile_slot( 15, 2, 207, 185, 247 ); //Skin
set_color_profile_slot( 15, 3, 183, 5, 227 ); //Energy
set_color_profile_slot( 15, 4, 140, 4, 173 ); //Energy2
set_color_profile_slot( 15, 5, 125, 39, 204 ); //Coat
set_color_profile_slot( 15, 6, 92, 50, 102 ); //Gun
set_color_profile_slot( 15, 7, 46, 18, 71 ); //Pants

// Ольга and Анна are made by Ferry, please listen to PAFL
// Reisen's note is from Lyrica Live's edition of Lunatic Red Eyes, be sure to give it a listen 
*/
// DEFAULT COLOR

// Hair_light
set_color_profile_slot( 0, 0, 76, 50, 39 );
set_color_profile_slot_range( 0, 1, 1, 1 );

// Hair_dark
set_color_profile_slot( 0, 1, 50, 38, 34 );
set_color_profile_slot_range( 1, 1, 1, 1 );

// Skin
set_color_profile_slot( 0, 2, 253, 233, 185 );
set_color_profile_slot_range( 2, 18, 24, 22 );

// Energy_light
set_color_profile_slot( 0, 3, 249, 202, 53 );
set_color_profile_slot_range( 3, 1, 1, 1 );

// Energy_dark
set_color_profile_slot( 0, 4, 225, 160, 76 );
set_color_profile_slot_range( 4, 1, 1, 1 );

// Metal
set_color_profile_slot( 0, 5, 41, 32, 38 );
set_color_profile_slot_range( 5, 9, 6, 10 );

// Shirt
set_color_profile_slot( 0, 6, 39, 70, 94 );
set_color_profile_slot_range( 6, 7, 9, 9 );

// Pants
set_color_profile_slot( 0, 7, 33, 33, 38 );
set_color_profile_slot_range( 7, 1, 1, 1 );


// ALTERNATE COLORS
set_num_palettes( 24 );

// Hime
set_color_profile_slot( 1, 0, 109, 140, 161 ); //Hair_light
set_color_profile_slot( 1, 1, 74, 95, 110 ); //Hair_dark
set_color_profile_slot( 1, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 1, 3, 116, 184, 229 ); //Energy_light
set_color_profile_slot( 1, 4, 81, 153, 209 ); //Energy_dark
set_color_profile_slot( 1, 5, 186, 121, 78 ); //Metal
set_color_profile_slot( 1, 6, 110, 74, 61 ); //Shirt
set_color_profile_slot( 1, 7, 22, 28, 61 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Sprite
set_color_profile_slot( 2, 0, 46, 42, 42 ); //Hair_light
set_color_profile_slot( 2, 1, 30, 27, 26 ); //Hair_dark
set_color_profile_slot( 2, 2, 155, 125, 95 ); //Skin
set_color_profile_slot( 2, 3, 245, 243, 248 ); //Energy_light
set_color_profile_slot( 2, 4, 198, 194, 204 ); //Energy_dark
set_color_profile_slot( 2, 5, 90, 89, 88 ); //Metal
set_color_profile_slot( 2, 6, 186, 8, 47 ); //Shirt
set_color_profile_slot( 2, 7, 39, 38, 37 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Gummy
set_color_profile_slot( 3, 0, 179, 224, 101 ); //Hair_light
set_color_profile_slot( 3, 1, 130, 145, 48 ); //Hair_dark
set_color_profile_slot( 3, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 3, 3, 200, 255, 0 ); //Energy_light
set_color_profile_slot( 3, 4, 176, 217, 74 ); //Energy_dark
set_color_profile_slot( 3, 5, 52, 74, 53 ); //Metal
set_color_profile_slot( 3, 6, 240, 155, 78 ); //Shirt
set_color_profile_slot( 3, 7, 33, 33, 38 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Santiana
set_color_profile_slot( 4, 0, 51, 51, 51 ); //Hair_light
set_color_profile_slot( 4, 1, 28, 28, 28 ); //Hair_dark
set_color_profile_slot( 4, 2, 207, 171, 155 ); //Skin
set_color_profile_slot( 4, 3, 255, 243, 69 ); //Energy_light
set_color_profile_slot( 4, 4, 204, 191, 22 ); //Energy_dark
set_color_profile_slot( 4, 5, 145, 145, 145 ); //Metal
set_color_profile_slot( 4, 6, 130, 101, 69 ); //Shirt
set_color_profile_slot( 4, 7, 33, 33, 38 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Misato
set_color_profile_slot( 5, 0, 81, 65, 107 ); //Hair_light
set_color_profile_slot( 5, 1, 57, 43, 87 ); //Hair_dark
set_color_profile_slot( 5, 2, 255, 218, 192 ); //Skin
set_color_profile_slot( 5, 3, 224, 95, 49 ); //Energy_light
set_color_profile_slot( 5, 4, 206, 68, 22 ); //Energy_dark
set_color_profile_slot( 5, 5, 138, 150, 150 ); //Metal
set_color_profile_slot( 5, 6, 138, 30, 30 ); //Shirt
set_color_profile_slot( 5, 7, 57, 37, 37 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// rox
set_color_profile_slot( 6, 0, 88, 111, 127 ); //Hair_light
set_color_profile_slot( 6, 1, 49, 66, 74 ); //Hair_dark
set_color_profile_slot( 6, 2, 255, 230, 210 ); //Skin
set_color_profile_slot( 6, 3, 232, 155, 227 ); //Energy_light
set_color_profile_slot( 6, 4, 230, 106, 221 ); //Energy_dark
set_color_profile_slot( 6, 5, 169, 169, 169 ); //Metal
set_color_profile_slot( 6, 6, 65, 72, 80 ); //Shirt
set_color_profile_slot( 6, 7, 33, 33, 38 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Ольга
set_color_profile_slot( 7, 0, 249, 237, 194 ); //Hair_light
set_color_profile_slot( 7, 1, 199, 190, 155 ); //Hair_dark
set_color_profile_slot( 7, 2, 255, 230, 212 ); //Skin
set_color_profile_slot( 7, 3, 189, 184, 122 ); //Energy_light
set_color_profile_slot( 7, 4, 127, 130, 80 ); //Energy_dark
set_color_profile_slot( 7, 5, 74, 74, 74 ); //Metal
set_color_profile_slot( 7, 6, 231, 228, 224 ); //Shirt
set_color_profile_slot( 7, 7, 155, 178, 179 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Анна
set_color_profile_slot( 8, 0, 178, 136, 112 ); //Hair_light
set_color_profile_slot( 8, 1, 125, 105, 91 ); //Hair_dark
set_color_profile_slot( 8, 2, 243, 222, 210 ); //Skin
set_color_profile_slot( 8, 3, 118, 204, 227 ); //Energy_light
set_color_profile_slot( 8, 4, 77, 176, 201 ); //Energy_dark
set_color_profile_slot( 8, 5, 74, 77, 74 ); //Metal
set_color_profile_slot( 8, 6, 49, 50, 49 ); //Shirt
set_color_profile_slot( 8, 7, 90, 162, 181 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// maki roll
set_color_profile_slot( 9, 0, 64, 64, 53 ); //Hair_light
set_color_profile_slot( 9, 1, 25, 30, 26 ); //Hair_dark
set_color_profile_slot( 9, 2, 255, 218, 192 ); //Skin
set_color_profile_slot( 9, 3, 252, 44, 44 ); //Energy_light
set_color_profile_slot( 9, 4, 176, 21, 21 ); //Energy_dark
set_color_profile_slot( 9, 5, 201, 201, 200 ); //Metal
set_color_profile_slot( 9, 6, 127, 58, 54 ); //Shirt
set_color_profile_slot( 9, 7, 33, 33, 38 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// zetta?
set_color_profile_slot( 10, 0, 245, 245, 245 ); //Hair_light
set_color_profile_slot( 10, 1, 217, 210, 224 ); //Hair_dark
set_color_profile_slot( 10, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 10, 3, 83, 224, 35 ); //Energy_light
set_color_profile_slot( 10, 4, 61, 163, 25 ); //Energy_dark
set_color_profile_slot( 10, 5, 37, 48, 41 ); //Metal
set_color_profile_slot( 10, 6, 66, 91, 26 ); //Shirt
set_color_profile_slot( 10, 7, 32, 32, 32 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Reisen
set_color_profile_slot( 11, 0, 220, 168, 251 ); //Hair_light
set_color_profile_slot( 11, 1, 149, 102, 222 ); //Hair_dark
set_color_profile_slot( 11, 2, 253, 233, 185 ); //Skin
set_color_profile_slot( 11, 3, 237, 36, 72 ); //Energy_light
set_color_profile_slot( 11, 4, 194, 30, 59 ); //Energy_dark
set_color_profile_slot( 11, 5, 248, 197, 104 ); //Metal
set_color_profile_slot( 11, 6, 255, 160, 156 ); //Shirt
set_color_profile_slot( 11, 7, 56, 16, 21 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Liz
set_color_profile_slot( 12, 0, 255, 72, 135 ); //Hair_light
set_color_profile_slot( 12, 1, 155, 43, 162 ); //Hair_dark
set_color_profile_slot( 12, 2, 255, 203, 160 ); //Skin
set_color_profile_slot( 12, 3, 199, 237, 255 ); //Energy_light
set_color_profile_slot( 12, 4, 56, 186, 255 ); //Energy_dark
set_color_profile_slot( 12, 5, 254, 227, 95 ); //Metal
set_color_profile_slot( 12, 6, 38, 27, 110 ); //Shirt
set_color_profile_slot( 12, 7, 255, 72, 135 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Lumi
set_color_profile_slot( 13, 0, 225, 237, 255 ); //Hair_light
set_color_profile_slot( 13, 1, 158, 179, 240 ); //Hair_dark
set_color_profile_slot( 13, 2, 240, 201, 161 ); //Skin
set_color_profile_slot( 13, 3, 164, 225, 233 ); //Energy_light
set_color_profile_slot( 13, 4, 78, 203, 229 ); //Energy_dark
set_color_profile_slot( 13, 5, 16, 18, 33 ); //Metal
set_color_profile_slot( 13, 6, 76, 65, 120 ); //Shirt
set_color_profile_slot( 13, 7, 40, 39, 81 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Abyss
set_color_profile_slot( 14, 0, 93, 54, 127 ); //Hair_light
set_color_profile_slot( 14, 1, 64, 37, 87 ); //Hair_dark
set_color_profile_slot( 14, 2, 207, 185, 247 ); //Skin
set_color_profile_slot( 14, 3, 201, 98, 227 ); //Energy_light
set_color_profile_slot( 14, 4, 150, 54, 173 ); //Energy_dark
set_color_profile_slot( 14, 5, 60, 45, 64 ); //Metal
set_color_profile_slot( 14, 6, 107, 82, 130 ); //Shirt
set_color_profile_slot( 14, 7, 46, 18, 71 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Eltnum
set_color_profile_slot( 15, 0, 157, 122, 207 ); //Hair_light
set_color_profile_slot( 15, 1, 111, 73, 163 ); //Hair_dark
set_color_profile_slot( 15, 2, 255, 218, 192 ); //Skin
set_color_profile_slot( 15, 3, 227, 197, 111 ); //Energy_light
set_color_profile_slot( 15, 4, 219, 117, 0 ); //Energy_dark
set_color_profile_slot( 15, 5, 58, 63, 66 ); //Metal
set_color_profile_slot( 15, 6, 77, 60, 125 ); //Shirt
set_color_profile_slot( 15, 7, 86, 48, 99 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Mori Calliope
set_color_profile_slot( 16, 0, 245, 205, 223 ); //Hair_light
set_color_profile_slot( 16, 1, 228, 140, 166 ); //Hair_dark
set_color_profile_slot( 16, 2, 254, 236, 237 ); //Skin
set_color_profile_slot( 16, 3, 181, 5, 68 ); //Energy_light
set_color_profile_slot( 16, 4, 122, 4, 46 ); //Energy_dark
set_color_profile_slot( 16, 5, 112, 89, 64 ); //Metal
set_color_profile_slot( 16, 6, 40, 36, 42 ); //Shirt
set_color_profile_slot( 16, 7, 100, 78, 80 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Fio
set_color_profile_slot( 17, 0, 206, 113, 13 ); //Hair_light
set_color_profile_slot( 17, 1, 138, 67, 1 ); //Hair_dark
set_color_profile_slot( 17, 2, 247, 215, 128 ); //Skin
set_color_profile_slot( 17, 3, 197, 166, 104 ); //Energy_light
set_color_profile_slot( 17, 4, 126, 96, 50 ); //Energy_dark
set_color_profile_slot( 17, 5, 71, 41, 1 ); //Metal
set_color_profile_slot( 17, 6, 247, 239, 178 ); //Shirt
set_color_profile_slot( 17, 7, 199, 167, 99 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// BLM
set_color_profile_slot( 18, 0, 38, 35, 41 ); //Hair_light
set_color_profile_slot( 18, 1, 26, 26, 26 ); //Hair_dark
set_color_profile_slot( 18, 2, 115, 91, 58 ); //Skin
set_color_profile_slot( 18, 3, 255, 229, 0 ); //Energy_light
set_color_profile_slot( 18, 4, 232, 173, 0 ); //Energy_dark
set_color_profile_slot( 18, 5, 181, 181, 181 ); //Metal
set_color_profile_slot( 18, 6, 79, 79, 79 ); //Shirt
set_color_profile_slot( 18, 7, 26, 26, 26 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Bout
set_color_profile_slot( 19, 0, 55, 59, 55 ); //Hair_light
set_color_profile_slot( 19, 1, 0, 31, 0 ); //Hair_dark
set_color_profile_slot( 19, 2, 255, 238, 194 ); //Skin
set_color_profile_slot( 19, 3, 37, 81, 184 ); //Energy_light
set_color_profile_slot( 19, 4, 0, 20, 133 ); //Energy_dark
set_color_profile_slot( 19, 5, 68, 115, 70 ); //Metal
set_color_profile_slot( 19, 6, 0, 46, 84 ); //Shirt
set_color_profile_slot( 19, 7, 35, 59, 35 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Rat God
set_color_profile_slot( 20, 0, 179, 255, 125 ); //Hair_light
set_color_profile_slot( 20, 1, 45, 122, 87 ); //Hair_dark
set_color_profile_slot( 20, 2, 255, 246, 232 ); //Skin
set_color_profile_slot( 20, 3, 255, 168, 217 ); //Energy_light
set_color_profile_slot( 20, 4, 184, 23, 23 ); //Energy_dark
set_color_profile_slot( 20, 5, 46, 36, 79 ); //Metal
set_color_profile_slot( 20, 6, 168, 34, 88 ); //Shirt
set_color_profile_slot( 20, 7, 216, 221, 230 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Cotton Candy..?
set_color_profile_slot( 21, 0, 179, 254, 255 ); //Hair_light
set_color_profile_slot( 21, 1, 94, 179, 209 ); //Hair_dark
set_color_profile_slot( 21, 2, 255, 236, 176 ); //Skin
set_color_profile_slot( 21, 3, 255, 158, 255 ); //Energy_light
set_color_profile_slot( 21, 4, 230, 103, 218 ); //Energy_dark
set_color_profile_slot( 21, 5, 201, 205, 207 ); //Metal
set_color_profile_slot( 21, 6, 230, 248, 255 ); //Shirt
set_color_profile_slot( 21, 7, 232, 175, 214 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, 1, 1]

// Riptide
set_color_profile_slot( 22, 0, 255, 228, 156 ); //Hair_light
set_color_profile_slot( 22, 1, 235, 148, 148 ); //Hair_dark
set_color_profile_slot( 22, 2, 242, 184, 145 ); //Skin
set_color_profile_slot( 22, 3, 105, 248, 255 ); //Energy_light
set_color_profile_slot( 22, 4, 48, 200, 255 ); //Energy_dark
set_color_profile_slot( 22, 5, 255, 180, 61 ); //Metal
set_color_profile_slot( 22, 6, 20, 65, 143 ); //Shirt
set_color_profile_slot( 22, 7, 224, 101, 47 ); //Pants
//shading data: [1, 1, 1.1, 1, 1, 2.1, 2, 1]

// Big House
set_color_profile_slot( 23, 0, 64, 108, 255 ); //Hair_light
set_color_profile_slot( 23, 1, 62, 64, 176 ); //Hair_dark
set_color_profile_slot( 23, 2, 255, 238, 209 ); //Skin
set_color_profile_slot( 23, 3, 224, 255, 247 ); //Energy_light
set_color_profile_slot( 23, 4, 153, 202, 255 ); //Energy_dark
set_color_profile_slot( 23, 5, 31, 36, 87 ); //Metal
set_color_profile_slot( 23, 6, 255, 224, 8 ); //Shirt
set_color_profile_slot( 23, 7, 32, 41, 56 ); //Pants
//shading data: [1, 1, 1, 1, 1, 1, -5, 1]


/* This is a comment used by that one RoA colors.gml generator tool to store palette data. You can safely keep it in your colors.gml if you plan to re-use the tool later, or safely remove it if you don't.
=== BEGIN JSON PALETTE ===
{"formatversion":1,"data":[{"name":"Hair_light","colors":[{"r":"76","g":"50","b":"39","main":true}]},{"name":"Hair_dark","colors":[{"r":"50","g":"38","b":"34","main":true}]},{"name":"Skin","colors":[{"r":"253","g":"233","b":"185","main":true},{"r":231,"g":196,"b":147},{"r":199,"g":141,"b":99}]},{"name":"Energy_light","colors":[{"r":"249","g":"202","b":"53","main":true}]},{"name":"Energy_dark","colors":[{"r":"225","g":"160","b":"76","main":true}]},{"name":"Metal","colors":[{"r":"41","g":"32","b":"38","main":true},{"r":64,"g":53,"b":59},{"r":27,"g":22,"b":26}]},{"name":"Shirt","colors":[{"r":"39","g":"70","b":"94","main":true},{"r":25,"g":48,"b":75}]},{"name":"Pants","colors":[{"r":"33","g":"33","b":"38","main":true}]}]}
=== END JSON PALETTE ===
*/
