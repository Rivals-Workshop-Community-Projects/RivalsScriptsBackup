set_attack_value(AT_NAIR, AG_CATEGORY, 1);
set_attack_value(AT_NAIR, AG_SPRITE, sprite_get("nair"));
set_attack_value(AT_NAIR, AG_NUM_WINDOWS, 3);
set_attack_value(AT_NAIR, AG_HAS_LANDING_LAG, 1);
set_attack_value(AT_NAIR, AG_LANDING_LAG, 7);
set_attack_value(AT_NAIR, AG_HURTBOX_SPRITE, sprite_get("nair_hurt"));

set_window_value(AT_NAIR, 1, AG_WINDOW_TYPE, 1);
set_window_value(AT_NAIR, 1, AG_WINDOW_LENGTH, 8);
set_window_value(AT_NAIR, 1, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_NAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_NAIR, 1, AG_WINDOW_SFX, sound_get("nair"));
set_window_value(AT_NAIR, 1, AG_WINDOW_SFX_FRAME, 2);

set_window_value(AT_NAIR, 2, AG_WINDOW_TYPE, 1);
set_window_value(AT_NAIR, 2, AG_WINDOW_LENGTH, 22);
set_window_value(AT_NAIR, 2, AG_WINDOW_ANIM_FRAMES, 4);
set_window_value(AT_NAIR, 2, AG_WINDOW_ANIM_FRAME_START, 1);

set_window_value(AT_NAIR, 3, AG_WINDOW_TYPE, 1);
set_window_value(AT_NAIR, 3, AG_WINDOW_LENGTH, 7);
set_window_value(AT_NAIR, 3, AG_WINDOW_ANIM_FRAMES, 1);
set_window_value(AT_NAIR, 3, AG_WINDOW_ANIM_FRAME_START, 0);

set_num_hitboxes(AT_NAIR, 12);

//bowl
set_hitbox_value(AT_NAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_X, 12);
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_Y, -60);
set_hitbox_value(AT_NAIR, 1, HG_WIDTH, 65);
set_hitbox_value(AT_NAIR, 1, HG_HEIGHT, 65);
set_hitbox_value(AT_NAIR, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_NAIR, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_NAIR, 1, HG_ANGLE, 80);
set_hitbox_value(AT_NAIR, 1, HG_ANGLE_FLIPPER, 4);
set_hitbox_value(AT_NAIR, 1, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 1, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_NAIR, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_NAIR, 1, HG_HITSTUN_MULTIPLIER, 1.1);
set_hitbox_value(AT_NAIR, 1, HG_VISUAL_EFFECT, 301);
set_hitbox_value(AT_NAIR, 1, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_NAIR, 1, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_NAIR, 2, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 2, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 2, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_NAIR, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 2, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_X, 12);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_Y, -60);
set_hitbox_value(AT_NAIR, 2, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_NAIR, 3, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 3, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 3, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 3, HG_WINDOW_CREATION_FRAME, 11);
set_hitbox_value(AT_NAIR, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_X, 12);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_Y, -60);
set_hitbox_value(AT_NAIR, 3, HG_HITBOX_GROUP, 3);

set_hitbox_value(AT_NAIR, 4, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 4, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 4, HG_WINDOW_CREATION_FRAME, 17);
set_hitbox_value(AT_NAIR, 4, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_X, 12);
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_Y, -60);
set_hitbox_value(AT_NAIR, 4, HG_WIDTH, 75);
set_hitbox_value(AT_NAIR, 4, HG_HEIGHT, 75);
set_hitbox_value(AT_NAIR, 4, HG_PRIORITY, 2);
set_hitbox_value(AT_NAIR, 4, HG_DAMAGE, 5);
set_hitbox_value(AT_NAIR, 4, HG_ANGLE, 70);
set_hitbox_value(AT_NAIR, 4, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_NAIR, 4, HG_KNOCKBACK_SCALING, .8);
set_hitbox_value(AT_NAIR, 4, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_NAIR, 4, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_NAIR, 4, HG_HITSTUN_MULTIPLIER, 1.05);
set_hitbox_value(AT_NAIR, 4, HG_VISUAL_EFFECT, 280);
set_hitbox_value(AT_NAIR, 4, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_NAIR, 4, HG_VISUAL_EFFECT_Y_OFFSET, -20);
set_hitbox_value(AT_NAIR, 4, HG_HIT_SFX, asset_get("sfx_waterhit_medium"));
set_hitbox_value(AT_NAIR, 4, HG_HITBOX_GROUP, 4);

//front fish
set_hitbox_value(AT_NAIR, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_NAIR, 5, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 5, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 5, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 5, HG_HITBOX_X, 18);
set_hitbox_value(AT_NAIR, 5, HG_HITBOX_Y, -105);
set_hitbox_value(AT_NAIR, 5, HG_WIDTH, 30);
set_hitbox_value(AT_NAIR, 5, HG_HEIGHT, 30);
set_hitbox_value(AT_NAIR, 5, HG_PRIORITY, 2);
set_hitbox_value(AT_NAIR, 5, HG_DAMAGE, 2);
set_hitbox_value(AT_NAIR, 5, HG_ANGLE, 40);
set_hitbox_value(AT_NAIR, 5, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 5, HG_ANGLE_FLIPPER, 2);
set_hitbox_value(AT_NAIR, 5, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_NAIR, 5, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_NAIR, 5, HG_HITSTUN_MULTIPLIER, 1.1);
set_hitbox_value(AT_NAIR, 5, HG_VISUAL_EFFECT, 280);
set_hitbox_value(AT_NAIR, 5, HG_HIT_SFX, asset_get("sfx_waterhit_weak"));
set_hitbox_value(AT_NAIR, 5, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_NAIR, 6, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_NAIR, 6, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 6, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 6, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_NAIR, 6, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 6, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 6, HG_HITBOX_X, 42);
set_hitbox_value(AT_NAIR, 6, HG_HITBOX_Y, -112);
set_hitbox_value(AT_NAIR, 6, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_NAIR, 7, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_NAIR, 7, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 7, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 7, HG_WINDOW_CREATION_FRAME, 11);
set_hitbox_value(AT_NAIR, 7, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 7, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 7, HG_HITBOX_X, 68);
set_hitbox_value(AT_NAIR, 7, HG_HITBOX_Y, -60);
set_hitbox_value(AT_NAIR, 7, HG_HITBOX_GROUP, 3);

set_hitbox_value(AT_NAIR, 8, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_NAIR, 8, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 8, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 8, HG_WINDOW_CREATION_FRAME, 17);
set_hitbox_value(AT_NAIR, 8, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 8, HG_HITBOX_X, 72);
set_hitbox_value(AT_NAIR, 8, HG_HITBOX_Y, -15);
set_hitbox_value(AT_NAIR, 8, HG_WIDTH, 40);
set_hitbox_value(AT_NAIR, 8, HG_HEIGHT, 40);
set_hitbox_value(AT_NAIR, 8, HG_PRIORITY, 1);
set_hitbox_value(AT_NAIR, 8, HG_DAMAGE, 5);
set_hitbox_value(AT_NAIR, 8, HG_ANGLE, 70);
set_hitbox_value(AT_NAIR, 8, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_NAIR, 8, HG_KNOCKBACK_SCALING, .8);
set_hitbox_value(AT_NAIR, 8, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_NAIR, 8, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_NAIR, 8, HG_HITSTUN_MULTIPLIER, 1.05);
set_hitbox_value(AT_NAIR, 8, HG_VISUAL_EFFECT, 280);
set_hitbox_value(AT_NAIR, 8, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_NAIR, 8, HG_VISUAL_EFFECT_Y_OFFSET, -20);
set_hitbox_value(AT_NAIR, 8, HG_HIT_SFX, asset_get("sfx_waterhit_medium"));
set_hitbox_value(AT_NAIR, 8, HG_HITBOX_GROUP, 4);

//back fish
set_hitbox_value(AT_NAIR, 9, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_NAIR, 9, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 9, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 9, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 9, HG_HITBOX_X, -10);
set_hitbox_value(AT_NAIR, 9, HG_HITBOX_Y, -105);
set_hitbox_value(AT_NAIR, 9, HG_WIDTH, 30);
set_hitbox_value(AT_NAIR, 9, HG_HEIGHT, 30);
set_hitbox_value(AT_NAIR, 9, HG_PRIORITY, 2);
set_hitbox_value(AT_NAIR, 9, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 9, HG_DAMAGE, 2);
set_hitbox_value(AT_NAIR, 9, HG_ANGLE, 40);
set_hitbox_value(AT_NAIR, 9, HG_ANGLE_FLIPPER, 2);
set_hitbox_value(AT_NAIR, 9, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_NAIR, 9, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_NAIR, 9, HG_HITSTUN_MULTIPLIER, 1.1);
set_hitbox_value(AT_NAIR, 9, HG_VISUAL_EFFECT, 280);
set_hitbox_value(AT_NAIR, 9, HG_HIT_SFX, asset_get("sfx_waterhit_weak"));
set_hitbox_value(AT_NAIR, 9, HG_HITBOX_GROUP, 1);

set_hitbox_value(AT_NAIR, 10, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_NAIR, 10, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 10, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 10, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_NAIR, 10, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 10, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 10, HG_HITBOX_X, -32);
set_hitbox_value(AT_NAIR, 10, HG_HITBOX_Y, -102);
set_hitbox_value(AT_NAIR, 10, HG_HITBOX_GROUP, 2);

set_hitbox_value(AT_NAIR, 11, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_NAIR, 11, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 11, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 11, HG_WINDOW_CREATION_FRAME, 11);
set_hitbox_value(AT_NAIR, 11, HG_TECHABLE, 2);
set_hitbox_value(AT_NAIR, 11, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 11, HG_HITBOX_X, -52);
set_hitbox_value(AT_NAIR, 11, HG_HITBOX_Y, -60);
set_hitbox_value(AT_NAIR, 11, HG_HITBOX_GROUP, 3);

set_hitbox_value(AT_NAIR, 12, HG_PARENT_HITBOX, 0);
set_hitbox_value(AT_NAIR, 12, HG_HITBOX_TYPE, 1);
set_hitbox_value(AT_NAIR, 12, HG_WINDOW, 2);
set_hitbox_value(AT_NAIR, 12, HG_WINDOW_CREATION_FRAME, 17);
set_hitbox_value(AT_NAIR, 12, HG_LIFETIME, 3);
set_hitbox_value(AT_NAIR, 12, HG_HITBOX_X, -55);
set_hitbox_value(AT_NAIR, 12, HG_HITBOX_Y, -15);
set_hitbox_value(AT_NAIR, 12, HG_WIDTH, 40);
set_hitbox_value(AT_NAIR, 12, HG_HEIGHT, 40);
set_hitbox_value(AT_NAIR, 12, HG_PRIORITY, 1);
set_hitbox_value(AT_NAIR, 12, HG_DAMAGE, 5);
set_hitbox_value(AT_NAIR, 12, HG_ANGLE, 110);
set_hitbox_value(AT_NAIR, 12, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_NAIR, 12, HG_KNOCKBACK_SCALING, .8);
set_hitbox_value(AT_NAIR, 12, HG_BASE_HITPAUSE, 7);
set_hitbox_value(AT_NAIR, 12, HG_HITSTUN_MULTIPLIER, 1.05);
set_hitbox_value(AT_NAIR, 12, HG_HITPAUSE_SCALING, .2);
set_hitbox_value(AT_NAIR, 12, HG_VISUAL_EFFECT, 280);
set_hitbox_value(AT_NAIR, 12, HG_VISUAL_EFFECT_X_OFFSET, 20);
set_hitbox_value(AT_NAIR, 12, HG_VISUAL_EFFECT_Y_OFFSET, -20);
set_hitbox_value(AT_NAIR, 12, HG_HIT_SFX, asset_get("sfx_waterhit_medium"));
set_hitbox_value(AT_NAIR, 12, HG_HITBOX_GROUP, 4);