//DEFAULT COLOR (R,G,B)
set_color_profile_slot(0, 0, 64, 124, 48); //Dark Greens
set_color_profile_slot(0, 1, 8, 0, 27); //Ears, T-Shirt, shoes and gloves
set_color_profile_slot(0, 2, 216, 184, 0); //Ear edges, ribbons, shoe detail and headlight
set_color_profile_slot(0, 3, 200, 0, 0); //Scarf and Bike
set_color_profile_slot(0, 4, 160, 184, 56); // Skin
set_color_profile_slot(0, 5, 29, 51, 21);//Eyes

//COLOR RANGE (H,S,V)
set_color_profile_slot_range(0, 5, 40, 28);
set_color_profile_slot_range(1, 2, 1, 25);
set_color_profile_slot_range(2, 10, 2, 22);
set_color_profile_slot_range(3, 1, 1, 23);
set_color_profile_slot_range(4, 4, 5, 12);
set_color_profile_slot_range(5, 1, 1, 19);

set_num_palettes(20);

//Yellow Scarf
set_color_profile_slot(1, 0, 64, 124, 48);
set_color_profile_slot(1, 1, 0, 0, 0);
set_color_profile_slot(1, 2, 216, 184, 0);
set_color_profile_slot(1, 3, 248, 252, 0);
set_color_profile_slot(1, 4, 160, 184, 56);
set_color_profile_slot(1, 5, 64, 124, 48);

//Pink Scarf
set_color_profile_slot(2, 0, 64, 124, 48);
set_color_profile_slot(2, 1, 0, 0, 0);
set_color_profile_slot(2, 2, 216, 184, 0);
set_color_profile_slot(2, 3, 248, 100, 248);
set_color_profile_slot(2, 4, 160, 184, 56);
set_color_profile_slot(2, 5, 64, 124, 48);

//Blue Scarf
set_color_profile_slot(3, 0, 64, 124, 48);
set_color_profile_slot(3, 1, 0, 0, 0);
set_color_profile_slot(3, 2, 216, 184, 0);
set_color_profile_slot(3, 3, 98, 116, 248);
set_color_profile_slot(3, 4, 160, 184, 56);
set_color_profile_slot(3, 5, 64, 124, 48);

//White Scarf
set_color_profile_slot(4, 0, 64, 124, 48);
set_color_profile_slot(4, 1, 0, 0, 0);
set_color_profile_slot(4, 2, 216, 184, 0);
set_color_profile_slot(4, 3, 232, 232, 232);
set_color_profile_slot(4, 4, 160, 184, 56);
set_color_profile_slot(4, 5, 64, 124, 48);

//Corazon
set_color_profile_slot(5, 0, 64, 124, 48);
set_color_profile_slot(5, 1, 37, 37, 37);
set_color_profile_slot(5, 2, 216, 184, 0);
set_color_profile_slot(5, 3, 125, 0, 0);
set_color_profile_slot(5, 4, 160, 184, 56);
set_color_profile_slot(5, 5, 196, 164, 0);

//Lilac
set_color_profile_slot(6, 0, 120, 64, 176);
set_color_profile_slot(6, 1, 0, 64, 168);
set_color_profile_slot(6, 2, 216, 164, 24);
set_color_profile_slot(6, 3, 0, 144, 248);
set_color_profile_slot(6, 4, 176, 120, 248);
set_color_profile_slot(6, 5, 248, 0, 144);

//Milla
set_color_profile_slot(7, 0, 160, 160, 104);
set_color_profile_slot(7, 1, 255, 255, 255);
set_color_profile_slot(7, 2, 96, 200, 0);
set_color_profile_slot(7, 3, 230, 102, 27);
set_color_profile_slot(7, 4, 254, 189, 159);
set_color_profile_slot(7, 5, 24, 104, 8);

//Mayor Zhao
set_color_profile_slot(8, 0, 192, 80, 0);
set_color_profile_slot(8, 1, 122, 0, 0);
set_color_profile_slot(8, 2, 216, 184, 0);
set_color_profile_slot(8, 3, 255, 0, 0);
set_color_profile_slot(8, 4, 255, 255, 255);
set_color_profile_slot(8, 5, 0, 92, 0);

//Garfield
set_color_profile_slot(9, 0, 181, 74, 0);
set_color_profile_slot(9, 1, 74, 0, 0);
set_color_profile_slot(9, 2, 181, 74, 0);
set_color_profile_slot(9, 3, 181, 74, 0);
set_color_profile_slot(9, 4, 255, 181, 0);
set_color_profile_slot(9, 5, 255, 255, 255);

//Litten
set_color_profile_slot(10, 0, 30, 30, 30);
set_color_profile_slot(10, 1, 155, 155, 255);
set_color_profile_slot(10, 2, 30, 30, 30);
set_color_profile_slot(10, 3, 30, 30, 30);
set_color_profile_slot(10, 4, 255, 0, 0);
set_color_profile_slot(10, 5, 255, 255, 0);

//Sprigatito
set_color_profile_slot(11, 0, 104, 170, 47);
set_color_profile_slot(11, 1, 104, 170, 47);
set_color_profile_slot(11, 2, 104, 170, 47);
set_color_profile_slot(11, 3, 104, 170, 47);
set_color_profile_slot(11, 4, 206, 230, 178);
set_color_profile_slot(11, 5, 180, 95, 100);

//Big
set_color_profile_slot(12, 0, 42, 2, 23);
set_color_profile_slot(12, 1, 255, 255, 255);
set_color_profile_slot(12, 2, 255, 255, 255);
set_color_profile_slot(12, 3, 42, 2, 23);
set_color_profile_slot(12, 4, 72, 32, 104);
set_color_profile_slot(12, 5, 255, 255, 255);

// Abyss
set_color_profile_slot(13, 0, 152, 153, 232 );
set_color_profile_slot(13, 1, 83, 82, 140 );
set_color_profile_slot(13, 2, 214, 209, 255 );
set_color_profile_slot(13, 3, 83, 82, 140 );
set_color_profile_slot(13, 4, 83, 82, 140 );
set_color_profile_slot(13, 5, 83, 82, 140 );

//Ratchet
set_color_profile_slot(14, 0, 163, 119, 64);
set_color_profile_slot(14, 1, 162, 91, 27);
set_color_profile_slot(14, 2, 219, 173, 73);
set_color_profile_slot(14, 3, 71, 43, 23);
set_color_profile_slot(14, 4, 219, 173, 73);
set_color_profile_slot(14, 5, 24, 104, 8);

//Rivet
set_color_profile_slot(15, 0, 160, 160, 160);
set_color_profile_slot(15, 1, 0, 0, 0);
set_color_profile_slot(15, 2, 216, 184, 0);
set_color_profile_slot(15, 3, 200, 50, 0);
set_color_profile_slot(15, 4, 220, 220, 160);
set_color_profile_slot(15, 5, 0, 0, 77);

//Carrie
set_color_profile_slot(16, 0, 210, 197, 37);
set_color_profile_slot(16, 1, 166, 73, 0);
set_color_profile_slot(16, 2, 255, 255, 255);
set_color_profile_slot(16, 3, 166, 73, 0)
set_color_profile_slot(16, 4, 66, 66, 66);
set_color_profile_slot(16, 5, 64, 124, 48);

//SSL
set_color_profile_slot(17, 0, 167, 186, 74 );
set_color_profile_slot(17, 1, 35, 67, 49 );
set_color_profile_slot(17, 2, 83, 122, 62 );
set_color_profile_slot(17, 3, 211, 226, 154 );
set_color_profile_slot(17, 4, 167, 186, 74 );
set_color_profile_slot(17, 5, 167, 186, 74 );

//Gold Kitty
set_color_profile_slot(18, 0, 255, 162, 29 );
set_color_profile_slot(18, 1, 86, 23, 16 );
set_color_profile_slot(18, 2, 192, 92, 0 );
set_color_profile_slot(18, 3, 255, 162, 29 );
set_color_profile_slot(18, 4, 255, 234, 155 ); 
set_color_profile_slot(18, 5, 255, 162, 29 );

//Mio
set_color_profile_slot(19, 0, 225, 225, 225);
set_color_profile_slot(19, 1, 215, 215, 215);
set_color_profile_slot(19, 2, 200, 39, 51);
set_color_profile_slot(19, 3, 190, 39, 51);
set_color_profile_slot(19, 4, 237, 205, 180);
set_color_profile_slot(19, 5, 216, 184, 0);