...
av(atk, AG_SPRITE, sprite_get("jab"));
av(atk, AG_NUM_WINDOWS, 9);
av(atk, AG_HURTBOX_SPRITE, sprite_get("jab_hurt"));
av(atk, AG_MUNO_ATTACK_NAME, "Jab (Claw Combo)");
av(atk, AG_MUNO_ATTACK_MISC, "Cancels into Hit 2 on frame 10 and Hit 3 on frame 29. Can be cancelled into FTilt, UTilt and DTilt on frames 10 and 29 provided this and the others haven't been cancelled twice before. Maximum 7 hit combo.");
...
win(atk, 2, AG_WINDOW_HSPEED, 1);

win(atk, 3, AG_WINDOW_LENGTH, 15);
win(atk, 3, AG_WINDOW_TYPE, 9);
...
hv(atk, 1, HG_MUNO_HITBOX_MISC_ADD, "Untechable rather than Force Flinch due to the way the claw combo cancel system works. Becomes techable if the move is cancelled into.");
...
hv(atk, 2, HG_HITBOX_Y, -34);
hv(atk, 2, HG_WIDTH, 62);
