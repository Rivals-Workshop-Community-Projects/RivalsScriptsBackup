#macro atk AT_FSPECIAL_AIR
#macro av set_attack_value
#macro win set_window_value
#macro hv set_hitbox_value

av(atk, AG_CATEGORY, 2);
av(atk, AG_SPRITE, sprite_get("fspecial_air"));
av(atk, AG_NUM_WINDOWS, 5);
av(atk, AG_USES_CUSTOM_GRAVITY, 1);
av(atk, AG_HURTBOX_SPRITE, sprite_get("fspecial_air_hurt"));
av(atk, AG_MUNO_ATTACK_NAME, "FSpecial3 (Bike Throw)");
av(atk, AG_MUNO_ATTACK_MISC, "Pressing Shield will warp to the bike and get on the bike instead. Otherwise pressing or holding Special will turn it back into a disc, storing the bike for future use. Doing nothing will result in the bike coming backwards and go into storage.");
...
win(atk, 4, AG_WINDOW_CUSTOM_AIR_FRICTION, .35);
win(atk, 4, AG_WINDOW_CUSTOM_GROUND_FRICTION, .35);
...
hv(atk, 1, HG_HIT_PARTICLE_NUM, 1);
...
hv(atk, 1, HG_PROJECTILE_SPRITE, sprite_get("fspecial_air_proj"));
hv(atk, 1, HG_PROJECTILE_MASK, sprite_get("fspecial_air_proj"));
hv(atk, 1, HG_PROJECTILE_ANIM_SPEED, 1);
hv(atk, 1, HG_PROJECTILE_HSPEED, 16);
