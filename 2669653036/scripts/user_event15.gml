...
*/
initTip("USpecial: Flare Blitz");
initWords("A multihit move that sends Litleo spiraling upwards. You can move horizontally during this move.");
initWords("The final hit of the move sends the opponent farther away than the other hits.");
initImage_ext(sprite_get("uspecial"), -4, fa_center, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("DSpecial: Flare Burst");
initWords("Similar to Zetterburn's down special, creates a small explosion around Litleo.");
initWords("Doesn't use burn effect to punish, however using it while the projectile from NSpecial is out creates a smaller explosion from it.");
initImage_ext(sprite_get("dspecial"), -4, fa_center, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);

initTip("FStrong: Flamethrower");
initWords("Similar to Charizard's neutral special. Charging it only increases the damage, not the duration of the move.");
initImage_ext(sprite_get("fstrong"), -4, fa_center, 1, 1, true, c_white, 1, false, noone, noone, noone, noone);
	
initTip("Pokedex Entry");
initWords_ext("Ultra Sun/Moon", fa_center, c_gray, 0, false);
initWords("When they’re young, they live with a pride. Once they’re able to hunt prey on their own, they’re kicked out and have to make their own way.");
initWords("This hot-blooded Pokémon is filled with curiosity. When it gets angry or starts fighting, its short mane gets hot.");
...
initPatch("About Litleo", "");
initWords("Litleo, he's a lion pokemon, he was made for the pokejam, emjoy :)   -- YoshifanJordan");
initHeader("Litleo created by");
initSection("Pokemon Team");
initHeader("Additional SFX from");
initSection("Pokemon Sun and Moon");
initHeader("Compatible with");
initSection("Steve, Link, and Pit");
...
// Steve death message.
steve_death_message = "Steve blacked out!";
...
initCodecPit();
initCodecPagePit(SPK_PIT,	3,	9,	"Hey, it's Simba!");
initCodecPagePit(SPK_PALU,	4,	4,	"Simba...?");
initCodecPagePit(SPK_VIR,	5,	1,	"Really, Pit? You think some dumb movie character is getting into a fighting game?");
initCodecPagePit(SPK_PIT,	0,	8,	"No, that's what I named my Litleo in Pokemon Sun!");
initCodecPagePit(SPK_VIR,	2,	3,	"Are we just advertising now?!");
initCodecPagePit(SPK_PIT,	0,	0,	"No need for advice, I know litleo well!");
initCodecPagePit(SPK_PIT,	0,	0,	"Litleo's a fire type that looks weak at first, but can truly go all out!");
initCodecPagePit(SPK_PIT,	0,	0,	"If he can approach that is... he's a bit small, but that makes him even faster!");
initCodecPagePit(SPK_VIR,	2,	3,	"Whatever, this is a jam character, who would check for Palutena's guidence compatability?");
